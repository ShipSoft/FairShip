...
GFRaveTrackParameters::GFRaveTrackParameters()
   : originalTrack_(NULL), weight_(0), state_(6), cov_(6, 6), hasSmoothedData_(false)
{
   ;
}
...
GFRaveTrackParameters::GFRaveTrackParameters(const Track *track, MeasuredStateOnPlane *originalState, double weight,
   ...
   : originalTrack_(const_cast<Track *>(track)), weight_(weight), state_(state6), cov_(cov6x6),
...
GFRaveTrackParameters::GFRaveTrackParameters(const Track *track, MeasuredStateOnPlane *originalState, double weight)
   : originalTrack_(const_cast<Track *>(track)), weight_(weight), state_(1, 6), cov_(6, 6), hasSmoothedData_(false)
{
   ;
}

TVector3 GFRaveTrackParameters::getPos() const
{
   return TVector3(state_[0], state_[1], state_[2]);
}

TVector3 GFRaveTrackParameters::getMom() const
{
   return TVector3(state_[3], state_[4], state_[5]);
}

double GFRaveTrackParameters::getCharge() const
{
