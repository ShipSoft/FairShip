...
GFRaveVertex::GFRaveVertex() : cov_(3, 3), ndf_(0), chi2_(0), id_(-1)
{
#ifdef COUNT
...
GFRaveVertex::GFRaveVertex(const TVector3 &pos, const TMatrixDSym &cov,
                           const std::vector<GFRaveTrackParameters *> &smoothedTracks, double ndf, double chi2, int id)
   : pos_(pos), cov_(cov), ndf_(ndf), chi2_(chi2), id_(id), smoothedTracks_(smoothedTracks)
{
...
GFRaveVertex::GFRaveVertex(const GFRaveVertex &vertex)
   : TObject(vertex), pos_(vertex.pos_), cov_(vertex.cov_), ndf_(vertex.ndf_), chi2_(vertex.chi2_), id_(vertex.id_)
{
