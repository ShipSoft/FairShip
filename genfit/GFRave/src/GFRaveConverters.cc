...
std::vector<rave::Track> GFTracksToTracks(const std::vector<genfit::Track *> &GFTracks,
                                          ...
                                          std::map<int, genfit::trackAndState> &IdGFTrackStateMap, int startID)
{
   ...
   for (unsigned int i = 0; i < ntracks; ++i) {
      ...
      if (GFStates == NULL)
         IdGFTrackStateMap[startID].state_ = new MeasuredStateOnPlane(
            GFTracks[i]->getFittedState()); // here clones are made so that the state of the original GFTracks and their
                                            // TrackReps will not be altered by the vertexing process
      else
...
rave::Track GFTrackToTrack(trackAndState trackAndState, int id, std::string tag)
{
   ...
   // covariance
   rave::Covariance6D ravecov(cov(0, 0), cov(1, 0), cov(2, 0), cov(1, 1), cov(2, 1), cov(2, 2), cov(3, 0), cov(4, 0),
                              cov(5, 0), cov(3, 1), cov(4, 1), cov(5, 1), cov(3, 2), cov(4, 2), cov(5, 2), cov(3, 3),
                              cov(4, 3), cov(5, 3), cov(4, 4), cov(5, 4), cov(5, 5));
   ...
   rave::Track ret(id, ravestate, ravecov, trackAndState.track_->getFitStatus()->getCharge(),
                   trackAndState.track_->getFitStatus()->getChi2(), trackAndState.track_->getFitStatus()->getNdf(),
...
GFRaveVertex *
RaveToGFVertex(const rave::Vertex &raveVertex, const std::map<int, genfit::trackAndState> &IdGFTrackStateMap)
{
   ...
   return new GFRaveVertex(Point3DToTVector3(raveVertex.position()), Covariance3DToTMatrixDSym(raveVertex.error()),
                           trackParameters, raveVertex.ndf(), raveVertex.chiSquared(), raveVertex.id());
}

void RaveToGFVertices(std::vector<GFRaveVertex *> *GFVertices, const std::vector<rave::Vertex> &raveVertices,
...
{
   return SharedPlanePtr(new DetPlane(Point3DToTVector3(rplane.position()), Vector3DToTVector3(rplane.normalVector())));
}
