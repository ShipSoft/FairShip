...
void TGeoMaterialInterface::getMaterialParameters(double &density, double &Z, double &A, double &radiationLength,
                                                  double &mEE)
{
...
void TGeoMaterialInterface::getMaterialParameters(MaterialProperties &parameters)
{
   ...
   parameters.setMaterialProperties(mat->GetDensity(), mat->GetZ(), mat->GetA(), mat->GetRadLen(),
                                    MeanExcEnergy_get(mat));
}

double TGeoMaterialInterface::findNextBoundary(const RKTrackRep *rep, const M1x7 &stateOrig,
                                               double sMax, // signed
                                               bool varField)
{
   ...
   while (1) {
      if (++it > maxIt) {
         Exception exc("TGeoMaterialInterface::findNextBoundary ==> maximum number of iterations exceeded", __LINE__,
                       __FILE__);
         exc.setFatal();
         throw exc;
      }
      ...
      if (slDist < delta) {
         if (debug)
            std::cout << "   very close to the boundary -> return" << " stepSign*(s + slDist) = " << stepSign << "*("
                      << s + slDist << ")\n";
      ...
      if (step > safety && maxDeviation2 > epsilon * epsilon) {
         ...
         bool volChanged = initTrack(state7[0], state7[1], state7[2], stepSign * state7[3], stepSign * state7[4],
                                     stepSign * state7[5]);
