...
MaterialEffects::MaterialEffects()
   : noEffects_(false), energyLossBetheBloch_(true), noiseBetheBloch_(true), noiseCoulomb_(true),
     energyLossBrems_(true), noiseBrems_(true), ignoreBoundariesBetweenEqualMaterials_(true), me_(0.510998910E-3),
     stepSize_(0), mom_(0), beta_(0), dEdx_(0), gamma_(0), gammaSquare_(0), matDensity_(0), matZ_(0), matA_(0),
     radiationLength_(0), mEE_(0), pdg_(0), charge_(0), mass_(0), mscModelCode_(0), materialInterface_(nullptr)
{
}
...
double MaterialEffects::effects(const std::vector<RKStep> &steps, int materialsFXStart, int materialsFXStop,
                                const double &mom, const int &pdg, M7x7 *noise)
{
   ...
   for (std::vector<RKStep>::const_iterator it = steps.begin() + materialsFXStart;
        it != steps.begin() + materialsFXStop; ++it) { // loop over steps
...
void MaterialEffects::stepper(const RKTrackRep *rep, M1x7 &state7,
                              const double &mom,  // momentum
                              double &relMomLoss, // relative momloss for the step will be added
                              const int &pdg, MaterialProperties &currentMaterial, StepLimits &limits, bool varField)
{
   ...
   materialInterface_->initTrack(state7[0], state7[1], state7[2], limits.getStepSign() * state7[3],
                                 limits.getStepSign() * state7[4], limits.getStepSign() * state7[5]);
...
void MaterialEffects::noiseBetheBloch(M7x7 &noise) const
{
   ...
   if (kappa > 0.01) {                                    // Vavilov-Gaussian regime
      ...
      double Sigma1 =
         dEdx_ * 1.0E9 * f1 / e1 * (log(mbbgg2 / e1) - beta_ * beta_) / (log(mbbgg2 / I) - beta_ * beta_) * 0.6; // 1/cm
      double Sigma2 =
         dEdx_ * 1.0E9 * f2 / e2 * (log(mbbgg2 / e2) - beta_ * beta_) / (log(mbbgg2 / I) - beta_ * beta_) * 0.6; // 1/cm
...
void MaterialEffects::noiseCoulomb(M7x7 &noise, const M1x3 &direction) const
{
   ...
   if (mscModelCode_ == 0) { // PANDA report PV/01-07 eq(43); linear in step length
      sigma2 =
         ...
         log(287. *
             pow(matZ_, -0.5)); // sigma^2 = 225E-6*z^2/mom^2 * XX0/beta_^2 * Z/(Z+1) * ln(159*Z^(-1/3))/ln(287*Z^(-1/2)
