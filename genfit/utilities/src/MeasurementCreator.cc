...
MeasurementCreator::MeasurementCreator()
   : trackModel_(NULL), resolution_(0.01), resolutionWire_(0.1), outlierProb_(0), outlierRange_(2), thetaDetPlane_(90),
     phiDetPlane_(0), wireCounter_(0), wireDir_(0., 0., 1.), minDrift_(0), maxDrift_(2), idealLRResolution_(true),
     useSkew_(false), skewAngle_(5), nSuperLayer_(5), measurementCounter_(0), debug_(false)
{
   ;
}

std::vector<genfit::AbsMeasurement *>
MeasurementCreator::create(eMeasurementType type, double tracklength, bool &outlier, int &lr)
{
   ...
   default: std::cerr << "measurement type not defined!" << std::endl; exit(0);
   }
