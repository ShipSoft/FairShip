...
void KalmanFitterRefTrack::processTrackWithRep(Track *tr, const AbsTrackRep *rep, bool resortHits)
{
   ...
   for (;;) {
      ...
      try {
         if (debugLvl_ > 0) {
            std::cout << " KalmanFitterRefTrack::processTrack with rep " << rep << " (id == " << tr->getIdForRep(rep)
                      << ")"
                      << ", iteration nr. " << nIt << "\n";
         }
         ...
         if (debugLvl_ > 0) {
            ...
            std::cout << "old chi2s: " << oldChi2BW << ", " << oldChi2FW << " new chi2s: " << chi2BW << ", " << chi2FW
                      << std::endl;
            std::cout << "old pVals: " << oldPvalBW << ", " << oldPvalFW << " new pVals: " << PvalBW << ", " << PvalFW
                      << std::endl;
         }
   ...
   for (; i < nPoints; ++i) {
      ...
      try {
         ...
         if (fitterInfo->hasReferenceState()) {
            ...
            if (!prevNewRefState) {
               if (debugLvl_ > 0) {
                  std::cout << "TrackPoint already has referenceState and previous referenceState has not been altered "
                               "-> continue \n";
               }
            ...
            if (prevReferenceState == NULL) {
               if (debugLvl_ > 0) {
                  std::cout << "TrackPoint already has referenceState but previous referenceState is NULL -> reset "
                               "forward info of current reference state and continue \n";
               }
            ...
            double segmentLen =
               rep->extrapolateToPlane(stateToExtrapolate, fitterInfo->getReferenceState()->getPlane(), false, true);
            if (debugLvl_ > 0) {
         ...
         newRefState = true;
         changedSmthg = true;
         referenceState = new ReferenceStateOnPlane(stateToExtrapolate->getState(), stateToExtrapolate->getPlane(),
                                                    stateToExtrapolate->getRep(), stateToExtrapolate->getAuxInfo());
         referenceState->setForwardSegmentLength(segmentLen);
         referenceState->setForwardTransportMatrix(FTransportMatrix_);
         referenceState->setForwardNoiseMatrix(FNoiseMatrix_);
         referenceState->setForwardDeltaState(forwardDeltaState_);
...
void KalmanFitterRefTrack::removeForwardBackwardInfo(Track *tr, const AbsTrackRep *rep, int notChangedUntil,
...
void KalmanFitterRefTrack::processTrackPoint(KalmanFitterInfo *fi, const KalmanFitterInfo *prevFi, const TrackPoint *tp,
                                             double &chi2, double &ndf, int direction)
{
   ...
   if (prevFi != NULL) {
      ...
      C_.Similarity(F);
      C_ += N;
      fi->setPrediction(new MeasuredStateOnPlane(p_, C_, fi->getReferenceState()->getPlane(),
      ...
      if (fi->hasPrediction(direction)) {
         ...
         MeasuredStateOnPlane mop(p_, dummy, fi->getReferenceState()->getPlane(), rep,
                                  fi->getReferenceState()->getAuxInfo());
         TVector3 pos, mom;
   ...
   KalmanFittedStateOnPlane *upState =
      new KalmanFittedStateOnPlane(p_, C_, fi->getReferenceState()->getPlane(), fi->getReferenceState()->getRep(),
                                   fi->getReferenceState()->getAuxInfo(), chi2inc, ndfInc);
