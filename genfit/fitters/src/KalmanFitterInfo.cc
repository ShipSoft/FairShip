...
KalmanFitterInfo::KalmanFitterInfo() : AbsFitterInfo(), fixWeights_(false)
{
   ;
}
...
MeasurementOnPlane
KalmanFitterInfo::getResidual(unsigned int iMeasurement, bool biased, bool onlyMeasurementErrors) const
{
   ...
   if (smoothedState.getRep() != measurement->getRep()) {
      Exception e("KalmanFitterInfo::getResidual: smoothedState and measurement are not defined wrt the same TrackRep.",
                  __LINE__, __FILE__);
      throw e;
   }
   ...
   if (onlyMeasurementErrors) {
      return MeasurementOnPlane(res, measurement->getCov(), plane, smoothedState.getRep(), H->clone(),
...
void KalmanFitterInfo::setWeights(const std::vector<double> &weights)
{
   ...
   if (weights.size() != getNumMeasurements()) {
      Exception e("KalmanFitterInfo::setWeights: weights do not have the same size as measurementsOnPlane", __LINE__,
                  __FILE__);
      throw e;
   }
...
bool KalmanFitterInfo::checkConsistency() const
{
   ...
   if (measurementsOnPlane_.size() > 1) {
      ...
      for (unsigned int i = 1; i < measurementsOnPlane_.size(); ++i) {
         if (measurementsOnPlane_[i]->getState().GetNrows() != dim) {
            std::cerr
               << "KalmanFitterInfo::checkConsistency(): measurementsOnPlane_ do not all have the same dimensionality"
               << std::endl;
            retVal = false;
         }
      }
   ...
   if (referenceState_) {
      ...
      if (referenceState_->getRep() != rep_) {
         std::cerr << "KalmanFitterInfo::checkConsistency(): referenceState_ is not defined with the correct TrackRep"
                   << std::endl;
         retVal = false;
      }
   ...
   if (forwardPrediction_) {
      if (forwardPrediction_->getPlane() != plane) {
         std::cerr << "KalmanFitterInfo::checkConsistency(): forwardPrediction_ is not defined with the correct plane"
                   << std::endl;
         retVal = false;
      }
   ...
   if (backwardPrediction_) {
      if (backwardPrediction_->getPlane() != plane) {
         std::cerr << "KalmanFitterInfo::checkConsistency(): backwardPrediction_ is not defined with the correct plane"
                   << std::endl;
         retVal = false;
      }
   ...
   if (backwardUpdate_) {
      ...
      if (backwardUpdate_->getRep() != rep_) {
         std::cerr << "KalmanFitterInfo::checkConsistency(): backwardUpdate_ is not defined with the correct TrackRep"
                   << std::endl;
         retVal = false;
      }
   ...
   for (std::vector<MeasurementOnPlane *>::const_iterator it = measurementsOnPlane_.begin();
        it != measurementsOnPlane_.end(); ++it) {
