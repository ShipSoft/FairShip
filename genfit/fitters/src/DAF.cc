...
void DAF::setBetas(double b1, double b2, double b3, double b4, double b5, double b6, double b7, double b8, double b9,
                   double b10)
{
   betas_.clear();
   assert(b1 > 0);
   ...
   betas_.resize(maxIterations_,
                 betas_.back()); // make sure main loop has a maximum of maxIterations_ and also make sure the last beta
                                 // value is used for if more iterations are needed then the ones set by the user.
}
...
void DAF::setAnnealingScheme(double bStart, double bFinal, unsigned int nSteps)
{
   ...
   betas_.resize(maxIterations_,
                 betas_.back()); // make sure main loop has a maximum of 10 iterations and also make sure the last beta
                                 // value is used for if more iterations are needed then the ones set by the user.
   ...
   for (std::vector<TrackPoint *>::const_iterator tp = trackPoints.begin(); tp != trackPoints.end(); ++tp) {
      ...
      for (unsigned int j = 0; j < nMeas; j++) {
         ...
         try {
            ...
            assert(cutVal > 1.E-6);
            // the following assumes that in the competing hits could have different V otherwise calculation could be
            // simplified
