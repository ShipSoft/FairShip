...
bool KalmanFitter::fitTrack(Track *tr, const AbsTrackRep *rep, double &chi2, double &ndf, int startId, int endId,
                            int &nFailedHits)
{
   ...
   if (multipleMeasurementHandling_ == unweightedClosestToReference ||
      ...
      Exception exc(
         "KalmanFitter::fitTrack ==> cannot use (un)weightedClosestToReference(Wire) as multiple measurement handling.",
         __LINE__, __FILE__);
      exc.setFatal();
      throw exc;
   }
