...
TrackPoint::TrackPoint() : sortingParameter_(0), track_(NULL), thinScatterer_(NULL)
{
   ;
}

TrackPoint::TrackPoint(Track *track) : sortingParameter_(0), track_(track), thinScatterer_(NULL)
{
   ;
}
...
TrackPoint::TrackPoint(const TrackPoint &rhs)
   : TObject(rhs), sortingParameter_(rhs.sortingParameter_), track_(rhs.track_), thinScatterer_(NULL)
{
   // clone rawMeasurements
   for (std::vector<AbsMeasurement *>::const_iterator it = rhs.rawMeasurements_.begin();
        it != rhs.rawMeasurements_.end(); ++it) {
