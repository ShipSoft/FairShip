...
DetPlane::DetPlane(AbsFinitePlane *finite) : finitePlane_(finite)
{
   // default constructor
   o_.SetXYZ(0., 0., 0.);
   u_.SetXYZ(1., 0., 0.);
...
DetPlane::DetPlane(const TVector3 &o, const TVector3 &n, AbsFinitePlane *finite) : o_(o), finitePlane_(finite)
{
   setNormal(n);
}
...
DetPlane::DetPlane(const DetPlane &rhs) : TObject(rhs), o_(rhs.o_), u_(rhs.u_), v_(rhs.v_)
{
   if (rhs.finitePlane_)
...
{
   std::cout << "DetPlane: " << "O(" << o_.X() << ", " << o_.Y() << ", " << o_.Z() << ") " << "u(" << u_.X() << ", "
             << u_.Y() << ", " << u_.Z() << ") " << "v(" << v_.X() << ", " << v_.Y() << ", " << v_.Z() << ") " << "n("
             << getNormal().X() << ", " << getNormal().Y() << ", " << getNormal().Z() << ") " << std::endl;
...
void DetPlane::straightLineToPlane(const double &posX, const double &posY, const double &posZ, const double &dirX,
