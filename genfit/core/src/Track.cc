...
Track::Track() : cardinalRep_(0), fitStatuses_(), stateSeed_(6), covSeed_(6)
{
   ;
}
...
Track::Track(AbsTrackRep *trackRep, const TVectorD &stateSeed)
   : cardinalRep_(0), fitStatuses_(), stateSeed_(stateSeed), covSeed_(TMatrixDSym::kUnit, TMatrixDSym(6))
{
   addTrackRep(trackRep);
}

Track::Track(AbsTrackRep *trackRep, const TVector3 &posSeed, const TVector3 &momSeed)
   : cardinalRep_(0), fitStatuses_(), stateSeed_(6), covSeed_(TMatrixDSym::kUnit, TMatrixDSym(6))
{
   addTrackRep(trackRep);
...
Track::Track(const Track &rhs)
   : TObject(rhs), cardinalRep_(rhs.cardinalRep_), stateSeed_(rhs.stateSeed_), covSeed_(rhs.covSeed_)
{
...
TrackPoint *Track::getPointWithMeasurementAndFitterInfo(int id, const AbsTrackRep *rep) const
{
   ...
   for (std::vector<TrackPoint *>::const_iterator it = trackPointsWithMeasurement_.begin();
        it != trackPointsWithMeasurement_.end(); ++it) {
...
void Track::mergeTrack(const Track *other, int id)
{
   ...
   for (std::vector<AbsTrackRep *>::const_iterator otherRep = other->trackReps_.begin();
        otherRep != other->trackReps_.end(); ++otherRep) {
      bool found(false);
   ...
   for (std::vector<TrackPoint *>::const_iterator otherTp = other->trackPoints_.begin();
        otherTp != other->trackPoints_.end(); ++otherTp) {
...
void Track::deleteForwardInfo(int startId, int endId, const AbsTrackRep *rep)
{
   ...
   for (std::vector<TrackPoint *>::const_iterator pointIt = trackPoints_.begin() + startId;
        pointIt != trackPoints_.begin() + endId; ++pointIt) {
      if (rep != NULL) {
         ...
         for (std::vector<AbsFitterInfo *>::const_iterator fitterInfoIt = fitterInfos.begin();
              fitterInfoIt != fitterInfos.end(); ++fitterInfoIt) {
...
void Track::Print(const Option_t *option) const
{
   ...
   if (opt.Contains("C")) { // compact
      ...
      for (unsigned int i = 0; i < trackPoints_.size(); ++i) {
         ...
         switch (color) {
         case 0: std::cout << "\033[1;30m"; break;
         case 1: std::cout << "\033[0;34m"; break;
         case 2: std::cout << "\033[1;34m"; break;
         case 3: std::cout << "\033[0;32m"; break;
         case 4: std::cout << "\033[1;32m"; break;
         case 5: std::cout << "\033[0;36m"; break;
         case 6: std::cout << "\033[1;36m"; break;
         case 7: std::cout << "\033[0;31m"; break;
         case 8: std::cout << "\033[1;31m"; break;
         case 9: std::cout << "\033[0;35m"; break;
         case 10: std::cout << "\033[1;35m"; break;
         case 11: std::cout << "\033[0;33m"; break;
         case 12: std::cout << "\033[1;33m"; break;
         case 13: std::cout << "\033[0;37m"; break;
         default:;
         }
            ...
            if (prevFis[(*fi)->getRep()] != NULL && static_cast<KalmanFitterInfo *>(*fi)->hasReferenceState() &&
               ...
               if (fabs(prevLen + len) > 1E-10) {
                  std::cerr << "Track::checkConsistency(): segment lengths of reference states for rep "
                            << (*fi)->getRep() << " (id " << getIdForRep((*fi)->getRep()) << ") at TrackPoint " << (*tp)
