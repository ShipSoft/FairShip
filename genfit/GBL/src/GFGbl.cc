...
bool writeHistoDataForLabel(double label, TVectorD res, TVectorD measErr, TVectorD resErr, TVectorD downWeights,
                            TVectorD localPar, TMatrixDSym localCov)
{
   if (label < 1.)
      return false;
...
GFGbl::GFGbl()
   : AbsFitter(), m_milleFileName("millefile.dat"), m_gblInternalIterations("THC"), m_pValueCut(0.), m_minNdf(1),
     m_chi2Cut(0.), m_enableScatterers(true), m_enableIntermediateScatterer(true)
{
}
...
void GFGbl::beginRun()
{
   ...
   stats = new TH1I("stats", "0: NDF>0 | 1: fTel&VXD | 2: all | 3: VXD | 4: SVD | 5: all - PXD | 6: fTel&SVD | 7: bTel",
                    10, 0, 10);
...
void GFGbl::processTrackWithRep(Track *trk, const AbsTrackRep *rep, bool resortHits)
{
   ...
   for (int ipoint_meas = 0; ipoint_meas < npoints_meas; ipoint_meas++) {
      ...
      if (!fi) {
         cout
            << " ERROR: KalmanFitterInfo (with reference state) for measurement does not exist. Track will be skipped."
            << endl;
         return;
      }
      ...
      if (point_meas->getRawMeasurement(0)->getDim() != 2 &&
         ...
         if (raw_meas1->constructHMatrix(rep)->isEqual(genfit::HMatrixU()) &&
            ...
            cout << " ERROR: Incompatible 1D measurements at meas. point " << ipoint_meas << ". Track will be skipped."
                 << endl;
            return;
         }
      ...
      try {
         ...
         if (ipoint_meas < npoints_meas - 1) {
            ...
            rep->extrapolateToPlane(refCopy, trk->getPoint(ipoint_meas + 1)->getFitterInfo(rep)->getPlane(), false,
                                    false);
            getScattererFromMatList(trackLen, scatTheta, scatSMean, scatDeltaS, trackMomMag, particleMass,
                                    particleCharge, rep->getSteps());
            ...
            if (s2 < 1.e-4 || s2 >= trackLen - 1.e-4 || s2 <= 1.e-4) {
               cout << " WARNING: GBL points will be too close. GBLTrajectory construction might fail. Let's try it..."
                    << endl;
            }
         }
         ...
         if (ipoint_meas < npoints_meas - 1) {
            if (theta2 > scatEpsilon) {
               ...
               if (0. > nextStep) {
                  cout << " ERROR: The extrapolation to measurement point " << (ipoint_meas + 2) << " stepped back by "
                       << nextStep << "cm !!! Track will be skipped." << endl;
                  return;
               }
               rep->getForwardJacobianAndNoise(jacPointToPoint, noise, deltaState);
               ...
               rep->extrapolateToPlane(*reference, trk->getPoint(ipoint_meas + 1)->getFitterInfo(rep)->getPlane(),
                                       false, true);
...
#ifdef OUTPUT
            if (!writeHistoDataForLabel(listOfLayers.at(p), residuals, measErrors, resErrors, downWeights, localPar,
                                        localCov))
               return;
#endif
