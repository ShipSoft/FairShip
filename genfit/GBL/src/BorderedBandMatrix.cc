 ...
 */
void BorderedBandMatrix::addBlockMatrix(double aWeight, const std::vector<unsigned int> *anIndex,
      ...
      for (int j = std::max(0, i - nBand); j <= i; ++j) {
         ...
         for (int l = 0; l < nBorder; ++l) { // diagonal
            ...
            for (int k = 0; k < l; ++k) { // off diagonal
               sum += anArray(i, l) * aSymArray(l, k) * anArray(j, k) + anArray(i, k) * aSymArray(l, k) * anArray(j, l);
            }
         }
         aBand(i - j, j) = sum;
      }
