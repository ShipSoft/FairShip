...
class GblTrajectory {
public:
   GblTrajectory(const std::vector<GblPoint> &aPointList, bool flagCurv = true, bool flagU1dir = true,
                 bool flagU2dir = true);
   GblTrajectory(const std::vector<GblPoint> &aPointList, unsigned int aLabel, const TMatrixDSym &aSeed,
   ...
   unsigned int getMeasResults(unsigned int aLabel, unsigned int &numRes, TVectorD &aResiduals, TVectorD &aMeasErrors,
                               TVectorD &aResErrors, TVectorD &aDownWeights);
   unsigned int getScatResults(unsigned int aLabel, unsigned int &numRes, TVectorD &aResiduals, TVectorD &aMeasErrors,
                               TVectorD &aResErrors, TVectorD &aDownWeights);
...
private:
   ...
   void getFitToLocalJacobian(std::vector<unsigned int> &anIndex, SMatrix55 &aJacobian, const GblPoint &aPoint,
                              unsigned int measDim, unsigned int nJacobian = 1) const;
   ...
   void
   getResAndErr(unsigned int aData, double &aResidual, double &aMeadsError, double &aResError, double &aDownWeight);
};
} // namespace gbl
