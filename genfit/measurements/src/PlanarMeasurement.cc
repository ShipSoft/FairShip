...
PlanarMeasurement::PlanarMeasurement(int nDim) : AbsMeasurement(nDim), physicalPlane_(), planeId_(-1), stripV_(false)
{
   assert(nDim >= 1);
}
...
const AbsHMatrix *PlanarMeasurement::constructHMatrix(const AbsTrackRep *rep) const
{
   ...
   case 2: return new HMatrixUV();

   default:
      Exception exc("PlanarMeasurement default implementation can only handle 1D (strip) or 2D (pixel) measurements!",
                    __LINE__, __FILE__);
      throw exc;
   }
}
