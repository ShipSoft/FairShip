...
WirePointMeasurement::WirePointMeasurement(int nDim) : WireMeasurement(nDim)
{
   assert(nDim >= 8);
}

WirePointMeasurement::WirePointMeasurement(const TVectorD &rawHitCoords, const TMatrixDSym &rawHitCov, int detId,
...
{
   MeasurementOnPlane *mopR = new MeasurementOnPlane(TVectorD(2), TMatrixDSym(2), state.getPlane(), state.getRep(),
