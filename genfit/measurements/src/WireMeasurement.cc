...
WireMeasurement::WireMeasurement(int nDim) : AbsMeasurement(nDim), maxDistance_(2), leftRight_(0)
{
   assert(nDim >= 7);
}
...
std::vector<MeasurementOnPlane *> WireMeasurement::constructMeasurementsOnPlane(const StateOnPlane &state) const
{
   ...
   MeasurementOnPlane *mopL = new MeasurementOnPlane(TVectorD(1, &mL), TMatrixDSym(1, &V), state.getPlane(),
                                                     state.getRep(), constructHMatrix(state.getRep()));

   MeasurementOnPlane *mopR = new MeasurementOnPlane(TVectorD(1, &mR), TMatrixDSym(1, &V), state.getPlane(),
                                                     state.getRep(), constructHMatrix(state.getRep()));
