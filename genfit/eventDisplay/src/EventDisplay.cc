...
EventDisplay::EventDisplay()
   : errorScale_(1.), drawGeometry_(false), drawDetectors_(true), drawHits_(true), drawErrors_(true), drawPlanes_(true),
     drawTrackMarkers_(true), drawTrack_(true), drawRefTrack_(true), drawForward_(true), drawBackward_(true),
     drawAutoScale_(true), drawScaleMan_(false), drawSilent_(false), drawCardinalRep_(true), repId_(0),
     drawAllTracks_(true), trackId_(0), refit_(false), debugLvl_(0), fitterId_(SimpleKalman),
     mmHandling_(weightedAverage), squareRootFormalism_(false), dPVal_(1.E-3), dRelChi2_(0.2), nMinIter_(2),
     nMaxIter_(4), nMaxFailed_(-1), resort_(false)
{
...
void EventDisplay::setErrScale(double errScale)
{
   errorScale_ = errScale;
}

double EventDisplay::getErrScale()
{
   return errorScale_;
}
   ...
   for (unsigned int i = 0; i < events_.at(id)->size(); i++) { // loop over all tracks in an event
      ...
      if (refit_) {
         ...
         case DafSimple:
            ...
            break;
         case DafRef: fitter.reset(new DAF()); break;
         }
      ...
      for (unsigned int j = 0; j < numhits; j++) { // loop over all hits in the track
         ...
         for (unsigned int iMeas = 0; iMeas < fi->getNumMeasurements(); ++iMeas) {
            ...
            if (j > 0 && prevFi != NULL) {
               if (drawTrack_) {
                  makeLines(prevFittedState, fittedState, rep, charge > 0 ? kRed : kBlue, 1, drawTrackMarkers_,
                            drawErrors_, 3);
                  if (drawErrors_) { // make sure to draw errors in both directions
                     makeLines(prevFittedState, fittedState, rep, charge > 0 ? kRed : kBlue, 1, false, drawErrors_, 0,
                               0);
                  }
               }
               if (drawForward_)
                  makeLines(prevFi->getForwardUpdate(), fi->getForwardPrediction(), rep, kCyan, 1, drawTrackMarkers_,
                            drawErrors_, 1, 0);
               if (drawBackward_)
                  makeLines(prevFi->getBackwardPrediction(), fi->getBackwardUpdate(), rep, kMagenta, 1,
               ...
               if (drawRefTrack_ && fi->hasReferenceState() && prevFi->hasReferenceState())
                  makeLines(prevFi->getReferenceState(), fi->getReferenceState(), rep,
                            charge > 0 ? kRed + 2 : kBlue + 2, 2, drawTrackMarkers_, false, 3);
            ...
            if (drawDetectors_) {
               ...
               if (wire_hit) {
                  ...
                  det_shape->SetShape(
                     new TGeoTube(std::max(0., (double)(hit_u - 0.0105 / 2.)), hit_u + 0.0105 / 2., plane_size));
                  ...
                  TVector3 move =
                     v * (v * (track_pos - o)); // move the tube along the wire until the track goes through it
            ...
            if (drawHits_) {
               ...
               if (full_hit) {
                  ...
                  StateOnPlane dummy2(TVectorD(rep->getDim()),
                                      static_cast<const FullMeasurement *>(m)->constructPlane(dummy), rep);
               ...
               if (planar_hit) {
                  if (!planar_pixel_hit) {
                     ...
                     hit_box = boxCreator((o + move + hit_u * stripDir3), stripDir3, stripDir3perp,
                     ...
                     if (drawAutoScale_) {
                        ...
                        if (min_cov < 1e-5) {
                           std::cout << "Track " << i << ", Hit " << j
                                     << ": Invalid covariance matrix (Eigenvalue < 1e-5), autoscaling not possible!"
                                     << std::endl;
                        } else {
                           if (min_cov < 0.049) {
               ...
               if (space_hit) {
                  {
                     ...
                     TGeoRotation det_rot("det_rot", eVec1.Theta() * radDeg, eVec1.Phi() * radDeg,
                                          eVec2.Theta() * radDeg, eVec2.Phi() * radDeg, eVec3.Theta() * radDeg,
                                          eVec3.Phi() * radDeg);
                     ...
                     if (!det_rot.IsValid()) {
                        ...
                        det_rot.SetAngles(eVec1.Theta() * radDeg, eVec1.Phi() * radDeg, eVec2.Theta() * radDeg,
                                          eVec2.Phi() * radDeg, eVec3.Theta() * radDeg, eVec3.Phi() * radDeg);
                     }
...

                  {
                     ...
                     TGeoRotation det_rot("det_rot", u_semiaxis.Theta() * radDeg, u_semiaxis.Phi() * radDeg,
                                          v_semiaxis.Theta() * radDeg, v_semiaxis.Phi() * radDeg, norm.Theta() * radDeg,
                                          norm.Phi() * radDeg);
                     /*if (! det_rot.IsValid()){
                       u_semiaxis.Print();
                       v_semiaxis.Print();
               ...
               if (wire_hit) {
                  ...
                  if (drawAutoScale_) {
                     if (pseudo_res_0 < 1e-5) {
                        ...
                        if (pseudo_res_0 < 0.0049) {
                           ...
                           std::cout << "Track " << i << ", Hit " << j << ": Wire covariance too small, rescaling by "
                                     << cor;
                           errorScale_ *= cor;
                           pseudo_res_0 *= cor;
                           std::cout << " to " << errorScale_ << std::endl;
                  ...
                  hit_box = boxCreator((o + move + hit_u * u), u, v, errorScale_ * std::sqrt(hit_cov(0, 0)),
                                       pseudo_res_1, 0.0105);
                  hit_box->SetMainColor(kYellow);
                  hit_box->SetMainTransparency(0);
                  gEve->AddElement(hit_box);
...
void EventDisplay::makeLines(const StateOnPlane *prevState, const StateOnPlane *state, const AbsTrackRep *rep,
                             const Color_t &color, const Style_t &style, bool drawMarkers, bool drawErrors,
                             double lineWidth, int markerPos)
{
   ...
   lineSet->AddLine(intermediate1(0), intermediate1(1), intermediate1(2), intermediate2(0), intermediate2(1),
                    intermediate2(2));
...
void EventDisplay::makeGui()
{
   ...
   TGHorizontalFrame *hf = new TGHorizontalFrame(frmMain);
   {
      ...
      hf->AddFrame(lbl);
      guiEvent = new TGNumberEntry(hf, 0, 9, 999, TGNumberFormat::kNESInteger, TGNumberFormat::kNEANonNegative,
   ...
   hf = new TGHorizontalFrame(frmMain);
   {
      guiErrorScale_ = new TGNumberEntry(hf, errorScale_, 6, 999, TGNumberFormat::kNESReal,
                                         TGNumberFormat::kNEANonNegative, TGNumberFormat::kNELLimitMinMax, 1.E-4, 1.E5);
   ...
   hf = new TGHorizontalFrame(frmMain);
   {
      guiRepId_ = new TGNumberEntry(hf, repId_, 6, 999, TGNumberFormat::kNESInteger, TGNumberFormat::kNEANonNegative,
   ...
   hf = new TGHorizontalFrame(frmMain2);
   {
      ...
      lbl = new TGLabel(hf, "Go to event: ");
      hf->AddFrame(lbl);
      guiEvent2 = new TGNumberEntry(hf, 0, 9, 999, TGNumberFormat::kNESInteger, TGNumberFormat::kNEANonNegative,
                                    TGNumberFormat::kNELLimitMinMax, 0, 99999);
   ...
   hf = new TGHorizontalFrame(frmMain2);
   {
      guiDebugLvl_ = new TGNumberEntry(hf, debugLvl_, 6, 999, TGNumberFormat::kNESInteger,
                                       TGNumberFormat::kNEANonNegative, TGNumberFormat::kNELLimitMinMax, 0, 999);
   ...
   hf = new TGHorizontalFrame(frmMain2);
   {
      guiDPVal_ = new TGNumberEntry(hf, dPVal_, 6, 9999, TGNumberFormat::kNESReal, TGNumberFormat::kNEANonNegative,
   ...
   hf = new TGHorizontalFrame(frmMain2);
   {
      guiRelChi2_ = new TGNumberEntry(hf, dRelChi2_, 6, 9999, TGNumberFormat::kNESReal, TGNumberFormat::kNEANonNegative,
                                      TGNumberFormat::kNELLimitMinMax, 0, 999);
   ...
   hf = new TGHorizontalFrame(frmMain2);
   {
      guiNMinIter_ = new TGNumberEntry(hf, nMinIter_, 6, 999, TGNumberFormat::kNESInteger,
                                       TGNumberFormat::kNEANonNegative, TGNumberFormat::kNELLimitMinMax, 1, 100);
