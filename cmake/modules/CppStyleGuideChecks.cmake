...
set(CHANGED_SRC_FILES_ORIGIN_MASTER
    "${BUILD_SUPPORT_DIR}/get-changed-src-files-origin-master.sh")
...
add_executable(clang-tidy-header-helper EXCLUDE_FROM_ALL
               ${clang_tidy_header_helper})

# -------------------- "make format*", "make show-format*" and "make
...
function(add_clang_format_target make_target_id get_files_cmd)
  if(${CLANG_FORMAT_FOUND})
    add_custom_target(
      ${make_target_id}
      COMMAND
        ${BUILD_SUPPORT_DIR}/run-clang-format.sh ${CMAKE_CURRENT_SOURCE_DIR}
        ${CLANG_FORMAT_BIN} 1 `${get_files_cmd} ${PROJECT_SOURCE_DIR}`
    ...
    add_custom_target(
      ...
      COMMAND
        ${BUILD_SUPPORT_DIR}/run-clang-format.sh ${CMAKE_CURRENT_SOURCE_DIR}
        ${CLANG_FORMAT_BIN} 2 `${get_files_cmd} ${PROJECT_SOURCE_DIR}`
      WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
      COMMENT "Run clang-format on selected files and display differences")

    add_custom_target(
      check-${make_target_id}
      COMMAND
        ${BUILD_SUPPORT_DIR}/run-clang-format.sh ${CMAKE_CURRENT_SOURCE_DIR}
        ${CLANG_FORMAT_BIN} 0 `${get_files_cmd} ${PROJECT_SOURCE_DIR}`
      WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
      COMMENT
...
# -------------------- "make tidy*", "make show-tidy*" and "make check-tidy*"
# targets ------------
function(add_clang_tidy_target make_target_id get_files_cmd)
  if(${CLANG_TIDY_FOUND})
    add_custom_target(
      ${make_target_id}
      COMMAND
        ${BUILD_SUPPORT_DIR}/run-clang-tidy.sh ${CLANG_TIDY_BIN}
        ${clang_tidy_header_helper} ${CMAKE_BINARY_DIR}/compile_commands.json 1
        `${get_files_cmd} ${PROJECT_SOURCE_DIR} | grep -v -F -f
        ${PROJECT_SOURCE_DIR}/.clang-tidy-ignore`
      WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
      COMMENT
    ...
    add_custom_target(
      ...
      COMMAND
        ${BUILD_SUPPORT_DIR}/run-clang-tidy.sh ${CLANG_TIDY_BIN}
        ${clang_tidy_header_helper} ${CMAKE_BINARY_DIR}/compile_commands.json 2
        `${get_files_cmd} ${PROJECT_SOURCE_DIR} | grep -v -F -f
        ${PROJECT_SOURCE_DIR}/.clang-tidy-ignore`
      WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
    ...
    add_custom_target(
      ...
      COMMAND
        ${BUILD_SUPPORT_DIR}/run-clang-tidy.sh ${CLANG_TIDY_BIN}
        ${clang_tidy_header_helper} ${CMAKE_BINARY_DIR}/compile_commands.json 0
        `${get_files_cmd} ${PROJECT_SOURCE_DIR} | grep -v -F -f
        ${PROJECT_SOURCE_DIR}/.clang-tidy-ignore`
      WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
...
function(add_cpplint_target make_target_id get_files_cmd)
  add_custom_target(
    ${make_target_id}
    COMMAND ${BUILD_SUPPORT_DIR}/cpplint/runCppLint.sh `${get_files_cmd}
            ${PROJECT_SOURCE_DIR}`
    WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
...
if(GIT_FOUND)
  ...
  add_custom_target(
    ...
    COMMENT
      "fix-submission will attempt to fix the reported issues using clang-format and clang-tidy.
