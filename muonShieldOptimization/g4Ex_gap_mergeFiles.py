...
Yandex3 = (
    True  # spring 2016 production, 10B, Ecut>10GeV, Mo/W/ target, record start of track
)
...
if Yandex2:
    stats = {10.0: [1e10]}
    files = {10.0: ["$SHIPSOFT/data/result_10Bn_ecut_10.root"]}
    fnew = "pythia8_Geant4_total_Yandex2.root"
if Yandex3:
    stats = {10.0: [10.05e9]}
    files = {
        10.0: [
            "/media/truf/Elements/Data/HNL/ShipSoft/data/Yandex-April2016_10.05B.root"
        ]
    }
...
if JPsi:
    ...
    files = {10.0: []}
    jobs = {
        "b6276c506a_g4Ex_gap_": ["53", "54", "55", "56"],
        "b602f257b0_g4Ex_gap_": ["52"],
    }
    nevts = 0
    ntot = 0
    tag = "#events="
...
if MoTarget:
    ...
    files = {}
    jobs = {
        "b64a4b817c.cern.ch_g4Ex_gap_": ["60", "61", "62", "65", "66"],
        "b63edb1b12_g4Ex_gap_": [
            "63",
            "75",
            "77",
            "78",
        ...
        "b65dfad864_g4Ex_gap_": ["70"],
        "b6276c506a_g4Ex_gap_": ["71"],
        "b6e72959f6_g4Ex_gap_": ["72"],
        "b6429e4f38.cern.ch_g4Ex_gap_": [
            "73",
            "74",
            "76",
            "79",
...
def makeFinalNtuples(norm=5.0e13, opt=""):
    cuts = {
        "": "",
    ...
    for ecut in stats:
        for i in range(len(stats[ecut])):
            ...
            if first:
                ...
                if opt != "":
                    fxx = fxx.replace("_total", "")
                h["N"] = ROOT.TFile(fxx, "RECREATE")
                print("new file created", fxx)
                if afterHadronAbsorber:
                    h["ntuple"] = ROOT.TNtuple(
                        "pythia8-Geant4", "flux after 3m hadron absorber", tuples
                    )
                else:
                    h["ntuple"] = ROOT.TNtuple(
                        "pythia8-Geant4",
                        "flux after 3m hadron absorber, position&momentum before",
                        tuples,
                    )
            gROOT.cd()
            t.SetEventList(0)
            ...
            for iev in range(nev):
                ...
                if Yandex:
                    if Ekin < ecut:
                        continue
                    if Ekin > 5.0:
                        weight = norm / (ntot[5.0] + ntot[0.5])
                    else:
                        weight = norm / (ntot[0.5])
                if Yandex2 or Yandex3:
                    if Ekin < ecut:
                        continue
                    weight = norm / (ntot[10.0])
                if JPsi:
                    weight = norm / (ntot[10.0])
                if Tau:
                    weight = norm / (ntot[0.0])
                if MoTarget:
                    if Ekin < ecut:
                        continue
                    if Ekin > 50.0:
                        weight = norm / (
                            ntot[0.5] + ntot[10.0] + ntot[20.0] + ntot[50.0]
                        )
                    elif Ekin > 20.0:
                        weight = norm / (ntot[0.5] + ntot[10.0] + ntot[20.0])
                    elif Ekin > 10.0:
                        weight = norm / (ntot[0.5] + ntot[10.0])
                    else:
                        weight = norm / (ntot[0.5])
                vlist.append(weight)
...
makeFinalNtuples(norm=5.0e13, opt="")
makeFinalNtuples(norm=5.0e13, opt="_onlyMuons")
makeFinalNtuples(norm=5.0e13, opt="_onlyNeutrinos")
