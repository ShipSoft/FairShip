...
exitHadronAbsorber::exitHadronAbsorber()
   : FairDetector("exitHadronAbsorber", kTRUE, kVETO), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.),
     fLength(-1.), fOnlyMuons(kFALSE), fSkipNeutrinos(kFALSE), fzPos(3E8), withNtuple(kFALSE),
   ...
   if (gMC->IsTrackEntering()) {
      ...
      if (!(fOnlyMuons && TMath::Abs(pdgCode) != 13)) {
         ...
         if ((fMom.E() - fMom.M()) > EMax) {
            AddHit(fTrackID, 111, TVector3(fPos.X(), fPos.Y(), fPos.Z()), TVector3(fMom.Px(), fMom.Py(), fMom.Pz()),
                   fTime, fLength, 0, pdgCode, TVector3(p->Vx(), p->Vy(), p->Vz()),
...
vetoPoint *exitHadronAbsorber::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time,
                                      Double_t length, Double_t eLoss, Int_t pdgCode, TVector3 Lpos, TVector3 Lmom)
{
