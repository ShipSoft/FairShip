...
def init():
    ...
    ap.add_argument(
        "-W", "--FullTungsten", action="store_true", dest="fullTungsten", default=False
    )
    ap.add_argument(
        "-o",
        "--output",
...
if JpsiMainly:
    myPythia.ReadString(
        "443:new  J/psi  J/psi  4   0   0    3.09692    0.00009    3.09602    3.09782  0.   1   1   0   1   0"
    )
...
myPythia.Initialize(2212, 2212, 400.0, 0.0)  # required to hack TPythia8 in root !
...
if withNtuple:
    f = ROOT.TFile.Open(
        "pythia8_Geant4_" + str(runnr) + "_" + str(ecut) + ".root", "RECREATE"
    )
    h["ntuple"] = ROOT.TNtuple(
        "pythia8-Geant4",
        "muon/nu flux",
        "id:px:py:pz:x:y:z:opx:opy:opz:ox:oy:oz:pythiaid:parentid",
    )
...
class MyGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    pos = G4ThreeVector(0 * cm, 0 * cm, -world_r / 2.0)
    ...
    def GeneratePrimaries(self, anEvent):
        ...
        if particleGun:
            ...
            ztarget = G4ThreeVector(0 * cm, 0 * cm, -50.0 * m)
            vertex = G4PrimaryVertex(ztarget, 0.0)
            ...
            while npart == 0:
                ...
                if fullTungsten:
                    z = rnr.Exp(10 * cm) - 50.0 * m  # tungsten interaction length
                else:
                    z = (
                        rnr.Exp(16 * cm) - 50.0 * m
                    )  # Mo/H20/W average interaction length
                ...
                for p in particles:
                    ...
                    if JpsiMainly and abs(pid) != 13:
                        continue
                    if tauOnly and abs(pid) != 16:
                        continue
                    if JpsiMainly and mother.GetPdgCode() != 443:
                        continue
                    if tauOnly:
                        mmkey = mother.GetMother(0) + 1
                        mmother = myPythia.GetParticle(mmkey)
                        if (
                            abs(mother.GetPdgCode()) != 431
                            and abs(mmother.GetPdgCode()) != 431
                        ):
                            myPythia.EventListing()
                            continue
                    qed = (
                        pid in qedlist
                    )  # use cut only for photons, leptons/neutrinos, protons and neutrons
                    p.Momentum(v)
                    Ekin = v.E() - v.M()
                    if Ekin * GeV < ecut and (qed or allPart):
                        continue
                    G4particle = G4PrimaryParticle(pid)
                    G4particle.Set4Momentum(
                        v.Px() * GeV, v.Py() * GeV, v.Pz() * GeV, v.E() * GeV
                    )
                    # store mother ID
        ...
        if debug and not particleGun:
            print("new event at ", ztarget.z / m)
...
class MyRunAction(G4UserRunAction):
    ...
    def EndOfRunAction(self, run):
        global debug, nevTot
        print("*** End of Run")
        print("- Run summary : (id= %d, #events= %d)" % (run.GetRunID(), nevTot))
        h["ntuple"].Write()
...
class MyTrackingAction(G4UserTrackingAction):
    ...
    def PreUserTrackingAction(self, atrack):
        ...
        qed = (
            pid in qedlist
        )  # use cut only for photons, electrons, protons and neutrons
...
class MyTrackingActionD(G4UserTrackingAction):
    ...
    def PreUserTrackingAction(self, atrack):
        ...
        if pdg.GetParticle(pid):
            tid = pdg.GetParticle(pid).GetName()
        mom = atrack.GetMomentum()
        p = ROOT.TMath.Sqrt(mom.x * mom.x + mom.y * mom.y + mom.z * mom.z)
        if debug and abs(pid) == 13:
            print(
                "track",
                atrack.GetTrackID(),
                tid,
...
def ConstructGeom():
    ...
    g4py.ezgeom.ResizeWorld(world_r / 2.0, world_r / 2.0, world_r)
    ...
    if fullTungsten:
        ...
        target.CreateTubeVolume(tungsten, 0.0, 25.0 * cm, targetDZ)
        ...
        slit.CreateTubeVolume(water, 0.0, 25.0 * cm, slitDZ)
        ...
        for i in range(4):
            targetPhys.append(
                target.PlaceIt(
                    G4ThreeVector(0.0, 0.0, z0Pos + targetL + targetDZ), 1, snoopy
                )
            )
            slitPhys.append(
                slit.PlaceIt(
                    G4ThreeVector(0.0, 0.0, z0Pos + targetL + 2 * targetDZ + slitDZ),
                    1,
                    snoopy,
                )
            )
        ...
        moreShieldingPhys = moreShielding.PlaceIt(
            G4ThreeVector(0.0, 0.0, z0Pos + targetL / 2.0), 1, snoopy
        )
    #
        ...
        layout = {
            ...
            8: [molybdenum, 2.5 * cm],
            9: [molybdenum, 5.0 * cm],
            10: [molybdenum, 5.0 * cm],
            11: [molybdenum, 6.5 * cm],
            12: [molybdenum, 8.0 * cm],
            13: [molybdenum, 8.0 * cm],
            14: [tungsten, 5.0 * cm],
            15: [tungsten, 8.0 * cm],
            16: [tungsten, 10.0 * cm],
            17: [tungsten, 35.0 * cm],
        ...
        for i in range(1, 18):
            ...
            targetVol[i - 1].CreateBoxVolume(
                layout[i][0], diameter, diameter, layout[i][1]
            )
            ...
            targetPhys.append(
                targetVol[i - 1].PlaceIt(
                    G4ThreeVector(0.0, 0.0, z0Pos + targetL + layout[i][1] / 2.0),
                    1,
                    snoopy,
                )
            )
            if i < 17:
                slitPhys.append(
                    slit.PlaceIt(
                        G4ThreeVector(
                            0.0, 0.0, z0Pos + targetL + layout[i][1] + slitDZ / 2.0
                        ),
                        1,
                        snoopy,
                    )
        ...
        moreShieldingTopPhys = moreShieldingTopBot.PlaceIt(
            G4ThreeVector(
                0.0, diameter / 2.0 + spaceTopBot + yTot / 4.0, z0Pos + targetL / 2.0
            ),
            1,
            snoopy,
        )
        ...
        moreShieldingSide.CreateBoxVolume(
            iron, xTot / 2.0, diameter + 1.9 * spaceTopBot, targetL
        )
        moreShieldingLeftPhys = moreShieldingSide.PlaceIt(
            G4ThreeVector(
                diameter / 2.0 + spaceSide + xTot / 4.0, 0.0, z0Pos + targetL / 2.0
            ),
            1,
            snoopy,
        )
    ...
    absorberL = 2 * 150.0 * cm
    absorber = G4EzVolume("Absorber")
    #                             inner radius outer radius length
    absorber.CreateTubeVolume(iron, 0.0, 400.0 * cm, absorberL / 2.0)
    absorberPhys = absorber.PlaceIt(
        G4ThreeVector(0.0, 0.0, z0Pos + targetL + absorberL / 2.0 + 5.0 * cm), 1, snoopy
    )
    ...
    score.CreateTubeVolume(vac, 0.0, 20.0 * m, 1.0 * mm)
    scorePhys = score.PlaceIt(G4ThreeVector(0.0, 0.0, scorez), 1, snoopy)
...
if rangeCut:
    xx.SetCutValue(10.0, "gamma")
    xx.SetCutValue(10.0, "e-")
    xx.SetCutValue(10.0, "e+")
