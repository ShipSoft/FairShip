...
if withNtuple:
    ...
    h["ntuple"] = ROOT.TNtuple(
        "pythia8-Geant4", "muon/nu flux", "id:px:py:pz:x:y:z:opx:opy:opz:ox:oy:oz:pythiaid:parentid"
    )
        ...
        if particleGun:
            ...
            while npart == 0:
                ...
                if fullTungsten:
                    ...
                    z = rnr.Exp(16 * cm) - 50.0 * m  # Mo/H20/W average interaction length
...
class MyTrackingActionD(G4UserTrackingAction):
    ...
    def PreUserTrackingAction(self, atrack):
        ...
        if debug and abs(pid) == 13:
            print("track", atrack.GetTrackID(), tid, tmoid, moid, atrack.GetKineticEnergy() / MeV, p / MeV)
        if pid == 12:
            if moid in trackHistory:
...
def ConstructGeom():
    ...
    if fullTungsten:
        ...
        z0Pos = -50.0 * m
        for i in range(4):
            targetPhys.append(target.PlaceIt(G4ThreeVector(0.0, 0.0, z0Pos + targetL + targetDZ), 1, snoopy))
            slitPhys.append(slit.PlaceIt(G4ThreeVector(0.0, 0.0, z0Pos + targetL + 2 * targetDZ + slitDZ), 1, snoopy))
    ...
    #
        ...
        for i in range(1, 18):
            ...
            targetPhys.append(
                targetVol[i - 1].PlaceIt(G4ThreeVector(0.0, 0.0, z0Pos + targetL + layout[i][1] / 2.0), 1, snoopy)
            )
            if i < 17:
                slitPhys.append(
                    slit.PlaceIt(G4ThreeVector(0.0, 0.0, z0Pos + targetL + layout[i][1] + slitDZ / 2.0), 1, snoopy)
                )
    ...
    absorberPhys = absorber.PlaceIt(G4ThreeVector(0.0, 0.0, z0Pos + targetL + absorberL / 2.0 + 5.0 * cm), 1, snoopy)
