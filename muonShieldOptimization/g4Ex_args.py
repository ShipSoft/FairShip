...
if withNtuple:
    ...
    h["ntuple"] = ROOT.TNtuple(
        "pythia8-Geant4", "muon/nu flux", "id:px:py:pz:x:y:z:pythiaid:parentid"
    )
...
class MyGeneratorAction(G4VUserPrimaryGeneratorAction):
    ...
    def GeneratePrimaries(self, anEvent):
        ...
        while npart == 0:
            ...
            vertex = G4PrimaryVertex(pos, 0.0)
...
def ConstructGeom():
    ...
    targetPhys = target.PlaceIt(
        G4ThreeVector(0.0, 0.0, -50.0 * m + 25.0 * cm), 1, snoopy
    )
    ...
    absorber.CreateTubeVolume(iron, 0.0, 100.0 * cm, 150.0 * cm)
    absorberPhys = absorber.PlaceIt(
        G4ThreeVector(0.0, 0.0, -50 * m + 2 * 25.0 * cm + 150.0 * cm), 1, snoopy
    )
    ...
    # scoring plane
    scorez = -50.0 * m + 2 * 25.0 * cm + 2 * 150.0 * cm + 1 * mm
    score = G4EzVolume("Score")
    score.CreateTubeVolume(vac, 0.0, 50.0 * m, 1.0 * mm)
