...
for idnu in range(12, 17, 2):
    ...
    for idadd in range(-1, 2):
        ...
        key = idhnu + 100
        ut.bookHist(h, key, title, 100, -0.3, 1.7, 100, -2.0, 0.5)
        title = name + "  log10-p vs log10-pt"
        key = idhnu + 200
...
def processFile(fin, noCharm=True):
    ...
    for n in range(sTree.GetEntries()):
        ...
        for v in sTree.vetoPoint:
            ...
            if moID in charmExtern and noCharm:
                continue  # take heavy flavours from separate production
            idhnu = abs(pdgCode) + 1000
            if pdgCode < 0:
                idhnu += 1000
            l10ptot = ROOT.TMath.Min(
                ROOT.TMath.Max(ROOT.TMath.Log10(t.GetP()), -0.3), 1.69999
            )
            l10pt = ROOT.TMath.Min(
                ROOT.TMath.Max(ROOT.TMath.Log10(t.GetPt()), -2.0), 0.4999
            )
            key = idhnu
