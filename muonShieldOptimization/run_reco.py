        ...
        for x in jobs:
            ...
            if not geofile:
                ...
                if reset:
                    ...
                    os.system(
                        "python " + cmd + " -n 9999999 -f " + inputfile + " >> logRec &"
                    )
                os.chdir("../")
                time.sleep(10)
    nJobs = len(proc)
    while nJobs > 0:
        ...
        for p in sorted(proc.keys()):
            ...
            if completed:
                ...
                nproc = checkRunningProcesses()
                if nproc == 1:
                    print("rec job probably failed, only when python process running")
                time.sleep(100)
            os.chdir("../")
                ...
                if not f.find("geofile_full") < 0:
                    ...
                    process = subprocess.Popen(
                        [
                            ...
                            cmdAna,
                            "-n 9999999",
                            "-f " + inputfile.replace(".root", "_rec.root"),
                        ],
                        stdout=log[x],
                    )
                    process.wait()
...
def mergeHistosMakePlots(p):
    ...
    if 1 > 0:
        ...
        ut.bookCanvas(
            h, key="fitresults2", title="Fit Results", nx=1600, ny=1200, cx=2, cy=2
        )
    ...
    for prefix in prefixes:
        ...
        for x in jobs:
            for f in os.listdir(x):
                if not f.find("geofile_full") < 0:
                    inputfile = (f.replace("geofile_full", "ship")).replace(
                        ".root", "_rec.root"
                    )
        ...
        cmd = (
            "hadd -f "
            + inputfile.replace(".root", "_" + prefix + ".root")
            + haddCommand
        )
        os.system(cmd)
...
def checkProd(prefixes, quiet=False):
    ...
    for prefix in prefixes:
        ...
        for x in jobs:
            ...
            if len(rl) < 1:
                if not quiet:
                    print("simulation failed log file 0", x)
                summary["Sim"][x] = 0
                continue
...
def copyRecoToEos(pl):
    ...
    for x in result["Rec"]:
        if result["Rec"][x] < 1:
            print("Reco failed !", x, result["Rec"][x])
        else:
            cmd = (
                eos
                + " cp -r "
...
for p in os.sys.argv[1].split(","):
    ...
    if not xx.find("neutrino") < 0:
        pref = "neutrino"
    if not xx.find("vdis") < 0 or not xx.find("vetodis") < 0:
        pref = "disV"
