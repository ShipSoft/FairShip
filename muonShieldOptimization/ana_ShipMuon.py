...
def origin(sTree, it):
    ...
    if im == 0:
        ...
        rz_inter = (
            ROOT.TMath.Sqrt(at.GetStartX() ** 2 + at.GetStartY() ** 2),
            at.GetStartZ(),
        )
...
def bookHist(detName):
    for mu in ["", "_mu", "_muV0"]:
        ...
        ut.bookHist(
            h, tag + "_OP", "original particle mom GeV " + detName, 400, 0.0, 400.0
        )
        ut.bookHist(h, tag + "_id", "particle id " + detName, 5001, -2499.5, 2499.5)
        ut.bookHist(
            h, tag + "_mul", "multiplicity of hits/tracks " + detName, 100, -0.5, 99.5
        )
        ut.bookHist(
            h,
            tag + "_evmul",
            "multiplicity of hits/event " + detName,
            100,
            -0.5,
            9999.5,
        )
        ...
        for c in hitContainers:
            for ahit in c:
                ...
                if detName.find("LS") < 0:
                    ...
                    rc = h[detName].Fill(
                        ahit.GetZ() / u.m, ROOT.TMath.ATan2(y, x) / ROOT.TMath.Pi(), w
                    )
                ...
                if not trackID < 0:
                    r = (
                        ROOT.TMath.Sqrt(
                            aTrack.GetStartX() ** 2 + aTrack.GetStartY() ** 2
                        )
                        / u.m
                    )
                    ...
                    if abs(pdgID) == 13:
                        rc = h[detName + "_originmu"].Fill(
                            aTrack.GetStartZ() / u.m, r, w
                        )
    ...
    for n in range(nEvents):
        ...
        for ahit in sTree.vetoPoint:
            ...
            if abs(pid) != 13:
                continue
            P = ROOT.TMath.Sqrt(
                ahit.GetPx() ** 2 + ahit.GetPy() ** 2 + ahit.GetPz() ** 2
            )
            if P > 3 / u.GeV:
                h["ntuple"].Fill(
                    float(pid),
...
def analyzeConcrete():
    ...
    ut.bookCanvas(
        h, key="Resultsmu", title="muons hitting concrete", nx=1000, ny=600, cx=2, cy=2
    )
    ut.bookCanvas(
        h, key="Results", title="hitting concrete", nx=1000, ny=600, cx=2, cy=2
    )
    for m in ["", "mu", "V0"]:
        tc = h["Results" + m].cd(1)
        h["conc_hity" + m].Draw()
