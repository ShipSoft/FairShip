...
def GetGoodAndBadRuns(startDate, endDate):
    ...
    for r in runs:
        ...
        N += 1
        rc = os.system(
            'grep -q "Number of events produced with activity after hadron absorber" '
            + globalPath
            + str(theRun)
            + "/stdout"
        )
...
def addAllHistograms():
    ...
    Nmax = 45000
    path = (
        os.environ["EOSSHIP"] + "/eos/experiment/ship/data/Mbias/background-prod-2018/"
    )
...
def compactifyCascade(cycle):
    ...
    for i in range(cycle, cycle + ncpus):
        ...
        for l in f.readlines():
            if not l.find("Macro finished succesfully") < 0:
                success = True
        if not success:
...
def compactify(charm):
    ...
    if charm:
        ...
        output = (
            "pythia8_Geant4_" + str(runMin) + "-" + str(runMax) + "_" + ecut + ".root"
        )
        if not checkOnly:
            ldir = ""
            badFiles = []
            for d in os.listdir(globalPath):
                if d.find("run_fixedTarget") < 0:
                    continue
...
def makePrintout():
    ...
    for i in range(1, hbiased["pids"].GetNbinsX() + 1):
        ...
        if c > 0:
            p[int(hbiased["pids"].GetBinCenter(i))] = c
    ...
    for p in sorted_pr:
        if not p[0].find("Hadronic inelastic") < 0:
            if len(p[0]) > len("Hadronic inelastic"):
                continue
        denom = 0
        if p[0] in unbiased:
            denom = unbiased[p[0]]
...
def check4DoubleRuns():
    allRuns = [
        ...
        "goodAndBadRuns_2018-02-06_2018-02-12.pkl",
        "goodAndBadRuns_2018-02-12_2018-02-15.pkl",
        "goodAndBadRuns_2018-02-15_2018-02-21.pkl",
        "goodAndBadRuns_2018-02-21_2018-02-26.pkl",
    ]
    Nruns = 0
    for x in allRuns:
        fn = open(x)
