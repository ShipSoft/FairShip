...
splitcal::splitcal()
   : FairDetector("splitcal", kTRUE, kSplitCal), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fsplitcalPointCollection(new TClonesArray("splitcalPoint"))
{
}

splitcal::splitcal(const char *name, Bool_t active)
   : FairDetector(name, active, kSplitCal), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fsplitcalPointCollection(new TClonesArray("splitcalPoint"))
{
}
...
void splitcal::SetEmpty(Double_t Empty, Double_t BigGap, Double_t ActiveECAL_gas_gap, Double_t first_precision_layer,
                        Double_t second_precision_layer, Double_t third_precision_layer, Double_t num_precision_layers)
{
   fEmpty = Empty;
   fBigGap = BigGap;
...
void splitcal::SetThickness(Double_t ActiveECALThickness, Double_t ActiveHCALThickness, Double_t FilterECALThickness,
                            Double_t FilterECALThickness_first, Double_t FilterHCALThickness,
...
void splitcal::SetMaterial(Double_t ActiveECALMaterial, Double_t ActiveHCALMaterial, Double_t FilterECALMaterial,
...
void splitcal::ConstructGeometry()
{
   ...
   for (Int_t i_nlayECAL = 0; i_nlayECAL < fnECALSamplings; i_nlayECAL++) {
      ...
      i_nlayECAL_gas = -1000;
      if (i_nlayECAL == ffirst_precision_layer or i_nlayECAL == fsecond_precision_layer or
         ...
         if (i_nlayECAL % 2 == 0) {
            ...
            for (int mx = 0; mx < fNModulesInX; mx++) {
               for (int my = 0; my < fNModulesInY; my++) {
                  for (int j = 0; j < fNStripsPerModule; j++) {
                     ...
                     double xCoordinate = -fXMax + (fNStripsPerModule * mx + j + 0.5) * fStripHalfWidth *
                                                      2; // the times 2 is to get the total width from the half-width
                     double yCoordinate = -fYMax + (my + 0.5) * fStripHalfLength *
            ...
            for (int mx = 0; mx < fNModulesInX; mx++) {
               for (int my = 0; my < fNModulesInY; my++) {
                  for (int j = 0; j < fNStripsPerModule; j++) {
                     int index = (i_nlayECAL + 1) * 1e5 + (mx + 1) * 1e4 + (my + 1) * 1e3 + j + 1;
                     double xCoordinate = -fXMax + (mx + 0.5) * fStripHalfLength *
                                                      2; // the times 2 is to get the total length from the half-length
                     double yCoordinate = -fYMax + (fNStripsPerModule * my + j + 0.5) * fStripHalfWidth *
                                                      2; // the times 2 is to get the total width from the half-width
...
splitcalPoint *splitcal::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
