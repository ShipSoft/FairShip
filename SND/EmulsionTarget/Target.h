...
class Target : public FairDetector, public ISTLPointContainer {
 public:
  ...
  void SetEmulsionParam(Double_t EmTh, Double_t EmX, Double_t EmY,
                        Double_t PBTh, Double_t EPlW, Double_t LeadTh,
                        Double_t AllPW);
  void SetBrickParam(Double_t BrX, Double_t BrY, Double_t BrZ, Double_t BrPackX,
                     Double_t BrPackY, Double_t BrPackZ,
  ...
  // Functions to set dimension of both the base and the pillars in no magnet
  // configuration options. The pillars are defined in EmulsionMagnet class in
  // the option with magnet.
  void SetBaseDimension(Double_t X, Double_t Y, Double_t Z);
  void SetPillarDimension(Double_t X, Double_t Y, Double_t Z);

  static std::tuple<Int_t, Int_t, Int_t, Int_t, Bool_t> DecodeBrickID(
      Int_t detID);
 ...
 protected:
  ...
  Bool_t
      fsingleemulsionfilm;  // 0 = both top and bottom layers active, separated
                            // by plastic base; 1 = only one active layer
  Int_t fDesign;
  ...
  Double_t EmPlateWidth;   // Z dimension of the emulsion plates =
