...
ShipFieldMaker::ShipFieldMaker(Bool_t verbose)
   : TG4VUserPostDetConstruction(), globalField_(0), theFields_(), regionInfo_(), localInfo_(), verbose_(verbose),
     Tesla_(10.0), // To convert T to kGauss for VMC/FairRoot
     theNode_(0), gotNode_(kFALSE)
{
}
...
void ShipFieldMaker::defineUniform(const stringVect &inputLine)
{
   ...
   if (nWords == 5) {
      ...
      std::cout << "Expecting 5 words for the definition of the uniform field: " << "Uniform Name Bx By Bz"
                << std::endl;
   }
}
...
void ShipFieldMaker::defineBell(const TString &name, Double_t BPeak, Double_t zMiddle, Int_t orient, Double_t tubeR,
...
void ShipFieldMaker::defineFieldMap(const TString &name, const TString &mapFileName, const TVector3 &localCentre,
                                    const TVector3 &localAngles, Bool_t useSymmetry)
{
   // Check if the field is already in the map
   if (!this->gotField(name)) {
...
void ShipFieldMaker::defineFieldMapCopy(const TString &name, const TString &mapNameToCopy, const TVector3 &translation,
                                        const TVector3 &eulerAngles)
{
   ...
   if (!this->gotField(name)) {
      ...
      if (fieldToCopy) {
         ...
         if (verbose_) {
            std::cout << "Creating map field copy " << name.Data() << " based on " << mapNameToCopy.Data() << std::endl;
         }
...
void ShipFieldMaker::defineComposite(const TString &name, const TString &field1Name, const TString &field2Name,
...
void ShipFieldMaker::defineGlobalField(const stringVect &inputLine)
{
   ...
   if (nWords > 1) {
      ...
      std::cout << "Expecting at least two words for the global field definition: " << "Global Field1 ... FieldN"
                << std::endl;
   }
}

void ShipFieldMaker::defineGlobalField(const TString &field1Name, const TString &field2Name, const TString &field3Name,
...
void ShipFieldMaker::defineLocalField(const TString &volName, const TString &fieldName, Double_t scale)
{
   ...
   if (verbose_) {
      std::cout << "ShipFieldMaker::defineLocalField for volume " << volName.Data() << " and field " << fieldName.Data()
   ...
   for (localIter = localInfo_.begin(); localIter != localInfo_.end(); ++localIter) {
      ...
      if (theVol) {
         ...
         if (localField) {
            ...
            if (verbose_) {
               std::cout << "Setting local field " << localField->GetName() << " for volume " << volName << std::endl;
            }
...
void ShipFieldMaker::checkLocalFieldMap(TVirtualMagField *&localField, const TString &volName, Double_t scale)
{
   ...
   if (mapField) {
      ...
      if (!localMap && volName.Length() > 0) {
         ...
         localMap = new ShipBFieldMap(localName.Data(), *mapField, theInfo.x0_, theInfo.y0_, theInfo.z0_, theInfo.phi_,
...
void ShipFieldMaker::generateFieldMap(TString fileName, const float step, const float xRange, const float yRange,
