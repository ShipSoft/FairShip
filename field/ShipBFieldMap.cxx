...
ShipBFieldMap::ShipBFieldMap(const std::string &label, const std::string &mapFileName, Float_t xOffset, Float_t yOffset,
                             Float_t zOffset, Float_t phi, Float_t theta, Float_t psi, Float_t scale,
                             Bool_t isSymmetric)
   : TVirtualMagField(label.c_str()), fieldMap_(new floatArray()), mapFileName_(mapFileName), initialised_(kFALSE),
     isCopy_(kFALSE), Nx_(0), Ny_(0), Nz_(0), N_(0), xMin_(0.0), xMax_(0.0), dx_(0.0), xRange_(0.0), yMin_(0.0),
     yMax_(0.0), dy_(0.0), yRange_(0.0), zMin_(0.0), zMax_(0.0), dz_(0.0), zRange_(0.0), xOffset_(xOffset),
     yOffset_(yOffset), zOffset_(zOffset), phi_(phi), theta_(theta), psi_(psi), scale_(scale),
     isSymmetric_(isSymmetric), theTrans_(0), Tesla_(10.0)
{
   this->initialise();
}
...
ShipBFieldMap::ShipBFieldMap(const std::string &newName, const ShipBFieldMap &rhs, Float_t newXOffset,
                             Float_t newYOffset, Float_t newZOffset, Float_t newPhi, Float_t newTheta, Float_t newPsi,
                             Float_t newScale)
   : TVirtualMagField(newName.c_str()), fieldMap_(rhs.fieldMap_), mapFileName_(rhs.GetMapFileName()),
     initialised_(kFALSE), isCopy_(kTRUE), Nx_(rhs.Nx_), Ny_(rhs.Ny_), Nz_(rhs.Nz_), N_(rhs.N_), xMin_(rhs.xMin_),
     xMax_(rhs.xMax_), dx_(rhs.dx_), xRange_(rhs.xRange_), yMin_(rhs.yMin_), yMax_(rhs.yMax_), dy_(rhs.dy_),
     yRange_(rhs.yRange_), zMin_(rhs.zMin_), zMax_(rhs.zMax_), dz_(rhs.dz_), zRange_(rhs.zRange_), xOffset_(newXOffset),
     yOffset_(newYOffset), zOffset_(newZOffset), phi_(newPhi), theta_(newTheta), psi_(newPsi), scale_(newScale),
     isSymmetric_(rhs.isSymmetric_), theTrans_(0), Tesla_(10.0)
{
