...
def plot_my_hist(datum):
    ...
    H, xedges, yedges = np.histogram2d(
        plotData["x"], plotData["z"], bins=[50, 500], weights=plotData["by"]
    )
...
def generate_file(
    input_fileName, output, xSpace=73, ySpace=128, zSpace=1214, step=2.5, args=None
):
    # (min, max, max/stepSize + 1)  in case of Z: (0, nSteps*2.5 - 2.5, nSteps)
    field = pd.read_csv(
        input_fileName, skiprows=1, sep="\s+", names=["x", "y", "z", "bx", "by", "bz"]
    )
    ...
    if args.sidesOnly:
        ...
        temp_by = gaussian_filter(temp_by, sigma=args.sigma)
        field_new["by"] = temp_by.reshape(-1)
        field_new["by"] = field_new["by"] / (
            field_new["by"].abs().max()
        )  # *field_mask['by']
        field_new["by"] = field_new["by"] * field_mask["by"]
        rezult = field.copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        default=os.path.expandvars("$FAIRSHIP/files/fieldMap.csv"),
        type=str,
        action="store",
    )
    parser.add_argument(
        "--output",
        default=os.path.expandvars("$FAIRSHIP/files/noisy_fieldMap.csv"),
        type=str,
        action="store",
    )
