...
ShipBellField::ShipBellField()
    : FairField(), fPeak(0.), fMiddle(0.), fBtube(0.) {
  fType = 1;
  fInclTarget = kFALSE;
}
// -------------------------------------------------------------------------
...
ShipBellField::ShipBellField(const char* name, Double_t Peak, Double_t Middle,
                             Int_t orientation, Double_t Btube)
    : FairField(name), fPeak(Peak), fMiddle(Middle), fBtube(Btube) {
  fType = 1;
  fInclTarget = kFALSE;
  fOrient = orientation;
  fBtube = Btube;
...
ShipBellField::ShipBellField(ShipFieldPar* fieldPar)
    : FairField(), fPeak(0.), fMiddle(0.), fBtube(0.), fInclTarget(kFALSE) {
  if (!fieldPar) {
  ...
  if (fOrient == 1) {
    ...
    if ((fabs(x) < 2.7 * m) && (fabs(y) < fBtube + 0.2 * m)) {
      if (zlocal < 3.8) {
        ...
        bx = 0.19532 - 0.61512E-01 * zlocal + 0.68447E-02 * pow(zlocal, 2.) -
             0.25672E-03 * pow(zlocal, 3.);
      }
      bx = ((fPeak / tesla) / 0.14361) * bx * tesla;
    }
    // cout << "Bell GetBX " << x << ", " << y << ", " << z << ", Bx= " << bx <<
    // endl;
    return bx;
  }
}
...
Double_t ShipBellField::GetBy(Double_t x, Double_t y, Double_t z) {
  ...
  if (fInclTarget && z < targetZ0 + targetL && z > targetZ0) {
    ...
    if (fabs(x) > 2 * targetXY && fabs(x) < 3 * targetXY &&
        fabs(y) < targetXY) {
      by = -1 * tesla;
    }
  } else if (fOrient == 1) {
