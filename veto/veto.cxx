...
veto::veto()
   : FairDetector("veto", kTRUE, kVETO), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fvetoPointCollection(new TClonesArray("vetoPoint"))
{
   fUseSupport = 1;
   fPlasticVeto = 0;
   fLiquidVeto = 1;
...
veto::veto(const char *name, Bool_t active)
   : FairDetector(name, active, kVETO), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fFastMuon(kFALSE), fFollowMuon(kFALSE),
     ...
     fvetoPointCollection(new TClonesArray("vetoPoint")), vetoMed_name("Scintillator"), // for liquid scintillator
     supportMedIn_name("steel"),                                                        // for vacuum option
     supportMedOut_name("Aluminum"),                                                    // for vacuum option
     f_RibThickness(1.5 * cm), decayVolumeMed_name("vacuums"),                          // for vacuum option
     VetoStartInnerX(920. * mm), VetoStartInnerY(2620. * mm), VetoEndInnerX(4120. * mm), VetoEndInnerY(6120. * mm)
{
   fUseSupport = 1;
   fPlasticVeto = 0;
   fLiquidVeto = 1;
...
TGeoVolume *veto::GeoTrapezoid(TString xname, Double_t wz, Double_t wX_start, Double_t wX_end, Double_t wY_start,
                               Double_t wY_end, Int_t color, TGeoMedium *material, Bool_t sens = kFALSE)
{
...
TGeoVolume *veto::GeoTrapezoidNew(TString xname, Double_t thick, Double_t wz, Double_t wX_start, Double_t wX_end,
                                  Double_t wY_start, Double_t wY_end, Int_t color, TGeoMedium *material,
                                  Bool_t sens = kFALSE)
{
...
TGeoVolume *veto::GeoSideObj(TString xname, double dz, double a1, double b1, double a2, double b2, double dA, double dB,
...
TGeoVolume *veto::GeoCornerLiSc1(TString xname, double dz, bool isClockwise, double a, double b1, double b2, double dA,
                                 double dB, Int_t color, TGeoMedium *material, Bool_t sens = kFALSE)
{
...
TGeoVolume *veto::GeoCornerLiSc2(TString xname, double dz, bool isClockwise, double a, double b1, double b2, double dA,
                                 double dB, Int_t color, TGeoMedium *material, Bool_t sens = kFALSE)
{
...
TGeoVolumeAssembly *veto::GeoCornerRib(TString xname, double ribThick, double lt1, double lt2, double dz, double slopeX,
                                       double slopeY, Int_t color, TGeoMedium *material, Bool_t sens = kFALSE)
{
...
void veto::AddBlock(TGeoVolumeAssembly *tInnerWall, TGeoVolumeAssembly *tDecayVacuum, TGeoVolumeAssembly *tOuterWall,
                    TGeoVolumeAssembly *tLongitRib, TGeoVolumeAssembly *tVerticalRib, TGeoVolumeAssembly *ttLiSc,
                    int &liScCounter, int blockNr, int nx, int ny, double z1, double z2, double Zshift, double dist,
                    double distC, double wallThick, double liscThick1, double liscThick2, double ribThick)
{
   ...
   TGeoVolume *TOW = GeoTrapezoidNew(nameOuterWall, wallThick, wz, wx(z1) + 2 * (wallThick + liscThick1),
                                     wx(z2) + 2 * (wallThick + liscThick2), wy(z1) + 2 * (wallThick + liscThick1),
   ...
   for (int i = 0; i < nx; i++) {
      ...
      vLongitRibX.at(i) = GeoSideObj(name, dZ, ribThick, liscThick1, ribThick, liscThick2, xpos2 - xpos1, ypos2 - ypos1,
   ...
   for (int i = 0; i < ny; i++) {
      ...
      name = "";
      name.Form("vLongitRibX_%s_phi%d", blockName.Data(), makeId(0, xpos1, ypos1));
      vLongitRibY.at(i) = GeoSideObj(name, dZ, liscThick1, ribThick, liscThick2, ribThick, xpos2 - xpos1, ypos2 - ypos1,
                                     ribColor, supportMedIn);
   }
   ...
   TGeoVolume *LiSc_L1 = GeoCornerLiSc1(name, dZ, 1, distC - ribThick / sqrt(2) - ribThick / 2, liscThick1, liscThick2,
                                        xPos2 - xPos1, yPos2 - yPos1, kMagenta - 10, vetoMed, true);
   name = "";
   name.Form("LiSc_R1_%d", liscId("LiSc_R1", blockNr, 0, 0, 0));
   TGeoVolume *LiSc_R1 = GeoCornerLiSc1(name, dZ, 0, distC - ribThick / sqrt(2) - ribThick / 2, liscThick1, liscThick2,
                                        xPos2 - xPos1, yPos2 - yPos1, kMagenta - 10, vetoMed, true);
   ...
   TGeoVolume *LiSc_L2 = GeoCornerLiSc2(name, dZ, 1, distC - ribThick / sqrt(2) - ribThick / 2, liscThick1, liscThick2,
                                        xPos2 - xPos1, yPos2 - yPos1, kMagenta - 10, vetoMed, true);
   name = "";
   name.Form("LiSc_R2_%d", liscId("LiSc_R2", blockNr, 0, 0, 0));
   TGeoVolume *LiSc_R2 = GeoCornerLiSc2(name, dZ, 0, distC - ribThick / sqrt(2) - ribThick / 2, liscThick1, liscThick2,
                                        xPos2 - xPos1, yPos2 - yPos1, kMagenta - 10, vetoMed, true);
   ...
   for (double pos = z1; pos < z2; pos += dist) {
      ...
      TGeoVolume *TVR =
         GeoTrapezoidNew(nameVR, liscThick1, ribThick, wx(pos) + 2 * wallThick, wx(pos + ribThick) + 2 * wallThick,
                         wy(pos) + 2 * wallThick, wy(pos + ribThick) + 2 * wallThick, ribColor, supportMedIn);
      ...
      for (int i = 0; i < nx; i++) {
         ...
         tLongitRib->AddNode(vLongitRibX.at(i), makeId(pos, xpos1, ypos1),
         ...
         if (i < nx - 1) {
            ...
            ttLiSc->AddNode(
               LiScX, liscId("LiScX", blockNr, Zlayer, i, 2),
      ...
      for (int i = 0; i < ny; i++) {
         ...
         tLongitRib->AddNode(vLongitRibY.at(i), makeId(pos, xpos1, ypos1),
                             new TGeoCombiTrans(xpos1, ypos1, tZ, new TGeoRotation("r", 0, 0, 0)));
         tLongitRib->AddNode(vLongitRibY.at(i), makeId(pos, -xpos1, -ypos1),
                             new TGeoCombiTrans(-xpos1, -ypos1, tZ, new TGeoRotation("r", 0, 0, 180)));
         // lisc
         if (i < ny - 1) {
            ...
            TGeoVolume *LiScY =
               GeoSideObj(name, dZ, liscThick1, yStepZ1 - ribThick, liscThick2, yStepZ2 - ribThick, xpos2 - xpos1,
                          (ypos2 - yStepZ2) - (ypos1 - yStepZ1), kMagenta - 10, vetoMed, true);
            ...
            ttLiSc->AddNode(
               LiScY, liscId("LiScY", blockNr, Zlayer, i, 2),
      ...
      ttLiSc->AddNode(LiSc_L1, liscId("LiSc_L1", blockNr, Zlayer, 0, 1),
                      new TGeoCombiTrans(xP1, yP1, tZ, new TGeoRotation("r", 0, 0, 0)));
      ttLiSc->AddNode(LiSc_L1, liscId("LiSc_L1", blockNr, Zlayer, 0, 2),
                      new TGeoCombiTrans(-xP1, -yP1, tZ, new TGeoRotation("r", 0, 0, 180)));

      ttLiSc->AddNode(LiSc_R1, liscId("LiSc_R1", blockNr, Zlayer, 0, 1),
                      new TGeoCombiTrans(-xP1, yP1, tZ, new TGeoRotation("r", 0, 0, 0)));
      ttLiSc->AddNode(LiSc_R1, liscId("LiSc_R1", blockNr, Zlayer, 0, 2),
                      new TGeoCombiTrans(xP1, -yP1, tZ, new TGeoRotation("r", 0, 0, 180)));

      xP1 = wx(pos + ribThick) / 2 + wallThick;
      yP1 = wy(pos + ribThick) / 2 + wallThick - distC + ribThick / 2;
      ttLiSc->AddNode(LiSc_L2, liscId("LiSc_L2", blockNr, Zlayer, 0, 1),
                      new TGeoCombiTrans(xP1, yP1, tZ, new TGeoRotation("r", 0, 0, 0)));
      ttLiSc->AddNode(LiSc_L2, liscId("LiSc_L2", blockNr, Zlayer, 0, 2),
                      new TGeoCombiTrans(-xP1, -yP1, tZ, new TGeoRotation("r", 0, 0, 180)));

      ttLiSc->AddNode(LiSc_R2, liscId("LiSc_R2", blockNr, Zlayer, 0, 1),
                      new TGeoCombiTrans(xP1, -yP1, tZ, new TGeoRotation("r", 0, 0, 0)));
      ttLiSc->AddNode(LiSc_R2, liscId("LiSc_R2", blockNr, Zlayer, 0, 2),
                      new TGeoCombiTrans(-xP1, yP1, tZ, new TGeoRotation("r", 0, 0, 180)));
   }
}
...
TGeoVolume *veto::MakeMagnetSegment(Int_t seg)
{
   ...
   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_Y1, 1,
                       new TGeoCombiTrans(xPosOutetCover, 0, zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));
   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_Y1, 2,
                       new TGeoCombiTrans(-xPosOutetCover, 0, zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));
   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_Y1, 3,
                       new TGeoCombiTrans(xPosOutetCover, 0, -zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));
   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_Y1, 4,
                       new TGeoCombiTrans(-xPosOutetCover, 0, -zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));

   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_X1, 1,
                       new TGeoCombiTrans(0, yPosOutetCover, zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));
   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_X1, 2,
                       new TGeoCombiTrans(0, -yPosOutetCover, zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));
   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_X1, 3,
                       new TGeoCombiTrans(0, yPosOutetCover, -zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));
   if (isLongOutCover)
      tMagVol->AddNode(LongOutCover_X1, 4,
                       new TGeoCombiTrans(0, -yPosOutetCover, -zPosOutetCover, new TGeoRotation("r", 0, 0, 0)));
...
TGeoVolume *veto::MakeSegments(Double_t dz, Double_t dx_start, Double_t dy_start, Double_t slopeX, Double_t slopeY,
                               Double_t floorHeight)
{
   ...
   AddBlock(tInnerWall, tDecayVacuum, tOuterWall, tLongitRib, tVerticalRib, ttLiSc, liScCounter, 1, nx, ny, z1, z2,
            Zshift, dist, distC, wallThick, liscThick, liscThick, ribThick);
   ...
   AddBlock(tInnerWall, tDecayVacuum, tOuterWall, tLongitRib, tVerticalRib, ttLiSc, liScCounter, 2, nx, ny, z1, z2,
            Zshift, dist, distC, wallThick, liscThick, liscThick, ribThick);
   ...
   AddBlock(tInnerWall, tDecayVacuum, tOuterWall, tLongitRib, tVerticalRib, ttLiSc, liScCounter, 3, nx, ny, z1, z2,
            Zshift, dist, distC, wallThick, liscThick, liscThick, ribThick);
   ...
   AddBlock(tInnerWall, tDecayVacuum, tOuterWall, tLongitRib, tVerticalRib, ttLiSc, liScCounter, 4, nx, ny, z1, z2,
            Zshift, dist, distC, wallThick, liscThick, liscThick, ribThick);
...
Bool_t veto::ProcessHits(FairVolume *vol)
{
   ...
   if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
      ...
      //    cout << veto_uniqueId << " :(" << xmean << ", " << ymean << ", " << zmean << "): " << gMC->CurrentVolName()
      //    << endl;
      AddHit(fTrackID, veto_uniqueId, TVector3(xmean, ymean, zmean), TVector3(fMom.Px(), fMom.Py(), fMom.Pz()), fTime,
             fLength, fELoss, pdgCode, TVector3(Pos.X(), Pos.Y(), Pos.Z()), TVector3(Mom.Px(), Mom.Py(), Mom.Pz()));
...
void veto::ConstructGeometry()
{
   ...
   */
   ...
   tMaGVol->AddNode(T6Lid, 1,
                    new TGeoTranslation(0, 0, fTub6z + fTub6length + f_LidThickness / 2. + 0.1 * cm - zStartMagVol));
   ...
   if (fFastMuon && fFollowMuon) {
      ...
      while (volumeiterator <= lastvolume) {
         ...
         if (strstr(classname, Vol)) {
            if (strstr(volumename, Mag) || strstr(volumename, Cavern) || strstr(volumename, Ain) ||
...
vetoPoint *veto::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
