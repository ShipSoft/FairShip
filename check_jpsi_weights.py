...
the JSON side file that run_fixedTarget.py writes (--jpsi-data):
    ...
    python check_jpsi_weights.py sim_ft_inject.root          # injected sample
...
# weights are stored as floats, so agreement is exact only to about 1e-7
...
n_mu, n_bad, n_jpsi, n_vetoed = 0, 0, 0, 0
worst = 0.0
z_jpsi = []
def count(coll):
    # modern FairShip: std::vector<ShipMCTrack>; older: TClonesArray
    return len(coll) if hasattr(coll, "size") else coll.GetEntriesFast()
...
for event in tree:
    ...
    for i in range(count(tracks)):
        ...
        if tracks[mother].GetMotherId() >= 0:
            n_vetoed += 1  # Pythia8 J/psi vetoed in an injected sample: not ours
            continue
        n_mu += 1
...
if n_vetoed:
    print(f"muons from vetoed Pythia8 J/psi still in the MC truth (untracked): {n_vetoed}")
if z_jpsi:
    z_jpsi.sort()
    mean = sum(z_jpsi) / len(z_jpsi)
    rms = (sum((z - mean) ** 2 for z in z_jpsi) / len(z_jpsi)) ** 0.5
    print(
        f"J/psi production z [cm]: min {z_jpsi[0]:.1f}, median {z_jpsi[len(z_jpsi) // 2]:.1f}, "
        f"max {z_jpsi[-1]:.1f}, mean {mean:.1f}, rms {rms:.1f}"
    )
if n_mu == 0:
