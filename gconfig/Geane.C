...
void Config() {
  ...
  gMC3->SetMUNU(0);
  gMC3->SetCKOV(0);
  gMC3->SetHADR(0);  // Select pure GEANH (HADR 1) or GEANH/NUCRIN (HADR 3)//4
                     // fluka 5 gcalor
  gMC3->SetLOSS(4);
  ...
  gMC3->SetOPTI(
      0);  // Select optimisation level for GEANT geometry searches (0,1,2)
  gMC3->SetERAN(5.e-7);

  // -------->>>>> PAY ATTENTION!!!!!
  // For a correct use of GEANE, you MUST use the cuts as set below!!!
  // i.e. Since GEANE is tracking only the primary particle, DCUTE, DCUTM, BCUTE
  // and BCUTM must be put at very high values (10 TeV) in order to calculate
  // properly the energy loss. For a more complete explanation of the chosen
  // values, refer to GEANT manual
