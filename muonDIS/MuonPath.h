...
/* 
This class allows to consider a track with changes in directions between the different points of measurements (start, veto detectors, SST, etc...).

A MuonPath can be made of several slices. For each slice, there is a start and an end position. 

When getting a random z position along the path, the extrapolation is made linearly using a vertex position which corresponds to the relevant point of measurement along the trajectory.
*/
...
class MuonPath {
 ...
 public:
  ...
  inline void AddVolume(const std::string & aVol, const std::string & aMat, const double & aD){
  ...
  inline void SetLabel(const std::string & aLab){
    flabel=aLab;
  };
  ...
  inline double GetWeightedDensity() const{
    return fwdensity;
  };
  
  inline void SetDensity(const double & aD){
    fdensity=aD;
  };
  ...
  inline double GetZLength() const{
    return fzlength;
  };
  
  inline void SetLength(const double & aStep,
			const TVector3 & aStart,
			const double& aZ){
    flength += aStep;
    fzlength += aZ;
  ...
  inline const unsigned GetNSlices() const{
    return fvolName.size();
  };
  ...
  inline double GetX(const double & aZ,const unsigned & idx) const{
    if (idx>=GetNSlices()) return 0;
    return fvtx[idx].X() + (aZ-fvtx[idx].Z())*fpvec[idx].X()/fpvec[idx].Z();
  };

  inline double GetY(const double & aZ,const unsigned & idx) const{
    if (idx>=GetNSlices()) return 0;
    return fvtx[idx].Y() + (aZ-fvtx[idx].Z())*fpvec[idx].Y()/fpvec[idx].Z();
  };
