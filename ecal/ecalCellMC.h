...
public:
  ...
  // For python users
  size_t TrackEnergySize() const {return fTrackEnergy.size();}
  size_t TrackTimeSize() const {return fTrackEnergy.size();}
  // Don't use slow methods except in emergency!!!
  void GetTrackEnergySlow(unsigned n, Int_t& trackid, Double_t& energy_dep);
  void GetTrackTimeSlow(unsigned n, Int_t& trackid, Float_t& time);
