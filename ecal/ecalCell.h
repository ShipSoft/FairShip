...
public:
   ecalCell(Int_t cellnumber, Float_t x1 = 0, Float_t y1 = 0, Float_t x2 = 0, Float_t y2 = 0, Char_t type = 0,
            Float_t energy = 0)
      : TObject(), fNumber(cellnumber), fX1(x1), fY1(y1), fX2(x2), fY2(y2), fType(type), fEnergy(energy), fADC(-1111),
        fNeighbors(), f5x5Cluster(), fTime(-1111){};
   ...
   inline void GetNeighborsList(std::list<ecalCell *> &neib) const { neib = fNeighbors; }
   inline void SetNeighborsList(std::list<ecalCell *> &neib) { fNeighbors = neib; }

   // 5x5 cluster stuff
   inline void Get5x5Cluster(std::list<ecalCell *> &cls)
   {
      if (f5x5Cluster.size() == 0)
         Create5x5Cluster();
      cls = f5x5Cluster;
   }
