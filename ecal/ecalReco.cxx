   ...
   for (i = 0; i < nc; i++) {
      ...
      /*
         ...
         rawE+=(*p)->GetEnergy()*ourE/allE;
            ...
            reco=new ((*fReconstucted)[fN++]) ecalReconstructed(rawE, fCalib->Calibrate(maxs[j]->GetType(), rawE), x, y,
         cls->PeakNum(j), i);
          }
          cls->SetStatus(1);
      */
...
ecalReco::ecalReco(const char *name, const Int_t verbose)
   : FairTask(name, verbose), fEv(0), fN(0), fRejected(0), fRejectedP(0), fClusters(NULL), fReconstucted(NULL),
     fStr(NULL), fCalib(NULL)
{
   ;
}
...
ecalReco::ecalReco()
   : FairTask(), fEv(-1111), fN(0), fRejected(0), fRejectedP(0), fClusters(NULL), fReconstucted(NULL), fStr(NULL),
     fCalib(NULL)
{
   ;
}
