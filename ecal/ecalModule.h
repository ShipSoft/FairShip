...
public:
   ...
   ecalModule(char type = 1, Int_t cellnumber = -1, Float_t x1 = 0, Float_t y1 = 0, Float_t x2 = 0, Float_t y2 = 0,
              Int_t mc = 0, Float_t energy = 0);
...
{
   ResetEnergyFast();
   for (UInt_t i = 0; i < fCells.size(); i++)
      fCells[i]->ResetEnergyFast();
}
#endif
