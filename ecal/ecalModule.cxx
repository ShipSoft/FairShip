...
//-----------------------------------------------------------------------------
ecalModule::ecalModule(char type, Int_t cellnumber, Float_t x1, Float_t y1, Float_t x2, Float_t y2, Int_t mc,
                       Float_t energy)
   : ecalCell(cellnumber, x1, y1, x2, y2, type, energy), fDx(x2 - x1), fDy(y2 - y1), fCells()
{
   if (GetType() < 1)
      return;
   ...
   if (mc == 0)
      for (i = 0; i < mt; i++)
         for (j = 0; j < mt; j++)
            fCells[i * mt + j] = new ecalCell((num * 10 + i + 1) * 10 + j + 1, x1 + j * fDx / mt, y1 + i * fDy / mt,
                                              x1 + (j + 1) * fDx / mt, y1 + (i + 1) * fDy / mt, type);
   else
      for (i = 0; i < mt; i++)
         for (j = 0; j < mt; j++)
            fCells[i * mt + j] = new ecalCellMC((num * 10 + i + 1) * 10 + j + 1, x1 + j * fDx / mt, y1 + i * fDy / mt,
                                                x1 + (j + 1) * fDx / mt, y1 + (i + 1) * fDy / mt, type);
}
...
{
   if (x < 0 || y < 0 || x >= GetType() || y >= GetType())
      return NULL;
