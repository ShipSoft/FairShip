...
// -----   Default constructor   -------------------------------------------
ecal::ecal()
   : FairDetector("ECAL", kTRUE, kecal), fInf(NULL), fDebug(NULL), fTrackID(-1), fVolumeID(-1), fPos(), fMom(),
     fTime(-1.), fLength(-1.), fELoss(-1.), fPosIndex(0), fEcalCollection(new TClonesArray("ecalPoint")),
     fLiteCollection(new TClonesArray("ecalPoint")), fEcalSize(), fSimpleGeo(0), fXSize(0), fYSize(0), fDX(0.), fDY(0.),
     fModuleSize(0.), fZEcal(0.), fSemiX(0.), fSemiY(0.), fThicknessLead(0.), fThicknessScin(0.), fThicknessTyvk(0.),
     fThicknessLayer(0.), fThicknessSteel(0.), fEdging(0.), fHoleRad(0.), fFiberRad(0.), fXCell(), fYCell(), fNH(),
     fCF(), fLightMapNames(), fLightMaps(), fNLayers(0), fModuleLenght(0.), fGeoScale(0.), fNColumns1(0), fNRows1(0),
     fNColumns2(0), fNRows2(0), fNColumns(0), fNRows(0), fVolIdMax(0), fFirstNumber(0), fVolArr(), fModules(), fCells(),
     fScTiles(), fTileEdging(), fPbTiles(), fTvTiles(), fHoleVol(), fFiberVol(), fSteelTapes(), fHolePos(),
     fModulesWithType(), fRawNumber(), fStructureId()
{
   fVerboseLevel = 1;
...
ecal::ecal(const char *name, Bool_t active, const char *fileGeo)
   : FairDetector(name, active, kecal), fInf(NULL), fDebug(NULL), fTrackID(-1), fVolumeID(-1), fPos(), fMom(),
     fTime(-1.), fLength(-1.), fELoss(-1.), fPosIndex(0), fEcalCollection(new TClonesArray("ecalPoint")),
     fLiteCollection(new TClonesArray("ecalPoint")), fEcalSize(), fSimpleGeo(0), fXSize(0), fYSize(0), fDX(0.), fDY(0.),
     fModuleSize(0.), fZEcal(0.), fSemiX(0.), fSemiY(0.), fThicknessLead(0.), fThicknessScin(0.), fThicknessTyvk(0.),
     fThicknessLayer(0.), fThicknessSteel(0.), fEdging(0.), fHoleRad(0.), fFiberRad(0.), fXCell(), fYCell(), fNH(),
     ...
     fModulesWithType(), fRawNumber(), fStructureId()
{
   ...
   for (i = 1; i < cMaxModuleType; i++) {
      ...
      if (fLightMapNames[i] != "none") {
         ...
         Info("ecal", "Number of modules of type %d is %d (%d channels), lightmap %s", i, fModulesWithType[i],
              fModulesWithType[i] * i * i, fLightMapNames[i].Data());
      } else
         fLightMaps[i] = NULL;
      ...
      */
      ...
      if (id < 0) {
         ...
         cout << "neg id " << mx << " " << my << " " << cell << " " << gMC->CurrentVolName() << " "
              << gMC->CurrentVolOffName(1) << " " << gMC->CurrentVolOffName(2) << " " << fELoss << " " << fELoss * 1e10
              << endl;
      }
      /*
...
void ecal::FillWallPoint()
{
   ...
   if (fMom.Pz() > 0 && fPos.Z() < fZEcal + 0.01) {
      ...
      AddHit(fTrackID, fVolumeID, TVector3(fPos.X(), fPos.Y(), fPos.Z()), TVector3(fMom.Px(), fMom.Py(), fMom.Pz()),
             fTime, fLength, fELoss, part->GetPdgCode());
   }
   fTrackID = gMC->GetStack()->GetCurrentTrackNumber();
}
...
Bool_t ecal::FillLitePoint(Int_t volnum)
{
   ...
   while (part->GetFirstMother() >= 0 && part->Vz() >= zmin && part->Vz() <= zmax && TMath::Abs(part->Vx()) <= xecal &&
      ...
      //      cout << "-> " << part->GetFirstMother() << " : " << part->Vx() << ", " << part->Vy() << ", " << part->Vz()
      //      << endl;
   }
//  if (part->Vz()>500)
...
ecalPoint *ecal::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
...
void ecal::ConstructCell(Int_t type)
{
   ...
   for (i = 0; i < fNLayers; i++) {
      gGeoManager->Node(scin.Data(), i + 1, nm.Data(), 0.0, 0.0,
                        -thickness * fNLayers / 2.0 + fThicknessScin / 2.0 + i * thickness, 0, kTRUE, buf, 0);
      gGeoManager->Node(lead.Data(), i + 1, nm.Data(), 0.0, 0.0,
                        -thickness * fNLayers / 2.0 + fThicknessScin + i * thickness + fThicknessTyvk +
                           fThicknessLead / 2.0,
                        0, kTRUE, buf, 0);
      if (fThicknessTyvk > 0.0) {
         gGeoManager->Node(tyvek.Data(), 2 * i + 1, nm.Data(), 0.0, 0.0,
                           -thickness * fNLayers / 2.0 + fThicknessScin + i * thickness + 1.5 * fThicknessTyvk +
                              fThicknessLead,
                           0, kTRUE, buf, 0);
         gGeoManager->Node(tyvek.Data(), 2 * i + 2, nm.Data(), 0.0, 0.0,
                           -thickness * fNLayers / 2.0 + fThicknessScin + i * thickness + 0.5 * fThicknessTyvk, 0,
                           kTRUE, buf, 0);
      }
   }
   fCells[type] = cellv;
