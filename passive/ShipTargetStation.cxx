...
ShipTargetStation::ShipTargetStation(const char *name, const Double_t tl, const Double_t tz, const int nS,
...
void ShipTargetStation::ConstructGeometry()
{
   ...
   if (fnS > 10) {
      ...
      tTarget->AddNode(moreShieldingTopBot, 1,
                       new TGeoTranslation(0., fDiameter / 2. + spaceTopBot + yTot / 2., fTargetLength / 2.));
      tTarget->AddNode(moreShieldingTopBot, 2,
                       new TGeoTranslation(0., -fDiameter / 2. - spaceTopBot - yTot / 2., fTargetLength / 2.));
      TGeoVolume *moreShieldingSide = gGeoManager->MakeBox("moreShieldingSide", iron, xTot / 2.,
                                                           (fDiameter + 1.9 * spaceTopBot) / 2., fTargetLength / 2.);
      moreShieldingSide->SetLineColor(33);
      tTarget->AddNode(moreShieldingSide, 1,
                       new TGeoTranslation(fDiameter / 2. + spaceSide + xTot / 2., 0., fTargetLength / 2.));
      tTarget->AddNode(moreShieldingSide, 2,
                       new TGeoTranslation(-fDiameter / 2. - spaceSide - xTot / 2., 0., fTargetLength / 2.));
   } else {
