...
ShipMagneticSpectrometer::ShipMagneticSpectrometer(const char* name, const Int_t Design, const char* Title, 
  ...
  : FairModule(name ,Title)
{
 ...
 if (fDesign==2){
     ...
     dZ5 = L5;
     dZ6 = L6;
     fMgSpectroLength = 2*(dZ1+dZ2+dZ3+dZ4+dZ5+dZ6) + LE ; //leave some space for nu-tau detector   
    }
 zEndOfAbsorb = Z - fMfSpectroLength/2.;

}


// -----   Private method InitMedium 
Int_t ShipMagneticSpectrometer::InitMedium(const char* name) 
{
...
void ShipMuonShield::ConstructGeometry()
{
    ...
    // passive design with tungsten and lead
     ...
     TGeoVolume *Pbshield2 = gGeoManager->MakeCone("Pbshield2", lead, (fMgSpectroLength-Lz)/2., rW,Pbr2,rW,Pbr3);
     Pbshield2->SetLineColor(23);  // silver/grey
     zpos = zEndOfAbsorb  + Lz + (fMgSpectroLength-Lz)/2. ;
     top->AddNode(Pbshield2, 1, new TGeoTranslation(0, 0, zpos));
    
     cout << "passive muon shield postioned at " << (-fMgSpectroLength/2.-2500)/100. << "m"<< endl;
    }
    else if (fDesign==2){
