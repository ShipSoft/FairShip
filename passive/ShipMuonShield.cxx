...
ShipMuonShield::ShipMuonShield(TString geofile, Double_t floor, const Int_t withCoMagnet, const Bool_t StepGeo,
...
ShipMuonShield::ShipMuonShield(TVectorT<Double_t> in_params, Double_t floor, const Int_t withCoMagnet,
                               const Bool_t StepGeo, const Bool_t WithConstAbsorberField,
...
ShipMuonShield::ShipMuonShield(const char *name, const Int_t Design, const char *Title, Double_t Z, Double_t L0,
                               Double_t L1, Double_t L2, Double_t L3, Double_t L4, Double_t L5, Double_t L6,
                               Double_t L7, Double_t L8, Double_t gap, Double_t LE, Double_t, Double_t floor,
                               Double_t field, const Int_t withCoMagnet, const Bool_t StepGeo,
                               const Bool_t WithConstAbsorberField, const Bool_t WithConstShieldField)
...
void ShipMuonShield::CreateTube(TString tubeName, TGeoMedium *medium, Double_t dX, Double_t dY, Double_t dZ,
                                Int_t color, TGeoVolume *tShield, Double_t x_translation, Double_t y_translation,
...
void ShipMuonShield::CreateArb8(TString arbName, TGeoMedium *medium, Double_t dZ, std::array<Double_t, 16> corners,
                                Int_t color, TGeoUniformMagField *magField, TGeoVolume *tShield, Double_t x_translation,
                                Double_t y_translation, Double_t z_translation)
{
...
void ShipMuonShield::CreateArb8(TString arbName, TGeoMedium *medium, Double_t dZ, std::array<Double_t, 16> corners,
                                Int_t color, TGeoUniformMagField *magField, TGeoVolume *tShield, Double_t x_translation,
                                Double_t y_translation, Double_t z_translation, Bool_t stepGeo)
{
   if (!stepGeo) {
...
void ShipMuonShield::CreateMagnet(TString magnetName, TGeoMedium *medium, TGeoVolume *tShield,
                                  TGeoUniformMagField *fields[4], FieldDirection fieldDirection, Double_t dX,
                                  Double_t dY, Double_t dX2, Double_t dY2, Double_t dZ, Double_t middleGap,
                                  Double_t middleGap2, Double_t HmainSideMag, Double_t HmainSideMag2, Double_t gap,
                                  Double_t gap2, Double_t Z, Bool_t NotMagnet, Bool_t stepGeo, Bool_t SC_key = false)
{
   ...
   std::array<Double_t, 16> cornersMainL = {
      ...
      dX + middleGap,   dY - anti_overlap,           dX + middleGap,   -(dY - anti_overlap),
      middleGap2,       -(dY2 + dX2 - anti_overlap), middleGap2,       dY2 + dX2 - anti_overlap,
      dX2 + middleGap2, dY2 - anti_overlap,          dX2 + middleGap2, -(dY2 - anti_overlap)};
   ...
   std::array<Double_t, 16> cornersMainSideL =
      fDesign == 7
         ? std::array<Double_t, 16>{dX + middleGap + gap,        -HmainSideMag,
                                    ...
                                    2 * dX2 + middleGap2 + gap2, -HmainSideMag2}
         : std::array<Double_t, 16>{dX + middleGap + gap,        -(dY - anti_overlap),      dX + middleGap + gap,
   ...
   if (SC_key) {
      ...
      cornersMainSideL = fDesign == 7
                            ? std::array<Double_t, 16>{dX + middleGap + gap,        -HmainSideMag,
                                                       ...
                                                       4 * dX2 + middleGap2 + gap2, -HmainSideMag2}
                            : std::array<Double_t, 16>{dX + middleGap + gap,        -(dY - anti_overlap),
...
Int_t ShipMuonShield::Initialize(std::vector<TString> &magnetName, std::vector<FieldDirection> &fieldDirection,
                                 std::vector<Double_t> &dXIn, std::vector<Double_t> &dYIn, std::vector<Double_t> &dXOut,
                                 std::vector<Double_t> &dYOut, std::vector<Double_t> &dZ,
                                 std::vector<Double_t> &midGapIn, std::vector<Double_t> &midGapOut,
                                 std::vector<Double_t> &HmainSideMagIn, std::vector<Double_t> &HmainSideMagOut,
                                 std::vector<Double_t> &gapIn, std::vector<Double_t> &gapOut, std::vector<Double_t> &Z)
{

   const Int_t nMagnets = 9;
   magnetName.reserve(nMagnets);
   fieldDirection.reserve(nMagnets);
   for (auto i : {&dXIn, &dXOut, &dYIn, &dYOut, &dZ, &midGapIn, &midGapOut, &HmainSideMagIn, &HmainSideMagOut, &gapIn,
                  &gapOut, &Z}) {
      i->reserve(nMagnets);
   }
   ...
   if (fDesign == 8) {
      ...
      fieldDirection = {
         FieldDirection::up,   FieldDirection::up,   FieldDirection::up,   FieldDirection::up,   FieldDirection::up,
         FieldDirection::down, FieldDirection::down, FieldDirection::down, FieldDirection::down,
      };
...
void ShipMuonShield::ConstructGeometry()
{
   ...
   if (fDesign >= 7 && fDesign <= 9) {
      ...
      std::vector<Double_t> dXIn, dYIn, dXOut, dYOut, dZf, midGapIn, midGapOut, HmainSideMagIn, HmainSideMagOut, gapIn,
         gapOut, Z;
      const Int_t nMagnets = Initialize(magnetName, fieldDirection, dXIn, dYIn, dXOut, dYOut, dZf, midGapIn, midGapOut,
      ...
      auto *target_pit_shift =
         new TGeoTranslation("target_pit_shift", 0 * m, -2.5 * m, zEndOfAbsorb - 2 * m - z_transition);
      target_pit_shift->RegisterYourself();

      float mField = 1.6 * tesla;
      TGeoUniformMagField *fieldsAbsorber[4] = {
         new TGeoUniformMagField(0., mField, 0.), new TGeoUniformMagField(0., -mField, 0.),
         new TGeoUniformMagField(-mField, 0., 0.), new TGeoUniformMagField(mField, 0., 0.)};
      ...
      if (fDesign > 7) {
         ...
         auto *coat_shift =
            new TGeoTranslation("coat_shift", 0, 0, zEndOfAbsorb + absorber_half_length + absorber_offset);
         coat_shift->RegisterYourself();
         auto *coat_shift_transition = new TGeoTranslation(
            "coat_shift_transition", 0, 0, zEndOfAbsorb - z_transition + absorber_half_length + absorber_offset);
         ...
         auto *coatWall_shift = new TGeoTranslation("coatWall_shift", 0, 0,
                                                    zEndOfAbsorb + 2 * absorber_half_length + absorber_offset + 7 * cm);
         coatWall_shift->RegisterYourself();
         auto *coatWall_shift_transition =
            new TGeoTranslation("coatWall_shift_transition", 0, 0,
                                zEndOfAbsorb - z_transition + 2 * absorber_half_length + absorber_offset + 7 * cm);
         coatWall_shift_transition->RegisterYourself();
         coatWall->SetLineColor(kRed);
         tShield->AddNode(coatWall, 1, coatWall_shift);
      }

      auto *compRock = new TGeoCompositeShape("compRock", "rock - muon_shield_cavern:TCC8_shift"
      ...
      for (Int_t nM = 2; nM <= (nMagnets - 1); nM++) {
         ...
         CreateMagnet(magnetName[nM], iron, tShield, fields_s, fieldDirection[nM], dXIn[nM], dYIn[nM], dXOut[nM],
                      dYOut[nM], dZf[nM], midGapIn[nM], midGapOut[nM], HmainSideMagIn[nM], HmainSideMagOut[nM],
                      gapIn[nM], gapOut[nM], Z[nM], nM == 8, fStepGeo, SC_key);
         ...
         std::array<Double_t, 16> verticesIn = {
            -w1, -h1, +w1, -h1, +w1, +h1, -w1, +h1, -w1, -h1 + slope * 2. * length, +w1, -h1 + slope * 2. * length,
            +w1, +h1, -w1, +h1,
         };
         ...
         if (!fStepGeo) {
            ...
            TGeoVolume *pillar1 =
               gGeoManager->MakeArb8(TString::Format("pillar_%d", 2 * nM - 1), steel, length, verticesIn.data());
            TGeoVolume *pillar2 =
            ...
            tShield->AddNode(
               pillar1, 1,
               new TGeoTranslation(0, -0.5 * (dYIn[nM] + dXIn[nM] + 10 * m - fFloor), Z[nM] - dZf[nM] + length));
            tShield->AddNode(
               pillar2, 1,
               new TGeoTranslation(0, -0.5 * (dYOut[nM] + dXOut[nM] + 10 * m - fFloor), Z[nM] + dZf[nM] - length));
         }
      }
