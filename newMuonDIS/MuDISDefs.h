...
inline const std::array<TString, nMats> MatTypeStr = {
    "MS", "UBT", "SBTsens", "SBTfr", "SSTsens", "SSTfr", "HE", "AIR", "REST"};
...
struct MuonDISInBranches {
  ...
  bool SetupTree(TTree* aT, const TString& aLabel) {
    bool ok = true;
    ...
    ok &=
        (aT->SetBranchAddress("mudis_nDISdaughters_" + aLabel, &nDISdau) >= 0);
    ok &= (aT->SetBranchAddress("mudis_DISproducts_" + aLabel, &DISparticles) >=
           0);
    return ok;
  };
