...
# The rename script exchange all occurence of Snoopy or Snoopy
...
Prefix=$3
for i in $(ls Snoopy*); do 
  oldfile=$i
  newfile=$(echo $oldfile | sed "s/Snoopy/$DetectorName/")
  mv $oldfile $newfile
done 
...
find . -name "*.h" -exec sed -e "s/Snoopy/$DetectorName/g" $sedstring "{}" ";"
find . -name "*.h" -exec sed -e "s/Snoopy/$DetectorNameUpper/g" $sedstring "{}" ";"
find . -name "*.cxx" -exec sed -e "s/Snoopy/$DetectorName/g" $sedstring "{}" ";"
find . -name "*.cxx" -exec sed -e "s/Snoopy/$DetectorNameUpper/g" $sedstring "{}" ";"
...
sed -e "s#tutorial/Snoopy#$RelativeDir#g" $sedstring CMakeLists.txt
sed -e "s/Snoopy/$DetectorName/g" $sedstring CMakeLists.txt
sed -e "s/Snoopy/$DetectorNameUpper/g" $sedstring CMakeLists.txt
