...
public:
   hcalModule(Int_t modulenumber, Float_t x1 = 0, Float_t y1 = 0, Float_t x2 = 0, Float_t y2 = 0)
      : TObject(), fNumber(modulenumber), fX1(x1), fY1(y1), fX2(x2), fY2(y2), fEnergy(0), fEnergy2(0), fADC(-1111),
        fNeighbors(){};
   ...
   inline void GetNeighborsList(std::list<hcalModule *> &neib) const { neib = fNeighbors; }
   inline void SetNeighborsList(std::list<hcalModule *> &neib) { fNeighbors = neib; }
