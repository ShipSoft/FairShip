...
public:
   ...
   hcalInf()
      : TObject(), fVariables(NULL), fHcalStr(), fXPos(0.), fYPos(0.), fZPos(0.), fNLayers(0), fXSize(0), fYSize(0),
        fModuleSize(0.), fAbsorber(0.), fScin(0.), fTyveec(0.), fThicknessLayer(0.), fCellSize(0.), fHcalSize(),
        fECut(0.), fHCut(0.), fSemiX(0.0), fSemiY(0.0), fFastMC(-1), fSuccess(-1), fFileName(""){};
...
inline char hcalInf::GetType(Int_t x, Int_t y) const
{
   ...
   if (x < 0 || y < 0 || y > fHcalStr.GetLast())
      return 0;
   TObjString *str = (TObjString *)fHcalStr.At(y);
   if (str->GetString().Length() < x)
      return 0;
