...
hcalInf::hcalInf(const char *filename)
   : TObject(), fVariables(new TMap(200)), fHcalStr(), fXPos(0.), fYPos(0.), fZPos(0.), fNLayers(0), fN1Layers(0),
     fXSize(0), fYSize(0), fModuleSize(0.), fAbsorber(0.), fScin(0.), fTyveec(0.), fThicknessLayer(0.), fCellSize(0.),
     fHcalSize(), fECut(0.), fHCut(0.), fSemiX(0.), fSemiY(0.), fFastMC(-1), fSuccess(1), fFileName(filename)
{
   /**
      ...
      if (variable == "structure") {
         while (getline(file, buffer)) {
            ...
            if (!message.empty()) {
               if (-1 == ssize)
                  ssize = message.size();
               else if (ssize != (Int_t)message.size()) {
...
{
   if (parname.CompareTo("hcalversion") == 0)
      return kTRUE;
   return kFALSE;
}
...
void hcalInf::InitVariables()
{
   ...
   stri = "";
   stri += str->GetString().Length();
   AddVariable("xsize", stri);
   stri = "";
   stri += fHcalStr.GetLast() + 1;
