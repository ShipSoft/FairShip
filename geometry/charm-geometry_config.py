...
with ConfigRegistry.register_config("basic") as c:
    ...
    c.MufluxTargetStation.ironshield_x = 20.0 * u.cm
    ...
    c.MufluxTargetStation.concreteshield_z = 160.0 * u.cm
    ...
    c.MufluxTargetStation.floor_x = 500.0 * u.cm
    c.MufluxTargetStation.floor_y = 80.0 * u.cm
    c.MufluxTargetStation.floor_z = 800.0 * u.cm
    c.MufluxTargetStation.floorT34_x = 500.0 * u.cm
    c.MufluxTargetStation.floorT34_y = 16.0 * u.cm
    c.MufluxTargetStation.floorT34_z = 118.875 * u.cm
    c.MufluxTargetStation.floorRPC_x = 500.0 * u.cm
    ...
    c.MufluxTargetStation.floorRPC_z = 175.0 * u.cm
    ...
    if cTarget == 16:
        ...
        c.Box.PasSlabTh = (
            0.1 * u.cm
        )  # passive slab in ECC (lead for July 2018 measurement, molybdenum/tungsten for SHiP target replica)
    ...
    c.Box.GapPostTargetTh = (
        0.73 * u.cm
    )  # distance between end of the emulsion target and start of pixel box (Pixel origin)
    ...
    c.hadronAbsorber.z = -c.hadronAbsorber.length / 2.0
    ...
    c.target.sl = (
        0.54459 * u.cm
    )  # H20 slit *17 times; to get to the measured length by survey
    c.target.xy = 10.0 * u.cm  # new diameter of muflux target
    ...
    c.Spectrometer.H = (
        2 * c.Spectrometer.BasisH
        + c.Spectrometer.CoilD
        + c.Spectrometer.UpCoilH
        + c.Spectrometer.LowCoilH
    )

    # -----Drift tube part --------
    c.MufluxSpectrometer.v_drift = 1.0 / (
        72 * u.ns / u.mm
    )  # 1300 ns max for 36.3 mm drifttubes
    c.MufluxSpectrometer.sigma_spatial = 0.027 * u.cm  # from Daniel 8feb2018

    c.MufluxSpectrometer.TubeLength = 160.0 * u.cm
    c.MufluxSpectrometer.TubeLength12 = 110.0 * u.cm
    c.MufluxSpectrometer.tr12ydim = 110.0 * u.cm
    c.MufluxSpectrometer.tr34xdim = 200.0 * u.cm
    c.MufluxSpectrometer.tr12xdim = 50.0 * u.cm
    c.MufluxSpectrometer.tr34ydim = 160.0 * u.cm

    # parameters for drift tubes
    c.MufluxSpectrometer.InnerTubeDiameter = 3.63 * u.cm
    c.MufluxSpectrometer.WallThickness = 0.085 * u.cm
    c.MufluxSpectrometer.OuterTubeDiameter = (
        c.MufluxSpectrometer.InnerTubeDiameter + 2 * c.MufluxSpectrometer.WallThickness
    )
    ...
    c.MufluxSpectrometer.ViewvAngle = -60.0
    ...
    c.MufluxSpectrometer.distT1T2 = 11.0 * u.cm
    ...
    c.MufluxSpectrometer.goliathcentre_to_beam = (
        17.32 * u.cm + (c.Spectrometer.UpCoilH - c.Spectrometer.LowCoilH) / 2.0
    )
    c.MufluxSpectrometer.goliathcentre = 351.19 * u.cm

    c.MufluxSpectrometer.T1x_x = 4.22 * u.cm
    c.MufluxSpectrometer.T1x_y = -1.995 * u.cm
    c.MufluxSpectrometer.T1x_z = 0.0 * u.cm
    ...
    c.MufluxSpectrometer.T2v_z = 0.0 * u.cm
    ...
    if c.MufluxSpectrometer.muflux == True:
        c.Spectrometer.DX = 2.0 * u.m
        c.Spectrometer.DY = 1.6 * u.m
        c.Spectrometer.DZ = 16.0 * u.cm
    else:
    ...
    c.MufluxSpectrometer.DX = 2.0 * u.m
    ...
    c.MufluxSpectrometer.SurveyCharm_T3az = (
        15269 * u.mm + 70 * u.mm - 8896.09 * u.mm
    )  # todo: check th 70mm (probably forgt bolt and alignement sphere before)
    ...
    c.MufluxSpectrometer.SurveyCharm_T4az = (
        16572.225 * u.mm - 200 * u.mm - 8896.09 * u.mm
    )  # -200 mm because of wrong sign in first calcualtion
    ...
    c.MufluxSpectrometer.SurveyCharm_T4cz = (
        16571.625 * u.mm - 200 * u.mm - 8896.09 * u.mm
    )
    ...
    c.Spectrometer.D1Short = 3.36 * u.cm / 2.0
    ...
    c.PixelModules.D1short = 50.0 * u.um * 336  # = 1.68 u.cm. Each module has 336 rows.
    c.PixelModules.D1long = (
        ((78 * 250.0) + 450.0 + 500.0) * u.um * 2
    )  # = 4.09 * u.cm. Each module has 160 columns, the outer two and the centered two have different size.
    ...
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            -0.13 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 1
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            0.52 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 2
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            2.412 + i * c.PixelModules.DimZSithin / c.PixelModules.nSlice
        )
        # Module 3
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            3.09 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
    ...
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            5.17 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 5
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            5.79 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 6
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            7.77 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 7
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            8.46 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 8
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            10.462 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 9
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            11.17 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 10
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            13.162 + i * c.PixelModules.DimZSithick / c.PixelModules.nSlice
        )
        # Module 11
    for i in range(1 - c.PixelModules.nSlice, 1):
        ...
        c.PixelModules.zSi.append(
            13.85 + i * c.PixelModules.DimZSithin / c.PixelModules.nSlice
        )
    ...
    c.SciFi.DZ = (
        28.07 * u.cm
    )  # as difference between positions of SF_DHBR and x of SF_UHTL in the Survey document
    ...
    c.SciFi.zSi = []
    zscifigap = (
        c.SciFi.DZ / c.SciFi.nstations - c.SciFi.StationDimZ
    )  # distance between center of two stations, for the moment equally spaced
    # Module 0
    ...
    c.Scintillator.Scoring1X = 55.0 * u.cm
    c.Scintillator.Scoring1Y = 110.0 * u.cm
    ...
    c.Spectrometer.SZ = (
        c.Spectrometer.DZ * 2
        + c.PixelModules.zSi[c.PixelModules.numSi - 1]
        - c.PixelModules.zSi[0]
        + c.PixelModules.DimZSithick
        + 80 * u.cm
        + 4.5 * u.m
    )  # 4.5 m is the Goliath length

    c.PixelModules.DimZpixelbox = (
        c.PixelModules.zSi[c.PixelModules.numSi - 1]
        - c.PixelModules.zSi[c.PixelModules.nSlice - 1]
        + c.PixelModules.DimZSithick
    )
    ...
    c.PixelModules.DimZpixelbox = (
        34.0 * u.cm
    )  # correct physical size of surrounding box
    ...
    c.Spectrometer.zBox = (
        ...
        - c.Spectrometer.TS / 2
        - PixeltoGoliath
        - c.PixelModules.DimZpixelbox / 2.0
    )
    # position of mother volumes
    c.Box.zBox = (
        c.Spectrometer.zBox - c.PixelModules.DimZpixelbox / 2.0 - c.Box.GapPostTargetTh
    )
    ...
    c.SciFi.zBox = c.PixelModules.zBox - c.PixelModules.DimZpixelbox / 2.0 + 469.05
    ...
    c.MuonTagger.RPCthickness = 8.0 * u.cm
    ...
    if c.MufluxSpectrometer.muflux == True:
        ...
        c.MuonTagger.zBox = 791.0 * u.cm + 175.9875 * u.cm
    else:
        c.MuonTagger.zBox = (
            c.Spectrometer.zBox
            + c.PixelModules.DimZpixelbox / 2.0
