...
with ConfigRegistry.register_config("basic") as c:
    ...
    c.TimeDet.z = (
        c.Chamber6.z + c.chambers.Tub6length + c.TimeDet.DZ + 1 * u.cm
    )  # safety margin
    ...
    c.SplitCal.XMax = 480.0 * u.cm / 2.0  # 290.*u.cm  #half length
    c.SplitCal.YMax = (
        720.0 * u.cm / 2.0
    ...
    if nuTauTargetDesign == 4:
        ...
        c.tauMudet.GapM = (
            20 * u.cm
        )  # magnetized region length, gap between sepctrometer planes
    ...
    if c.NuTauTarget.Design == 4:  # new geometry, for ECN3
        ...
        c.NuTauTarget.EmX = 20.0 * u.cm
        c.NuTauTarget.EmY = 20.0 * u.cm
    ...
    if c.NuTauTT.design == 4:
        ...
        c.NuTauTT.scifimat_width = 13.06 * u.cm  # old value 13.045
        c.NuTauTT.scifimat_hor = (
            c.NuTauTT.n_hor_planes * c.NuTauTT.scifimat_width + 2.9 * u.cm
        )  # endpieces (~2.9cm from previous geom)
        c.NuTauTT.scifimat_vert = (
            c.NuTauTT.n_vert_planes * c.NuTauTT.scifimat_width + 2.9 * u.cm
        )  # endpieces (~2.9cm from previous geom)
        c.NuTauTT.scifimat_z = 0.145 * u.cm  # Scintillating fiber mat
