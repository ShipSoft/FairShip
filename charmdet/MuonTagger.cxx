...
MuonTagger::MuonTagger()
   : FairDetector("MuonTagger", "", kTRUE), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fMuonTaggerPointCollection(new TClonesArray("MuonTaggerPoint"))
{
}

MuonTagger::MuonTagger(const char *name, const Double_t BX, const Double_t BY, const Double_t BZ, const Double_t zBox,
                       Bool_t Active, const char *Title)
   : FairDetector(name, true, kMuonTagger), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fMuonTaggerPointCollection(new TClonesArray("MuonTaggerPoint"))
{
   BoxX = BX;
   BoxY = BY;
...
void MuonTagger::ConstructGeometry()
{
   ...
   for (int n = 0; n < npassive; n++) {
      if (n == 0) {
         ...
         if (n == 1) {
            VMuonBox->AddNode(VPassive, n + 1,
                              new TGeoTranslation(0, 0,
                                                  fRPCz[n] - PasThicknessz[n] / 2. -
                                                     (fRPCz[n] - fRPCz[n - 1] - PasThicknessz[n]) / 2.));
         }
         ...
         // //last wall had been disaligned with respect to the others
            ...
            VMuonBox->AddNode(VPassive1, n + 1,
                              new TGeoTranslation(0, 0,
                                                  fRPCz[n] - PasThicknessz[n] / 2. -
                                                     (fRPCz[n] - fRPCz[n - 1] - PasThicknessz[n]) / 2.));
         }
   ...
   for (int n = 1; n < nsensitive + 1; n++) {
      ...
      VSensitive[n - 1]->AddNode(
         Ground, 10000 * n + 2000,
         new TGeoTranslation(0, 0,
                             -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz - fStripz / 2));
      VSensitive[n - 1]->AddNode(Electrode, 10000 * n + 3000,
      ...
      VSensitive[n - 1]->AddNode(Electrode, 10000 * n + 5000,
                                 new TGeoTranslation(0, 0, fGapThickness / 2 + 0.025 + fElectrodeThickness / 2));
      VSensitive[n - 1]->AddNode(
         Ground, 10000 * n + 6000,
         new TGeoTranslation(0, 0,
      ...
      for (int m = 1; m < fNHstrips + 1; m++) {
         if (m == 1) {
            VSensitive[n - 1]->AddNode(
               HFoam_ext, 10000 * n + 7000 + m,
               new TGeoTranslation(0, SensY / 2 - fHstrip_ext / 2,
                                   -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz / 2));
            VSensitive[n - 1]->AddNode(
               Hstrip_ext, 10000 * n + m,
               new TGeoTranslation(0, SensY / 2 - fHstrip_ext / 2,
            ...
            if (m == fNHstrips) {
               VSensitive[n - 1]->AddNode(
                  HFoam_ext, 10000 * n + 7000 + m,
                  new TGeoTranslation(0, SensY / 2 - fHstripoffset * (m - 1) - fHstripy * (m - 2) - 3 * fHstrip_ext / 2,
                                      -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz / 2));
               VSensitive[n - 1]->AddNode(
                  Hstrip_ext, 10000 * n + m,
                  new TGeoTranslation(0, SensY / 2 - fHstripoffset * (m - 1) - fHstripy * (m - 2) - 3 * fHstrip_ext / 2,
               ...
               VSensitive[n - 1]->AddNode(
                  HFoam, 10000 * n + 7000 + m,
                  new TGeoTranslation(
                     0, SensY / 2 - fHstrip_ext - fHstripoffset * (m - 1) - fHstripy * (m - 2) - fHstripy / 2,
                     -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz / 2));
               VSensitive[n - 1]->AddNode(
                  Hstrip, 10000 * n + m,
                  new TGeoTranslation(
      ...
      for (int m = 1; m < fNVstrips + 1; m++) {
         if (m == 1) {
            VSensitive[n - 1]->AddNode(
               Vstrip_R, 10000 * n + 1000 + m,
               new TGeoTranslation(SensX / 2 - fVstrip_R / 2, 0,
                                   fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz / 2));
            VSensitive[n - 1]->AddNode(
               VFoam_R, 10000 * n + 8000 + m,
               new TGeoTranslation(SensX / 2 - fVstrip_R / 2, 0,
                                   fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz + fStripfoamz / 2));
         } else {
            if (m == fNVstrips) {
               VSensitive[n - 1]->AddNode(
                  Vstrip_L, 10000 * n + 1000 + m,
                  new TGeoTranslation(SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstrip_R -
                                         fVstrip_L / 2,
                                      0, fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz / 2));
               VSensitive[n - 1]->AddNode(
                  VFoam_L, 10000 * n + 8000 + m,
                  new TGeoTranslation(SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstrip_R -
                                         fVstrip_L / 2,
                                      0, fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz + fStripfoamz / 2));
            } else {
               VSensitive[n - 1]->AddNode(
                  Vstrip, 10000 * n + 1000 + m,
                  new TGeoTranslation(SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstripx / 2 -
                                         fVstrip_R,
                                      0, fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz / 2));
               VSensitive[n - 1]->AddNode(
                  VFoam, 10000 * n + 8000 + m,
                  new TGeoTranslation(SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstripx / 2 -
                                         fVstrip_R,
                                      0, fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz + fStripfoamz / 2));
            }
...
MuonTaggerPoint *MuonTagger::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time,
