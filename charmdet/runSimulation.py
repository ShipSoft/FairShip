...
def run_FixedTarget(start):
    ...
    for n in range(start, start + ncpus):
        cmd = (
            "python $FAIRSHIP/muonShieldOptimization/run_MufluxfixedTarget.py -n "
            + str(N)
            + " -e 1 -P -o run-"
            + str(n)
            + " &"
        ...
        os.system(cmd)
        while 1 > 0:
            if count_python_processes("run_MufluxfixedTarget") < ncpus:
                break
            time.sleep(100)
...
def submit2Condor(eospath, directory, filename):
    ...
    f = open(sub, "w")
    f.write(
        "executable            = /afs/cern.ch/user/t/trufship/muflux/runDriftTubeScript.sh\n"
    )
    f.write(
        "arguments             = "
        + command
...
def getFilesFromEOS(E="10.0_withCharmandBeauty"):  # E="1.0"
    ...
    temp = subprocess.check_output(
        "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eospath, shell=True
    )
    for x in temp.split("\n"):
        if x.find(E) < 0 or x.find("_mu") < 0:
            continue  # includes charm
        fname = x[x.find("/eos") :]
        nentries = 0
...
def getFilesEOS(D):
    ...
    temp = subprocess.check_output(
        "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eospathSim, shell=True
    )
    fl = []
    for x in temp.split("\n"):
        if x.find("pythia8_Geant4") < 0:
            continue
        d = x[x.rfind("/") + 1 :]
...
def simulationStep(fnames=[]):
    ...
    for fname in fnames:
        ...
        if odir in os.listdir("."):
            continue
        cmd = (
            "python $FAIRSHIP/macro/run_simScript.py -n "
            + str(N)
            + " --MuonBack --charm=1 --CharmdetSetup=0 --output "
            + odir
            + " -f "
            + fname
            + " &"
        ...
        os.system(cmd)
        while 1 > 0:
            if count_python_processes("run_simScript") < ncpus:
                break
            time.sleep(100)
    print("finished all the tasks.")
...
def digiStep(fnames=[]):
    ...
    for fname in fnames:
        ...
        cmd = (
            "python $FAIRSHIP/macro/runMufluxDigi.py -n 9999999 -f "
            + mcFile
            + " -g "
            + geoFile
            + " &"
        ...
        os.chdir("../")
        while 1 > 0:
            if count_python_processes("runMufluxDigi") < ncpus:
                break
            time.sleep(100)
    print("finished all the tasks.")
...
def findDirWithSim():
    ...
    for fname in fnames:
        ...
        for x in os.listdir(fname):
            if not x.find("geofile_full.conical.MuonBack-TGeant4") < 0:
                geo = True
            if not x.find("roottmp") < 0:
                tmp = True
            if not x.find("dig.root") < 0:
                digi = True
            if not x.find("dig_RT") < 0:
                reco += 1
            if not x.find("dig-") < 0:
                digiSplit += 1
        if tmp:
            stats["tmp"].append(fname)
        elif digi:
            stats["digi"].append(fname)
        elif geo:
            if reco == 0 and digiSplit == 0:
                stats["simOnly"].append(fname)
        else:
        ...
        try:
            if f.cbmsim.GetEntries() > 0:
                ...
                os.system(
                    "mv "
                    + x
                    + "/ship.conical.MuonBack-TGeant4.roottmp "
                    + x
        ...
        for i in range(splitFactor):
            ...
            if digiFile in os.listdir("."):
                ...
                if eospath:
                    ...
                    continue
            cmd = (
                "python $FAIRSHIP/charmdet/drifttubeMonitoring.py -c recoStep1 -u 1 -f "
                + recoFile
                + " &"
            )
...
def checkFilesWithTracks(D=".", splitFactor=5, dimuon=False):
    ...
    for fname in fnames:
        ...
        for i in range(splitFactor):
            ...
            if dimuon:
                recoFile = recoFile.replace(".root", "_dimuon99.root")
            if D == ".":
                dirList = os.listdir(fname)
            else:
                dirList = subprocess.check_output(
                    "xrdfs " + os.environ["EOSSHIP"] + " ls " + fname, shell=True
                )
            if recoFile.replace(fname + "/", "") in dirList:
                ...
                if sTree:
                    if sTree.GetBranch("FitTracks"):
                        ...
                        size = sTree.GetBranch("FitTracks").GetTotalSize() / float(
                            sTree.GetEntries()
                        )
                        fileListPer[fname][
                            recoFile.replace(D + "." + fname + "/", "")
                        ] = size
...
def makeHistos(D=".", splitFactor=5, command="anaResiduals"):
    commandToHist = {
        "anaResiduals": "histos-analysis-",
        "momResolution": "histos-momentumResolution-",
        "plotDTPoints": "histos-DTPoints-",
    }
    if D == ".":
        ...
        for df in fileList:
            ...
            if not commandToHist[command] + tmp[1] in os.listdir("."):
                cmd = (
                    "python $FAIRSHIP/charmdet/drifttubeMonitoring.py -c "
                    + command
                    + " -f "
                    + tmp[1]
                    + " &"
        ...
        for x in temp.split("\n"):
            ...
            for y in temp2.split("\n"):
                ...
                if interactive:
                    ...
                    submit2Condor(eospathSim, d, y[y.rfind("/") + 1 :])
            os.chdir("../")
    else:
        eospathSim10GeV = "/eos/experiment/ship/data/muflux/MC/19feb2019"
        fileList = []
        temp = subprocess.check_output(
            "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eospathSim10GeV, shell=True
        )
        for x in temp.split("\n"):
            if x.find("RT.root") < 0:
                continue
        ...
        for fname in fileList:
            if os.path.isfile(commandToHist[command] + fname[fname.rfind("/") + 1 :]):
                continue
            cmd = (
                "python $FAIRSHIP/charmdet/drifttubeMonitoring.py -c "
                + command
    ...
    if D == ".":
        ...
        for df in fileList:
            ...
            if not "histos-momentumResolution-" + tmp[1] in os.listdir("."):
                cmd = (
                    "python $FAIRSHIP/charmdet/drifttubeMonitoring.py -c momResolution -f "
                    + tmp[1]
                    + " &"
                )
                print("execute:", cmd)
    ...
    if D == ".":
        ...
        for df in fileList:
            ...
            if not "histos-residuals-" + tmp[1] in os.listdir("."):
                cmd = (
                    "python $FAIRSHIP/charmdet/drifttubeMonitoring.py -c alignment -f "
                    + tmp[1]
                    + " &"
                )
                print("execute:", cmd)
        ...
        temp = subprocess.check_output(
            "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eospathSim1GeV, shell=True
        )
        for x in temp.split("\n"):
            if x.find("pythia8_Geant4") < 0:
                continue
            ...
            os.chdir(d)
            temp2 = subprocess.check_output(
                "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eospathSim1GeV + "/" + d,
                shell=True,
            )
            fileList = []
            for y in temp2.split("\n"):
...
def exportToEos(
    destination="/eos/experiment/ship/user/truf/muflux-sim/1GeV", update=True
):
    remote = subprocess.check_output(
        "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + destination, shell=True
    ).split("\n")
    fnames = getFilesLocal()
    for D in fnames:
        if not D in remote:
            os.system(
                "xrdfs " + os.environ["EOSSHIP"] + " mkdir  " + destination + "/" + D
            )
        remoteD = subprocess.check_output(
            "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + destination + "/" + D,
            shell=True,
        ).split("\n")
        for f in os.listdir(D):
            if f in remoteD and update:
                continue
            fname = D + "/" + f
...
def mergeHistos(command="anaResiduals"):
    ...
    commandToSum = {
        "anaResiduals": "momDistributions-",
        "momResolution": "momentumResolution-",
        "plotDTPoints": "DTPoints-",
        "alignment": "residuals-",
    }
    ...
    for d in dirList:
        for x in os.listdir(d):
            ...
            if d.find("charm") > 0:
                z = "charm"
            if not x.find(commandToHist[command]) < 0:
                cmd[z] += d + "/" + x + " "
    for z in ["charm", "mbias"]:
        os.system(cmd[z])
...
def checkForFilesWithTracks():
    ...
    for x in temp.split("\n"):
        ...
        if test.GetKey("cbmsim"):
            if test.cbmsim.GetBranch("FitTracks"):
                trList.append(fname)
...
def mergeHistos10(case="residuals"):
    ...
    for x in os.listdir("."):
        if n == 0:
            if case == "residuals":
                cmd = "hadd -f residuals-" + str(N) + ".root "
            elif case == "momResolution":
                cmd = "hadd -f momentumResolution-" + str(N) + ".root "
            else:
                cmd = "hadd -f momDistributions-" + str(N) + ".root "
        if case == "residuals" and not x.find("histos-residuals") < 0:
            cmd += x + " "
        elif case == "momResolution" and not x.find("momentumResolution") < 0:
            cmd += x + " "
        elif case == "momDistribution" and not x.find("analysis") < 0:
            cmd += x + " "
        n += 1
        if n == 500:
            os.system(cmd)
            ...
            N += 1
    if case == "residuals":
        histname = "residuals.root "
    elif case == "momResolution":
        histname = "momentumResolution.root "
    else:
        histname = "momDistributions.root "
    cmd = "hadd -f " + histname
...
def redoMuonTracks():
    ...
    for df in fileList:
        ...
        if len(tmp) > 1:
            os.chdir(tmp[0])
            cmd = (
                "python $FAIRSHIP/charmdet/drifttubeMonitoring.py -c  recoMuonTaggerTracks -u 1 -f "
                + tmp[1]
                + " &"
            )
            print("execute:", cmd)
        ...
        for f in os.listdir(remote + "/" + d):
            if f.find("histo") < 0 and not f.find("ship.conical") < 0:
                if not check:
                    ...
                    cmd = (
                        "python /home/truf/muflux/simulation/drifttubeMonitoring.py -c  splitOffBoostedEvents -f "
                        + f
                        + " &"
                    )
                    print("execute:", cmd)
...
def checkStatistics(splitFactor=5):
    ...
    for f in simFiles:
        if f.find("charm") > 0:
            Nsim["charm"] += simFiles[f]
        else:
            Nsim["mbias"] += simFiles[f]
    allFiles = {}
    ...
    for dname in allFiles:
        n = 0
        for x in reco:
            if not x.find(dname) < 0:
                n += 1
        fraction = n / float(splitFactor)
        print("fraction:", dname, fraction)
        if dname.find("charm") > 0:
            Nreco["charm"] += fraction * allFiles[dname]
        else:
            Nreco["mbias"] += fraction * allFiles[dname]
    ...
    print(
        "internal MC normalization, to be applied to charm",
        1.8 * Nreco["mbias"] / Nsim["mbias"] / (10.2 * Nreco["charm"] / Nsim["charm"]),
    )
