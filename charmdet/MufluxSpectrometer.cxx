...
MufluxSpectrometer::MufluxSpectrometer()
   : FairDetector("HighPrecisionTrackers", kTRUE, kMufluxSpectrometer), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(),
     fMom(), fTime(-1.), fLength(-1.), fELoss(-1),
     fMufluxSpectrometerPointCollection(new TClonesArray("MufluxSpectrometerPoint"))
{
}

MufluxSpectrometer::MufluxSpectrometer(const char *name, const Double_t DX, const Double_t DY, const Double_t DZ,
                                       Bool_t Active, const char *Title)
   : FairDetector(name, Active, kMufluxSpectrometer), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(),
     fTime(-1.), fLength(-1.), fELoss(-1),
     fMufluxSpectrometerPointCollection(new TClonesArray("MufluxSpectrometerPoint"))
{
   DimX = DX;
   DimY = DY;
...
void MufluxSpectrometer::SetTubePitch_T1u(Double_t tubepitch_T1u, Double_t T1u_const, Double_t T1u_const_2,
                                          Double_t T1u_const_3, Double_t T1u_const_4)
{
...
void MufluxSpectrometer::SetTubePitch_T2v(Double_t tubepitch_T2v, Double_t T2v_const, Double_t T2v_const_2,
                                          Double_t T2v_const_3, Double_t T2v_const_4)
{
...
void MufluxSpectrometer::SetTStationsZ(Double_t T1z, Double_t T1x_z, Double_t T1u_z, Double_t T2z, Double_t T2v_z,
                                       Double_t T2x_z, Double_t T3z, Double_t T4z)
{
   fT1z = T1z;
   fT1x_z = T1x_z;
   fT1u_z = T1u_z;
...
void MufluxSpectrometer::SetTStationsX(Double_t T1x_x, Double_t T1u_x, Double_t T2x_x, Double_t T2v_x, Double_t T3x,
                                       Double_t T4x)
{
   fT1x_x = T1x_x;
   fT1u_x = T1u_x;
...
void MufluxSpectrometer::SetTStationsY(Double_t T1x_y, Double_t T1u_y, Double_t T2x_y, Double_t T2v_y, Double_t T3y,
                                       Double_t T4y)
{
   fT1x_y = T1x_y;
   fT1u_y = T1u_y;
...
void MufluxSpectrometer::ConstructGeometry()
{
   ...
   if (fMuonFlux) {
      ...
      TGeoBBox *platebox_12 = new TGeoBBox("platebox_12", ftr12xdim / 2. + fTubes_pitch, plate_thickness / 2.,
      ...
      TGeoBBox *DriftTube1 = new TGeoBBox("DriftTube1", DimX / 2 + 1. * m / 2, DimY / 2 + 0.9 * m / 2,
                                          DimZ + 2 * cm + fdiststereoT1 / 2 + eps);
      TGeoVolume *volDriftTube1 = new TGeoVolume("volDriftTube1", DriftTube1, air);
      volDriftTube1->SetLineColor(kBlue - 5);

      TGeoBBox *DriftTube2 = new TGeoBBox("DriftTube2", DimX / 2 + 1. * m / 2, DimY / 2 + 1. * m / 2,
                                          DimZ + 2 * cm + fdiststereoT2 / 2 + eps);
      ...
      for (Int_t statnb = 1; statnb < 3; statnb++) {
         ...
         for (Int_t vnb = 0; vnb < 2; vnb++) {
            ...
            if (angle == 0.) {
               ...
               if (statnb == 1) {
                  t5.SetTranslation(fT1u_x - (ftr12ydim / 2. + eps + plate_thickness / 2 + fTubes_pitch) *
                                                sin(TMath::Pi() * angle / 180.),
                                    fT1u_y +
                                       (ftr12ydim / 2. + eps + plate_thickness / 2) * cos(TMath::Pi() * angle / 180.),
               ...
               if (statnb == 2) {
                  t5b.SetTranslation(fT2v_x - (ftr12ydim / 2. + eps + plate_thickness / 2 + fTubes_pitch) *
                                                 sin(TMath::Pi() * angle / 180.),
                                     fT2v_y +
                                        (ftr12ydim / 2. + eps + plate_thickness / 2) * cos(TMath::Pi() * angle / 180.),
                                     (vnb - 1) * fDeltaz_view);
            ...
            for (Int_t pnb = 0; pnb < 2; pnb++) {
               ...
               nmplane_12 += pnb;
               TGeoBBox *plane_12 = new TGeoBBox("plane box_12", ftr12xdim / 2. + eps / 2. + 2 * fTubes_pitch,
                                                 ftr12ydim / 2. + eps / 2., planewidth / 2. + eps / 2 + 0.0111);
               ...
               if (statnb == 1) {
                  if (angle == 0.) {
                     ...
                     t3.SetTranslation(fT1u_x, fT1u_y,
                                       (vnb - 1.) * (fDeltaz_view) + (pnb - 1. / 2.) * fDeltaz_plane12 + fdiststereoT1);
                  }
               }
               if (statnb == 2) {
                  if (angle == 0.) {
                     t3.SetTranslation(fT2x_x, fT2x_y,
                                       (vnb - 1.) * (fDeltaz_view) + (pnb - 1. / 2.) * fDeltaz_plane12 + fdiststereoT2);
                  } else {
               ...
               for (Int_t lnb = 0; lnb < 2; lnb++) {
                  ...
                  nmlayer_12 += lnb;
                  TGeoBBox *layer_12 = new TGeoBBox("layer box_12", ftr12xdim / 2. + 2 * fTubes_pitch, ftr12ydim / 2.,
                                                    layerwidth / 2. + 0.011);
                  TGeoVolume *layerbox_12 = new TGeoVolume(nmlayer_12, layer_12, air);
                  layerbox_12->SetVisibility(kFALSE);
                  if ((statnb == 1 && pnb == 0) && (lnb == 0)) {
                     planebox_12->AddNode(layerbox_12, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000,
                  ...
                  for (Int_t snb = 1; snb < fTubes_per_layer_tr12 + 1; snb++) {
                     ...
                     if (angle != 0.) {
                        if (statnb == 2) {
                           if (lnb == 0 && pnb == 0) {
                              t6s.SetTranslation(
                                 fT2v_const - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT2v_z);
                           }
                           if (lnb == 1 && pnb == 0) {
                              t6s.SetTranslation(
                                 fT2v_const_2 - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT2v_z);
                           }
                           if (lnb == 0 && pnb == 1) {
                              t6s.SetTranslation(
                                 fT2v_const_3 - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT2v_z);
                           }
                           if (lnb == 1 && pnb == 1) {
                              t6s.SetTranslation(
                                 fT2v_const_4 - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT2v_z);
                           }
                        }
                        if (statnb == 1) {
                           if (lnb == 0 && pnb == 0) {
                              t6s.SetTranslation(
                                 fT1u_const - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT1u_z);
                           }
                           if (lnb == 1 && pnb == 0) {
                              t6s.SetTranslation(
                                 fT1u_const_2 - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT1u_z);
                           }
                           if (lnb == 0 && pnb == 1) {
                              t6s.SetTranslation(
                                 fT1u_const_3 - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT1u_z);
                           }
                           if (lnb == 1 && pnb == 1) {
                              t6s.SetTranslation(
                                 fT1u_const_4 - ftr12xdim / 2. + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1) -
                                    fOffset_plane12 * (pnb - 1) - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                 0, fT1u_z);
                           }
                        }
                     ...
                     layerbox_12->AddNode(
                        drifttube_12, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000 + snb, h6s);
                     layerbox_12->AddNode(
                        gas_12, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000 + snb, h6s);
                     layerbox_12->AddNode(
                        wire_12, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 3000 + snb, h6s);
   ...
   TGeoTranslation *tr1 = new TGeoTranslation(
      -TransversalSize / 2 + side1 / 2, Height / 2 - BasisHeight - UpCoilHeight / 2, -LongitudinalSize / 2 + base1 / 2);
   ...
   TGeoTranslation *tr1low = new TGeoTranslation(
      -TransversalSize / 2 + side1 / 2, Height / 2 - BasisHeight - UpCoilHeight - (CoilDistance + LowCoilHeight) / 2,
   ...
   TGeoTranslation *tr2low = new TGeoTranslation(
      -TransversalSize / 2 + side1 + side3 / 2,
      Height / 2 - BasisHeight - UpCoilHeight - (CoilDistance + LowCoilHeight) / 2, -LongitudinalSize / 2 + base2 / 2);
   ...
   TGeoTranslation *tr1blow = new TGeoTranslation(
      -TransversalSize / 2 + side1 / 2, Height / 2 - BasisHeight - UpCoilHeight - (CoilDistance + LowCoilHeight) / 2,
      LongitudinalSize / 2 - side2 / 2);
   ...
   TGeoTranslation *tr2blow = new TGeoTranslation(
      -TransversalSize / 2 + side1 + side3 / 2,
      Height / 2 - BasisHeight - UpCoilHeight - (CoilDistance + LowCoilHeight) / 2, LongitudinalSize / 2 - base2 / 2);
   ...
   TGeoTranslation *tr1_d = new TGeoTranslation(
      TransversalSize / 2 - side1 / 2, Height / 2 - BasisHeight - (UpCoilHeight + LowCoilHeight + CoilDistance) / 2,
   ...
   TGeoTranslation *tr1_c = new TGeoTranslation(
      TransversalSize / 2 - side1 / 2, Height / 2 - BasisHeight - (UpCoilHeight + LowCoilHeight + CoilDistance) / 2,
      LongitudinalSize / 2 - base1 / 2);
   ...
   if (fMuonFlux) {
      ...
      for (Int_t statnb = 3; statnb < 5; statnb++) {
         ...
         // fDeltaz_view/2.);
         TGeoBBox *platebox_34 =
            new TGeoBBox("platebox_34", ftr34xdim / 2. + fTubes_pitch / 2., plate_thickness / 2., fDeltaz_view / 2.);
         ...
         for (Int_t pnb = 0; pnb < 2; pnb++) {
            ...
            nmplane_34 += pnb;
            TGeoBBox *plane_34 = new TGeoBBox("plane box_34", ftr34xdim / 2. + eps / 2. + 2 * fTubes_pitch,
                                              ftr34ydim / 2 + eps / 2., planewidth / 2. + eps / 2 + 0.137);
            ...
            for (Int_t lnb = 0; lnb < 2; lnb++) {
               ...
               nmlayer_34 += lnb;
               TGeoBBox *layer_34 = new TGeoBBox("layer box_34", ftr34xdim / 2. + 2 * fTubes_pitch, ftr34ydim / 2.,
                                                 layerwidth / 2. + 0.136);
               TGeoVolume *layerbox_34 = new TGeoVolume(nmlayer_34, layer_34, air);
               layerbox_34->SetVisibility(kFALSE);
               planebox_34->AddNode(layerbox_34, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000,
                                    new TGeoTranslation(0, 0, (lnb - 1. / 2.) * fDeltaz_layer12));
               ...
               for (Int_t snb = 1; snb < fTubes_per_layer_tr34 + 1; snb++) {
                  ...
                  TGeoHMatrix *h6s = new TGeoHMatrix(c6s);
                  layerbox_34->AddNode(
                     drifttube, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000 + snb, h6s);
                  layerbox_34->AddNode(gas, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000 + snb,
                                       h6s);
                  layerbox_34->AddNode(
      ...
      for (Int_t statnb = 3; statnb < 5; statnb++) {
         for (Int_t mnb = 0; mnb < 5; mnb++) {
            ...
            TGeoBBox *platebox =
               new TGeoBBox("platebox", ftr12xdim / 2. + fTubes_pitch / 2., plate_thickness / 2., fDeltaz_view / 2.);
            ...
            if (statnb == 3) {
               ...
               if (mnb == 4) {
                  t5.SetTranslation(fSurveyCharm_T3x[mnb],
                                    fSurveyCharm_T3y[mnb] + 1.1 / 1.6 * fTube_length / 2. + eps + plate_thickness / 2 +
                                       0.5 * cm,
                                    fSurveyCharm_T3z[mnb] - zcenter_t34);
                  t6.SetTranslation(fSurveyCharm_T3x[mnb],
                                    fSurveyCharm_T3y[mnb] + 1.1 / 1.6 * -fTube_length / 2. - eps - plate_thickness / 2 -
            ...
            if (statnb == 4) {
               ...
               if (mnb == 4) {
                  t5.SetTranslation(fSurveyCharm_T4x[mnb],
                                    fSurveyCharm_T4y[mnb] + 1.1 / 1.6 * fTube_length / 2. + eps + plate_thickness / 2 +
                                       0.5 * cm,
                                    fSurveyCharm_T4z[mnb] - zcenter_t34);
                  t6.SetTranslation(fSurveyCharm_T4x[mnb],
                                    fSurveyCharm_T4y[mnb] + 1.1 / 1.6 * -fTube_length / 2. - eps - plate_thickness / 2 -
            ...
            for (Int_t pnb = 0; pnb < 2; pnb++) {
               ...
               nmplane_34 += "_module_";
               nmplane_34 += mnb;
               TGeoBBox *plane_34 = new TGeoBBox("plane box_34", ftr12xdim / 2. + eps / 2. + 2 * fTubes_pitch,
                                                 ftr34ydim / 2 + eps / 2., planewidth / 2. + eps / 2);
               TGeoVolume *planebox_34 = new TGeoVolume(nmplane_34, plane_34, air);
               ...
               volDriftTubeCharm->AddNode(planebox_34,
                                          mnb * 100000000 + statnb * 10000000 + vnb * 1000000 + pnb * 100000, j3);
               ...
               for (Int_t lnb = 0; lnb < 2; lnb++) {
                  ...
                  nmlayer_34 += lnb;
                  TGeoBBox *layer_34 =
                     new TGeoBBox("layer box_34", ftr12xdim / 2. + 2 * fTubes_pitch, ftr34ydim / 2., layerwidth / 2.);
                  TGeoVolume *layerbox_34 = new TGeoVolume(nmlayer_34, layer_34, air);
                  layerbox_34->SetVisibility(kFALSE);
                  planebox_34->AddNode(layerbox_34,
                                       mnb * 100000000 + statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000,
                                       new TGeoTranslation(0, 0, (lnb - 1. / 2.) * fDeltaz_layer12));
                  ...
                  for (Int_t snb = 1; snb < fTubes_per_layer_tr12 + 1; snb++) {
                     ...
                     if (statnb == 3) {
                        t6s.SetTranslation(fSurveyCharm_T3x[mnb] + fTubes_pitch * (snb - 1) - 225.5 * mm -
                                              fOffset_plane12 * (pnb - 1) -
                                              fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                           fSurveyCharm_T3y[mnb], fSurveyCharm_T3z[mnb] - zcenter_t34);
                     }
                     if (statnb == 4) {
                        t6s.SetTranslation(fSurveyCharm_T4x[mnb] + fTubes_pitch * (snb - 1) - 225.5 * mm -
                                              fOffset_plane12 * (pnb - 1) -
                                              fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                           fSurveyCharm_T4y[mnb], fSurveyCharm_T4z[mnb] - zcenter_t34);
                     }
                     ...
                     if (mnb == 4) {
                        layerbox_34->AddNode(
                           short_drifttube,
                           statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000 + snb + 12 * mnb, h6s);
                        layerbox_34->AddNode(
                           short_gas,
                           statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000 + snb + 12 * mnb, h6s);
                        layerbox_34->AddNode(
                           short_wire,
                           statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 3000 + snb + 12 * mnb, h6s);
                     } else {
                        layerbox_34->AddNode(
                           drifttube,
                           statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000 + 12 * mnb + snb, h6s);
                        layerbox_34->AddNode(
                           gas, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000 + 12 * mnb + snb,
                           h6s);
                        layerbox_34->AddNode(
                           wire, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 3000 + 12 * mnb + snb,
                           h6s);
                     }
                     // end of straw loop
                  }
...
Bool_t MufluxSpectrometer::ProcessHits(FairVolume *vol)
{
   ...
   if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
      ...
      AddHit(fTrackID, tube_uniqueId, TVector3(xmean, ymean, zmean), TVector3(fMom.Px(), fMom.Py(), fMom.Pz()), fTime,
             deltaTrackLength, fELoss, pdgCode, dist2Wire);
      if (dist2Wire > 2.23) {
         std::cout << "addhit " << dist2Wire << " tube id " << tube_uniqueId << " pdgcode " << pdgCode << " dot prod "
                   << pq.Dot(uCrossv) << std::endl;
...
MufluxSpectrometerPoint *MufluxSpectrometer::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom,
