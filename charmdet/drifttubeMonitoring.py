...
def displayDTLayers():
    ...
    if not 'layerDisplay' in h: ut.bookCanvas(h,key='layerDisplay',title='Upstream Layers',nx=1600,ny=1200,cx=1,cy=0)
...
def plotEvent(n=-1):
    ...
    if not 'simpleDisplay' in h: ut.bookCanvas(h,key='simpleDisplay',title='simple event display',nx=1200,ny=800,cx=1,cy=0)
    rc = h[ 'simpleDisplay'].cd(1)
    ...
    if not onlyPlotting:
        for event in sTree:
            for hit in event.Digi_MufluxSpectrometerHits:
                ...
                if not channel+tot in h: h[channel+tot]=h['TDC'+str(nRT)+tot].Clone(channel)
                rc = h[channel+tot].Fill(hit.GetDigi()-t0)
    if not 'hitMapsX' in h: ut.bookCanvas(h,key='hitMapsX',title='Hit Maps All Layers',nx=1600,ny=1200,cx=4,cy=6)
    if not 'TDCMapsX' in h: ut.bookCanvas(h,key='TDCMapsX',title='TDC Maps All Layers',nx=1600,ny=1200,cx=5,cy=10)
    if not 'TDCMapsX_noToT' in h: ut.bookCanvas(h,key='TDCMapsX_noToT',title='TDC Maps All Layers noToT',nx=1600,ny=1200,cx=5,cy=10)
    j  = 0
    jt = 0
    for s in range(1,5):
        for view in ['_x','_u','_v']:
            for p in range(2):
                for l in range(2):
                    if not view in xLayers[s][p][l]:continue
...
def printScalers():
    ...
    if not 'rates' in h: ut.bookCanvas(h,key='rates',title='Rates',nx=800,ny=400,cx=2,cy=1)
    rc = h['rates'].cd(1)
...
def plotRPCHitmap():
    if not 'rpcPlot' in h: ut.bookCanvas(h,key='rpcPlot',title='RPC Hitmaps',nx=1200,ny=600,cx=4,cy=3)
    j=0
    for n in range(1,6):
        for l in range(2):
            j+=1
...
# rt relation, drift time to distance
    ...
    if MCdata:
        ...
        if t > h['tMinAndTmax'][name][1]:  r = R
            ...
            r = h['rt'+name].Eval(t)
        if not 'RTcorr' in h: r+=h['RTcorrFun_'+str(s)+view+str(2*p+l)].Eval(r)
    h['TDC2R'].Fill(t-t0,r)
    return r
...
def checkMCSmearing():
    ...
    for i in range(sTree.GetEntries()):
        ...
        for n in range(sTree.Digi_MufluxSpectrometerHits.GetEntries()):
            ...
            rc = h['MCposX'].Fill(pmc.GetX()-vbot[0]-r)
            if not mcp in trackID: trackID[mcp]=[]
...
# onurs files, ~18 M POT 
    ...
    for event in sTree:
      ...
      for track in event.FitTracks:
           ...
           hname = 'mom'+str(pid)
           if not hname in h: h[hname]=h['mom'].Clone(hname)
...
def momDisplay():
    ...
    for x in ['','mu']:
        t = 'mom'+x
        if not t in h: ut.bookCanvas(h,key=t,title='trackfit'+x,nx=1200,ny=600,cx=4,cy=2)
        rc = h[t].cd(1)
        h['p/pt'+x].SetStats(0)
...
def ghostFraction(aTrack):
    ...
    for p in aTrack.getPointsWithMeasurement():
        ...
        if not trackID in trackIDs: trackIDs[trackID]=0
        trackIDs[trackID]+=1
...
def plotBiasedResiduals(nEvent=-1,nTot=1000,PR=11,onlyPlotting=False,minP=3.):
    ...
    if not onlyPlotting:
        ...
        if not 'hitMapsX' in h: plotHitMaps()
        for s in xLayers:
            for p in xLayers[s]:
...
def plot2dResiduals(minEntries=-1):
    if not 'biasedResiduals2dX' in h: 
...
def efficiencyEstimates(method=2,MCdata=False):
    ...
    if not 'biasedResiduals' in h: plotBiasedResiduals(onlyPlotting=True)
...
def plotLinearResiduals():
    if not 'linearResiduals2dX' in h: 
        plotRPCExtrap(0,-1)
...
#
    ...
    t = 'momResolution'
    if not t in h: ut.bookCanvas(h,t,'momentum Resolution',900,600,1,1)
    tc=h[t].cd(1)
    for res in ['-0',v]:
        hname = 'momResol'+res
...
def testForSameDetID(nEvent=-1,nTot=1000):
    ...
    for Nr in range(eventRange[0],eventRange[1]):
        ...
        for hit in sTree.Digi_MufluxSpectrometerHits:
            detID = hit.GetDetectorID()
            if detID<0: continue # feature for converted data in February'19
            if not detID in listOfDigits: 
                listOfDigits[detID]=[0,[]]
                listOfTDCs[detID]={}
            tdcModule = hit.GetTDC()
            if not tdcModule in listOfTDCs[detID]:
...
def init(database='muflux_RTrelations.pkl',remake=False,withReco=False):
    ...
    N = sTree.GetEntries()
    if not rname in RTrelations or remake:
        withTDC = False
        sTree.SetBranchStatus("FitTracks",0)
    ...
    for n in range(N):
        ...
        for hit in sTree.MufluxSpectrometerPoint: 
            ...
            if muP!=0: 
                ...
                muP = pName
            if not pName in muonO:
                muonO[pName]=0
                muonO2[pName]={}
            if not moID in muonO2[pName]: muonO2[pName][moID]=0
            muonO[pName]+=1
            muonO2[pName][moID]+=1
...
# effect on mom distribution of eff tuning
    ...
    if not t in h: ut.bookCanvas(h,key=t,title='MC-Comparison eff tuning',nx=900,ny=600,cx=1,cy=1)
    tc = h[t].cd(1)
...
# some code for 2 track events
    ...
    for s in osign:
        ...
        for case in txt:
            ...
            if not t in h: ut.bookCanvas(h,key=t,title=' momentum of muons in 2-track events '+osign[s],nx=800,ny=800,cx=1,cy=1)
            tc = h[t].cd(pad)
    ...
    for n in range(sTree.GetEntries()):
        ...
        for m in sTree.MCTrack:
            if abs(m.GetPdgCode())==13:
                ...
                if not p in muon: muon[p]=0
                muon[p]+=1
        if len(muon)==0:
            print("MCchecks",sTree.GetCurrentFile().GetName())
            sTree.MCTrack.Dump()
        for p in muon:
            if not p in mult: mult[p]={}
            N = muon[p]
            if not N in mult[p]: mult[p][N]=0
            mult[p][N]+=1
    return mult
...
def plotTDCExample():
    ...
    for x in range(50):
        ...
        h['TDCMaps'].cd(n)
        if not 'xTDC'+str(x) in h: continue
        z = 'xTDC'+str(x)
        h[z].Draw()
