...
cuts["delxAtGoliath"] = 8.0
cuts["lateArrivalsToT"] = 9999.0
...
cuts["muTrackMatchX"] = 5.0
cuts["muTrackMatchY"] = 10.0
cuts["muTaggerCluster_grouping"] = 3
cuts["RPCmaxDistance"] = 10.0
...
parser.add_argument(
    "-l", "--fileCatalog", dest="catalog", help="list of files in file", default=False
)
parser.add_argument(
    "-c", "--cmd", dest="command", help="command to execute", default=""
)
parser.add_argument(
    "-d", "--Display", dest="withDisplay", help="detector display", default=True
)
parser.add_argument("-e", "--eos", dest="onEOS", help="files on EOS", default=False)
parser.add_argument(
    "-u", "--update", dest="updateFile", help="update file", default=False
)
parser.add_argument(
    "-i", "--input", dest="inputFile", help="input histo file", default="residuals.root"
)
...
if options.updateFile:
    ...
    for f in fnames:
        print("add ", f)
        if options.onEOS:
            sTree.Add(os.environ["EOSSHIP"] + f)
        else:
            sTree.Add(f)
...
Lcorrection["T1_MA_01"] = -(20.0 + 150.0 + 8.65) / 10.0
Lcorrection["T1_MA_02"] = -(20.0 + 150.0 + 8.65) / 10.0
Lcorrection["T1_MA_03"] = (20.0 + 50.0 + 8.35) / 10.0
Lcorrection["T1_MA_04"] = (20.0 + 50.0 + 8.35) / 10.0
Lcorrection["T1_MB_01"] = -(20.0 + 150.0) / 10.0
Lcorrection["T1_MB_02"] = -(20.0 + 150.0) / 10.0
Lcorrection["T1_MB_03"] = (20.0 + 50.0) / 10.0
Lcorrection["T1_MB_04"] = (20.0 + 50.0) / 10.0
Lcorrection["T2_MC_01"] = -(20.0 + 150.0) / 10.0
Lcorrection["T2_MC_02"] = -(20.0 + 150.0) / 10.0
Lcorrection["T2_MC_03"] = (20.0 + 50.0) / 10.0
Lcorrection["T2_MC_04"] = (20.0 + 50.0) / 10.0
Lcorrection["T2_MD_01"] = -(20.0 + 150.0 + 7.3) / 10.0
Lcorrection["T2_MD_02"] = -(20.0 + 150.0 + 7.3) / 10.0
Lcorrection["T2_MD_03"] = (20.0 + 50.0 + 8.45) / 10.0  # 8.45 or 4.45
Lcorrection["T2_MD_04"] = (20.0 + 50.0 + 8.45) / 10.0  # 8.45 or 4.45

surveyXYZ = {}
for s in survey:
    surveyXYZ[s] = [survey[s][1] * 100.0, survey[s][2] * 100.0, survey[s][0] * 100.0]
for s in daniel:
    daniel[s] = [daniel[s][0] / 10.0, daniel[s][1] / 10.0, daniel[s][2] / 10.0]
...
for s in survey:
    if s.find("T1") < 0 and s.find("T2") < 0:
        continue
    if s.find("01") > 0:
        a = s.replace("01", "01-04")
    elif s.find("04") > 0:
        a = s.replace("04", "01-04")
    elif s.find("02") > 0:
        a = s.replace("02", "02-03")
    elif s.find("03") > 0:
        a = s.replace("03", "02-03")
...
for k in range(4):
    for s in ["T3_T0", "T4_T0"]:
        ...
        rc = h["lengthCalibration"].Fill(delta - 8.0 * 4.2)
...
ypos["T1X"] = [
    (daniel["T1_MA_01"][1] + daniel["T1_MA_02"][1]) / 2.0,
    (daniel["T1_MA_04"][1] + daniel["T1_MA_03"][1]) / 2.0,
]
test = ROOT.MufluxSpectrometerHit(10002012, 0.0)
test.MufluxSpectrometerEndPoints(vbot, vtop)
deltaZ = zpos["T1X"] - (vbot[2] + vtop[2]) / 2.0
n = 10002012
start = daniel[
    "T1_MA_01"
][
    0
]  # (daniel['T1_MA_01'][0]+daniel['T1_MA_04'][0])/2. bottom survey measurements do not match nominal distance
delta = (
    start - (daniel["T1_MA_02"][0] + daniel["T1_MA_03"][0]) / 2.0 - 1.1 - 2.1
) / 10.0
delta = 4.2
for i in range(12):
...
n = 10012001
start = (
    daniel["T1_MA_02"][0] + 1.1
)  #   (daniel['T1_MA_02'][0]+daniel['T1_MA_03'][0])/2. +1.1
for i in range(12):
    xpos[n + i] = start + delta * i
    ypos[n + i] = ypos["T1X"]
...
angleu1 = ROOT.TMath.ATan2(
    (daniel["T1_MB_01"][0] - daniel["T1_MB_04"][0]),
    (daniel["T1_MB_01"][1] - daniel["T1_MB_04"][1]),
)
angleu2 = ROOT.TMath.ATan2(
    (daniel["T1_MB_02"][0] - daniel["T1_MB_03"][0]),
    (daniel["T1_MB_02"][1] - daniel["T1_MB_03"][1]),
)
angleu = (angleu1 + angleu2) / 2.0
...
for i in range(1, 5):
    p = "T1_MB_0" + str(i)
    tx += daniel[p][0] - (
        rn[p][0] * ROOT.TMath.Cos(angle) - rn[p][1] * ROOT.TMath.Sin(angle)
    )
    ty += daniel[p][1] - (
        rn[p][0] * ROOT.TMath.Sin(angle) + rn[p][1] * ROOT.TMath.Cos(angle)
    )
    if debug:
        print(
            "%s: %5.4F, %5.4F"
            % (
                ...
                (
                    daniel[p][0]
                    - (
                        rn[p][0] * ROOT.TMath.Cos(angle)
                        - rn[p][1] * ROOT.TMath.Sin(angle)
                    )
                )
                * 10,
                (
                    daniel[p][1]
                    - (
                        rn[p][0] * ROOT.TMath.Sin(angle)
                        + rn[p][1] * ROOT.TMath.Cos(angle)
                    )
                )
                * 10,
            )
...
L = 110.0
start = (rn["T1_MB_01"][0] + rn["T1_MB_04"][0]) / 2.0
delta = (start - ((rn["T1_MB_02"][0] + rn["T1_MB_03"][0]) / 2.0 + 1.1 + 2.1)) / 10.0
delta = 4.2
...
n = 11012001
start = (rn["T1_MB_02"][0] + rn["T1_MB_03"][0]) / 2.0 + 1.1
for i in range(12):
    xnom = start + delta * i
    ynom = L / 2.0
    xpos[n + i] = xnom * ROOT.TMath.Cos(angle) - ynom * ROOT.TMath.Sin(angle) + tx
...
n = 11102001
start = (rn["T1_MB_02"][0] + rn["T1_MB_03"][0]) / 2.0
for i in range(12):
    xnom = start + delta * i
    ynom = L / 2.0
    xpos[n + i] = xnom * ROOT.TMath.Cos(angle) - ynom * ROOT.TMath.Sin(angle) + tx
...
ypos["T2X"] = [
    (daniel["T2_MD_01"][1] + daniel["T2_MD_02"][1]) / 2.0,
    (daniel["T2_MD_04"][1] + daniel["T2_MD_03"][1]) / 2.0,
]
n = 21112001
start = daniel[
    "T2_MD_03"
    ...
    0
]  # (daniel['T2_MD_02'][0]+daniel['T2_MD_03'][0])/2.  top x survey do not match nominal distance, makes 1.8mm difference to FairShip
delta = (
    (daniel["T2_MD_01"][0] + daniel["T2_MD_04"][0]) / 2.0 - (start + 1.1 - 2.1)
) / 11.0
...
anglev1 = ROOT.TMath.ATan2(
    (daniel["T2_MC_02"][0] - daniel["T2_MC_03"][0]),
    (daniel["T2_MC_02"][1] - daniel["T2_MC_03"][1]),
)
anglev2 = ROOT.TMath.ATan2(
    (daniel["T2_MC_01"][0] - daniel["T2_MC_04"][0]),
    (daniel["T2_MC_01"][1] - daniel["T2_MC_04"][1]),
)
anglev = (anglev1 + anglev2) / 2.0
...
n = 20112001
start = (rn["T2_MC_02"][0] + rn["T2_MC_03"][0]) / 2.0
delta = (
    (rn["T2_MC_01"][0] + rn["T2_MC_04"][0]) / 2.0
    - ((rn["T2_MC_02"][0] + rn["T2_MC_03"][0]) / 2.0 - 2.1 + 1.1)
) / 11.0
delta = 4.2
...
n = 20012012
start = (rn["T2_MC_01"][0] + rn["T2_MC_04"][0]) / 2.0
for i in range(12):
    xnom = start - delta * i
    ynom = L / 2.0
    xpos[n - i] = xnom * ROOT.TMath.Cos(angle) - ynom * ROOT.TMath.Sin(angle) + tx
...
ypos["T3aX"] = [
    (daniel["T3_T01"][1] + daniel["T3_T02"][1]) / 2.0,
    (daniel["T3_B01"][1] + daniel["T3_B02"][1]) / 2.0,
]

delta = (
    (daniel["T3_T01"][0] + daniel["T3_B01"][0]) / 2.0
    - (daniel["T3_T02"][0] + daniel["T3_B02"][0]) / 2.0
) / 8.0
delta = 4.2

n = 30002037
start = (daniel["T3_T02"][0] + daniel["T3_B02"][0]) / 2.0 - delta + 2.1 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3aX"] - deltaZ
n = 30012037
start = (daniel["T3_T02"][0] + daniel["T3_B02"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3aX"] + 3.64 - deltaZ
n = 30102037
start = (daniel["T3_T02"][0] + daniel["T3_B02"][0]) / 2.0 - delta - 1.1
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3aX"] + 3.64 + 4.06 - deltaZ
n = 30112037
start = (daniel["T3_T02"][0] + daniel["T3_B02"][0]) / 2.0 - delta - 1.1 - 2.1
for i in range(12):
    xpos[n + i] = start + delta * i
...
ypos["T3bX"] = [
    (daniel["T3_T03"][1] + daniel["T3_T04"][1]) / 2.0,
    (daniel["T3_B03"][1] + daniel["T3_B04"][1]) / 2.0,
]

delta = (
    (daniel["T3_T03"][0] + daniel["T3_B03"][0]) / 2.0
    - (daniel["T3_T04"][0] + daniel["T3_B04"][0]) / 2.0
) / 8.0
delta = 4.2

n = 30002025
start = (daniel["T3_T04"][0] + daniel["T3_B04"][0]) / 2.0 - delta + 2.1 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3bX"] - deltaZ
n = 30012025
start = (daniel["T3_T04"][0] + daniel["T3_B04"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3bX"] + 3.64 - deltaZ
n = 30102025
start = (daniel["T3_T04"][0] + daniel["T3_B04"][0]) / 2.0 - delta - 1.1
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3bX"] + 3.64 + 4.06 - deltaZ
n = 30112025
start = (daniel["T3_T04"][0] + daniel["T3_B04"][0]) / 2.0 - delta - 1.1 - 2.1
for i in range(12):
    xpos[n + i] = start + delta * i
...
ypos["T3cX"] = [
    (daniel["T3_T05"][1] + daniel["T3_T06"][1]) / 2.0,
    (daniel["T3_B05"][1] + daniel["T3_B06"][1]) / 2.0,
]

delta = (
    (daniel["T3_T05"][0] + daniel["T3_B05"][0]) / 2.0
    - (daniel["T3_T06"][0] + daniel["T3_B06"][0]) / 2.0
) / 8.0
delta = 4.2

n = 30002013
start = (daniel["T3_T06"][0] + daniel["T3_B06"][0]) / 2.0 - delta + 2.1 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3cX"] - deltaZ
n = 30012013
start = (daniel["T3_T06"][0] + daniel["T3_B06"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
...
ypos["T3dX"] = [
    (daniel["T3_T07"][1] + daniel["T3_T08"][1]) / 2.0,
    (daniel["T3_B07"][1] + daniel["T3_B08"][1]) / 2.0,
]

delta = (
    (daniel["T3_T07"][0] + daniel["T3_B07"][0]) / 2.0
    - (daniel["T3_T08"][0] + daniel["T3_B08"][0]) / 2.0
) / 8.0
delta = 4.2


n = 30002001
start = (daniel["T3_T08"][0] + daniel["T3_B08"][0]) / 2.0 - delta + 2.1 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3dX"] - deltaZ
n = 30012001
start = (daniel["T3_T08"][0] + daniel["T3_B08"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3dX"] + 3.64 - deltaZ
n = 30102001
start = (daniel["T3_T08"][0] + daniel["T3_B08"][0]) / 2.0 - delta - 1.1
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T3dX"] + 3.64 + 4.06 - deltaZ
n = 30112001
start = (daniel["T3_T08"][0] + daniel["T3_B08"][0]) / 2.0 - delta - 1.1 - 2.1
for i in range(12):
    xpos[n + i] = start + delta * i
...
ypos["T4aX"] = [
    (daniel["T4_T01"][1] + daniel["T4_T02"][1]) / 2.0,
    (daniel["T4_B01"][1] + daniel["T4_B02"][1]) / 2.0,
]

delta = (
    (daniel["T4_T01"][0] + daniel["T4_B01"][0]) / 2.0
    - (daniel["T4_T02"][0] + daniel["T4_B02"][0]) / 2.0
) / 8.0
delta = 4.2

n = 40002048
start = (daniel["T4_T02"][0] + daniel["T4_B02"][0]) / 2.0 - delta + 45.2
for i in range(12):
    xpos[n - i] = start - delta * i
    zpos[n - i] = zpos["T4aX"] - deltaZ
n = 40012037
start = (daniel["T4_T02"][0] + daniel["T4_B02"][0]) / 2.0 - delta + 1.1
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4aX"] + 3.64 - deltaZ
n = 40102037
start = (daniel["T4_T02"][0] + daniel["T4_B02"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4aX"] + 3.64 + 4.06 - deltaZ
n = 40112037
start = (daniel["T4_T02"][0] + daniel["T4_B02"][0]) / 2.0 - delta - 2.1
for i in range(12):
    xpos[n + i] = start + delta * i
...
ypos["T4bX"] = [
    (daniel["T4_T03"][1] + daniel["T4_T04"][1]) / 2.0,
    (daniel["T4_B03"][1] + daniel["T4_B04"][1]) / 2.0,
]

delta = (
    (daniel["T4_T03"][0] + daniel["T4_B03"][0]) / 2.0
    - (daniel["T4_T04"][0] + daniel["T4_B04"][0]) / 2.0
) / 8.0
delta = 4.2

n = 40002036
start = (daniel["T4_T04"][0] + daniel["T4_B04"][0]) / 2.0 - delta + 45.2
for i in range(12):
    xpos[n - i] = start - delta * i
    zpos[n - i] = zpos["T4bX"] - deltaZ
n = 40012025
start = (
    (daniel["T4_T04"][0] + daniel["T4_B04"][0]) / 2.0 - delta + 45.2 - 10 * delta - 2.1
)
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4bX"] + 3.64 - deltaZ
n = 40102025
start = (daniel["T4_T04"][0] + daniel["T4_B04"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4bX"] + 3.64 + 4.06 - deltaZ
n = 40112025
start = (daniel["T4_T04"][0] + daniel["T4_B04"][0]) / 2.0 - delta - 2.1
for i in range(12):
    xpos[n + i] = start + delta * i
...
ypos["T4cX"] = [
    (daniel["T4_T05"][1] + daniel["T4_T06"][1]) / 2.0,
    (daniel["T4_B05"][1] + daniel["T4_B06"][1]) / 2.0,
]

delta = (
    (daniel["T4_T05"][0] + daniel["T4_B05"][0]) / 2.0
    - (daniel["T4_T06"][0] + daniel["T4_B06"][0]) / 2.0
) / 8.0
delta = 4.2

n = 40002024
start = (daniel["T4_T06"][0] + daniel["T4_B06"][0]) / 2.0 - delta + 45.2
for i in range(12):
    xpos[n - i] = start - delta * i
    zpos[n - i] = zpos["T4cX"] - deltaZ
n = 40012013
start = (
    (daniel["T4_T06"][0] + daniel["T4_B06"][0]) / 2.0 - delta + 45.2 - 10 * delta - 2.1
)
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4cX"] + 3.64 - deltaZ
n = 40102013
start = (daniel["T4_T06"][0] + daniel["T4_B06"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4cX"] + 3.64 + 4.06 - deltaZ
n = 40112013
start = (daniel["T4_T06"][0] + daniel["T4_B06"][0]) / 2.0 - delta - 2.1
for i in range(12):
    xpos[n + i] = start + delta * i
...
ypos["T4dX"] = [
    (daniel["T4_T07"][1] + daniel["T4_T08"][1]) / 2.0,
    (daniel["T4_B07"][1] + daniel["T4_B08"][1]) / 2.0,
]

delta = (
    (daniel["T4_T07"][0] + daniel["T4_B07"][0]) / 2.0
    - (daniel["T4_T08"][0] + daniel["T4_B08"][0]) / 2.0
) / 8.0
delta = 4.2

n = 40002012
start = (daniel["T4_T08"][0] + daniel["T4_B08"][0]) / 2.0 - delta + 45.2
for i in range(12):
    xpos[n - i] = start - delta * i
    zpos[n - i] = zpos["T4dX"] - deltaZ
n = 40012001
start = (
    (daniel["T4_T08"][0] + daniel["T4_B08"][0]) / 2.0 - delta + 45.2 - 10 * delta - 2.1
)
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4dX"] + 3.64 - deltaZ
n = 40102001
start = (daniel["T4_T08"][0] + daniel["T4_B08"][0]) / 2.0 - delta
for i in range(12):
    xpos[n + i] = start + delta * i
    zpos[n + i] = zpos["T4dX"] + 3.64 + 4.06 - deltaZ
n = 40112001
start = (daniel["T4_T08"][0] + daniel["T4_B08"][0]) / 2.0 - delta - 2.1
for i in range(12):
    xpos[n + i] = start + delta * i
...
if debug:
    for a in Langle:
        print(a, Langle[a] * 180.0 / ROOT.TMath.Pi())
    for s in ["T1_MA_", "T1_MB_", "T2_MC_", "T2_MD_"]:
        for x in ["01", "02", "03", "04"]:
            print(
                "delta survey corrected - Daniel: %s %6.4F,%6.4F,%6.4F"
                % (
                    s + x,
                    surveyXYZ[s + x][0] - daniel[s + x][0],
                    surveyXYZ[s + x][1] - daniel[s + x][1],
                    surveyXYZ[s + x][2] - daniel[s + x][2],
                )
            )
    for s in ["T1_MA_", "T1_MB_", "T2_MC_", "T2_MD_"]:
        for top in ["01", "02"]:
            delx = (
                surveyXYZ[s + top][0]
                - surveyXYZ[s + top.replace("01", "04").replace("02", "03")][0]
            )
            dely = (
                surveyXYZ[s + top][1]
                - surveyXYZ[s + top.replace("01", "04").replace("02", "03")][1]
            )
            r = ROOT.TMath.Sqrt(delx**2 + dely**2)
            angle = ROOT.TMath.ATan2(delx, dely) * 180.0 / ROOT.TMath.Pi()
            print(s + top, angle, r)
...
def compareAlignment():
    ut.bookHist(
        h, "alignCompare", "compare Alignments", 100, -120.0, 120.0, 100, -120.0, 120.0
    )
    ut.bookHist(h, "alignCompareDiffs", "compare Alignments", 100, -1.0, 1.0)
    for d in sorted(xpos.keys()):
        test = ROOT.MufluxSpectrometerHit(d, 0.0)
        test.MufluxSpectrometerEndPoints(vbot, vtop)
        statnb, vnb, pnb, lnb, view, channelID, tdcId, nRT = stationInfo(test)
        angle = (
            ROOT.TMath.ATan2(-vtop[0] + vbot[0], -vtop[1] + vbot[1])
            / ROOT.TMath.Pi()
            * 180
        )
        ...
        if view == "_x":
            vbotD, vtopD = (
                ROOT.TVector3(xpos[d], L / 2.0, zpos[d]),
                ROOT.TVector3(xpos[d], -L / 2.0, zpos[d]),
            )
        else:
            vbotD, vtopD = (
                ROOT.TVector3(xposb[d], yposb[d], zpos[d]),
                ROOT.TVector3(xpos[d], ypos[d], zpos[d]),
            )
        angleD = (
            ROOT.TMath.ATan2(vtopD[0] - vbotD[0], vtopD[1] - vbotD[1])
            / ROOT.TMath.Pi()
            * 180
        )
        if abs(vtop[0] - vbot[0]) < 1:
            x0 = (vtop[0] + vbot[0]) / 2.0
        else:
            m = (vtop[1] - vbot[1]) / (vtop[0] - vbot[0])
            b = vtop[1] - m * vtop[0]
            x0 = -b / m
        if abs(vtopD[0] - vbotD[0]) < 1:
            x0D = (vtopD[0] + vbotD[0]) / 2.0
        else:
        ...
        if abs(diff) > 0.1:
            txt = "!!! "
        if abs(z - vbotD[2]) > 0.1:
            txt += "!!z"
        print(
            "%s %i x/y pos from Daniel %7.4F %7.4F %7.4F %7.4F %7.4F  from FairShip  %7.4F %7.4F %7.4F %7.4F %7.4F diff %5.4F zdiff %5.4F"
            % (
                txt,
                d,
                x0D,
        ...
        if abs(vbot[2] - vtop[2]) > 0.1:
            print("!!! z tilt", vbot[2], vtop[2])
        rc = h["alignCompare"].Fill(x0D, x0)
        rc = h["alignCompareDiffs"].Fill(x0D - x0)
    z0_D = zpos["T1X"] / 10.0
    test = ROOT.MufluxSpectrometerHit(10002001, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z0_F = (vbot[2] + vtop[2]) / 2.0
    # distance to T1 u
    test = ROOT.MufluxSpectrometerHit(11002001, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z1_U = (vbot[2] + vtop[2]) / 2.0 - z0_F
    print("distance T1 X to T1 U:", zpos["T1U"] / 10.0 - z0_D, z1_U)
    # distance to T2 V
    test = ROOT.MufluxSpectrometerHit(20002001, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z2_V = (vbot[2] + vtop[2]) / 2.0 - z0_F
    print("distance T1 X to T2 V:", zpos["T2V"] / 10.0 - z0_D, z2_V)
    # distance to T2 X
    test = ROOT.MufluxSpectrometerHit(21002001, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z2_X = (vbot[2] + vtop[2]) / 2.0 - z0_F
    print("distance T1 X to T2 X:", zpos["T2X"] / 10.0 - z0_D, z2_X)
    # distance to T3 X
    test = ROOT.MufluxSpectrometerHit(30002001, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z3_X = (vbot[2] + vtop[2]) / 2.0 - z0_F
    print(
        "distance T1 X to T3 X:",
        (zpos["T3aX"] + zpos["T3bX"] + zpos["T3cX"] + zpos["T3dX"]) / (4 * 10.0) - z0_D,
        z3_X,
    )
    # distance to T4 X
    test = ROOT.MufluxSpectrometerHit(40002001, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z4_X = (vbot[2] + vtop[2]) / 2.0 - z0_F
    print(
        "distance T1 X to T4 X:",
        (zpos["T4aX"] + zpos["T4bX"] + zpos["T4cX"] + zpos["T4dX"]) / (4 * 10.0) - z0_D,
        z4_X,
    )
...
def surveyVSfairship():
    ...
    test = ROOT.MufluxSpectrometerHit(10002012, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z1 = (surveyXYZ["T1_MA_01"][2] + surveyXYZ["T1_MA_04"][2]) / 2.0 + 3.03
    z1F = vtop[2]
    print("                               X       topY    botY     Z")
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
        % (
            "T1_MA_01/04 survey corrected",
            (surveyXYZ["T1_MA_01"][0] + surveyXYZ["T1_MA_04"][0]) / 2.0,
            surveyXYZ["T1_MA_01"][1],
            surveyXYZ["T1_MA_04"][1],
            z1 - z1,
        )
    )
    print(
        ...
        % (
            "T1_MA_01/04 Daniel          ",
            (daniel["T1_MA_01"][0] + daniel["T1_MA_04"][0]) / 2.0,
            daniel["T1_MA_01"][1],
            daniel["T1_MA_04"][1],
            (daniel["T1_MA_01"][2] + daniel["T1_MA_04"][2]) / 2.0 + 3.03 - z1,
        )
    )
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
    ...
    test2 = ROOT.MufluxSpectrometerHit(10102001, 0.0)
    test2.MufluxSpectrometerEndPoints(vbot2, vtop2)
    delx = (daniel["T1_MA_01"][0] + daniel["T1_MA_04"][0]) / 2.0 - (
        daniel["T1_MA_02"][0] + daniel["T1_MA_03"][0]
    ) / 2.0
    dely = [
        daniel["T1_MA_01"][1] - daniel["T1_MA_02"][1],
        daniel["T1_MA_04"][1] - daniel["T1_MA_03"][1],
    ]
    delz = ((daniel["T1_MA_01"][2] + daniel["T1_MA_04"][2]) / 2.0 + 3.03) - (
        (daniel["T1_MA_02"][2] + daniel["T1_MA_03"][2]) / 2.0 + 3.03 + 3.64 + 4.06
    )
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
    ...
    test = ROOT.MufluxSpectrometerHit(21112001, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z2 = (surveyXYZ["T2_MD_02"][2] + surveyXYZ["T2_MD_03"][2]) / 2.0 - 3.03
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
        % (
            "T2_MD_02/03 survey corrected",
            (surveyXYZ["T2_MD_02"][0] + surveyXYZ["T2_MD_03"][0]) / 2.0,
            surveyXYZ["T2_MD_02"][1],
            surveyXYZ["T2_MD_03"][1],
            z2 - z1,
        )
    )
    print(
        ...
        % (
            "T2_MD_02/03 Daniel          ",
            (daniel["T2_MD_02"][0] + daniel["T2_MD_03"][0]) / 2.0,
            daniel["T2_MD_02"][1],
            daniel["T2_MD_03"][1],
            (daniel["T2_MD_02"][2] + daniel["T2_MD_03"][2]) / 2.0 - 3.03 - z1,
        )
    )
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
    ...
    test = ROOT.MufluxSpectrometerHit(11002012, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z1b = (
        (surveyXYZ["T1_MB_01"][2] + surveyXYZ["T1_MB_04"][2]) / 2.0
        - 3.03
        - 3.64
        - 4.06
        - 3.64
    ...
    z1bF = vtop[2]
    m = (surveyXYZ["T1_MB_01"][1] - surveyXYZ["T1_MB_04"][1]) / (
        surveyXYZ["T1_MB_01"][0] - surveyXYZ["T1_MB_04"][0]
    )
    b = surveyXYZ["T1_MB_01"][1] - m * surveyXYZ["T1_MB_01"][0]
    xposAty0 = -b / m
    m = (daniel["T1_MB_01"][1] - daniel["T1_MB_04"][1]) / (
        daniel["T1_MB_01"][0] - daniel["T1_MB_04"][0]
    )
    ...
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
        % (
            "T1_MB_01/04 survey corrected",
            xposAty0,
            surveyXYZ["T1_MB_01"][1],
            surveyXYZ["T1_MB_04"][1],
            z1b - z1,
        )
    )
    print(
        ...
        % (
            ...
            daniel["T1_MB_01"][1],
            daniel["T1_MB_04"][1],
            (daniel["T1_MB_01"][2] + daniel["T1_MB_04"][2]) / 2.0
            - 3.03
            - 3.64
            - 4.06
            - 3.64
    ...
    test = ROOT.MufluxSpectrometerHit(20012012, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z2b = (surveyXYZ["T2_MC_01"][2] + surveyXYZ["T2_MC_04"][2]) / 2.0 + 3.03 + 3.64
    m = (surveyXYZ["T2_MC_01"][1] - surveyXYZ["T2_MC_04"][1]) / (
        surveyXYZ["T2_MC_01"][0] - surveyXYZ["T2_MC_04"][0]
    )
    b = surveyXYZ["T2_MC_01"][1] - m * surveyXYZ["T2_MC_01"][0]
    xposAty0 = -b / m
    m = (daniel["T2_MC_01"][1] - daniel["T2_MC_04"][1]) / (
        daniel["T2_MC_01"][0] - daniel["T2_MC_04"][0]
    )
    ...
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
        % (
            "T2_MC_01/04 survey corrected",
            xposAty0,
            surveyXYZ["T2_MC_01"][1],
            surveyXYZ["T2_MC_04"][1],
            z2b - z1,
        )
    )
    print(
        ...
        % (
            ...
            daniel["T2_MC_01"][1],
            daniel["T2_MC_04"][1],
            (daniel["T2_MC_01"][2] + daniel["T2_MC_04"][2]) / 2.0 + 3.03 + 3.64 - z1,
        )
    )
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
    ...
    test = ROOT.MufluxSpectrometerHit(30012046, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    # For T3 it is placed 7cm in front and for T4 7cm behind the endplate
    z3 = (surveyXYZ["T3_T01"][2] + surveyXYZ["T3_B01"][2]) / 2.0 + 4.33 + 3.64 + 7.0
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
        % (
            "T3_T01/B01  survey corrected",
            (surveyXYZ["T3_T01"][0] + surveyXYZ["T3_B01"][0]) / 2.0,
            surveyXYZ["T3_T01"][1],
            surveyXYZ["T3_B01"][1],
            z3 - z1,
        )
    ...
    print(
        ...
        % (
            ...
            daniel["T3_T01"][1],
            daniel["T3_B01"][1],
            (daniel["T3_T01"][2] + daniel["T3_B01"][2]) / 2.0 + 4.33 + 3.64 - z1,
        )
    )
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
    ...
    test = ROOT.MufluxSpectrometerHit(40102046, 0.0)
    test.MufluxSpectrometerEndPoints(vbot, vtop)
    z4 = (surveyXYZ["T4_T01"][2] + surveyXYZ["T4_B01"][2]) / 2.0 - 4.33 - 3.64 - 7.0
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
        % (
            "T4_T01/B01  survey corrected",
            (surveyXYZ["T4_T01"][0] + surveyXYZ["T4_B01"][0]) / 2.0,
            surveyXYZ["T4_T01"][1],
            surveyXYZ["T4_B01"][1],
            z4 - z1,
        )
    ...
    print(
        ...
        % (
            ...
            daniel["T4_T01"][1],
            daniel["T4_B01"][1],
            (daniel["T4_T01"][2] + daniel["T4_B01"][2]) / 2.0 - 4.33 - 3.64 - z1,
        )
    )
    print(
        "%s, %7.3F,%7.3F,%7.3F,%7.3F"
...
def checkZtilts():
    ...
    for s in ["T1_MA_", "T1_MB_", "T2_MC_", "T2_MD_"]:
        ...
        diffTopBot = (
            daniel[s + "01"][2] - daniel[s + "04"][2],
            daniel[s + "02"][2] - daniel[s + "03"][2],
        )
        diffLefRig = (
            daniel[s + "01"][2] - daniel[s + "02"][2],
            daniel[s + "04"][2] - daniel[s + "03"][2],
        )
...
def displayDTLayers():
    ...
    if "layerDisplay" not in h:
        ut.bookCanvas(
            h, key="layerDisplay", title="Upstream Layers", nx=1600, ny=1200, cx=1, cy=0
        )
...
def plotEvent(n=-1):
    ...
    for c in h["hitCollection"]:
        rc = h["hitCollection"][c][1].SetName(c)
    for c in h["hitCollection"]:
        rc = h["hitCollection"][c][1].Set(0)
    ut.bookHist(h, "xz", "x (y) vs z", 500, 0.0, 1200.0, 100, -150.0, 150.0)
    ...
    h["xz"].Draw("b")
    for hit in sTree.Digi_MufluxSpectrometerHits:
        if not hit.hasTimeOverThreshold():
            continue  # 16% of the hits, isn't it a bit too much?
        statnb, vnb, pnb, lnb, view, channelID, tdcId, nRT = stationInfo(hit)
    ...
    if sTree.GetBranch("Digi_MuonTaggerHits"):
        for hit in sTree.Digi_MuonTaggerHits:
            ...
            if view == 1:
                x, z = (vtop[0] + vbot[0]) / 2.0, (vtop[2] + vbot[2]) / 2.0
                rc = c[1].SetPoint(c[0], z, x)
                c[0] += 1
            if view == 0:
                y, z = (vtop[1] + vbot[1]) / 2.0, (vtop[2] + vbot[2]) / 2.0
    ...
    for c in h["hitCollection"]:
        rc = h["hitCollection"][c][1].SetMarkerStyle(30)
    for c in h["hitCollection"]:
        if h["hitCollection"][c][1].GetN() < 1:
            continue
    ...
    for p in range(2):
        ...
        for l in range(2):
            ...
            for view in ["_x", "_u", "_v"]:
                ...
                if s > 2:
                    channels[s][p][l][view] = 48
for x in xpos.keys():
    ut.bookHist(h, "TDC" + str(x), "TDC " + str(x), 1500, -500.0, 2500.0)
    ut.bookHist(h, "TDC" + str(x), "noToT " + str(x), 1500, -500.0, 2500.0)

ut.bookHist(h, "T0tmp", "T0 temp", 1250, -500.0, 2000.0)
ut.bookHist(h, "T0", "T0", 1250, -500.0, 2000.0)
...
for n in range(1, 5):
    for x in ROOT.gGeoManager.GetTopVolume().GetNodes():
        if x.GetName().find("volDriftTube") < 0:
            continue
        for y in x.GetVolume().GetNodes():
            if y.GetName().find("plane") < 0:
                continue
...
def DTEfficiencyFudgefactor(method=-1):
    ...
    for x in effFudgeFactors:
        muflux_Reco.setEffFudgeFactor(x, effFudgeFactors[x])
...
def MakeKeysToDThits(minToT=-999):
    ...
    for hit in sTree.Digi_MufluxSpectrometerHits:
        ...
        if not hit.isValid() and MCdata:
            continue
        detID = hit.GetDetectorID()
        if detID < 0:
            continue  # feature for converted data in February'19
        if detID in keysToDThits:
            prevTDC = sTree.Digi_MufluxSpectrometerHits[
                keysToDThits[detID][0]
            ].GetDigi()
            prevToT = sTree.Digi_MufluxSpectrometerHits[
                keysToDThits[detID][0]
    ...
    for hit in sTree.Digi_LateMufluxSpectrometerHits:
        ...
        if not hit.hasTimeOverThreshold():
            continue
        if hit.GetTimeOverThreshold() < minToT:
            continue
        detID = hit.GetDetectorID()
...
def studyLateDTHits(nevents=1000, nStart=0):
    ...
    for n in range(nStart, nevents):
        ...
        for channel in keysToDThits:
            if keysToDThits[channel][0] < 0:
                rc = h["multLateDTHits"].Fill(-1)
            else:
                nHits += 1
                ...
                for n in range(1, len(keysToDThits[channel])):
                    ...
                    rc = h["ToverTvsTDC"].Fill(
                        aHit.GetTimeOverThreshold(), aHit.GetDigi()
                    )
        ...
        for v in [0, 1, 2]:
            ...
            for l in range(4):
                ...
                for hit in hits[v][s][l]:
                    statnb, vnb, pnb, lnb, view, channelID, tdcId, nRT = stationInfo(
                        hit
                    )
                    txt += str(channelID) + " "
                    tdc += "%5.0F %5.0F " % (
                        hit.GetDigi() - t0,
                        hit.GetTimeOverThreshold(),
                    )
    ...
    for s in range(1, 5):
        for p in range(2):
            for l in range(2):
                ...
                if s == 1:
                    xLayers[s][p][l]["_u"].Reset()
                if s == 2:
                    xLayers[s][p][l]["_v"].Reset()
    for event in sTree:
        for trInfo in event.TrackInfos:
            for n in range(trInfo.N()):
                if trInfo.wL(n) < 0.1 and trInfo.wR(n) < 0.1:
                    continue
...
def plotHitMapsOld(onlyPlotting=False):
    noisyChannels = []
    deadThreshold = 1.0e-4  # ~1% typical occupancy
    deadChannels = []
    ...
    if not onlyPlotting:
        for event in sTree:
            for hit in event.Digi_MufluxSpectrometerHits:
                ...
                if channel + tot not in h:
                    h[channel + tot] = h["TDC" + str(nRT) + tot].Clone(channel)
                rc = h[channel + tot].Fill(hit.GetDigi() - t0)
    if "hitMapsX" not in h:
        ut.bookCanvas(
            h, key="hitMapsX", title="Hit Maps All Layers", nx=1600, ny=1200, cx=4, cy=6
        )
    if "TDCMapsX" not in h:
        ut.bookCanvas(
            h,
            key="TDCMapsX",
            title="TDC Maps All Layers",
            nx=1600,
            ny=1200,
            cx=5,
            cy=10,
        )
    if "TDCMapsX_noToT" not in h:
        ut.bookCanvas(
            h,
            key="TDCMapsX_noToT",
            title="TDC Maps All Layers noToT",
            nx=1600,
            ny=1200,
            cx=5,
            cy=10,
    ...
    for s in range(1, 5):
        for view in ["_x", "_u", "_v"]:
            for p in range(2):
                for l in range(2):
                    if view not in xLayers[s][p][l]:
                        continue
                    if s > 2 and view != "_x":
                        continue
                    if s == 1 and view == "_v" or s == 2 and view == "_u":
                        continue
                    j += 1
                    rc = h["hitMapsX"].cd(j)
                    ...
                    for i in range(2, int(xLayers[s][p][l][view].GetEntries()) + 1):
                        ...
                        if (
                            xLayers[s][p][l][view].GetBinContent(i)
                            > noiseThreshold * mean
                        ):
                            print(
                                "noisy channel:",
                            ...
                            if not channel in noisyChannels:
                                noisyChannels.append(myDetID + i - 1)
                        if xLayers[s][p][l][view].GetBinContent(i) < max(
                            1, deadThreshold * mean
                        ):
                            print(
                                "dead channel:",
                                s,
    ...
    for n in noisyChannels:
        print(n)
    print("list of dead channels")
    for n in deadChannels:
        print(n)
    if sTree.GetBranch("FitTracks"):
        sTree.SetBranchStatus("FitTracks", FitTracksBrStatus)
...
def printScalers():
    ...
    if "rates" not in h:
        ut.bookCanvas(h, key="rates", title="Rates", nx=800, ny=400, cx=2, cy=1)
    rc = h["rates"].cd(1)
...
ut.bookHist(h, "delx", "delta x", 200, -50.0, 50.0)
ut.bookHist(h, "delta_mean_uv", "delta to mean u and v", 200, -10.0, 10.0)
ut.bookHist(h, "magPos", "XY at goliath, PR", 100, -50.0, 50.0, 100, -50.0, 50.0)
for dets in ["34", "stereo12", "y12"]:
    ut.bookHist(h, "tracklets" + dets, "hits per view", 10, -0.5, 9.5)
...
def plotRPCHitmap():
    ...
    for n in range(1, 6):
        for l in range(2):
            ut.bookHist(
                ...
                "rpc Hitmaps station " + str(n) + "layer " + str(l),
                200,
                -0.5,
                199.5,
            )
    ...
    if "rpcPlot" not in h:
        ut.bookCanvas(
            h, key="rpcPlot", title="RPC Hitmaps", nx=1200, ny=600, cx=4, cy=3
        )
    j = 0
    for n in range(1, 6):
        for l in range(2):
...
def plotTimeOverThreshold(N, Debug=False):
    ut.bookHist(h, "ToverT", "Time over threshold", 3000, -1000.0, 2000.0)
    ut.bookHist(h, "endTime", "End Time", 100, 0.0, 2000.0)
    ut.bookHist(h, "tdc", "tdc", 100, -200.0, 2000.0)
    for n in range(N):
        rc = sTree.GetEvent(n)
        flag = False
        for aHit in sTree.Digi_MufluxSpectrometerHits:
            ...
            if aHit.GetTimeOverThreshold() < 10:
                flag = True
        if flag and Debug:
            print(n)
            spectrHitsSorted = ROOT.nestedList()
            muflux_Reco.sortHits(
                sTree.Digi_MufluxSpectrometerHits, spectrHitsSorted, True
            )
            for s in range(1, 5):
        ...
        for x in p.GetListOfPrimitives():
            if x.InheritsFrom("TH1"):
                ...
                for m in range(tmp.GetNbinsX() - 1, 0, -1):
                    runningMean = max(
                        tmp.Integral(m, tmp.GetNbinsX()) / float(tmp.GetNbinsX() - m), 2
                    )
                    if tmp.GetBinContent(m) > mean / 5.0:
                        tmax = tmp.GetBinCenter(m)
                        break
                h["tMinAndTmax"][x.GetName()] = [tmin, tmax]
                h[x.GetName() + "tMin"] = ROOT.TArrow(tmin, -5.0, tmin, 0.8, 0.05, ">")
                h[x.GetName() + "tMax"] = ROOT.TArrow(tmax, -5.0, tmax, 0.8, 0.05, ">")
...
def extractRTPanda(hname="TDC1000_x"):
    R = ShipGeo.MufluxSpectrometer.InnerTubeDiameter / 2.0  #  = 3.63*u.cm
    ...
    for n in range(n0, n1):
        ...
        h["rt" + hname].SetPoint(
            n - n0, h[hname].GetBinCenter(n), N / float(Ntot + 1e-20) * R
        )
    h["rt" + hname].SetTitle("rt" + hname)
    h["rt" + hname].SetLineWidth(2)
    if not hname.find("TDC1") < 0:
        h["rt" + hname].SetLineColor(ROOT.kBlue)
    elif not hname.find("TDC2") < 0:
        h["rt" + hname].SetLineColor(ROOT.kCyan)
    elif not hname.find("TDC3") < 0:
        h["rt" + hname].SetLineColor(ROOT.kGreen)
    elif not hname.find("TDC4") < 0:
        h["rt" + hname].SetLineColor(ROOT.kGreen + 2)
    h["RTrelations"].cd(1)
...
def makeRTrelations():
    if "RTrelations" not in h:
        ut.bookCanvas(
            h, key="RTrelations", title="RT relations", nx=800, ny=500, cx=1, cy=1
        )
        h["RTrelations"].cd(1)
        x = h["TDC0"]
        h["emptyHist"] = ROOT.TH2F(
            "empty",
            " ;[ns];[cm] ",
            100,
            x.GetBinCenter(1),
            x.GetBinCenter(x.GetNbinsX()),
            100,
            0.0,
            2.0,
        )
    ...
    for p in h["TDCMapsX"].GetListOfPrimitives():
        for o in p.GetListOfPrimitives():
            if o.InheritsFrom("TH1"):
                extractRTPanda(hname=o.GetName())
    h["legRT"].Draw("same")


ut.bookHist(h, "TDC2R", "RT relation; t [ns] ; r [cm]", 100, 0.0, 3000.0, 100, 0.0, 2.0)
...
def RT(hit, t):
    ...
    if MCdata:
        ...
        if nRT in [16, 17]:
            nRT = 18
        if nRT in [30, 31]:
            nRT = 32  # not enough statistics for left and right of T3
        name = "TDC" + str(nRT)
        if t == 0:
            t = 0.1
        if t > h["tMinAndTmax"][name][1]:
            r = R
        elif t < h["tMinAndTmax"][name][0]:
            r = 0
        else:
            r = h["rt" + name].Eval(t)
        if "RTcorr" in h:
...
def checkMCSmearing():
    ut.bookHist(h, "spr", "spatial resolution MC", 100, -1.0, 1.0)
    ut.bookHist(h, "MCposX", "diff position X", 100, -1.0, 1.0)
    ut.bookHist(h, "nMeasMC", "number of hits per track", 50, -0.5, 49.5)
    for i in range(sTree.GetEntries()):
        trackID = {}
        rc = sTree.GetEvent(i)
        if (
            sTree.Digi_MufluxSpectrometerHits.GetEntries()
            != sTree.MufluxSpectrometerPoint.GetEntries()
        ):
        ...
        for n in range(sTree.Digi_MufluxSpectrometerHits.GetEntries()):
            ...
            if mcp < 0:
                continue
            if abs(sTree.MCTrack[mcp].GetPdgCode()) != 13:
                continue
            t = p.GetDigi()
...
def originMCmuons():
    ut.bookHist(
        h, "origin z/r", "origin of muons, z vs r", 1000, -600.0, 600.0, 100, 0.0, 10.0
    )
    for i in range(sTree.GetEntries()):
        rc = sTree.GetEvent(i)
        r = ROOT.TMath.Sqrt(
            sTree.MCTrack[0].GetStartX() ** 2 + sTree.MCTrack[0].GetStartY() ** 2
        )
...
def extrapolateToPlane(fT, z, cplusplus=True):
    ...
    # etrapolate to a plane perpendicular to beam direction (z)
        ...
        if (
            z > DT["Station_1_x_plane_0_layer_0_10000000"][2] - 10
            and z < DT["Station_4_x_plane_1_layer_1_40110000"][2] + 10
        ):
            # find closest measurement
            mClose = 0
            mZmin = 999999.0
            M = min(
                fT.getNumPointsWithMeasurement(), 30
            )  # for unknown reason, get stuck for track with large measurements
            for m in range(0, M):
            ...
            NewPosition = ROOT.TVector3(0.0, 0.0, z)
            ...
            try:
                ...
                error = (
                    "extrapolateToPlane: error with extrapolation: z=%7.3F m %7.3F %7.3F %7.3F %7.3F %7.3F %7.3F "
                    % (z / u.m, pos.X(), pos.Y(), pos.Z(), mom.X(), mom.Y(), mom.Z())
                )
...
for x in ["", "mu"]:
    for s in [
        ...
        ut.bookHist(
            h,
            "p/pt" + x + s,
            "momentum vs Pt (GeV);p [GeV/c]; p_{T} [GeV/c]",
            500,
            0.0,
            500.0,
            100,
        ...
        ut.bookHist(
            h,
            "p/px" + x + s,
            "momentum vs Px (GeV);p [GeV/c]; p_{X} [GeV/c]",
            500,
            0.0,
            500.0,
            200,
        ...
        ut.bookHist(h, "chi2" + x + s, "chi2/nDoF", 100, 0.0, 10.0)
        ut.bookHist(
            h, "Nmeasurements" + x + s, "number of measurements used", 25, -0.5, 24.5
        )
        ut.bookHist(
            h,
            "xy" + x + s,
            "xy of first state;x [cm];y [cm]",
            100,
            -30.0,
            30.0,
            100,
        ...
        ut.bookHist(
            h,
            "p1/p2" + x + s,
            "momentum p1 vs p2;p [GeV/c]; p [GeV/c]",
            500,
            0.0,
            500.0,
            500,
        ...
        ut.bookHist(
            h,
            "pt1/pt2" + x + s,
            "P_{T} 1 vs P_{T} 2;p [GeV/c]; p [GeV/c]",
            100,
            0.0,
            10.0,
            100,
        ...
        ut.bookHist(
            h,
            "p1/p2s" + x + s,
            "momentum p1 vs p2 same sign;p [GeV/c]; p [GeV/c]",
            500,
            0.0,
            500.0,
            500,
        ...
        ut.bookHist(
            h,
            "pt1/pt2s" + x + s,
            "P_{T} 1 vs P_{T} 2 same sign;p [GeV/c]; p [GeV/c]",
            100,
            0.0,
            10.0,
            100,
        ...
        ut.bookHist(
            h,
            "trueMom" + x + s,
            "true MC momentum;P [GeV/c];#sigma P/P",
            500,
            0.0,
            500.0,
        )
        ut.bookHist(
            h,
            "recoMom" + x + s,
            "reco MC momentum;P [GeV/c];#sigma P/P",
            500,
            0.0,
            500.0,
        )
        ut.bookHist(
            h,
            "momResol" + x + s,
            "momentum resolution function of momentum;P [GeV/c];#sigma P/P",
            200,
            -0.5,
            0.5,
            40,
...
ut.bookHist(
    h, "weightVsSource", "weight vs source MC check", 10, -0.5, 9.5, 1000, 0.0, 1000.0
)
        ...
        if not rc:
            ...
            if debug:
                ...
                print(
                    ...
                    % (pos[0], pos[1], pos[2], Bx, By, Bz, mom[0], mom[1], mom[2])
                )
            # ptkick 1.03 / dalpha
        if nP == 0:
            fitStatus = theTrack.getFitStatus()
            print(
                "trackinfo P/Pt/chi2/DoF/Ndf:%6.2F %6.2F %6.2F %6.2F"
                % (
                    mom.Mag(),
                    mom.Pt(),
            ...
            if debug:
                ...
                for j in range(theTrack.getNumPointsWithMeasurement()):
                    ...
                    print(
                        "%i %5.2F %5.2F %5.2F %5.2F %5.2F  %5.2F %i %i "
                        % (
                            j,
                            pos[0],
                            pos[1],
...
def plotMuonTaggerTrack(muTracks):
    for view in ["X", "Y"]:
        for muTrack in muTracks[view]:
            ...
            h["dispTrackY"].append(
                ROOT.TGraph(2)
            )  # tricky, framework expects equal number of x and y projections
            zStart = 850.0
            ...
            if view == "X":
                h["dispTrack"][nt].SetLineColor(ROOT.kRed)
            else:
...
def findSimpleEvent(event, nmin=2, nmax=6):
    ...
    for s in range(1, 5):
        for l in range(4):
            for hit in spectrHitsSorted[0][s][l]:
                nH[s] += 1
        if nH[s] < nmin or nH[s] > nmax:
            passed = False
    nu = 0
    for l in range(4):
        for hit in spectrHitsSorted[1][1][l]:
            nu += 1
    if nu < nmin or nu > nmax:
        passed = False
    nv = 0
    for l in range(4):
        for hit in spectrHitsSorted[2][2][l]:
            nv += 1
    if nv < nmin or nv > nmax:
        passed = False
    return passed
...
def fitTracks(
    ...
    if not withDisplay and not Debug and not simpleEvents:
        muflux_Reco.trackKinematics(3.0)
        momDisplay()
        return
    for n in range(nStart, sTree.GetEntries()):
        ...
        if MCdata:
            if (
                ...
                break
                if PR < 10 and sTree.ShipEventHeader.GetUniqueID() == 1:
                    continue  # non reconstructed events
        if not withDisplay and n % 10000 == 0:
            print("event #", n)
        if nMax == 0:
            break
        if simpleEvents and simpleEvents < 2:
            if not findSimpleEvent(sTree):
                continue
        if withDisplay:
            print("event #", n)
            plotEvent(n)
        if PR == 3:
            theTracks = bestTracks()
        else:
            theTracks = findTracks(PR)
        if simpleEvents == 2 and len(theTracks) < simpleEvents:
            continue
        if PR == 1 and len(theTracks) > 1:
            RPCclusters, RPCtracks = muonTaggerClustering(
                11
            )  # in case file was made with simple reco
        else:
        ...
        if len(theTracks) > 0:
            for aTrack in theTracks:
                ...
                if not fitStatus.isFitConverged():
                    ...
                    # track quality
                if PR < 10:
                    hitsPerStation = countMeasurements(N, PR)
                else:
                    hitsPerStation = countMeasurements(aTrack, PR)
                if len(hitsPerStation["x1"]) < 2:
                    continue
                if len(hitsPerStation["x2"]) < 2:
                    continue
                if len(hitsPerStation["x3"]) < 2:
                    continue
                if len(hitsPerStation["x4"]) < 2:
                    continue
                ...
                if rc:
                    ...
                    for proj in ["X", "Y"]:
                        for m in RPCtracks[proj]:
                            if (
                                abs(posRPC[0] - m[0] * zRPC1 + m[1])
                                < cuts["muTrackMatch" + proj]
                            ):
                                tagged[proj] = True
                ...
                if len(theTracks) == 2 and N == 0:
                    ...
                    rc = h["pt1/pt2"].Fill(
                        ROOT.TMath.Sqrt(Px * Px + Py * Py),
                        ROOT.TMath.Sqrt(Pbx * Pbx + Pby * Pby),
                    )
            if withRT:
                for hit in sTree.Digi_MufluxSpectrometerHits:
                    if not hit.hasTimeOverThreshold():
...
def momDisplay():
    ...
    for x in ["", "mu"]:
        t = "mom" + x
        if t not in h:
            ut.bookCanvas(h, key=t, title="trackfit" + x, nx=1200, ny=600, cx=4, cy=2)
        rc = h[t].cd(1)
        h["p/pt" + x].SetStats(0)
...
def fitTrack(hitlist, Pstart=3.0):
    # need measurements
    global fitter
    hitPosLists = {}
    trID = 0
    posM = ROOT.TVector3(0, 0, 20.0)
    momM = ROOT.TVector3(0, 0, Pstart * u.GeV)
    # approximate covariance
    covM = ROOT.TMatrixDSym(6)
    resolution = sigma_spatial
    if not withTDC:
        resolution = 10 * sigma_spatial
    for i in range(3):
        covM[i][i] = resolution * resolution
    # covM[0][0]=resolution*resolution*100.
    for i in range(3, 6):
        covM[i][i] = ROOT.TMath.Power(resolution / (4.0 * 2.0) / ROOT.TMath.Sqrt(3), 2)
    ...
    for k in sorted_z:
        ...
        measurement = ROOT.genfit.WireMeasurement(
            unSortedList[k[0]][0], hitCov, 1, 6, tp
        )  # the measurement is told which trackpoint it belongs to
        measurement.setMaxDistance(ShipGeo.MufluxSpectrometer.InnerTubeDiameter / 2.0)
    ...
    timer.Start()
    try:
        fitter.processTrack(theTrack)  # processTrackWithRep(theTrack,rep,True)
    except:
        print("fit failed")
        timer.Stop()
        theTrack.Delete()
    ...
    if timer.RealTime() > 1:  # make a new fitter, didn't helped
        error = "fitTrack::very long fit time %8.6F  %6i" % (
            timer.RealTime(),
            len(hitlist),
        )
        ut.reportError(error)
...
ut.bookHist(
    h,
    "biasResTrackMom",
    "momentum of tracks used for biased residuals",
    400,
    0.0,
    400.0,
)
for s in range(1, 5):
    ...
    for view in ["_x", "_u", "_v"]:
        ...
        if s == 1 and view == "_v" or s == 2 and view == "_u":
            continue
        ut.bookHist(
            h, "del" + view + str(s), "del x for " + str(s) + view, 100, -20.0, 20.0
        )
        clusters[s][view] = {}
        dx = 0
        for layer in range(0, 4):  # p*2+l
            ...
            ut.bookHist(
                ...
                "biased residual for " + str(s) + view + " " + str(layer),
                100,
                -0.5,
                0.5,
                20,
            ...
            ut.bookHist(
                ...
                "linear track model residual for " + str(s) + view + " " + str(layer),
                100,
                -20.0,
                20.0,
                10,
...
ut.bookHist(
    h, "biasResDist", "residual versus drift radius", 100, 0.0, 2.0, 1000, -2.0, 2.0
)
ut.bookHist(
    h, "biasResDist2", "residual versus track distance", 100, 0.0, 2.0, 1000, -2.0, 2.0
)

ut.bookHist(h, "clsN", "cluster sizes", 10, -0.5, 9.5)
ut.bookHist(h, "Ncls", "number of clusters / event", 10, -0.5, 9.5)
ut.bookHist(h, "delY", "del Y from stereo; [cm]", 100, -40.0, 40.0)
ut.bookHist(h, "yest", "estimated Y from stereo; [cm]", 100, -100.0, 100.0)

myGauss = ROOT.TF1(
    "gauss", "abs([0])/(abs([2])*sqrt(2*pi))*exp(-0.5*((x-[1])/[2])**2)+abs([3])", 4
)
...
def testPR(onlyHits=False):
    ...
    for nTrack in track_hits:
        h["magPos"].Fill(
            track_hits[nTrack]["x_in_magnet"], track_hits[nTrack]["y_in_magnet"]
        )
    ...
    for nTrack in track_hits:
        ...
        for x in [track_hits[nTrack]["y12"], track_hits[nTrack]["34"]]:
            ...
            for hits in x:
                ...
                if detID > 2:
                    detID -= 2
                clus[detID].append([0, (hits["xtop"] + hits["xbot"]) / 2.0, hits["z"]])
            slopeA, bA = getSlopes(clus[1], clus[2])
            x1 = zgoliath * slopeA + bA
            nt = len(h["dispTrackSeg"])
            h["dispTrackSeg"].append(ROOT.TGraph(2))
            if upStream:
                h["dispTrackSeg"][nt].SetPoint(0, 0.0, bA)
                h["dispTrackSeg"][nt].SetPoint(1, 400.0, slopeA * 400 + bA)
                h["dispTrackSeg"][nt].SetLineColor(ROOT.kRed)
                upStream = False
            else:
                h["dispTrackSeg"][nt].SetPoint(0, 300.0, slopeA * 300 + bA)
                h["dispTrackSeg"][nt].SetPoint(1, 900.0, slopeA * 900 + bA)
    ...
    for s in range(1, 5):
        ...
        for view in viewsI[s]:
            ...
            if removeBigClusters:
                clustersPerLayer = {}
                for l in range(4):
                    clustersPerLayer[l] = dict(
                        enumerate(grouper(list(allHits[l].keys()), 1), 1)
                    )
                    for Acl in clustersPerLayer[l]:
                        if (
                            len(clustersPerLayer[l][Acl]) > cuts["maxClusterSize"]
                ...
                if len(cl) > 5:
                    for hit in cl:
                        if hit[3] not in tmp:
                            tmp[hit[3]] = []
                            check[hit[3]] = []
                        if hit[0].GetDetectorID() in check[hit[3]]:
                            continue
...
def findDTClustersDebug1(n, tmp):
    for hit in tmp[n]:
        s, v, p, l, view, channelID, tdcId, nRT = stationInfo(hit)
        bot, top = strawPositionsBotTop[hit.GetDetectorID()]
        print(s, v, p * 2 + l, channelID, (bot[0] + top[0]) / 2.0)
...
def findTracks(PR=1, linearTrackModel=False, withCloneKiller=True):
    if PR < 3 and sTree.GetBranch("FitTracks"):
        return sTree.FitTracks
    if PR % 2 == 0:
        trackCandidates = testPR()
        if len(trackCandidates) > 1:
            trackCandidates = cloneKiller(trackCandidates)
        return trackCandidates
    ...
    allStations = True
    for s in range(1, 5):
        if len(clusters[s][0]) == 0:
            allStations = False
    if len(clusters[1][1]) == 0 or len(clusters[2][2]) == 0:
        allStations = False
    if allStations:
        t1t2cand = []
        ...
        for nt1t2 in range(len(t1t2cand)):
            ...
            for nt3t4 in range(len(t3t4cand)):
                ...
                if abs(delx) < cuts["delxAtGoliath"]:
                    ...
                    if Debug:
                        print(
                            "stereo clusters", len(clusters[1][1]), len(clusters[2][2])
                        )
                    for nu in range(len(clusters[1][1])):
                        ...
                        for cl in clu:
                            ...
                            rc = h["yest"].Fill(yest)
                            if yest > botA[1] + cuts["yMax"]:
                                continue
                            if yest < topA[1] - cuts["yMax"]:
                                continue
                            stereoHits[1][cl[0].GetDetectorID()] = [
                                cl[0],
                                sl,
                                b,
                                yest,
                        ...
                        for nv in range(len(clusters[2][2])):
                            ...
                            for cl in clv:
                                ...
                                if yest < topA[1] - cuts["yMax"]:
                                    continue
                                stereoHits[2][cl[0].GetDetectorID()] = [
                                    cl[0],
                                    sl,
                                    b,
                                    yest,
                            ...
                            if (
                                len(stereoHits[1]) < cuts["minLayersUV"]
                                or len(stereoHits[2]) < cuts["minLayersUV"]
                            ):
                                continue
                            ...
                            if Debug:
                                ...
                                print("----> u")
                                for x in stereoHits[1]:
                                    print(stereoHits[1][x][3], stereoHits[1][x][4])
                                print("----> v")
                                for x in stereoHits[2]:
                            ...
                            if Debug:
                                print(
                                    ...
                                    T3ybot,
                                    T3ytop,
                                    (yAtT3 - T3ybot) > 2 * cuts["yMax"],
                                    (T3ytop - yAtT3) > 2 * cuts["yMax"],
                                )
                            if (yAtT3 - T3ybot) > 2 * cuts["yMax"] or (
                                T3ytop - yAtT3
                            ) > 2 * cuts["yMax"]:
                                continue
                            if Debug:
                                ...
                                h["dispTrackSeg"].append(ROOT.TGraph(2))
                                h["dispTrackSeg"][nt].SetPoint(0, 0.0, bA)
                                h["dispTrackSeg"][nt].SetPoint(
                                    1, 900.0, slopeA * 900 + bA
                                )
                                h["dispTrackSeg"][nt].SetLineColor(ROOT.kRed + nTrx)
                                h["dispTrackSeg"][nt].SetLineWidth(2)
                                h["simpleDisplay"].cd(1)
                            ...
                            for p in range(2):
                                for cl in t1t2[p]:
                                    hitList.append(cl[0])
                            for p in stereoHits:
                                for cl in stereoHits[p]:
                                    hitList.append(stereoHits[p][cl][0])
                            for p in range(2):
                                for cl in t3t4[p]:
                                    hitList.append(cl[0])
                            # add late arrivals
                            if cuts["lateArrivalsToT"] < 3000:
                                ...
                                for x in hitList:
                                    ...
                                    for k in range(1, l):
                                        ...
                                        tmp.append(
                                            sTree.Digi_LateMufluxSpectrometerHits[key]
                                        )
                                hitList = hitList + tmp
                            if linearTrackModel:
                                ...
                                if zeroField:
                                    momFromptkick = 1000.0
                                else:
                                    momFromptkick = ROOT.TMath.Abs(
                                        1.03 / (t3t4[3] - t1t2[3] + 1e-20)
                                    )
                                if Debug:
                                    print(
                                        "fit track t1t2 %i t3t4 %i stereo %i,%i, with hits %i,  delx %6.3F, pstart %6.3F"
                                        % (
                                            nt1t2,
                                            nt3t4,
                                            nu,
                                ...
                                if type(aTrack) != type(1):
                                    ...
                                    reject = False
                                    if (pos[1] - T3ybot) > 1.2 * cuts["yMax"] or (
                                        T3ytop - pos[1]
                                    ) > 1.2 * cuts["yMax"]:
                                        reject = True
                                    mStatistics = countMeasurements(aTrack, PR)
                                    if (
                                        len(mStatistics["u"]) < cuts["minLayersUV"]
                                        or len(mStatistics["v"]) < cuts["minLayersUV"]
                                    ):
                                        reject = (
                                            True  # require 2 measurements in each view
                                        )
                                    if not reject:
                                        trackCandidates.append(aTrack)
                                    else:
                                        ...
                                        if Debug:
                                            print(
                                                "track rejected, outside T3 acceptance or not enough u/v measurements"
                                            )
                                            print(
                                                (pos[1] - T3ybot) > 1.2 * cuts["yMax"],
                                                (T3ytop - pos[1]) > 1.2 * cuts["yMax"],
                                                len(mStatistics["u"]) < 2,
                                                len(mStatistics["v"]) < 2,
                                            )
    if withMaterial:
    ...
    if withCloneKiller:
        ...
        if withMaterial:
            for aTrack in trackCandidates:
                ...
                if Debug:
                    printTrackMeasurements(aTrack, PR)
    ...
    if PR == 1 and sTree.GetBranch("TrackInfos"):
        ...
        for n in range(trInfo.N()):
            ...
            if view != "_x":
                ...
                if view == "_u":
                    mStatistics["u"].append(detID)
                if view == "_v":
                    mStatistics["v"].append(detID)
            else:
                mStatistics["xAll"].append(detID)
                mStatistics["x" + str(s)].append(detID)
            if s > 2:
                mStatistics["xDown"].append(detID)
            else:
                mStatistics["xUp"].append(detID)
    else:
        for p in aTrack.getPointsWithMeasurement():
            ...
            if not info:
                continue
            detID = rawM.getDetId()
            test = ROOT.MufluxSpectrometerHit(detID, 0.0)
...
def cloneKiller(trackCandidates):
    ...
    for j in range(len(detIDs) - 1):
        ...
        for k in range(j + 1, len(detIDs)):
            ...
            if max(len(o) / tj, len(o) / tk) > 0.49:
                ...
                if sj.getNdf() < sk.getNdf() - cuts["deltaNdf"]:
                    detIDs[j]["xDown"] = []
                elif sk.getNdf() < sj.getNdf() - cuts["deltaNdf"]:
                    detIDs[k]["xDown"] = []
                elif sj.getChi2() / sj.getNdf() < sk.getChi2() / sk.getNdf():
                    detIDs[k]["xDown"] = []
                elif sk.getChi2() / sk.getNdf() < sj.getChi2() / sj.getNdf():
                    detIDs[j]["xDown"] = []
                else:
                    detIDs[j]["xDown"] = []
                if Debug:
    ...
    for aTrack in trackCandidates:
        j += 1
        if Debug:
            print("clone killer at work", j, len(detIDs[j]["xDown"]))
        if len(detIDs[j]["xDown"]) > 0:
...
def printResiduals(aTrack):
    if not aTrack.getNumPointsWithMeasurement() > 0:
        return
    ...
    for hit in sTree.Digi_MufluxSpectrometerHits:
        ...
        tmp = -tmp / ROOT.TMath.Sqrt(
            (vtop[0] - vbot[0]) ** 2 + (vtop[1] - vbot[1]) ** 2
        )  # to have same sign as difference in X
        xL = tmp - distance
        xR = tmp + distance
        if abs(xL) < abs(xR):
            res = xL
            ...
            res = xR
        tmpList[k] = pos[2]
        txt[k] = "%i %s %i %5.3F %5.3F %5.3F %5.3F " % (
            s,
            view,
            2 * p + l,
            pos[0],
...
def plotBiasedResiduals(nEvent=-1, nTot=1000, PR=1, onlyPlotting=False, minP=3.0):
    timerStats = {
        "fit": 0,
        "analysis": 0,
        "prepareTrack": 0,
            ...
            for p in xLayers[s]:
                for l in xLayers[s][p]:
                    for view in xLayers[s][p][l]:
                        ...
                        h["biasResDist2Wire_" + hkey] = h["biasResDist2"].Clone(
                            "biasResDist2Wire_" + hkey
                        )
                        ut.bookHist(
                            h,
                            "distIfNoHit_" + hkey,
                            "shortest distance to wire if no hit",
                            100,
                            0.0,
                            5.0,
                        )
        for n in range(576 / cuts["RTsegmentation"]):
            h["TDC" + str(n)].Reset()
            #
        pos = ROOT.TVector3()
        mom = ROOT.TVector3()
            ...
            if len(trackCandidates) == 1:
                ...
                for aTrack in trackCandidates:
                    ...
                    try:
                        ...
                        print(
                            "problem with getting state, event",
                            sTree.GetCurrentFile().GetName(),
                            Nr,
                        )
                        continue
                    if sta.getMomMag() < minP and not zeroField:
                        continue
                    ...
                    for s in range(1, 5):
                        for view in viewsI[s]:
                            for l in range(4):
                                hitFound = False
                                for hit in spectrHitsSorted[view][s][l]:
                                    ss, vv, pp, ll, vw, channelID, tdcId, nRT = (
                                        stationInfo(hit)
                                    )
                                    vbot, vtop = strawPositionsBotTop[
                                        hit.GetDetectorID()
                                    ]
                                    z = (vbot[2] + vtop[2]) / 2.0
                                    timer.Start()
                                    # rc,pos,mom = extrapolateToPlane(aTrack,z)
                                    trackLength = muflux_Reco.extrapolateToPlane(
                                        aTrack, z, pos, mom
                                    )
                                ...
                                if not hitFound:
                                    ...
                                    v = 0
                                    if (s == 1 and view == 1) or (s == 2 and view == 0):
                                        v = 1000000
                                    firstWire = (
                                        s * 10000000
                                        + v
    ...
    if "biasedResiduals" not in h:
        ...
        ut.bookCanvas(
            h,
            key="biasedResidualsX",
            title="biasedResiduals function of X",
            nx=1600,
            ny=1200,
            cx=4,
            cy=6,
        ...
        ut.bookCanvas(
            h,
            key="biasedResidualsY",
            title="biasedResiduals function of Y",
            nx=1600,
            ny=1200,
            cx=4,
            cy=6,
        ...
        for view in ["_x", "_u", "_v"]:
            ...
            for l in range(0, 4):
                if withTDC:
                    ...
                    hname = "biasResXL_" + str(s) + view + str(l)
                ...
                if not rc:
                    ...
                    fitFunction.SetParameter(
                        0, h[hnameProjX].GetEntries() * h[hnameProjX].GetBinWidth(1)
                    )
                    ...
                    fitFunction.SetParameter(
                        0,
                        rc.GetParams()[0]
                        * ROOT.TMath.Sqrt(2 * ROOT.TMath.Pi())
                        * rc.GetParams()[2],
                    )
                ...
                Erms = rc.GetErrors()[2]
                print(
                    "%i, %s, %i mean=%5.2F+/-%5.2F RMS=%5.2F+/-%5.2F [mm]"
                    % (s, view, l, mean * 10, Emean * 10, rms * 10, Erms * 10)
                )
                residuals[j - 1] = h[
                    hnameProjX
                ].GetMean()  # fitresult too unstable, mean
                # make plot of mean as function of X,Y
                for p in ["X", "Y"]:
                    if withTDC:
                        hname = "biasRes" + p + "_" + str(s) + view + str(l)
                    else:
                        hname = "biasRes" + p + "L_" + str(s) + view + str(l)
                    hmean = hname + "_mean" + p
                    ...
                    for k in range(1, h[hname].GetNbinsY() + 1):
                        ...
                        if not rc:
                            print("simple gaus fit failed")
                            fitFunction.SetParameter(
                                0, h[sli].GetEntries() * h[sli].GetBinWidth(1)
                            )
                            fitFunction.SetParameter(1, 0.0)
                            fitFunction.SetParameter(2, 0.1)
                            fitFunction.SetParameter(3, 1.0)
...
def plotSigmaRes():
    ut.bookHist(h, "resDistr", "residuals", 50, 0.0, 0.1)
...
def calculateRTcorrection():
    for hist in h.keys():
        if hist.find("biasResDist") != 0:
            continue
        if not hist.find("proj") < 0:
            continue
        if hist == "biasResDist2":
            continue
    ...
    if "RTCorrection" not in h:
        ut.bookCanvas(
            h, key="RTCorrection", title="RTCorrection", nx=1200, ny=1400, cx=1, cy=2
        )
...
def analyzeSingleDT():
    for detID in sorted(xpos.keys()):
        ...
        if mean < -900:
            print(
                "channel:%i : not enough statistics, integral=%i, under- over-flow: %i,%i"
                % (detID, histo.GetSumOfWeights(), uf, of)
            )
        else:
            print(
                "channel:%i : mean=%6.3Fmm,  sigma=%6.3Fmm"
                % (detID, mean * 10, rms * 10)
            )
...
myGauss2 = ROOT.TF1(
    "test",
    "abs([0])/(abs([2])*sqrt(2*pi))*exp(-0.5*((x-[1])/[2])**2)+abs( [3]+[6]*x+[7]*x**2 )+abs([4])/(abs([5])*sqrt(2*pi))*exp(-0.5*((x-[1])/[5])**2)",
    8,
)
...
myGauss2.FixParameter(6, 0.0)
myGauss2.FixParameter(7, 0.0)
...
def binoEff(n=4, k=2):
    ...
    for i in range(k, n + 1):
        totEff += ROOT.TMath.Binomial(n, i) * eff**i * (1 - eff) ** (n - i)
    print("global efficiency = %5.4F  %i %i" % (totEff, n, k))


def DTeffWithRPCTracks(Nevents=0, onlyPlotting=False):
    align2RPC = {4: [-1.0, 3.2], 3: [-2.0, 5.0], 2: [-6.0, 8.0], 1: [-6.0, 8.0]}
    if not onlyPlotting:
        if Nevents == 0:
        ...
        for tag_s in range(1, 5):
            for s in range(1, 5):
                ut.bookHist(
                    h,
                    "hitsIn" + str(s) + "_" + str(tag_s),
                    "number of hits " + str(s),
                    10,
                    -0.5,
                    9.5,
                )
                ut.bookHist(
                    h,
                    "hits" + str(s) + "_" + str(tag_s),
                    "number of hits in layer " + str(s),
                    5,
                    -0.5,
                    4.5,
                )
                for l in range(4):
                    if tag_s == 1:
                        ut.bookHist(
                            h,
                            "distX" + str(s) + str(l),
                            "distance of RPC track to hit " + str(s) + str(l),
                            100,
                            -10.0,
                            10.0,
                        )
                    ut.bookHist(
                        h,
                        "distXref" + str(s) + str(l) + "_" + str(tag_s),
                        "distance of refitted RPC track to hit " + str(s) + str(l),
                        100,
                        -10.0,
                        10.0,
                    )
        ...
        for n in range(Nevents):
            ...
            if not findSimpleEvent(sTree):
                continue
            if len(sTree.RPCTrackX) > 1 or len(sTree.RPCTrackY) > 1:
                continue
            spectrHitsSorted = ROOT.nestedList()
            candidates = {
                1: {0: [], 1: [], 2: [], 3: []},
                ...
                for tag_s in range(1, 5):
                    ...
                    for l in candidates[tag_s]:
                        for hit in candidates[tag_s][l]:
                            vbot, vtop = strawPositionsBotTop[hit.GetDetectorID()]
                            pos[(vbot[2] + vtop[2]) / 2.0] = (vbot[0] + vtop[0]) / 2.0
                    if len(pos) < 3:
                        continue  # 1 RPC point + >1 DT point
                    coefficients = numpy.polyfit(
                        list(pos.keys()), list(pos.values()), 1
                    )
                    Ntot[tag_s] += 1
                    ...
                    if nhits[4] < 2:
                        Ineff += 1
                    for s in range(1, 5):
                        h["hits" + str(s) + "_" + str(tag_s)].SetBinContent(
                            6, Ntot[tag_s]
                        )
        print(
            "rough estimate of station inefficiency:",
            float(Ineff) / (Ntot[tag_s] + 1e-5),
        )
        ut.writeHists(h, "histos-DTEff" + rname)
    else:
        # analysis part
        if "hits1_1" not in h:
            ut.readHists(h, "DTEff.root")
        effPerLayer = {}
        for tag_s in range(1, 5):
            ...
            if t not in h:
                ut.bookCanvas(
                    h,
                    key=t,
                    title="with tagging station " + str(tag_s),
                    nx=1600,
                    ny=1200,
                    cx=4,
                    cy=4,
            ...
            for s in range(1, 5):
                for l in range(4):
                    ...
                    fitFunction = myGauss
                    fitFunction.SetParameter(
                        0, h[hname].GetEntries() * h[hname].GetBinWidth(1)
                    )
                    fitFunction.SetParameter(1, 0.0)
                    fitFunction.SetParameter(2, 1.2)
                    fitFunction.SetParameter(3, 0.0)
                    fitResult = h[hname].Fit(fitFunction, "SQ", "", -10.0, 10.0)
                    rc = fitResult.Get()
                    background = rc.Parameter(3) * h[hname].GetNbinsX()
                    signal = h[hname].GetSumOfWeights() - background
                    effPerLayer[tag_s][10 * s + l] = signal / float(
                        h["hits" + str(s) + "_" + str(tag_s)].GetBinContent(6)
                    )
        ...
        for s in choice:
            tag_s = choice[s]
            for l in range(4):
                h["DTeffPerLayer"].SetBinContent(
                    s * 10 + l + 1, effPerLayer[tag_s][10 * s + l]
                )
        ...
        for s in choice:
            ...
            print("station %i ineff=%5.2F%%" % (s, inEff / ntracks * 100.0))
        h["leghits"].Draw()
        ...
        for s in range(1, 5):
            for view in ["_x", "_u", "_v"]:
                ...
                for l in range(0, 4):
                    ...
                    h[hname] = (
                        h["biasResDist_" + str(s) + view + str(l)]
                        .ProjectionY()
                        .Clone(hname)
                    )
    j = 0
        ...
        for view in ["_x", "_u", "_v"]:
            ...
            for l in range(0, 4):
                ...
                if method == 0 or method == 1:
                    ...
                    hname = "biasResXL_" + str(s) + view + str(l) + "_projx"
                ...
                if not rc:
                    ...
                    fitFunction.SetParameter(5, fitFunction.GetParameter(2) * 10.0)
                fitResult = h[hname].Fit(fitFunction, "SQ", "", xmin, xmax)
                h[hname].Draw()
                rc = fitResult.Get()
                if method == 0:
                    estSignal = h[hname].Integral(375, 625)
                elif method == 1 or method == 3:
                    estSignal = (abs(rc.GetParams()[0]) + abs(rc.GetParams()[4])) / h[
                        hname
                    ].GetBinWidth(1)
                elif method == 2:
                    estSignal = (
                        h[hname].GetSumOfWeights()
                        - rc.GetParams()[3] * h[hname].GetNbinsX()
                    )
                ...
                j += 1
            print(
                "station, %i %s, average efficiency: %5.3F"
                % (s, view, effStation / 4.0)
            )
    for p in h["biasedResiduals"].GetListOfPrimitives():
        p.SetLogy(1)
    tc1 = ROOT.gROOT.FindObject("c1")
    tc1.cd()
    for n in range(1, h["effLayer"].GetNbinsX() + 1):
        h["effLayer"].SetBinError(n, 0.02)
    h["effLayer"].SetMaximum(1.0)
    h["effLayer"].SetMinimum(0.0)
...
def checkEffectOfEffCor():
    ...
    ut.readHists(
        h["MCtuned2"], "momDistributions-mbias-effTuned-M2.root", interestingHistos
    )
    ut.readHists(
        h["MCtuned0"], "momDistributions-mbias-effTuned-M0.root", interestingHistos
    )
    ut.readHists(
        h["MCtuned0rec"],
        "momDistributions-mbias-effTuned-M0-reco.root",
        interestingHistos,
    )
    ut.readHists(
        h["MCNotune"], "momDistributions-mbias-NoeffTuned.root", interestingHistos
    )
    ut.readHists(h["MC"], "momDistributions-charm.root", interestingHistos)
    ut.readHists(
        h["MCtuned2"], "momDistributions-charm-effTuned-M2.root", interestingHistos
    )
    ut.readHists(
        h["MCtuned0"], "momDistributions-charm-effTuned-M0.root", interestingHistos
    )
    ut.readHists(
        h["MCtuned0rec"],
        "momDistributions-charm-effTuned-M0-reco.root",
        interestingHistos,
    )
    ut.readHists(
        h["MCNotune"], "momDistributions-charm-NoeffTuned.root", interestingHistos
    )
    ...
    for x in ["MCtuned2", "MCtuned0rec"]:
        h[x + "ratioB"].Draw("same")
...
def printTrackMeasurements(atrack, PR=1):
    ...
    if PR == 1 and sTree.GetBranch("TrackInfos"):
        ...
        for n in range(trInfo.N()):
            ...
            s, v, p, l, view, channelID, tdcId, nRT = stationInfo(hit)
            print(
                "%3i %3i %3i %3i %3s %3i %3i %4.2F %4.2F "
        ...
        for p in atrack.getPointsWithMeasurement():
            ...
            print(
                "%3i %3i %3i %3i %3s %3i %3i %4.2F %4.2F %5.2F %5.2F %5.2F "
                % (
                    s,
                    v,
                    p,
...
def debugTrackFit(nEvents, nStart=0, simpleEvents=True, singleTrack=True, PR=1):
    matches = {"good": [], "bad": []}
    ut.bookHist(h, "residuals", "all residuals", 100, -1.0, 1.0)
    ut.bookHist(h, "extrapX", "extrap in X", 100, -20.0, 20.0)
    ut.bookHist(h, "extrapY", "extrap in Y", 100, -20.0, 20.0)
    ut.bookHist(
        h, "fitfail_good", "fitfailure by channel for good events", 700, 0.5, 700.5
    )
    ut.bookHist(
        h, "fitfail_bad", "fitfailure by channel for bad events", 700, 0.5, 700.5
    )
    #
    fitFailures = [{}, {}]
    fitSuccess = [{}, {}]
    for n in range(nStart, nStart + nEvents):
        ...
        if singleTrack and len(tracks) != 1:
            continue
            # select RPC tracks with good Y info
        clusters, RPCtracks = muonTaggerClustering()
        if len(RPCtracks["X"]) > 1 or len(RPCtracks["Y"]) > 1:
            print(n, len(RPCtracks["X"]), len(RPCtracks["Y"]))
        if len(RPCtracks["X"]) != 1 or len(RPCtracks["Y"]) != 1:
            continue
        ...
        for atrack in tracks:
            ...
            if not st.isFitConverged():
                continue
            rc, pos, mom = extrapolateToPlane(atrack, zRPC1)
            if not rc:
                continue
            if abs(pos[0] - X) > 5.0:
                continue  # not worth checking Y
            delta = pos[1] - Y
            if Debug:
                print(
                    "event# %i difference in X,Y %5.3F %5.3F " % (n, pos[0] - X, delta)
                )
            rc = h["extrapX"].Fill(pos[0] - X)
            rc = h["extrapY"].Fill(delta)
            if abs(delta) < 10.0:
                matches["good"].append(n)  # within ~3sigma
            else:
            ...
            for kp in range(Npoints):
                ...
                if abs(delta) < 10.0:
                    k = 0
                elif abs(pos[1] - Y) > 50.0:
                    k = 1
                else:
                    continue
                if wL < 0.1 and wR < 0.1:
                    # record failure rate
                    if not detID in fitFailures[k]:
                        fitFailures[k][detID] = 0
                    fitFailures[k][detID] += 1
                else:
                    if not detID in fitSuccess[k]:
                        fitSuccess[k][detID] = 0
    ...
    for k in range(2):
        for detID in fitFailures[k]:
            ...
            if s > 2:
                ...
                x += 12 * (2 * l + p)
            r = 1
            if detID in fitSuccess[k]:
                r = fitFailures[k][detID] / float(
                    fitSuccess[k][detID] + fitFailures[k][detID]
                )
            if k == 0:
                rc = h["fitfail_good"].SetBinContent(x, r)
            else:
                rc = h["fitfail_bad"].SetBinContent(x, r)
    # only look at the pathological cases
    print(
        "Summary: good matches: %i   bad matches: %i    failure rate %5.2F"
        % (
            len(matches["good"]),
            len(matches["bad"]),
            len(matches["bad"]) / float(len(matches["bad"]) + len(matches["good"])),
        )
    )
    ROOT.gROOT.FindObject("c1").cd()
    h["extrapY"].Draw()
...
def plotLinearResiduals():
    if "linearResiduals2dX" not in h:
        ...
        ut.bookCanvas(
            h,
            key="linearResiduals2dX",
            title="linear track model, residuals function of X",
            nx=1600,
            ny=1200,
            cx=4,
            cy=4,
        ...
        ut.bookCanvas(
            h,
            key="linearResidualsX",
            title="linear track model, residuals",
            nx=1600,
            ny=1200,
            cx=4,
            cy=4,
...
def mergeHistosForMomResol():
    ...
    sim10fact = 1.8 / (
        65.0 * (1.0 - 0.016)
    )  # normalize 10GeV to 1GeV stats, 1.6% of 10GeV stats not processed.
    charmNorm = {1: 0.176, 10: 0.424}
    beautyNorm = {1: 0.0, 10: 0.01218}
    sources = {
        "": 1.0,
        "Hadronic inelastic": 100.0,
        "Lepton pair": 100.0,
        "Positron annihilation": 100.0,
        "charm": 1.0 / charmNorm[10],
        "beauty": 1.0 / beautyNorm[10],
    }
    if len(hMC) == 0:
        interestingHistos = []
        for a in [
            ...
            for source in sources:
                interestingHistos.append(a + source)
        ut.readHists(
            h["270"]["1GeV"], "momDistributions-1GeV-mbias-0.root", interestingHistos
        )
        ut.readHists(
            h["270"]["1GeVCharm"],
            "momDistributions-1GeV-charm-0.root",
            interestingHistos,
        )
        ut.readHists(
            h["270"]["10GeV"], "momDistributions-10GeV-mbias-0.root", interestingHistos
        )
        ut.readHists(
            h["500"]["1GeV"], "momDistributions-1GeV-mbias.root", interestingHistos
        )
        ut.readHists(
            h["500"]["1GeVCharm"], "momDistributions-1GeV-charm.root", interestingHistos
        )
        ut.readHists(
            h["500"]["10GeV"], "momDistributions-10GeV.root", interestingHistos
        )
        for res in ["270", "500"]:
            for a in [
                "trueMom",
                ...
                h["MC10" + res + a].Add(
                    h[res]["10GeV"][a + "charm"], -1.0 + charmNorm[10]
                )
                h["MC10" + res + a].Add(
                    h[res]["10GeV"][a + "beauty"], -1.0 + beautyNorm[10]
                )
            ...
            for a in ["trueMom", "recoMom"]:
                ...
                for n in range(h[a].GetNbinsX()):
                    if h[res + a].GetBinCenter(n) < 20.0:
            ...
            h["I-" + res + "trueMom"].GetXaxis().SetRangeUser(5.0, 500.0)
    # true mom, reco mom
    t = "true Mom"
    if t not in h:
        ut.bookCanvas(h, t, "true and reco momentum", 900, 600, 1, 1)
    tc = h[t].cd(1)
    tc.SetLogy()
    h["I-270trueMom"].Draw()
    ...
    t = "momResolution"
    if t not in h:
        ut.bookCanvas(h, t, "momentum Resolution", 900, 600, 1, 1)
    tc = h[t].cd(1)
    for res in ["270", "500"]:
        hname = "momResol" + res
        ...
        for n in range(1, h[hname + "P"].GetNbinsX() + 1):
            ...
            if h[hname + str(n)].GetEntries() < 50:
                continue
            if n > 10:
                h[hname + str(n)].Rebin(5)
            ...
            fitFunction.FixParameter(3, 0.0)
            fitResult = h[hname + str(n)].Fit(fitFunction, "S", "", -1.0, 1.0)
            rc = fitResult.Get()
            if not rc:
                continue
            mean = rc.GetParams()[1]
    ...
    for res in ["270", "500"]:
        ...
        if res == "500":
            h["text" + res] = ROOT.TLatex(
                50.0, 0.09, "#sigmaP/P = (" + p0 + "+" + p1 + ")%"
            )
            h[hname + "P"].Draw("same")
...
def hitResolution():
    ...
    for n in range(sTree.GetEntries()):
        ...
        for k in range(sTree.Digi_MufluxSpectrometerHits.GetEntries()):
            ...
            TDC = hit.GetDigi() - (vtop[0] - trueHit.GetX()) / (
                ROOT.TMath.C() * 100.0 / 1000000000.0
            )
            distance = RT("x", TDC)
            h["hitResol"].Fill(distance - trueHit.dist2Wire())


def matchedRPCHits(aTrack, maxDistance=10.0):
    matchedHits = {
        1: {0: [], 1: []},
        2: {0: [], 1: []},
        3: {0: [], 1: []},
    ...
    if rc:
        if (
            pos[0] > cuts["xLRPC1"]
            and pos[0] < cuts["xRRPC1"]
            and pos[1] > cuts["yBRPC1"]
            and pos[1] < cuts["yTRPC1"]
        ):
            inAcc = True
        for hit in sTree.Digi_MuonTaggerHits:
            channelID = hit.GetDetectorID()
...
def plotRPCExtrap(nEvent=-1, nTot=1000, PR=1, onlyPlotting=False):
    if not onlyPlotting:
        ...
        for s in range(1, 6):
            for v in range(2):
                ...
                ut.bookHist(
                    ...
                    "mom of tracks extr to RPC k with RPC k+1 matched",
                    100,
                    0.0,
                    100.0,
                )
                ut.bookHist(
                    ...
                    "mom of tracks extr to RPC k and matched with RPC k+1 matched",
                    100,
                    0.0,
                    100.0,
                )
            ut.bookHist(
                h,
                "RPCfired_or_" + str(s),
                "mom of tracks extr to RPC k and matched with RPC k+1 or of 0 and 1",
                100,
                0.0,
                100.0,
            )
        ut.bookHist(
            h,
            "RPCResX1_p",
            "RPC residual for station 1 function of track momentum",
            100,
            -dx,
            dx,
            100,
        ...
        ut.bookHist(
            h,
            "RPCMatchedHits",
            "matched RPC hits as function of track momentum",
            10,
            0.5,
            10.5,
            20,
        ...
        ut.bookHist(
            h,
            "RPCMeanMatchedHits",
            "mean matched RPC hits as function of track momentum",
            100,
            0.0,
            100.0,
        )
        ut.bookHist(h, "RPC>1", "fraction of tracks with > 1 RPC hits", 100, 0.0, 100.0)
        ut.bookHist(
            h,
            "RPC<2XY",
            "position of tracks with < 2 RPC hits and p>30GeV",
            50,
            -100.0,
            100.0,
            50,
        ...
        ut.bookHist(h, "RPC_p", "momentum of tracks pointing to RPC", 100, 0.0, 100.0)
        for k in range(2, 20):
            ut.bookHist(
                h,
                "RPC<" + str(k) + "_p",
        ...
        for Nr in range(eventRange[0], eventRange[1]):
            ...
            if not sTree.Digi_MuonTaggerHits.GetEntries() > 0:
                continue
            if not findSimpleEvent(sTree):
                continue
            trackCandidates = findTracks(PR)
            for aTrack in trackCandidates:
                ...
                if (
                    pos1[0] > cuts["xLRPC1"]
                    and pos1[0] < cuts["xRRPC1"]
                    and pos1[1] > cuts["yBRPC1"]
                    and pos1[1] < cuts["yTRPC1"]
                ):
                    inAcc = True
                for hit in sTree.Digi_MuonTaggerHits:
                    nHit += 1
                    ...
                    if v == 0:
                        ...
                        if s == 1:
                            h["RPCResX1_p"].Fill(res, sta.getMomMag())
                ...
                if inAcc:
                    ...
                    for k in range(1, 5):
                        for v in range(0, 2):
                            if len(matchedHits[k + 1][v]) == 0:
                                continue
                            rc = h["RPCextTrack_" + str(k) + str(v)].Fill(p)
                            if len(matchedHits[k][v]) > 0:
                                rc = h["RPCfired_" + str(k) + str(v)].Fill(p)
                            if v == 0:
                                if (
                                    len(matchedHits[k][v]) > 0
                                    or len(matchedHits[k][v + 1]) > 0
                                ):
                                    rc = h["RPCfired_or_" + str(k)].Fill(p)
                    for s in matchedHits:
                        for v in matchedHits[s]:
                            rc = h["RPCMatchedHits"].Fill(
                                2 * s - 1 + v, len(matchedHits[s][v]), p
                            )
                            Nmatched += len(matchedHits[s][v])
                    if Nmatched < 2 and p > 30:
                        rc = h["RPC<2XY"].Fill(pos1[0], pos1[1])
                    rc = h["RPC_p"].Fill(p)
                    for k in range(2, 20):
                        if Nmatched < k:
                            rc = h["RPC<" + str(k) + "_p"].Fill(p)
    if "RPCResiduals" not in h:
        ut.bookCanvas(
            h, key="RPCResiduals", title="RPCResiduals", nx=1600, ny=1200, cx=2, cy=5
        )
        ut.bookCanvas(
            h,
            key="RPCResidualsXY",
            title="RPCResiduals function of Y/X",
            nx=1600,
            ny=1200,
            cx=2,
            cy=5,
        ...
        ut.bookCanvas(
            h,
            key="RPCResidualsP",
            title="RPCResiduals function of muon momentum",
            nx=900,
            ny=900,
            cx=1,
            cy=1,
    ...
    for s in range(1, 6):
        for v in range(
            0, 2
        ):  # 1 = x layer vertical strip, 0 = y layer horizontal strips
            if v == 1:
            ...
            if h[hname].GetEntries() == 0:
                continue
            h[hnameProjX] = h[hname].ProjectionX()
            myGauss.SetParameter(0, h[hnameProjX].GetMaximum())
            myGauss.SetParameter(1, 0.0)
            myGauss.SetParameter(2, 10.0)
            myGauss.SetParameter(3, 1.0)
            rc = h["RPCResiduals"].cd(jk)
            if v == 0:
                fitResult = h[hnameProjX].Fit(myGauss, "SQ", "", -40.0, 40.0)
            else:
                fitResult = h[hnameProjX].Fit(myGauss, "SQ", "", -10.0, 10.0)
            rc = fitResult.Get()
            if not rc:
                continue
            mean = rc.GetParams()[1]
            ...
            for k in range(1, h[hname].GetNbinsY() + 1):
                ...
                myGauss.SetParameter(1, 0.0)
                myGauss.SetParameter(2, 4.0)
                myGauss.SetParameter(3, 1.0)
                if v == 0:
                    fitResult = h[sli].Fit(myGauss, "SQ", "", -40.0, 40.0)
                else:
                    fitResult = h[sli].Fit(myGauss, "SQ", "", -10.0, 10.0)
                rc = fitResult.Get()
                if not rc:
                    continue
                params = rc.GetParams()
    ...
    if "RPCResiduals2dXY" not in h:
        ut.bookCanvas(
            h,
            key="RPCResiduals2dXY",
            title="muon tagger Residuals function of X/Y",
            nx=1600,
            ny=1200,
            cx=2,
            cy=5,
    ...
    for ip in range(1, zax.GetNbins() + 1):
        ...
        for ks in range(1, xax.GetNbins() + 1):
            Ntracks1 = h["RPCMatchedHits"].GetBinContent(ks, 1, ip) + h[
                "RPCMatchedHits"
            ].GetBinContent(ks, 2, ip)
            for n in range(1, yax.GetNbins() + 1):
                Nmean += h["RPCMatchedHits"].GetBinContent(
                    ks, n, ip
                ) * xax.GetBinCenter(n)
    ...
    ut.bookCanvas(
        h, key="RPCEff", title="RPC efficiencies", nx=1600, ny=1200, cx=3, cy=4
    )
    ...
    for s in range(1, 5):
        for v in range(3):
            if v == 2:
                ...
                h["Eff" + hname] = ROOT.TEfficiency(
                    h[hname], h["RPCextTrack_" + str(s) + str(0)]
                )
            else:
                hname = "RPCfired_" + str(s) + str(v)
                h["Eff" + hname] = ROOT.TEfficiency(
                    h[hname], h["RPCextTrack_" + str(s) + str(v)]
                )
            pad = h["RPCEff"].cd(l)
            l += 1
            ...
            h["txtEff" + hname] = ROOT.TMathText(
                25,
                0.92,
                "eff = (%5.2F\pm%5.2F)%s" % (eff * 100, err * 100, "\\hbox{%}"),
            )
...
def debugRPCstrips():
    ut.bookHist(h, "RPCstrips", "RPC strips", 1000, -100.0, 100.0, 1000, -100.0, 100.0)
    h["RPCstrips"].Draw()
    s = 1
    for v in range(2):
        for c in range(1, 185):
            ...
            if c % 5 == 0:
                ...
                if v == 0:
                    h["RPCstrip" + str(v) + str(c)].SetLineColor(ROOT.kRed)
                if v == 1:
                    h["RPCstrip" + str(v) + str(c)].SetLineColor(ROOT.kBlue)
                h["RPCstrip" + str(v) + str(c)].Draw("same")


def debugRPCYCoordinate():
    ut.bookHist(h, "y1y2", "y1 vs y2 of RPC", 100, -100.0, 100.0, 100, -100.0, 100.0)
...
def strawPosition():
    for detID in alignConstants["strawPositions"]:
        ...
        strawPositionsBotTop[detID] = [
            ROOT.TVector3(b[0], b[1], b[2]),
            ROOT.TVector3(t[0], t[1], t[2]),
        ]
...
def correctAlignment(hit):
    ...
    if withDefaultAlignment and not withCorrections:
        return vbot, vtop
    s, v, p, l, view, channelID, tdcId, nRT = stationInfo(hit)
...
def makeFit():
    ...
    for Nr in range(50000):
        ...
        if len(track_hits) != 1:
            continue
        listOfTracks[Nr] = []
        for nhit in track_hits[0]:
            listOfTracks[Nr].append(nhit)
    npar = 24
    ierflg = ROOT.Long(0)
    ...
    for s in ["T1_x", "T1_u", "T2_v", "T2_x", "T3_x", "T4_x"]:
        for l in range(4):
            gMinuit.mnparm(p, s + str(l), vstart[p], 0.5, 0.0, 0.0, ierflg)
            p += 1
    for p in range(4, 24):
...
def debugGeometrie():
    ...
    if statnb < 3:
        wire = "gas_12_" + str(test.GetDetectorID())
...
def residualLoop(nstart=0, nend=50000):
    ...
    if "unbiasedResiduals" not in h:
        ut.bookCanvas(
            h,
            key="unbiasedResiduals",
            title="unbiasedResiduals",
            nx=1600,
            ny=1200,
            cx=4,
            cy=6,
        ...
        ut.bookCanvas(
            h,
            key="unbiasedResidualsX",
            title="unbiasedResiduals function of X",
            nx=1600,
            ny=1200,
            cx=4,
            cy=6,
        ...
        ut.bookCanvas(
            h,
            key="unbiasedResidualsY",
            title="unbiasedResiduals function of Y",
            nx=1600,
            ny=1200,
            cx=4,
            cy=6,
            ...
            for l in range(0, 4):
                ...
                for p in ["X", "Y"]:
                    ...
                    for k in range(1, h[hname].GetNbinsY() + 1):
                        ...
                        myGauss.SetParameter(2, 0.1)
                        myGauss.SetParameter(3, 1.0)
                        fitResult = h[sli].Fit(myGauss, "SQ", "", -5.0, 5.0)
                        rc = fitResult.Get()
                        mean, rms = 0, 0
                        if rc:
                            mean = rc.GetParams()[1]
                    ...
                    amin, amax, nmin, nmax = ut.findMaximumAndMinimum(h[hmean])
                    if amax < 3.0 and amin > -3.0:
                        h[hmean].SetMaximum(2.0)
                        h[hmean].SetMinimum(-2.0)
                    else:
                        h[hmean].SetLineColor(ROOT.kRed)
                        h[hmean].SetMaximum(10.0)
                        h[hmean].SetMinimum(-10.0)
                    h[hmean].Draw()
                j += 1
    ...
    for l in range(2):
        for s in range(1, 6):
            ...
            temp = dict(
                enumerate(
                    grouper(
                        hitsPerStation[10 * s + l], cuts["muTaggerCluster_grouping"]
                    ),
                    1,
                )
            )
            clustersPerStation[10 * s + l] = {}
            for cl in temp:
                if len(temp[cl]) > cuts["muTaggerCluster_max"]:
                    continue
                clusCentre = 0
                zCentre = 0
                for hit in temp[cl]:
                    ...
                    clusCentre += vbot[1 - l]
                    zCentre += (vbot[2] + vtop[2]) / 2.0
                clusCentre = clusCentre / float(len(temp[cl]))
                zCentre = zCentre / float(len(temp[cl]))
                clusCentre = (
                    int(clusCentre * 1000) + float(s) / 10.0
                ) / 1000.0  # encode station number
                ...
                for s in range(1, 6):
                    ...
                    for cl in clustersPerStation[10 * s + l]:
                        if clustersPerStation[10 * s + l][cl][1] == coord:
                            ...
                            break
            tmp = list(zpositions)
            test = dict(enumerate(grouper(tmp, 10.0), 1))
            if len(test) > 2:
...
def testForSameDetID(nEvent=-1, nTot=1000):
    ...
    ut.bookHist(h, "multHits_deltaT", "DT multiple hits delta T", 100, 0.0, 2000.0)
    ut.bookHist(h, "multHits_deltaTz", "DT multiple hits delta T", 400, -200.0, 200.0)
    eventRange = [0, sTree.GetEntries()]
    if not nEvent < 0:
        eventRange = [nEvent, nEvent + nTot]
    listOfTDCs = {}
            ...
            for view in viewsI[s]:
                for l in range(4):
                    ...
                    for Acl in clustersPerLayer:
                        rc = h["multHits_" + str(s) + viewC[view] + str(l)].Fill(
                            len(clustersPerLayer[Acl])
                        )
    ...
    for s in range(1, 5):
        for view in views[s]:
            for l in range(4):
                ...
                h[hname + "x"].Scale(1.0 / float(h[hname].GetEntries()))
                h[hname + "x"].Draw()
                j += 1
...
def studyDeltaRays():
    # MC
    ut.bookHist(h, "station1Occ", "station1 Occupancy", 50, 0.0, 50.0)
    ut.bookHist(h, "station1OccX", "station1 Occupancy deltaRay present", 50, 0.0, 50.0)
    ut.bookHist(h, "deltaRay", "E vs z", 100, -1.0, 1.0, 100, 0.0, 10.0)
    ut.bookHist(h, "deltaRayN", "hits vs z", 100, -1.0, 1.0, 50, 0.0, 50.0)
    ut.bookHist(h, "deltaRayNvsE", "hits vs E", 100, 0.0, 10.0, 50, 0.0, 50.0)
    for n in range(sTree.GetEntries()):
        rc = sTree.GetEvent(n)
        if not (sTree.RPCTrackX.GetEntries() == 1) or not (
            sTree.RPCTrackY.GetEntries() == 1
        ):
            continue
        N = 0
        spectrHitsSorted = ROOT.nestedList()
        ...
        if MCdata:
            ...
            for m in sTree.MCTrack:
                ...
                if not pName.find("Delta ray") < 0:
                    if m.GetStartZ() / 100.0 < 0.05 and m.GetStartZ() / 100.0 > -0.3:
                        rc = h["deltaRayNvsE"].Fill(m.GetP(), N)
                        if m.GetP() > 0.01:
                            rc = h["station1OccX"].Fill(N)
                            found = True
                    rc = h["deltaRay"].Fill(m.GetStartZ() / 100.0, m.GetP())
                    rc = h["deltaRayN"].Fill(m.GetStartZ() / 100.0, N)
            # if not found and N>8: sTree.MCTrack.Dump()


def studyScintillator():
    ut.bookHist(h, "sc", "sc", 1000, -500.0, 2000.0)
    ut.bookHist(h, "sc6", "sc", 1000, -500.0, 2000.0)
    ut.bookHist(h, "sc7", "sc", 1000, -500.0, 2000.0)
...
def findV0(nstart=0, nmax=-1, PR=1):
    if nmax < 0:
        nmax = sTree.GetEntries()
    ut.bookHist(h, "doca", "distance between two tracks", 100, 0.0, 50.0)
    ut.bookHist(h, "nRPC", "matchedRPCHits", 50, 0.0, 50.0)
    ut.bookHist(
        h,
        "v0mass_wc",
        "V0 mass wrong charge combinations",
        100,
        0.2,
        1.8,
        100,
    ...
    ut.bookHist(h, "v0mass", "V0 mass ", 100, 0.2, 1.8, 100, -120.0, 120.0)
    ...
    for n in range(nstart, nmax):
        ...
        for tr1 in range(tr - 1):
            for tr2 in range(tr1 + 1, tr):
                ...
                if doca > 5:
                    continue
                if PosDir[tr1][3] * PosDir[tr2][3] < 0:
                    rc = h["v0mass"].Fill(V0Mom.M(), zv)
                else:
                    rc = h["v0mass_wc"].Fill(V0Mom.M(), zv)
    ...
    if sTree.GetListOfFiles().GetEntries() > 1:
        ...
        if curFile != rname:
            ...
            if withTDC:
                ...
                for s in h["tMinAndTmax"]:
                    h["rt" + s] = RTrelations[rname]["rt" + s]
            if withDefaultAlignment:
...
def importAlignmentConstants():
    ...
    if not sTree.GetCurrentFile().Get("alignConstants") or withCorrections:
        for straw in xpos:
            hit = ROOT.MufluxSpectrometerHit(straw, 0.0)
    ...
    try:
        ...
        print(
            "importing alignment constants from file", sTree.GetCurrentFile().GetName()
        )
        strawPosition()
    except:
        print(
            "loading of alignment constants failed for file",
            sTree.GetCurrentFile().GetName(),
        )
...
def importRTrel():
    for fname in fnames:
        ...
        try:
            ...
            for x in RTrelations[rname]["tMinAndTmax"]:
                RTrelations[rname]["rt" + x] = f.RT.Get("rt" + x)
            h["tMinAndTmax"] = RTrelations[rname]["tMinAndTmax"]
            for s in h["tMinAndTmax"]:
                h["rt" + s] = RTrelations[rname]["rt" + s]
        except:
...
def analyzeRTrel():
    ...
    for x in RTrelations[fnames[0]]["tMinAndTmax"]:
        ut.bookHist(h, x + "Tmin", x + "Tmin", 100, -130.0, -30.0)
        ut.bookHist(h, x + "Tmax", x + "Tmax", 600, 1100.0, 1700.0)
...
def monitorMasterTrigger():
    ut.bookHist(h, "masterTrigger", "t of master trigger", 1000, 200.0, 400.0)
    ut.bookHist(h, "delay", "delay time", 1000, -2000.0, 2000.0)
    ut.bookHist(h, "tdcCor", "corrected TDC time", 1000, -2000.0, 3000.0)
    ut.bookHist(h, "tdc", "TDC time from hit", 1000, -2000.0, 3000.0)
    ut.bookHist(h, "tdc#4", "TDC 4", 1000, -2000.0, 10000.0)
    for n in range(sTree.GetEntries()):
        rc = sTree.GetEvent(n)
        for mt in sTree.Digi_MasterTrigger:
            rc = h["masterTrigger"].Fill(mt.GetDigi())
        if sTree.Digi_MasterTrigger.GetEntries() == 0:
            rc = h["masterTrigger"].Fill(201.0)
        else:
            tdcDict = {}
            ...
            for hit in sTree.Digi_MufluxSpectrometerHits:
                ...
                if not hit.hasDelay():
                    ...
                    if tdcID not in tdcDict:
                        ...
                        if hit.hasTrigger():
                            print(
                                "this should not happen, no trigger but hasTrigger", n
                            )
                        continue
                    if not hit.hasTrigger():
                        print("this should not happen, trigger but hasTrigger false", n)
                    lt = tdcDict[tdcID]
                    tdcCor = (
                        hit.GetDigi() - delay - lt - 1323.0
                    )  # default value used to make the correction during conversion
                    rc = h["tdcCor"].Fill(tdcCor)


def disableBranches():
    # if sTree.GetBranchStatus("FitTracks"): sTree.SetBranchStatus("FitTracks",0)
    if sTree.GetBranchStatus("Digi_BeamCounters"):
        sTree.SetBranchStatus("Digi_BeamCounters", 0)
    if sTree.GetBranchStatus("Digi_LateMufluxSpectrometerHits"):
        sTree.SetBranchStatus("Digi_LateMufluxSpectrometerHits", 0)
    if sTree.GetBranchStatus("Digi_MufluxSpectrometerHits"):
        sTree.SetBranchStatus("Digi_MufluxSpectrometerHits", 0)
    if sTree.GetBranchStatus("Digi_Scintillators"):
        sTree.SetBranchStatus("Digi_Scintillators", 0)
    if sTree.GetBranchStatus("Digi_Triggers"):
        sTree.SetBranchStatus("Digi_Triggers", 0)
    if sTree.GetBranchStatus("Digi_MasterTrigger"):
        sTree.SetBranchStatus("Digi_MasterTrigger", 0)
    if sTree.GetBranchStatus("Digi_MuonTaggerHits"):
        sTree.SetBranchStatus("Digi_MuonTaggerHits", 0)
...
def checkForDiMuon():
    ...
    for t in sTree.MCTrack:
        ...
        if not (
            pName.find("Lepton pair") < 0
            and pName.find("Positron annihilation") < 0
            and pName.find("Hadronic inelastic") < 0
        ):
            boost = True
            break
    return boost
...
def muonOrigin():
    ...
    for p in sorted_o:
        print("%30s %5.2F %%" % (p[0], p[1] / float(doubleProc[0]) * 100.0))
    for p in ["Primary particle emission", "Decay"]:
        ...
        for x in sorted_p:
            ...
            if not part:
                particleName = str(x[0])
            else:
                particleName = PDG.GetParticle(x[0]).GetName()
            print(
                "   %20s %5.2F %%" % (particleName, x[1] / float(doubleProc[0]) * 100.0)
            )
...
def splitOffBoostedEvents():
    ...
    for n in range(sTree.GetEntries()):
        rc = sTree.GetEntry(n)
        if checkForDiMuon() and rnr.Uniform(0.0, 1.0) < 0.99:
            rc = newTree1.Fill()  # dimuon99
        else:
            rc = newTree2.Fill()  # dimuon1
    sTree.Clear()
    newTree1.AutoSave()
    newf1.Close()
    newTree2.AutoSave()
...
def plotDTPoints(onlyPlotting=False):
    ...
    if not onlyPlotting:
        for view in views:
            ...
            ut.bookHist(
                h, "Fitpoints" + view, "points" + view, 100, 0, 400, 10, -0.5, 9.5
            )
        for n in range(sTree.GetEntries()):
            rc = sTree.GetEvent(n)
            hitsPerTrack = {}
            ...
            for p in sTree.MufluxSpectrometerPoint:
                ...
                if t not in hitsPerTrack:
                    ...
                    if P.Mag() < 10:
                        continue
                    if abs(p.GetX()) > 10 or abs(p.GetY()) > 10:
                        continue
                    hitsPerTrack[t] = {
                        "_u1": 0,
                        "_v2": 0,
                    ...
                    mom[t] = P.Mag()
                test = ROOT.MufluxSpectrometerHit(p.GetDetectorID(), 0.0)
                statnb, vnb, pnb, lnb, view, channelID, tdcId, nRT = stationInfo(test)
                key = view + str(statnb)
    ...
    for t in cases:
        ...
        for view in views:
            ...
            h[cases[t] + view + "proj"] = (
                h[cases[t] + view].ProjectionY().Clone(cases[t] + view + "proj")
            )
            ...
            for k in range(1, h[mean].GetNbinsX()):
                ...
                ratio = float(test.GetBinContent(2)) / (
                    test.GetBinContent(3)
                    + test.GetBinContent(4)
                    + test.GetBinContent(5)
                    + 1e-6
                )
                h[oneHit234].SetBinContent(k, ratio)
                h[oneHit234].SetBinError(k, 0)
                ratio = float(test.GetBinContent(3)) / (
                    test.GetBinContent(4) + test.GetBinContent(5) + 1e-6
                )
                h[twoHit34].SetBinContent(k, ratio)
                h[twoHit34].SetBinError(k, 0)
            tc = h[t].cd(n)
            h[mean].SetMinimum(2.0)
            h[mean].SetMaximum(4.0)
...
def plotFitPoints():
    ...
    interestingHistos = []
    for view in views:
        interestingHistos.append(t + view)
    ...
    for data in ["data", "MC"]:
        ...
        for view in views:
            ...
            h[data + t + view + "proj"] = (
                hx[t + view].ProjectionY().Clone(data + t + view + "proj")
            )
            h[oneHit234].SetMaximum(-1111)
            h[oneHit234].SetMinimum(-1111)
            h[twoHit34].SetMaximum(-1111)
...
def plotEnergyLoss():
    ...
    PinPout.Draw("box")
    lx = ROOT.TLine(0.0, 1.0, 10.0, 1.0)
    lx.DrawClone()
    ly = ROOT.TLine(5.0, 0.0, 5.0, 10.0)
    ly.DrawClone()
...
def checkMCEffTuning():
    ...
    ut.readHists(
        hMC0, "momDistributions-1GeV-mbias-effTuned-M0.root", interestingHistos
    )
    ut.readHists(
        hMC2, "momDistributions-1GeV-mbias-effTuned-M2.root", interestingHistos
    )
    ut.readHists(
        hMCrec0, "momDistributions-1GeV-mbias-effTuned-M0-reco.root", interestingHistos
    )
    ut.readHists(
        hCharm0, "momDistributions-1GeV-charm-effTuned-M0.root", interestingHistos
    )
    ut.readHists(
        hCharm2, "momDistributions-1GeV-charm-effTuned-M2.root", interestingHistos
    )
    ut.readHists(
        hCharmrec0,
        "momDistributions-1GeV-charm-effTuned-M0-reco.root",
        interestingHistos,
    )
    ut.readHists(
        hMC10GeV0, "momDistributions-10GeV-mbias-effTuned-M0.root", interestingHistos
    )
    ut.readHists(
        hMC10GeV2, "momDistributions-10GeV-mbias-effTuned-M2.root", interestingHistos
    )
    ut.readHists(
        hMC10GeVrec0,
        "momDistributions-10GeV-mbias-effTuned-M0-reco.root",
        interestingHistos,
    )
    a = "p/pt_projx"
    ...
    print(
        "1GeV mbias:",
        hMC0[a].GetEntries() / hMC[a].GetEntries(),
        hMCrec0[a].GetEntries() / hMC[a].GetEntries(),
    )
    print(
        "1GeV charm:",
        hCharm0[a].GetEntries() / hCharm[a].GetEntries(),
        hCharmrec0[a].GetEntries() / hCharm[a].GetEntries(),
    )
    print(
        "10GeV     :",
        hMC10GeV0[a].GetEntries() / hMC10GeV[a].GetEntries(),
        hMC10GeVrec0[a].GetEntries() / hMC10GeV[a].GetEntries(),
    )
    ...
    methods = {
        ...
        "M0": [hMC0, hCharm0, hMC10GeV0],
        "M2": [hMC2, hCharm2, hMC10GeV2],
    ...
    for m in methods:
        ...
        h["MC10" + m + a].Add(
            methods[m][2][a.replace("_", "charm_")], -1.0 + charmNorm[10]
        )
        h["MC10" + m + a].Add(
            methods[m][2][a.replace("_", "beauty_")], -1.0 + beautyNorm[10]
        )
    ...
    if t not in h:
        ut.bookCanvas(
            h, key=t, title="MC-Comparison eff tuning", nx=900, ny=600, cx=1, cy=1
        )
    tc = h[t].cd(1)
    ...
    for m in methods:
        ...
        h[ihist].SetTitle(
            "efficiency correction as function of momentum "
            + m
            + ";p [GeV/c];efficiency correction"
        )
        if n == 0:
            h[ihist].Draw()
        else:
...
def MCcomparison(pot=-1, pMin=5.0, effCor=True, eric=False):
    # 1GeV mbias,      1.8 Billion PoT
    # 1GeV charm,     10.2 Billion PoT,  10 files
    # 10GeV MC,         65 Billion PoT
    ...
    if len(hMC) == 0:
        ...
        if not effCor:
            ...
            ut.readHists(hCharm, "momDistributions-charm.root", interestingHistos)
            if eric:
                ut.readHists(hMC10GeV, "momDistributions-10GeV.root", interestingHistos)
            else:
                ut.readHists(
                    hMC10GeV, "momDistributions-10GeVTR.root", interestingHistos
                )
        else:
            ut.readHists(
                hMC,
                "momDistributions-1GeV-mbias-effTuned-M0-reco.root",
                interestingHistos,
            )
            ut.readHists(
                hMC, "momDistributions-1GeV-mbias-effTuned-M2.root", interestingHistos
            )
            ut.readHists(
                hCharm,
                "momDistributions-1GeV-charm-effTuned-M0-reco.root",
            ...
            ut.readHists(
                hCharm,
                "momDistributions-1GeV-charm-effTuned-M2.root",
                interestingHistos,
            )
            ut.readHists(
                hMC10GeV,
            ...
            ut.readHists(
                hMC10GeV,
                "momDistributions-10GeV-mbias-effTuned-M2.root",
                interestingHistos,
            )
            MCStats = MCStats * 2.0
        for a in ["p/pt", "p/Abspx", "p1/p2", "p1/p2s"]:
            for x in ["", "mu"]:
                if a.find("p1/p2") == 0 and x == "mu":
                    continue
                for hstore in [hMC, hCharm, hMC10GeV]:
                    # for new category "G4default", subtract dimuons made by SHiP modification to G4
                    hstore[a + x + "G4default"] = hstore[a + x].Clone(
                        a + x + "G4default"
                    )
                    hstore[a + x + "G4default"].Add(
                        hstore[a + x + "Hadronic inelastic"], -1.0
                    )
            ...
            if eric:  # data with boosted channels
                for source in sources:
                    if source == "":
                        continue
                    xxx = a + x + source
                    h["MC10" + a + x].Add(hMC10GeV[xxx], -1.0 + 1.0 / sources[source])
                    h["MC10" + xxx] = hMC10GeV[xxx].Clone("MC10" + xxx)
                    h["MC10" + xxx].Scale(1.0 / sources[source])
                    h["MC10" + xxx].Scale(1.8 / 65.0)  # scale it to 1GeV MC statistics
                h["MC10" + a + x].Scale(1.8 / 65.0)
            else:
                h["MC10" + a + x].Add(hMC10GeV[a + x + "charm"], -1.0 + charmNorm[10])
                h["MC10" + a + x].Add(hMC10GeV[a + x + "beauty"], -1.0 + beautyNorm[10])
                for source in sources:
                    if source == "":
                        continue
                    xxx = a + x + source
                    h["MC10" + xxx] = hMC10GeV[xxx].Clone("MC10" + xxx)
                    if source == "charm" or source == "beauty":
                        h["MC10" + xxx].Scale(sim10fact / sources[source] * norm)
                    else:
                        h["MC10" + xxx].Scale(sim10fact * norm)
    ...
    ]  # decay removed, only covers part
    opt = {
        "": ["", ROOT.kBlue, "data"],
        "MC": ["same", ROOT.kRed, "MC inclusive"],
        "MC10": ["same", ROOT.kRed, "MC 10GeV total"],
        "MCcharm": ["same", ROOT.kGreen, "Charm"],
        "MC10charm": ["same", ROOT.kGreen, "Charm"],
        ...
        "MCLepton pair": ["same", ROOT.kGreen + 1, "Lepton pair"],
        "MCPositron annihilation": ["same", ROOT.kRed + 2, "Positron annihilation"],
        "MC10Lepton pair": ["same", ROOT.kGreen + 1, "Lepton pair"],
        "MC10Positron annihilation": ["same", ROOT.kRed + 2, "Positron annihilation"],
    ...
    for d in ["", "I-"]:
        for x in ["", "mu"]:
            ...
            if t not in h:
                ut.bookCanvas(
                    h, key=t, title=d + " MC / Data " + x, nx=1200, ny=600, cx=3, cy=2
                )
            if d == "":
                ...
                for i1 in opt:
                    ...
                    if i == "MC":
                        h[i + "p/pt" + z + "_y"] = h[i + "p/pt" + z].ProjectionY(
                            i + "p/pt" + z + "_y",
                            h[i + "p/pt" + x + "_x"].FindBin(pMin),
                            h[i + "p/pt" + x + "_x"].FindBin(20.0),
                        )
                        tmp = h[i + "10p/pt" + z].ProjectionY(
                            "tmp",
                            h[i + "p/pt" + x + "_x"].FindBin(20) + 1,
                            h[i + "p/pt" + x + "_x"].GetNbinsX(),
                        )
                        # attempt to make pt/px distribution with 1 GeV and 10 GeV MC
                        h[i + "p/pt" + z + "_y"].Add(tmp)
                        h[i + "p/Abspx" + z + "_y"] = h[i + "p/Abspx" + z].ProjectionY(
                            i + "p/Abspx" + z + "_y",
                            h[i + "p/pt" + x + "_x"].FindBin(pMin),
                            h[i + "p/pt" + x + "_x"].FindBin(20.0),
                        )
                        tmp = h[i + "10p/Abspx" + z].ProjectionY(
                            "tmp",
                            h[i + "p/pt" + x + "_x"].FindBin(20) + 1,
                            h[i + "p/pt" + x + "_x"].GetNbinsX(),
                        )
                        ...
                        for pInterval in [
                            ...
                            interval = (
                                "_y" + str(pInterval[0]) + "-" + str(pInterval[1])
                            )
                            if pInterval[0] < 20:
                                h[i + "p/pt" + z + interval] = h[
                                    i + "p/pt" + z
                                ].ProjectionY(
                                    i + "p/pt" + z + interval,
                                    h[i + "p/pt" + x + "_x"].FindBin(pInterval[0]),
                                    h[i + "p/pt" + x + "_x"].FindBin(20.0),
                                )
                                tmp = h[i + "10p/pt" + z].ProjectionY(
                                    "tmp",
                                    h[i + "p/pt" + x + "_x"].FindBin(20.0) + 1,
                                    h[i + "p/pt" + x + "_x"].FindBin(pInterval[1]),
                                )
                                # attempt to make pt/px distribution with 1 GeV and 10 GeV MC
                                h[i + "p/pt" + z + interval].Add(tmp)
                                h[i + "p/Abspx" + z + interval] = h[
                                    i + "p/Abspx" + z
                                ].ProjectionY(
                                    i + "p/Abspx" + z + interval,
                                    h[i + "p/pt" + x + "_x"].FindBin(pInterval[0]),
                                    h[i + "p/pt" + x + "_x"].FindBin(20.0),
                                )
                                tmp = h[i + "10p/Abspx" + z].ProjectionY(
                            ...
                            h[i + "p/pt" + z + interval].SetTitle(
                                "pt for "
                                + str(pInterval[0])
                                + " < P < "
                                + str(pInterval[1])
                            )
                            h[i + "p/Abspx" + z + interval].SetTitle(
                                "|px| for "
                                + str(pInterval[0])
                                + " < P < "
                                + str(pInterval[1])
            ...
            for i1 in optSorted:
                ...
                h[d + i + "p/pt" + xx + "_x"].SetMaximum(hMax * 10.0)
                h[d + i + "p/pt" + xx + "_x"].SetMinimum(1.0)
                ...
                if i == "MC":
                    h[d + i + "p/pt" + xx + "_x"].GetXaxis().SetRangeUser(0, 50)
                if i == "MC10":
                    h[d + i + "p/pt" + xx + "_x"].GetXaxis().SetRangeUser(20, 500)
                if i.find("MC10") < 0:
                    h["leg" + t + str(tc)].AddEntry(
                        h[d + i + "p/pt" + xx + "_x"], opt[i1][2], "PL"
                    )
            ...
            for i1 in optSorted:
                ...
                if not i.find("MC10") < 0:
                    ...
                    source = i1.split("MC")[1]
                if i == "MC10":
                    continue  # Pt/Px distributions from 1GeV and 10GeV are merged into one
                xx = x + source
                h[d + i + "p/pt" + xx + "_y"].SetTitle(
                    "transverse momentum Pt, P>" + str(pMin)
                )
                h[d + i + "p/pt" + xx + "_y"].SetMaximum(hMay * 10.0)
                h[d + i + "p/pt" + xx + "_y"].SetMinimum(1.0)
            ...
            for i1 in optSorted:
                ...
                h[d + i + "p/Abspx" + xx + "_y"].SetMaximum(hMaPx * 10.0)
                h[d + i + "p/Abspx" + xx + "_y"].SetMinimum(1.0)
                ...
                if i.find("MC10") < 0:
                    h["leg" + t + str(tc)].AddEntry(
                        h[d + i + "p/Abspx" + xx + "_y"], opt[i1][2], "PL"
                    )
            h["leg" + t + str(tc)].Draw("same")
            tc = 4
            rc = h[t].cd(tc)
            ...
            for i1 in optSorted:
                ...
                if i == "MC10":
                    h["lin" + d + i + "p/pt" + xx + "_x"].GetXaxis().SetRangeUser(
                        20, 500
                    )
            h["leg" + t + str(tc)].Draw("same")
            tc = 5
            ...
            for i1 in optSorted:
                ...
                if i == "MC10":
                    continue
                xx = x + source
                h["lin" + d + i + "p/Abspx" + xx + "_y"] = h[
                    d + i + "p/Abspx" + xx + "_y"
                ].Clone("lin" + d + i + "p/Abspx" + xx + "_y")
                h["lin" + d + i + "p/Abspx" + xx + "_y"].GetXaxis().SetRange(1, 25)
                h["lin" + d + i + "p/Abspx" + xx + "_y"].SetMaximum(hMaPx * 1.1)
                h["lin" + d + i + "p/Abspx" + xx + "_y"].SetMinimum(0.0)
    ...
    for x in ["", "mu"]:
        if x != "":
            ...
            print("=== all tracks  ====")
        for P in [5.0, 10.0, 50.0, 100.0]:
            nbin = h["p/pt" + x + "_x"].FindBin(P)
            print(
                "data/MC P>%5i GeV: %5.2F"
                % (
                    int(P),
                    h["I-p/pt" + x + "_x"].GetBinContent(nbin)
                    / h["I-MCp/pt" + x + "_x"].GetBinContent(nbin),
                )
            )
    # pt in slices of P
    x = "mu"
    for t in ["MC-Comparison Pt", "MC-Comparison Px"]:
        ...
        y = 1
        for pInterval in [[pMin, 50.0], [51.0, 100.0], [101.0, 200.0], [201.0, 300.0]]:
            ...
            for i1 in optSorted:
                ...
                hMaPx = max(mx1, mx2)
                h[hname].GetXaxis().SetRangeUser(0.0, 5.0)
                ...
                if i.find("MC10") < 0:
                    h["leg" + t + str(tc)].AddEntry(h[hname], opt[i1][2], "PL")
            h["leg" + t + str(tc)].Draw("same")
        h[t].Update()
    ...
    for s in osign:
        ...
        if t not in h:
            ut.bookCanvas(
                h,
                key=t,
                title=" momentum of muons in 2-track events " + osign[s],
                nx=800,
                ny=1200,
                cx=1,
                cy=2,
        ...
        for case in txt:
            ...
            h[hn].Scale(1.0 / norm)
            h[hn].SetTitle(
                "P in 2-track events " + txt[case] + "; GeV/c;N2/Nall with P > / "
            )
            h[hn].SetStats(0)
            h[hn].SetMaximum(0.1)
            ...
            if rebin > 1:
                h[hn].Rebin(rebin)
            if case < 20:
                norm = h["MCp/ptmu_x"].Integral(case, 500)
            else:
                norm = h["MC10p/ptmu_x"].Integral(case, 500)
            h[hn].Scale(1.0 / norm)
            h[hn].SetStats(0)
            h[hn].SetTitle(
                "P in 2-track events, one track with " + txt[case] + "; GeV/c"
            )
            ...
            for i1 in optSorted:
                if i1.find("MC") < 0:
                    continue
                if i1.find("MC10") < 0 and case == 21:
                    continue
                if not i1.find("MC10") < 0 and case == 6:
                    continue
                ...
                h[hn].Add(
                    h[i + "p1/p2" + s + source].ProjectionY(
                        i + "p1/p2y" + source + txt[case] + s, case, 500
                    )
                )
                if rebin > 1:
                    h[hn].Rebin(5)
...
def printSources():
    ...
    for xx in sources:
        ...
        ratio10 = (
            h[hname10].GetBinContent(21) / h[d + i + "10p/pt_x"].GetBinContent(21) * 100
        )
...
def compareRuns(runs=[]):
    ...
    if "RunComparison" not in h:
        ut.bookCanvas(
            h, key="RunComparison", title="Momentum", nx=1600, ny=1200, cx=1, cy=0
        )
        ut.bookCanvas(
            h,
            key="EventStatistics",
        ...
        ut.bookHist(
            h, "HEventStatistics", "Event Statistics;run number", 100, 2000, 3000
        )
    ...
    for r in runs:
        ...
        print(
            "events with tracks %5.2F%%"
            % (
                hruns[r]["Trscalers"].GetBinContent(2)
                / hruns[r]["Trscalers"].GetBinContent(1)
                * 100
            )
        )
        print(
            "tracks/event                     %5.2F%%"
            % (
                hruns[r]["Trscalers"].GetBinContent(3)
                / hruns[r]["Trscalers"].GetBinContent(1)
                * 100
            )
        )
        print(
            "ratio of muon tagged tracks / all tracks %5.2F%%"
            % (hruns[r]["p/ptmu"].GetEntries() / (hruns[r]["p/pt"].GetEntries() + 1e-6))
        )
        print(
            "mean p %5.2F GeV/c, rms %5.2F GeV/c"
            % (hruns[r][hname].GetMean(), hruns[r][hname].GetRMS())
        )
        eventStats[r] = [
            hruns[r]["Trscalers"].GetBinContent(2)
            / hruns[r]["Trscalers"].GetBinContent(1),
        ...
        if first:
            ...
            hruns[r][hname].Draw("same PLC PMC")
    ...
    for r in eventStats:
        ...
        h["eventStats3"].SetPoint(n, r, eventStats[r][2] / 100.0)
        h["eventStats4"].SetPoint(n, r, eventStats[r][4] / 5.0)
        n += 1
...
def mergeGoodRuns(excludeRPC=False, path="."):
    ...
    for x in temp:
        ...
        if not os.path.isdir(path + "/" + x):
            continue
        r = int(x[x.rfind("/") + 1 :].split("_")[2])
        if r in badRuns or r in noTracks or r in intermediateField or r in noField:
            continue
        if excludeRPC and (r in RPCbad or (r > 2198 and r < 2275)):
            continue
...
def fcn(npar, gin, f, par, iflag):
    ...
    for proj in ["p/Abspx_y" + x, "p/pt" + x + "_x"]:
        for n in range(1, h[proj].GetNbinsX() + 1):
            if proj == "p/pt" + x + "_x" and h[proj].GetBinCenter(n) < 5:
                continue
            delta = h[proj].GetBinContent(n) - dataMC * (
                hMC[proj].GetBinContent(n) + charmMbias * hCharm[proj].GetBinContent(n)
            )
            errSq = (
                h[proj].GetBinContent(n)
                + dataMC**2 * hMC[proj].GetBinContent(n)
...
def doFit(p0=5.7, p1=0.17):
    ...
    a = "p/Abspx"
    for H in [h, hMC, hCharm]:
        H[a + "_y" + x] = H[a + x].ProjectionY(
            a + "_y" + x, h[a + "_x" + x].FindBin(pMin), h[a + "_x" + x].GetNbinsX()
        )
    npar = 2
    ...
    step = array("d", [2.0, 2.0])
    ierflg = ROOT.Long(0)
    name = [ROOT.TString("dataMC"), ROOT.TString("charmMbias")]
    for i in range(npar):
        gMinuit.mnparm(i, name[i], vstart[i], step[i], 0.0, 0.0, ierflg)
...
def additionalMomSmearing():
    ...
    fudge = 2.0
    sigma = [0.36 / 100.0 * fudge, 0.036 / 100.0 * fudge]
...
def recoStep1(PR=11):
    ...
    fRPCTrackArray = {
        "X": ROOT.TClonesArray("RPCTrack"),
        "Y": ROOT.TClonesArray("RPCTrack"),
    }
    RPCTrackbranch = {}
    ...
    for n in range(sTree.GetEntries()):
        ...
        for aTrack in theTracks:
            ...
            aTrack.prune(
                "CFL"
            )  # aTrack.prune("CURM")  # FL keep first and last point only, C deleteTrackRep, W deleteRawMeasurements, I U R M
...
def recoMuonTaggerTracks():
    ...
    if sTree.GetBranch("RPCTrackX"):
        ...
        os.system(
            "cp " + fname + " " + fname.replace(".root", "orig.root")
        )  # make backup
    ...
    if ftest.GetKey("cbmsim"):
        ...
        check += sTree.GetBranch("RPCTrackY").GetZipBytes()
        if check / float(sTree.GetBranch("FitTracks").GetZipBytes()) > 0.003:
            OK = True
    if not OK:
...
if options.command == "":
    ...
    print(
        " --- plotHitMaps(): hitmaps / layer, TDC / layer, together with list of noisy channels"
    )
    print(
        " --- plotEvent(n) : very basic event display, just x hits in x/z projection "
    )
    print(" --- plotRPCHitmap() : basic plots for RPC ")
    print(" --- momentum plot and track fitting tests:")
    print(
        " ---     fitTracks(100) and fitTracks(100,True,True) with simple Display and 3d display of tracks with detector, low occupancy events"
    )
    print(
        " --- testClusters(nstart,nevents), clustering of hits and pattern recognition followed by track fit"
    )
    print(
        " --- plotBiasedResiduals(nstart,nevents), fit tracks in low occupancy events and plot residuals, plot2dResiduals() for display del vs x, del vs y"
    )
    print(" --- plotLinearResiduals(), to be used for zero field")
    print(" --- plotRPCExtrap(nstart,nevents), extrapolate track to RPC hits")
    print(" --- printScalers()")
    print(
        " --- init(): outdated! do boostrapping, determine RT relation using fitted tracks, do plotBiasedResiduals and plotRPCExtrap with TDC"
    )
...
if options.command == "recoStep0":
...
elif options.command == "splitOffBoostedEvents":
    splitOffBoostedEvents()
elif options.command == "plotDTPoints":
    plotDTPoints()
    ...
    for x in sTree.GetListOfBranches():
        sTree.SetBranchStatus(x.GetName(), 0)
