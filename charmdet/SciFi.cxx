...
SciFi::SciFi()
   : FairDetector("HighPrecisionTrackers", kTRUE, kSciFi), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(),
     fTime(-1.), fLength(-1.), fELoss(-1), fSciFiPointCollection(new TClonesArray("SciFiPoint"))
{
}

SciFi::SciFi(const char *name, const Double_t DX, const Double_t DY, const Double_t DZ, Bool_t Active,
             const char *Title)
   : FairDetector(name, Active, kSciFi), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(), fTime(-1.),
     fLength(-1.), fELoss(-1), fSciFiPointCollection(new TClonesArray("SciFiPoint"))
{
   DimX = DX;
   DimY = DY;
   DimZ = DZ;
...
void SciFi::ConstructGeometry()
{
   ...
   for (int iscifi = 0; iscifi < nSciFi; iscifi++) {
      if (vertical[iscifi])
         volSciFiBox->AddNode(
            volSciFiy, SciFiIDlist[iscifi],
            new TGeoTranslation(xs[iscifi], ys[iscifi], -SBoxZ / 2. + zs[iscifi])); // compensation for the Node offset
      else
         volSciFiBox->AddNode(volSciFix, SciFiIDlist[iscifi],
                              new TGeoTranslation(xs[iscifi], ys[iscifi], -SBoxZ / 2. + zs[iscifi]));
   }
}
...
SciFiPoint *SciFi::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
