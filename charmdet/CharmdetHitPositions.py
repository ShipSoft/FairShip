...
parser.add_argument(
    ...
    "--scififile",
    dest="scififilename",
    help="root file with scifi data",
    required=True,
)
parser.add_argument(
    "-nev", "--nevent", dest="nevent", help="number of event to plot", default=0
)
parser.add_argument(
    "-w",
    "--write",
    dest="writentuple",
    help="option to write an ntuple",
    default=False,
    action="store_true",
)
parser.add_argument(
    ...
    "--outputfile",
    dest="outputfilename",
    help="outputfile with the ntuples",
    default="positions.root",
)
parser.add_argument(
...
parser.add_argument(
    ...
    "--Display",
    dest="withDisplay",
    help="detector display",
    default=False,
    action="store_true",
)
parser.add_argument(
    "-e", "--eos", dest="onEOS", help="files on EOS", default=False, action="store_true"
)
parser.add_argument(
    "-u",
    "--update",
...
ShipGeo = ConfigRegistry.loadpy(
    "$FAIRSHIP/geometry/charm-geometry_config.py", Setup=1, cTarget=1
)
    ...
    if fittedtracks:
        ...
        for itrk in range(maxtrackID):
            for istation in range(5):
                ...
                npoint = 5
                if x == 0:
                    subdetector = 40  # not paired cluster
                if y == 0:
                    subdetector = 40
                if writentuple:
                    hitnumber = leafnhits[0]
...
def DrawTrack(theta, phi, lastpoint):
    ...
    )  #'vertex' of track, here used as starting point
    fakemomentum = 400
    recotrack.fP.Set(
        fakemomentum * ROOT.TMath.Sin(theta) * ROOT.TMath.Cos(phi),
        fakemomentum * ROOT.TMath.Sin(theta) * ROOT.TMath.Sin(phi),
        -fakemomentum * ROOT.TMath.Cos(theta),
    )  # track propagated backwards
    recotrack.fSign = 1
...
if writentuple:
    ...
    leafposx = array("f", maxdim * [0.0])
    leafposy = array("f", maxdim * [0.0])
    leafposz = array("f", maxdim * [0.0])
    eventtime = array("f", [0.0])
