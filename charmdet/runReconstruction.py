...
ncpus = int(multiprocessing.cpu_count() * 3.0 / 4.0)
...
def recoStep0(local=False):
    ...
    while Ndone < Nfiles:
        ...
        if Ndone % 100 == 0:
            cleanUp()
    while count_python_processes("drifttubeMonitoring") > 0:
        time.sleep(200)
...
def recoStep1():
    ...
    for fname in fileList:
        cmd = (
            "python "
            + pathToMacro
            + "drifttubeMonitoring.py -c recoStep1 -u 1 -f "
            + fname
            + " &"
        )
        print("step 1:", cmd)
...
def checkAlignment(fileList=[]):
    ...
    for fname in fileList:
        cmd = (
            "python "
            + pathToMacro
            + "drifttubeMonitoring.py -c alignment -f "
            + fname
            + " &"
        )
...
def runMC():
    ...
    os.system(
        "python $FAIRSHIP/macro/run_simScript.py -n 100000 --MuonBack --charm=1 --CharmdetSetup=0 -f "
        + inputFile
    )
    # full simulation
    os.system(
        "python $SHIPBUILD/FairShip/macro/run_simScript.py --Muflux -n 1000 --charm=1 --CharmdetSetup=0 --charm=1 --CharmdetSetup=0"
    )
        ...
        for x in os.listdir(D):
            if x.find("_RT") > 0 and x.find("histos") < 0:
                ...
                if not test.GetKey("cbmsim"):
                    zombie.append(x)
                elif test.cbmsim.GetBranch("FitTracks"):
                    fileList.append(x)
                else:
                    rest.append(x)
    else:
        temp = subprocess.check_output(
            "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + D, shell=True
        )
        for x in temp.split("\n"):
            if x.find(".root") < 0:
                continue
...
def checkFilesWithTracks3(D="."):
    ...
    for x in os.listdir(D):
        if x.find("_RT") > 0 and x.find("histos") < 0:
            ...
            if b:
                if b.GetZipBytes() / 1.0e6 < 1.0:
                    badFile[x] = b.GetZipBytes() / 1.0e6
    return badFile
...
def copyMissingFiles(remote="../../ship-ubuntu-1710-64/RUN_8000_2395", exclude=[]):
    ...
    for fname in allFilesR:
        if not fname.find("histos") < 0:
            continue
        if fname.find("RT") < 0:
            continue
        if fname in exclude:
            continue
        if not fname in allFilesL:
            toCopy.append(fname)
    print("len", len(toCopy))
    for fname in toCopy:
        os.system("cp " + remote + "/" + fname + " .")


def importRTFiles(
    local=".", remote="/home/truf/ship-ubuntu-1710-32/home/truf/muflux/Jan08"
):
...
def importRecoFiles(
    local=".",
    remote="/media/truf/disk2/home/truf/ShipSoft/ship-ubuntu-1710-16/home/truf/muflux/Jan08",
):
    fileWithTracks = checkFilesWithTracks(remote)
    for x in fileWithTracks:
        os.system("cp " + remote + "/" + x + " .")
...
def checkRecoRun(eosLocation=eospath, local="."):
    temp = subprocess.check_output(
        "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eosLocation, shell=True
    )
    for x in temp.split("\n"):
        if x.find(".root") < 0:
            continue
...
def exportRunToEos(
    eosLocation="/eos/experiment/ship/user/truf/muflux-reco", run=run, local="."
):
    temp = os.system(
        "xrdfs " + os.environ["EOSSHIP"] + " mkdir " + eosLocation + "/" + run
    )
    failures = []
...
def makeMomDistributions(run=0):
    if run == 0:
        ...
        fileList = []
        temp = subprocess.check_output(
            "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eospathReco, shell=True
        )
        for x in temp.split("\n"):
            if x.find(".root") < 0:
                continue
    ...
    for fname in fileList:
        if not fname.find("sys") < 0:
            continue
        if os.path.isfile("histos-analysis-" + fname[fname.rfind("/") + 1 :]):
            continue
        cmd = (
            "python "
            + pathToMacro
            + "drifttubeMonitoring.py -c anaResiduals -f "
            + fname
            + " &"
        )
    ...
    if merge:
        ...
        for x in fnames:
            ...
            temp2 = subprocess.check_output(
                "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + eospathReco + run,
                shell=True,
            )
            if temp2.find(".root") < 0:
                continue
            skip = False
            for x in zeroField:
                if not run.find(x) < 0:
...
def redoMuonTracks():
    ...
    for fname in fileList:
        cmd = (
            "python "
            + pathToMacro
            + "drifttubeMonitoring.py -c  recoMuonTaggerTracks -u 1 -f "
            + fname
            + " &"
        )
    ...
    for fname in os.listdir("."):
        if not merge and fname.find("SPILL") == 0:
            cmd = (
                "python "
                + pathToMacro
                + "drifttubeMonitoring.py -c DTeffWithRPCTracks -f "
                + fname
                + " &"
            )
            os.system(cmd)
