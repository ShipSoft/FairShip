...
* Resolve the strawtube L/R drift-isochrone ambiguity per hit via a Legendre-transform pattern recognition step in `shipPatRec.py` (Aliev et al., NIM A 592, 456, 2008). Each Legendre stage finds the common tangent line to a set of drift circles in one projection plane: stage 1 in (z, y) for Y-view hits gives the y-trajectory; stage 2 in (z, x) for stereo hits (projected via the y-track) gives the x-trajectory. Per-hit L/R signs are derived from the sign of the perpendicular distance from the resulting straight track to each wire, and passed to GenFit via `WireMeasurement::setLeftRightResolution`. The DAF is no longer asked to resolve the L/R weighting iteratively, eliminating the bistable basin that previously produced a bimodal `Delta p_y` per HNL daughter (~14% RMS reduction in the non-bending plane on truth-matched tracks). Opt-out via `global_variables.use_legendre_lr = False`.
