...
def test_get_detector(cdb_api, detector_id):
    ...
    value_error_parameters = ["/detector_id_exist/", "detector///subdetector", "//detector/subdetector//"]
...
def test_add_detector(cdb_api, name, parent_id):
    ...
    if has_correct_parameter_type:
        if parent_id == "detector_id_not_exist":
            ...
            new_detector_id = cdb_api.get_detector(parent_id + '/' + name)
            tmp = parent_id + '/' + name
            cdb_api.remove_detector(tmp)
            assert new_detector_id['name'] == name, "Should be equal"
...
def test_list_detectors(cdb_api, parent_id):
    ...
    if type(parent_id) != str and parent_id is not None:
        ...
        for el in result:
            ...
            detector_path = parent_id + "/" + el
            assert cdb_api.get_detector() is not None, "Cannot find this detector: " + detector_path
...
@pytest.mark.parametrize("detector_id, name, values, test_type, tag, collected_at, valid_since, valid_until", [
    ...
    ("detector_id_not_exist", "strawPositions", {"dictPar1": "dictVal1"}, "calibration", "tagsp",
     "2020-03-12 11:05:27", "2020-03-12 11:05:27", "2020-04-10 11:05:27"),
])
def test_add_condition(cdb_api, detector_id, name, values, test_type, tag, collected_at, valid_since, valid_until):
    ...
    if type(collected_at) == str or type(valid_until) == str:
        # if the format is wrong, the function must raise ValueError
        if not has_correct_format(collected_at) or not has_correct_format(valid_until):
            has_correct_parameter_type = False
            with pytest.raises(ValueError):
                assert cdb_api.add_condition(detector_id, name, tag, collected_at, values, test_type, valid_since,
                                             valid_until)
    ...
    if (type(valid_since) == datetime and type(valid_until) == datetime) and (valid_since > valid_until):
        has_correct_parameter_type = False
        with pytest.raises(ValueError):
            assert cdb_api.add_condition(detector_id, name, tag, collected_at, values, test_type, valid_since,
                                         valid_until)
    ...
    if has_correct_parameter_type:
        ...
        assert detector["name"] == name, "the new inserted detector has different name"
        ...
        assert new_condition["collected_at"] == collected_at, "collected_at is not correct"

        # Check if the valid_until exists
        assert new_condition["valid_until"] == valid_until, "valid_until is not correct"
...
def test_get_conditions_by_name_and_validity(cdb_api, detector_id, name, date):
    ...
    if has_correct_parameter_type:
        ...
        if detector_id == "detector_id_exist" and name == "condition_exist" and \
                date == datetime.datetime(2020, 3, 12, 11, 5, 27, 555555):
            ...
            assert result[0]["collected_at"] == "2020,03,12,11,05,27,767563", "condition collected_at value is wrong"
            assert result[0]["valid_since"] == "2020,03,12,11,05,27,767563", "condition valid_since value is wrong"
            assert result[0]["valid_until"] == "2020,04,10,11,05,27,767563", "condition valid_until value is wrong"
...
def test_get_condition_by_name_and_tag(cdb_api, detector_id, name, tag):
    ...
    if has_correct_parameter_type:
        ...
        if detector_id == "detector_id_exist" and name == "condition_exist" and tag == "1":
            ...
            assert result["collected_at"] == "2020,03,12,11,05,27,767563", "condition collected_at value is wrong"
            assert result["valid_since"] == "2020,03,12,11,05,27,767563", "condition valid_since value is wrong"
            assert result["valid_until"] == "2020,04,12,11,05,27,767563", "condition valid_until value is wrong"
...
def test_update_condition_by_name_and_tag(cdb_api, detector_id, name, tag, test_type, valid_since, valid_until):
    ...
    if has_correct_parameter_type:
        ...
        assert result_con[valid_since] == datetime.datetime(2020, 4, 16, 12, 7, 30, 129910) and \
               result_con[valid_until] == datetime.datetime(2020, 4, 17, 12, 7, 30, 129910), \
            "date is not updated successfully"
...
def test_remove_detector(cdb_api, detector_id):
    ...
    if type(detector_id) != str:
        ...
        assert cdb_api.get_detector(detector_id) is None, "detector_id still exists: " + detector_id
