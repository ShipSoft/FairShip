...
class ShipDigiReco:
    ...
    def __init__(self, fout, fgeo):
        ...
        if self.sTree.GetBranch("FitTracks"):
            ...
            if sTree.GetBranch("Pid"):
                sTree.SetBranchStatus("Pid", 0)
            ...
            if sTree.GetBranch("Digi_TimeDetHits"):
                sTree.SetBranchStatus("Digi_TimeDetHits", 0)
            if sTree.GetBranch("Digi_UpstreamTaggerHits"):
                sTree.SetBranchStatus("Digi_UpstreamTaggerHits", 0)
            if sTree.GetBranch("Digi_MuonHits"):
                sTree.SetBranchStatus("Digi_MuonHits", 0)
        ...
        self.goodTracksBranch = self.sTree.Branch(
            "goodTracks", self.goodTracksVect, 32000, -1
        )
        ...
        self.digiStrawBranch = self.sTree.Branch(
            "Digi_StrawtubesHits", self.digiStraw, 32000, -1
        )
        ...
        self.vetoHitOnTrackBranch = self.sTree.Branch(
            "VetoHitOnTrack", self.vetoHitOnTrackArray, 32000, -1
        )
        ...
        self.digiTimeDetBranch = self.sTree.Branch(
            "Digi_TimeDetHits", self.digiTimeDet, 32000, -1
        )
        self.digiUpstreamTagger = ROOT.TClonesArray("UpstreamTaggerHit")
        self.digiUpstreamTaggerBranch = self.sTree.Branch(
            "Digi_UpstreamTaggerHits", self.digiUpstreamTagger, 32000, -1
        )
        self.digiMuon = ROOT.TClonesArray("muonHit")
        self.digiMuonBranch = self.sTree.Branch(
            "Digi_muonHits", self.digiMuon, 32000, -1
        )
        ...
        if self.sTree.GetBranch("splitcalPoint"):
            ...
            self.digiSplitcalBranch = self.sTree.Branch(
                "Digi_SplitcalHits", self.digiSplitcal, 32000, -1
            )
            self.recoSplitcal = ROOT.TClonesArray("splitcalCluster")
            self.recoSplitcalBranch = self.sTree.Branch(
                "Reco_SplitcalClusters", self.recoSplitcal, 32000, -1
            )

        # setup ecal reconstruction
        self.caloTasks = []
        if self.sTree.GetBranch("EcalPoint") and not self.sTree.GetBranch(
            "splitcalPoint"
        ):
            # Creates. exports and fills calorimeter structure
            dflag = 10 if global_variables.debug else 0
            ecalGeo = (
                global_variables.ecalGeoFile
                + "z"
                + str(global_variables.ShipGeo.ecal.z)
                + ".geo"
            )
            if not ecalGeo in os.listdir(os.environ["FAIRSHIP"] + "/geometry"):
                shipDet_conf.makeEcalGeoFile(
                    global_variables.ShipGeo.ecal.z, global_variables.ShipGeo.ecal.File
                )
        ...
        if len(self.caloTasks) > 0:
            ...
            self.EcalClusters = self.sTree.Branch(
                "EcalClusters", self.ecalClusters, 32000, -1
            )
            self.ecalReconstructed = ecalReco.InitPython(
                self.sTree.EcalClusters, self.ecalStructure, self.ecalCalib
            )
            self.EcalReconstructed = self.sTree.Branch(
                "EcalReconstructed", self.ecalReconstructed, 32000, -1
            )
            ecalMatch.InitPython(
                self.ecalStructure, self.ecalReconstructed, self.sTree.MCTrack
            )
            if global_variables.EcalDebugDraw:
                ecalDrawer.InitPython(
                    self.sTree.MCTrack,
                    self.sTree.EcalPoint,
                    self.ecalStructure,
                    self.ecalClusters,
                )
        else:
            ...
            self.EcalClusters = self.sTree.Branch(
                "EcalClusters", ecalClusters, 32000, -1
            )
            self.EcalReconstructed = self.sTree.Branch(
                "EcalReconstructed", ecalReconstructed, 32000, -1
            )
        #
    ...
    def reconstruct(self):
        ...
        for x in self.caloTasks:
            if hasattr(x, "execute"):
                x.execute()
            elif x.GetName() == "ecalFiller":
                x.Exec("start", self.sTree.EcalPointLite)
            elif x.GetName() == "ecalMatch":
                x.Exec("start", self.ecalReconstructed, self.sTree.MCTrack)
            else:
                x.Exec("start")
        if len(self.caloTasks) > 0:
    ...
    def digitizeSplitcal(self):
        ...
        for i in list_clusters_of_hits:
            ...
            for hit in list_clusters_of_hits[i]:
                ...
                if hit.IsY():
                    list_hits_y.append(
                        hit
                    )  # FIXME: check if this could work also with high precision layers
            ...
            for index_subcluster in list_of_subclusters_x:
                subcluster_energy_x = self.GetClusterEnergy(
            ...
            for index_subcluster in list_of_subclusters_y:
                subcluster_energy_y = self.GetClusterEnergy(
                ...
                for ix in list_of_subclusters_x:
                    for iy in list_of_subclusters_y:
                        ...
                        list_final_clusters[index_final_cluster] = (
                            list_of_subclusters_y[iy] + list_of_subclusters_x[ix]
                        )
                        index_final_cluster += 1
        ...
        for i in list_final_clusters:
            ...
            for j, h in enumerate(list_final_clusters[i]):
                if j == 0:
                    aCluster = ROOT.splitcalCluster(h)
                else:
                    aCluster.AddHit(h)
    ...
    def GetSubclustersExcludingFragments(self):
        ...
        for k in self.list_subclusters_of_hits:
            ...
            if (
                subcluster_size < 5
            ):  # FIXME: it can be tuned on a physics case (maybe use fraction of hits or energy)
        ...
        if (
            len(subclusters_indices) == 0 and len(fragment_indices) != 0
        ):  # only fragments
        ...
        for index_fragment in fragment_indices:
            ...
            if (
                minIndex != index_fragment
            ):  # in case there were only fragments - this is to prevent to sum twice fragment 0
    ...
    def digitizeTimeDet(self):
        ...
        for aMCPoint in self.sTree.TimeDetPoint:
            ...
            if self.digiTimeDet.GetSize() == index:
                self.digiTimeDet.Expand(index + 1000)
    ...
    def digitizeUpstreamTagger(self):
        ...
        for aMCPoint in self.sTree.UpstreamTaggerPoint:
            aHit = ROOT.UpstreamTaggerHit(
                aMCPoint, global_variables.modules["UpstreamTagger"], self.sTree.t0
            )
            if self.digiUpstreamTagger.GetSize() == index:
                self.digiUpstreamTagger.Expand(index + 1000)
    ...
    def digitizeMuon(self):
        ...
        for aMCPoint in self.sTree.muonPoint:
            ...
            if self.digiMuon.GetSize() == index:
                self.digiMuon.Expand(index + 1000)
    ...
    def digitizeStrawTubes(self):
        ...
        for aMCPoint in self.sTree.strawtubesPoint:
            ...
            if self.digiStraw.GetSize() == index:
                self.digiStraw.Expand(index + 1000)
    ...
    def findTracks(self):
        ...
        if global_variables.realPR:
            # Do real PatRec
            track_hits = shipPatRec.execute(
                self.SmearedHits, global_variables.ShipGeo, global_variables.realPR
            )
            ...
            for i_track in track_hits.keys():
                ...
                atrack_smeared_hits = (
                    list(atrack_y12)
                    + list(atrack_stereo12)
                    + list(atrack_y34)
                    + list(atrack_stereo34)
                )
        ...
        for atrack in hitPosLists:
            ...
            for m in meas:
                ...
                measurement.setMaxDistance(
                    global_variables.ShipGeo.strawtubes.InnerStrawDiameter / 2.0
                )
    ...
    def linkVetoOnTracks(self):
        ...
        for goodTrak in self.goodTracksVect:
            ...
            if self.vetoHitOnTrackArray.GetSize() == index:
                self.vetoHitOnTrackArray.Expand(index + 1000)
