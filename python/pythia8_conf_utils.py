...
def make_particles_stable(P8gen, above_lifetime):
    ...
    while n != 0:
        ...
        if p.tau0() > above_lifetime:
            ...
            print(
                "Pythia8 configuration: Made {} stable for Pythia, should decay in Geant4".format(
                    p.name()
                )
            )
...
def parse_histograms(filepath):
    ...
    header_line_idx = [
        i for i in range(len(lines)) if th1f_exp.match(lines[i]) is not None
    ]
...
def add_particles(P8gen, particles, data):
    ...
    for particle_id in particles:
        ...
        particle = next(
            (p for p in data["particles"] if particle_id in [p["id"], p["name"]]), None
        )
        if particle is None:
            raise ValueError(
                "Could not find particle ID {0} in file {1}".format(particle, datafile)
            )
        # Add the particle
...
def add_dummy_channel(P8gen, particle, remainder):
    ...
    if charge > 0:
        P8gen.SetParameters(
            "{}:addChannel      1   {:.16}    0       22      -11".format(
                particle, remainder
            )
        )
    elif charge < 0:
        ...
        P8gen.SetParameters(
            "{}:addChannel      1   {:.16}    0       22      22".format(
                particle, remainder
            )
        )
...
def compute_max_total_br(decay_chains):
    ...
    top_level_particles = get_top_level_particles(decay_chains)
    total_branching_ratios = [
        compute_total_br(particle, decay_chains) for particle in top_level_particles
    ]
...
def exit_if_zero_br(max_total_br, selection, mass, particle="HNL"):
    if max_total_br <= 0:
        print(
            "No phase space for {0} from {1} at this mass: {2}. Quitting.".format(
                particle, selection, mass
            )
        )
        sys.exit()
