...
class ModuleLoader:
    ...
    def register_module(self, name: str, module_name: str, 
                       condition: Optional[Callable[[], bool]] = None) -> LazyModule:
...
class SimulationModuleLoader(ModuleLoader):
    ...
    def _setup_simulation_modules(self):
        ...
        self.register_module(
            'pythia8_conf', 
            'pythia8_conf',
            condition=lambda: self._is_pythia8_needed()
        )
        
        self.register_module(
            'pythia8darkphoton_conf',
            'pythia8darkphoton_conf', 
            condition=lambda: self.options and getattr(self.options, 'DarkPhoton', False)
        )
        ...
        self.register_module(
            'geomGeant4', 
            'geomGeant4',
            condition=lambda: self._is_geometry_needed()
        )
...
def lazy_import_decorator(module_dependencies: List[str]):
    """Decorator to ensure required modules are loaded before function execution."""
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            # Create a temporary loader for this function
            with controlled_import_context(module_dependencies) as loader:
...
class ImportManager:
    ...
    def get_module(self, name: str):
        ...
        if not self.is_initialized:
            raise RuntimeError("ImportManager not initialized. Call initialize() first.")
...
def setup_lazy_imports(options=None):
    """Convenience function to set up lazy imports for the simulation script."""
