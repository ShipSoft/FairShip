...
class DarkPhoton:
    ...
    def readPDGtable(self):
        ...
        """ecm,ratio = [],[]"""
        with open(
            os.path.expandvars("$FAIRSHIP/input/rpp2012-hadronicrpp_page1001.dat"), "r"
        ) as f:
            for line in f:
                line = line.split()
                try:
                    ...
                    if (
                        ("EXCLSUM" in strType)
                        or ("EDWARDS" in strType)
                        or ("BLINOV" in strType)
                    ):
    ...
    def leptonicDecayWidth(self, lepton):  # mDarkPhoton in GeV
        ...
        if 2.0 * ml < self.mDarkPhoton:
            rad = math.sqrt(
                1.0 - (4.0 * ml * ml) / (self.mDarkPhoton * self.mDarkPhoton)
            )
        else:
            rad = 0.0
