...
def config(GenieGen):
    ...
    zpos = (dVec["volArm2Mudet_1/volIron_12"].Z() + dVec["volArm2Mudet_1/volIron_23"].Z()) / 2.0
    box["volArm2Mudet_1/volIron_12-23"] = ROOT.TVector3(
        box["volArm2Mudet_1/volIron_12"].X(), box["volArm2Mudet_1/volIron_12"].Y(), length
    )
    ...
    if GenieGen == "debug":
        for aVol in box:
            print(
                ...
                % (aVol, box[aVol].X(), box[aVol].Y(), box[aVol].Z(), dVec[aVol].X(), dVec[aVol].Y(), dVec[aVol].Z())
            )
    else:
        for aVol in box:
