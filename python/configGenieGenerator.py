...
def config(GenieGen):
        ...
        if anode.GetName() in yokes:
            ...
            for bnode in anode.GetNodes():
                if bnode.GetName() in vols:
                    ...
                    x = []
                    for k in range(3):
                        x.append(aTrans[k] + bTrans[k] + muSpectrometerTrans[k])
                    dVec[nm].SetXYZ(x[0], x[1], x[2])
                    sbnode = bnode.GetVolume().GetShape()
                    box[nm] = ROOT.TVector3(
                        sbnode.GetDX(), sbnode.GetDY(), sbnode.GetDZ()
                    )
                    print("Debug muonSpectro ", nm, dVec[nm], box[nm])
    length = (
        dVec["volArm2Mudet_1/volIron_23"].Z() - dVec["volArm2Mudet_1/volIron_12"].Z()
    )
    zpos = (
        dVec["volArm2Mudet_1/volIron_12"].Z() + dVec["volArm2Mudet_1/volIron_23"].Z()
    ) / 2.0
    box["volArm2Mudet_1/volIron_12-23"] = ROOT.TVector3(
        box["volArm2Mudet_1/volIron_12"].X(),
        box["volArm2Mudet_1/volIron_12"].Y(),
        length,
    )
    ...
    if GenieGen == "debug":
        for aVol in box:
            print(
                "%50s %6.2F %6.2F %6.2F %5.2F %7.2F %7.2F "
                % (
                    aVol,
                    box[aVol].X(),
                    box[aVol].Y(),
