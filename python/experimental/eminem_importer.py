...
Examples:
  ...
  %(prog)s muons_FullMC.pkl --format rntuple --output my_data.root
    ...
    parser.add_argument(
        "input_file",
        nargs="?",
        help="Path to the input pickled data file (.pkl or .pkl.gz)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument("--config", help="Path to custom column configuration file (JSON format)")
...
def load_pickled_data(input_file, verbose=False):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load pickled data: {e}") from e

    if isinstance(data, dict):
        required = ["px", "py", "pz", "x", "y", "z", "pdg_id", "weight"]

        missing = [k for k in required if k not in data]
        if missing:
            raise ValueError(f"Missing keys in dict: {missing}")

        # convert to column stack (ONLY here, once)
        stacked = np.column_stack(
            [
                ...
                np.asarray(data["y"]),
                np.asarray(data["z"]),
                np.asarray(data["pdg_id"]),
                np.asarray(data["weight"]),
            ]
        )

        if verbose:
            print(f"Loaded data shape: {stacked.shape}")
    ...
    for col_name, col_config in custom_config.items():
        ...
        if "unit" in col_config and isinstance(col_config["unit"], str):
            ...
            if unit_str in unit_map:
                ...
                print(f"Warning: Unknown unit '{unit_str}' for column '{col_name}', ignoring unit")
        ...
        if "offset_unit" in col_config and isinstance(col_config["offset_unit"], str):
            ...
            if offset_unit_str in unit_map:
                ...
                print(f"Warning: Unknown offset unit '{offset_unit_str}' for column '{col_name}', ignoring offset unit")
...
def main():
    ...
    if not args.input_file:
        print("Error: input_file is required unless --list-config is used", file=sys.stderr)
        sys.exit(2)
