...
globalDesigns = {
    "2023": {"dy": 6.0, "dv": 6, "nud": 4, "caloDesign": 3, "strawDesign": 10}
}
default = "2023"
...
parser.add_argument(
    "-E",
    dest="simEngine",
    help="Possible options: EvtCalc,Pythia6, Pythia8, PG, Genie, nuRadiography, Ntuple, MuonBack, Nuage, muonDIS, Cosmics",
    required=True,
    default="Pythia8",
)
parser.add_argument(
...
parser.add_argument(
    "-f",
    dest="yaml_file",
    help="Yaml configuration file for simEngine parameters",
    required=True,
    default="shipgen/shipgen_parameters.yaml",
)
parser.add_argument(
    "-g",
    dest="geofile",
...
parser.add_argument(
    "--shieldName",
    help="The name of the SC shield in the database. SC default: sc_v6, Warm default: warm_opt",
    default="sc_v6",
    choices=["sc_v6", "warm_opt"],
)
parser.add_argument(
    "--debug",
...
if options.simEngine == "PG":
    tag = options.simEngine + "_" + str(config["ParticleGun"]["pID"]) + "-" + mcEngine
else:
    tag = options.simEngine + "-" + mcEngine
if config["Pythia8"]["charmonly"] == True:
    tag = options.simEngine + "CharmOnly-" + mcEngine
