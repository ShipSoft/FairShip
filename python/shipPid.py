...
class Task:
    ...
    def __init__(self, main):
        ...
        for i in range(4):
            ...
            if muonDet:
                self.zpositions["muon" + str(i)] = muonDet.GetMatrix().GetTranslation()[
                    2
                ]
            else:
                self.zpositions["muon" + str(i)] = ShipGeo["MuonStation" + str(i)].z
        ...
        self.Ecal_dx = 15  # cm
        self.Ecal_dy = 15  # cm
        self.Ecal_EP_threshold_min = (
            0.92  # acceptence region for E over P in Ecal >  self.Ecal_EP_threshold_min
        )
        self.Ecal_EP_threshold_max = 10
        self.Hit_number_threshold = (
            3  # number of pad-hits around the extrapolated tracks in MuonDet
        )
        self.hitlimit_x, self.hitlimit_y = (
            5,
            5,
        )  # x-y area to check the number of pad-hits
        self.cutNdf = 25
    ...
    def HcalHits(self):
        ...
        self.hcal2 = []
        self.new_hcal2 = []
        if not hasattr(self.sTree, "HcalPointLite"):
            return
        for chit in self.sTree.HcalPointLite:
            if not chit.GetEnergyLoss() > 0:
                continue
            ...
            if ELoss > 0:
                self.hcal1.append([x.value, y.value, section.value, ELoss])
    ...
    def muonDigitHit(self):
        ...
        for ahit in self.sTree.muonPoint:
            ...
            if m.fabs(ahit.GetZ() - self.zpositions["muon0"]) < self.muly_acceptance:
                ...
                if (pos_hit_x0 < 0 or pos_hit_x0 > self.ncells_x) or (
                    pos_hit_y0 < 0 or pos_hit_y0 > self.ncells_y
                ):
                    continue
                E = ahit.GetEnergyLoss() / u.MeV
                #      print "--*--->>  ", E,pos_hit_x0,pos_hit_y0, mul
                if not E < self.threshold_pad_energy:
                    self.list1.append([pos_hit_x0, pos_hit_y0, E, mul])
            if m.fabs(ahit.GetZ() - self.zpositions["muon1"]) < self.muly_acceptance:
                ...
                if (pos_hit_x1 < 0 or pos_hit_x1 > self.ncells_x) or (
                    pos_hit_y1 < 0 or pos_hit_y1 > self.ncells_y
                ):
                    continue
                E1 = ahit.GetEnergyLoss() / u.MeV
                #      print "--**--->>  ", E1,pos_hit_x1,pos_hit_y1, mul1
                if not E1 < self.threshold_pad_energy:
                    self.list1.append([pos_hit_x1, pos_hit_y1, E1, mul1])
            if m.fabs(ahit.GetZ() - self.zpositions["muon2"]) < self.muly_acceptance:
                ...
                if (pos_hit_x2 < 0 or pos_hit_x2 > self.ncells_x) or (
                    pos_hit_y2 < 0 or pos_hit_y2 > self.ncells_y
                ):
                    continue
                E2 = ahit.GetEnergyLoss() / u.MeV
                #      print "--***--->>  ", E2,pos_hit_x2,pos_hit_y2, mul2
                if not E2 < self.threshold_pad_energy:
                    self.list1.append([pos_hit_x2, pos_hit_y2, E2, mul2])
            if m.fabs(ahit.GetZ() - self.zpositions["muon3"]) < self.muly_acceptance:
                ...
                if (pos_hit_x3 < 0 or pos_hit_x3 > self.ncells_x) or (
                    pos_hit_y3 < 0 or pos_hit_y3 > self.ncells_y
                ):
                    continue
                E3 = ahit.GetEnergyLoss() / u.MeV
                #      print "--****--->>  ", E3,pos_hit_x3,pos_hit_y3, mul3
                if not E3 < self.threshold_pad_energy:
                    self.list1.append([pos_hit_x3, pos_hit_y3, E3, mul3])
        for i in range(len(self.list1)):
            ...
            for j in range(len(self.list1)):
                ...
                if x == xx and y == yy and mull == mulll:
                    E_tot += EE
                j += 1
            # print 'E_tot', E_tot
            if not mull == 0:
                self.list2.append([x, y, E_tot, mull])
            i += 1
        for elem in self.list2:
            if elem not in self.new_list2:
                self.new_list2.append(elem)
    ...
    def PID(self):
        ...
        for fT in self.sTree.FitTracks:
            ...
            self.pid20, self.pid21, self.pid22, self.pid30, self.pid31 = (
                False,
                False,
                False,
                False,
    ...
    def run_muon_ID(self):
        for i in range(len(self.list2)):
            ...
            if Mull == 0:
                ...
                if (
                    (self.P > self.P_min00 and self.P < self.P_max00)
                    and (delta_x0 > self.dx00_min and delta_x0 < self.dx00_max)
                    and (delta_y0 > self.dy00_min and delta_y0 < self.dy00_max)
                ):
                    self.pid00 = True
                if (
                    (self.P > self.P_min01 and self.P < self.P_max01)
                    and (delta_x0 > self.dx01_min and delta_x0 < self.dx01_max)
                    and (delta_y0 > self.dy01_min and delta_y0 < self.dy01_max)
                ):
                    self.pid01 = True
                if (
                    (self.P > self.P_min02 and self.P < self.P_max02)
                    and (delta_x0 > self.dx02_min and delta_x0 < self.dx02_max)
                    and (delta_y0 > self.dy02_min and delta_y0 < self.dy02_max)
                ):
                    self.pid02 = True
                if (
                    (self.P > self.P_min03 and self.P < self.P_max03)
                    and (delta_x0 > self.dx03_min and delta_x0 < self.dx03_max)
                    and (delta_y0 > self.dy03_min and delta_y0 < self.dy03_max)
                ):
                    self.pid03 = True
            ...
            if Mull == 1:
                ...
                if (
                    (self.P > self.P_min10 and self.P < self.P_max10)
                    and (delta_x1 > self.dx10_min and delta_x1 < self.dx10_max)
                    and (delta_y1 > self.dy10_min and delta_y1 < self.dy10_max)
                ):
                    self.pid10 = True
                if (
                    (self.P > self.P_min11 and self.P < self.P_max11)
                    and (delta_x1 > self.dx11_min and delta_x1 < self.dx11_max)
                    and (delta_y1 > self.dy11_min and delta_y1 < self.dy11_max)
                ):
                    self.pid11 = True
                if (
                    (self.P > self.P_min12 and self.P < self.P_max12)
                    and (delta_x1 > self.dx12_min and delta_x1 < self.dx12_max)
                    and (delta_y1 > self.dy12_min and delta_y1 < self.dy12_max)
                ):
                    self.pid12 = True
                if (
                    (self.P > self.P_min13 and self.P < self.P_max13)
                    and (delta_x1 > self.dx13_min and delta_x1 < self.dx13_max)
                    and (delta_y1 > self.dy13_min and delta_y1 < self.dy13_max)
                ):
                    self.pid13 = True
            if Mull == 2:
                ...
                if (
                    (self.P > self.P_min20 and self.P < self.P_max20)
                    and (delta_x2 > self.dx20_min and delta_x2 < self.dx20_max)
                    and (delta_y2 > self.dy20_min and delta_y2 < self.dy20_max)
                ):
                    self.pid20 = True
                if (
                    (self.P > self.P_min21 and self.P < self.P_max21)
                    and (delta_x2 > self.dx21_min and delta_x2 < self.dx21_max)
                    and (delta_y2 > self.dy21_min and delta_y2 < self.dy21_max)
                ):
                    self.pid21 = True
                if (
                    (self.P > self.P_min22 and self.P < self.P_max22)
                    and (delta_x2 > self.dx22_min and delta_x2 < self.dx22_max)
                    and (delta_y2 > self.dy22_min and delta_y2 < self.dy22_max)
                ):
                    self.pid22 = True
            if Mull == 3:
                ...
                if (
                    (self.P > self.P_min30 and self.P < self.P_max30)
                    and (delta_x3 > self.dx30_min and delta_x3 < self.dx30_max)
                    and (delta_y3 > self.dy30_min and delta_y3 < self.dy30_max)
                ):
                    self.pid30 = True
                if (
                    (self.P > self.P_min31 and self.P < self.P_max31)
                    and (delta_x3 > self.dx31_min and delta_x3 < self.dx31_max)
                    and (delta_y3 > self.dy31_min and delta_y3 < self.dy31_max)
                ):
                    self.pid31 = True
        ...
        if not self.P > 5 and (
            ...
            if chmu1 < 0.5 and chmu2 < 0.5:
                self.pid_mu = True
        check_vol_mu1 = (m.pow(self.extrap_X_mu1, 2) / m.pow(self.rx, 2)) + (
            m.pow(self.extrap_Y_mu1, 2) / m.pow(self.ry, 2)
        )
        if check_vol_mu1 > 1:
            self.vol_mu1 = True
    ...
    def run_elec_ID(self):
        for aClus in self.sTree.EcalReconstructed:
            ...
            EP = Ene1_Clus / self.P
            if (
                (EP > self.Ecal_EP_threshold_min and EP < self.Ecal_EP_threshold_max)
                and m.fabs(Ecal_delta_x) < self.Ecal_dx
                and m.fabs(Ecal_delta_y) < self.Ecal_dy
            ):
                self.El = True
        if (
            self.pid10 == False
            ...
            self.Hl = True
        check_vol_ecal = (m.pow(self.extrap_X_ecal, 2) / m.pow(self.rx, 2)) + (
            m.pow(self.extrap_Y_ecal, 2) / m.pow(self.ry, 2)
        )
        if check_vol_ecal > 1:
            self.vol_ecal = True
    ...
    def run_hcal_ID(self):
        N1, N2, M, E_est, E_totale = 0, 0, 0, 0.0, 0.0
        self.E_tot_h1, self.E_tot_h2, self.E_tot = 0.0, 0.0, 0.0
        ...
        check_vol_hcal = (m.pow(self.extrap_X_hcal, 2) / m.pow(self.rx, 2)) + (
            m.pow(self.extrap_Y_hcal, 2) / m.pow(self.ry, 2)
        )
        if check_vol_hcal > 1:
            self.vol_hcal = True
