        ...
        for i_track in range(nTracklets):
            ...
            if (
                ...
                if (
                    ...
                    if (
                        tmax_y12 in reconstructible_tracks
                        ...
                        tmax_y12 in reconstructible_tracks
                        and tmax_y12 in found_track_ids
                    ):
                        n_clones += 1
...
def getReconstructibleTracks(iEvent, sTree, sGeo, ShipGeo):
    ...
    VetoStationEndZ = (
        VetoStationZ
        + (ShipGeo.strawtubes.DeltazView + ShipGeo.strawtubes.OuterStrawDiameter) / 2
    )

    TStationz = ShipGeo.TrackStation1.z
    Zpos = (
        TStationz
        - 3.0 / 2.0 * ShipGeo.strawtubes.DeltazView
        - 1.0 / 2.0 * ShipGeo.strawtubes.DeltazPlane
        - 1.0 / 2.0 * ShipGeo.strawtubes.DeltazLayer
    )
    ...
    for key in hits1.keys():
        if (key in hits2 and key in hits3) and key in hits4:
            if (
                key not in tracks_with_hits_in_all_stations
                and key not in trackoutsidestations
            ):
...
def init_book_hist():
    ...
    ut.bookHist(
        h, "TracksPassed", "Tracks passing the pattern recognition", 8, -0.5, 7.5
    )
    ...
    ut.bookHist(
        h,
        "TracksPassedU",
        "Tracks passing the pattern recognition. No clones.",
        8,
        -0.5,
        7.5,
    )
    ...
    ut.bookProf(
        h,
        "TracksPassed_p",
        "Tracks passing the pattern recognition from momentum",
        30,
        0,
        150,
    )
    ...
    ut.bookHist(
        h, "n_hits_reco", "Number of recognized hits per track, total", 64, 0.0, 64.01
    )
    ut.bookHist(
        h,
        "n_hits_reco_12",
        "Number of recognized hits per track, station 1&2",
        32,
        0.0,
        32.01,
    )
    ut.bookHist(
        h,
        "n_hits_reco_y12",
        "Number of recognized hits per track, Y view station 1&2",
        32,
        0.0,
        32.01,
    )
    ut.bookHist(
        ...
        "Number of recognized hits per track, Stereo view station 1&2",
        32,
        0.0,
        32.01,
    )
    ut.bookHist(
        h,
        "n_hits_reco_34",
        "Number of recognized hits per track, station 3&4",
        32,
        0.0,
        32.01,
    )
    ut.bookHist(
        h,
        "n_hits_reco_y34",
        "Number of recognized hits per track, Y view station 3&4",
        32,
        0.0,
        32.01,
    )
    ut.bookHist(
        ...
        "Number of recognized hits per track, Stereo view station 3&4",
        32,
        0.0,
        32.01,
    )
    ...
    ut.bookProf(
        h, "n_hits_total", "Number of recognized hits per track, total", 30, 0, 150
    )
    ...
    ut.bookProf(
        h, "n_hits_12", "Number of recognized hits per track, station 1&2", 30, 0, 150
    )
    ...
    ut.bookProf(
        h, "n_hits_34", "Number of recognized hits per track, station 3&4", 30, 0, 150
    )
    ...
    ut.bookProf(
        h,
        "perr_direction",
        "(p - p-true)/p from track direction in YZ plane",
        20,
        0,
        10.01,
    )
    ...
    ut.bookProf(
        h, "frac_total", "Fraction of hits the same as MC hits, total", 30, 0, 150
    )
    ...
    ut.bookProf(
        h,
        "frac_y12",
        "Fraction of hits the same as MC hits, Y view station 1&2",
        30,
        0,
        150,
    )
    ...
    ut.bookProf(
        h,
        "frac_stereo12",
        "Fraction of hits the same as MC hits, Stereo view station 1&2",
        30,
        0,
        150,
    )
    ...
    ut.bookProf(
        h, "frac_12", "Fraction of hits the same as MC hits, station 1&2", 30, 0, 150
    )
    ...
    ut.bookProf(
        h,
        "frac_y34",
        "Fraction of hits the same as MC hits, Y view station 3&4",
        30,
        0,
        150,
    )
    ...
    ut.bookProf(
        h,
        "frac_stereo34",
        "Fraction of hits the same as MC hits, Stereo view station 3&4",
        30,
        0,
        150,
    )
    ...
    ut.bookProf(
        h, "frac_34", "Fraction of hits the same as MC hits, station 3&4", 30, 0, 150
    )
    ...
    ut.bookHist(
        h,
        "frac_12_dist",
        "Fraction of hits the same as MC hits, station 1&2",
        50,
        0.5,
        1.001,
    )
    ...
    ut.bookHist(
        h,
        "frac_34_dist",
        "Fraction of hits the same as MC hits, station 3&4",
        50,
        0.5,
        1.001,
    )
    ut.bookHist(
        h,
        "frac_total_dist",
        "Fraction of hits the same as MC hits, total",
        50,
        0.5,
        1.001,
    )
    ...
    ut.bookHist(
        h,
        "Reco_tracks",
        "Number of recognized tracks, clones and ghosts for reconstructible tracks.",
        5,
        -0.5,
        4.5,
    )
    ...
    # Fitted values
    ut.bookHist(
        h, "chi2fittedtracks", "Chi^2 per NDOF for fitted tracks", 210, -0.05, 20.05
    )
    ut.bookHist(h, "pvalfittedtracks", "pval for fitted tracks", 110, -0.05, 1.05)
    ut.bookHist(
        h, "momentumfittedtracks", "momentum for fitted tracks", 251, -0.05, 250.05
    )
    ut.bookHist(
        h, "xdirectionfittedtracks", "x-direction for fitted tracks", 91, -0.5, 90.5
    )
    ut.bookHist(
        h, "ydirectionfittedtracks", "y-direction for fitted tracks", 91, -0.5, 90.5
    )
    ut.bookHist(
        h, "zdirectionfittedtracks", "z-direction for fitted tracks", 91, -0.5, 90.5
    )
    ut.bookHist(h, "massfittedtracks", "mass fitted tracks", 210, -0.005, 0.205)
    ut.bookHist(
        h, "pvspfitted", "p-patrec vs p-fitted", 401, -200.5, 200.5, 401, -200.5, 200.5
    )
...
if __name__ == "__main__":
    ...
    metrics = run_track_pattern_recognition(
        options.input, options.geo, options.output, options.method
    )
