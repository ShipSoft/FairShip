...
class HNLbranchings:
    ...
    def Width_nu_f_fbar(self, alpha, beta):
        ...
        if (alpha not in [1, 2, 3]) or (beta not in [1, 2, 3, 4, 5, 6, 7, 8, 9]):
            print(
                "Width_nu_f_fbar ERROR: unknown channel alpha =", alpha, " beta =", beta
            )
            quit()
        ...
        if x > 0.5:  # the decay is kinematically forbidden
            return 0
        width = (
            (c.GF**2.0) * (self.MN**5.0) * self.U2[alpha - 1] / (192.0 * (math.pi**3.0))
        )
        L = 0.0
        if x > 0.01:
            logContent = (
                1.0 - 3.0 * x**2.0 - (1.0 - x**2.0) * math.sqrt(1.0 - 4.0 * x**2.0)
        ...
        if beta < 4:  # lepton decay
            ...
            if beta in [4, 7, 9]:  # up quarks
                C1 = 0.25 * (
                    1.0 - 8.0 / 3.0 * c.s2thetaw + 32.0 / 9.0 * c.s2thetaw**2.0
                )
        ...
        width = (
            ...
            * (
                C1
                * (
                    (1.0 - 14.0 * x**2.0 - 2.0 * x**4.0 - 12.0 * x**6.0)
                    * math.sqrt(1 - 4.0 * x**2)
                ...
                * (
                    x**2.0
                    * (2.0 + 10.0 * x**2.0 - 12.0 * x**4.0)
                    * math.sqrt(1.0 - 4.0 * x**2)
    ...
    def Width_l1_l2_nu2(self, alpha, beta):
        ...
        if (alpha not in [1, 2, 3]) or (beta not in [1, 2, 3]):
            print(
                "Width_l1_l2_nu2 ERROR: unknown channel alpha =", alpha, " beta =", beta
            )
            quit()
        if (
            alpha == beta
    ...
    def Width_l_u_d(self, alpha, beta, gamma):
        ...
        if (
            (alpha not in [1, 2, 3])
            or (beta not in [1, 2, 3])
            or (gamma not in [1, 2, 3])
        ):
            print(
                "Width_l_u_d ERROR: unknown channel alpha =",
                alpha,
                " beta =",
                beta,
                " gamma =",
    ...
    def Width_H0_nu(self, H, alpha):
        ...
        """
        if (H not in ["pi0", "eta", "rho0", "omega", "eta1", "phi", "eta_c"]) or (
            alpha not in [1, 2, 3]
        ):
    ...
    def findBranchingRatio(self, decayString):
        ...
        if (decayString not in self.decays) and (
            decayString not in ["N -> hadrons", "N -> charged hadrons"]
        ):
        ...
        if decayString == "N -> charged hadrons":
            br = (
                max([self.Width_charged_mesons(), self.Width_quarks_lepton()])
                / totalWidth
            )
        return br
