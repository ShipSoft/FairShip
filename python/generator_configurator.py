...
class GeneratorConfigurator:
    ...
    def __init__(
        self, ROOT, units, utils, ship_geo, pythia_configurator, cosmics_configurator
    ):
    ...
    def configure_pythia8_generators(self, primGen, options, config_values):
        ...
        primGen.SetTarget(self.ship_geo.target.z0, 0.0)
        ...
        if HNL or options.RPVSUSY:
            ...
            if HNL:
                ...
                if theProductionCouplings is None and theDecayCouplings is None:
                    ...
                    theProductionCouplings is not None and theDecayCouplings is not None
                ):
        ...
        if P8gen and (HNL or options.RPVSUSY or options.DarkPhoton):
            ...
            P8gen.SetLmin(
                (self.ship_geo.Chamber1.z - self.ship_geo.chambers.Tub1length)
        ...
        if charmonly:
            ...
            if self.ship_geo.Box.gausbeam:
                ...
                primGen.SetBeam(
                    0.0, 0.0, self.ship_geo.Box.TX - 1.0, self.ship_geo.Box.TY - 1.0
    ...
    def configure_pythia6_generator(self, primGen, options):
        ...
        primGen.SetTarget(self.ship_geo.target.z0 + self.ship_geo.muShield.length, 0.0)

        # Force loading of Pythia6 library
        test = self.ROOT.TPythia6()  # noqa: F841 - Intentional: forces library loading
        P6gen = self.ROOT.tPythia6Generator()
        P6gen.SetMom(50.0 * self.u.GeV)
    ...
    def configure_fixed_target_generator(self, primGen, options):
        ...
        P8gen.SetTarget("cave_1/target_vacuum_box_1/TargetArea_1/HeVolume_1", 0.0, 0.0)
        P8gen.SetMom(400.0 * self.u.GeV)
    ...
    def configure_evtcalc_generator(self, primGen, options, inputFile):
        ...
        EvtCalcGen.SetPositions(
            zTa=self.ship_geo.target.z, zDV=self.ship_geo.decayVolume.z
        )
    ...
    def configure_particle_gun(self, primGen, options):
        ...
        if options.multiplePG:
            ...
            # point source
            myPgun.SetXYZ(
                options.Vx * self.u.cm, options.Vy * self.u.cm, options.Vz * self.u.cm
            )
    ...
    def configure_mudis_generator(self, primGen, options, inputFile):
        ...
        mu_start, mu_end = (
            self.ship_geo.Chamber1.z
            - self.ship_geo.chambers.Tub1length
            - 10.0 * self.u.cm,
    ...
    def configure_ntuple_generator(self, primGen, options, inputFile):
        ...
        self.ut.checkFileExists(inputFile)
        primGen.SetTarget(self.ship_geo.target.z0 + 50 * self.u.m, 0.0)
    ...
    def configure_muon_background_generator(
        self, primGen, options, inputFile, config_values, modules
    ):
        ...
        if fileType == "tree":
            ...
            primGen.SetTarget(self.ship_geo.target.z0 + 70.845 * self.u.m, 0.0)
        else:
            primGen.SetTarget(self.ship_geo.target.z0 + 50 * self.u.m, 0.0)
        ...
        print(
            ...
            options.PaintBeam,
            ", with smear radius r = ",
            options.SmearBeam * self.u.cm,
            "with MCTracksWithHitsOnly",
            config_values["MCTracksWithHitsOnly"],
        )
    ...
    def configure_all_generators(self, run, options, config_values, modules):
        ...
        nu_generators = self.configure_neutrino_generators(
            primGen, run, options, inputFile
        )
        ...
        mbgen = self.configure_muon_background_generator(
            primGen, options, inputFile, config_values, modules
        )
        if mbgen:
