...
class ROOTInitializer:
    ...
    def get_initialization_summary(self) -> str:
        ...
        summary.append(f"  Status: {'Initialized' if self.is_initialized else 'Not initialized'}")
    ...
    def load_library(self, library_name: str, required: bool = True) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"Failed to load library {library_name} (result: {result})")
                if required:
                    raise RuntimeError(f"Required library {library_name} failed to load")
...
class InitializationManager:
    ...
    def initialize_simulation_environment(self, 
                                        seed: int = 0,
                                        load_ship_libs: bool = True,
                                        config_level: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            # Initialize ROOT
            ROOT = self.library_manager.ensure_root(seed=seed, config_level=config_level)
    ...
    def get_full_summary(self) -> str:
        ...
        summary.append(f"  Status: {'Fully Initialized' if self.is_fully_initialized else 'Not Initialized'}")
        summary.append("")
        summary.append(self.library_manager.root_initializer.get_initialization_summary())
        summary.append("")
...
def initialize_simulation_environment(**kwargs):
    """Convenience function to initialize the simulation environment."""
