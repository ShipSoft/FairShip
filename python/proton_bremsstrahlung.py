...
def wba(p, theta, mDarkPhoton, epsilon):
    ...
    p2 = (
        ...
        * (1.0 - zeta(p, theta))
        * (
            (2.0 * mp2 + mA2) / H(p, theta, mDarkPhoton)
            - pow(zeta(p, theta), 2.0) * 2.0 * mp2 * mp2 / h2
        )
    )
...
def sigmaRatio(p, mDarkPhoton):
    """sigma(s') / sigma(s)"""
    return sigma(es(p, mDarkPhoton)) / sigma(
        2.0 * mProton * energy(protonMomentum, mProton)
    )
...
def hProdPDF(
    ...
    momenta = np.linspace(
        pMin(mDarkPhoton), pMax(mDarkPhoton), binsp, endpoint=False
    ).tolist()
    hPDF = r.TH2F(
    ...
    for theta in angles:
        for p in momenta:
            ...
            hPDFp.Fill(p, w)
    hPdfFilename = sys.modules[
        "__main__"
    ].outputDir + "/ParaPhoton_eps%s_m%s%s.root" % (epsilon, mDarkPhoton, suffix)
