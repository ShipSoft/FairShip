...
def execute(smeared_hits, ship_geo, method=""):
    ...
    if method == "TemplateMatching":
        recognized_tracks = template_matching_pattern_recognition(
            smeared_hits, ship_geo
        )
    elif method == "FH":
        recognized_tracks = fast_hough_transform_pattern_recognition(
            smeared_hits, ship_geo
        )
    elif method == "AR":
        recognized_tracks = artificial_retina_pattern_recognition(
            smeared_hits, ship_geo
        )
    else:
...
def template_matching_pattern_recognition(SmearedHits, ShipGeo):
    ...
    #### Separate hits
    SmearedHits_12y, SmearedHits_12stereo, SmearedHits_34y, SmearedHits_34stereo = (
    ...
    ### PatRec in stereo12
    recognized_tracks_12 = pat_rec_stereo_views(
        SmearedHits_12stereo, recognized_tracks_y12, min_hits
    )
    ...
    ### PatRec in stereo34
    recognized_tracks_34 = pat_rec_stereo_views(
        SmearedHits_34stereo, recognized_tracks_y34, min_hits
    )
...
def fast_hough_transform_pattern_recognition(SmearedHits, ShipGeo):
    ...
    recognized_tracks_12 = fast_hough_pat_rec_stereo_views(
        SmearedHits_12stereo, recognized_tracks_y12, min_hits
    )
    # recognized_tracks_12 = [{'hits_y': [hit1, hit2, ...], 'hits_stereo': [hit1, hit2, ...], 'k_y': float, 'b_y': float}, {...}, ...]
    ...
    recognized_tracks_34 = fast_hough_pat_rec_stereo_views(
        SmearedHits_34stereo, recognized_tracks_y34, min_hits
    )
    # recognized_tracks_34 = [{'hits_y': [hit1, hit2, ...], 'hits_stereo': [hit1, hit2, ...], 'k_y': float, 'b_y': float}, {...}, ...]
    ...
    for atrack_y in recognized_tracks_y:
        ...
        for ahit1 in SmearedHits_stereo:
            for ahit2 in SmearedHits_stereo:
                ...
                if (
                    abs(ahit1["zx_projection"]) > 300
                    or abs(ahit2["zx_projection"]) > 300
                ):
                    continue

                k_seed = (
                    1.0
                    * (ahit2["zx_projection"] - ahit1["zx_projection"])
                    / (ahit2["z"] - ahit1["z"])
                )
                ...
                for ahit3 in SmearedHits_stereo:
                    if (
                        ahit3["digiHit"] == ahit1["digiHit"]
                        or ahit3["digiHit"] == ahit2["digiHit"]
                    ):
                        continue
...
def artificial_retina_pattern_recognition(SmearedHits, ShipGeo):
    ...
    ### PatRec in stereo12
    # recognized_tracks_12 = pat_rec_stereo_views(SmearedHits_12stereo, recognized_tracks_y12, min_hits)
    recognized_tracks_12 = artificial_retina_pat_rec_stereo_views(
        SmearedHits_12stereo, recognized_tracks_y12, min_hits
    )
    # recognized_tracks_12 = [{'hits_y': [hit1, hit2, ...], 'hits_stereo': [hit1, hit2, ...], 'k_y': float, 'b_y': float}, {...}, ...]
    ...
    ### PatRec in stereo34
    # recognized_tracks_34 = pat_rec_stereo_views(SmearedHits_34stereo, recognized_tracks_y34, min_hits)
    recognized_tracks_34 = artificial_retina_pat_rec_stereo_views(
        SmearedHits_34stereo, recognized_tracks_y34, min_hits
    )
    # recognized_tracks_34 = [{'hits_y': [hit1, hit2, ...], 'hits_stereo': [hit1, hit2, ...], 'k_y': float, 'b_y': float}, {...}, ...]
...
def artificial_retina_pat_rec_y_view(SmearedHits, min_hits):
    ...
    for i in range(len(SmearedHits)):
        ...
        best_seed_params = get_best_seed(
            hits_z_unused, hits_y_unused, sigma, sample_weight=None
        )
...
def artificial_retina_pat_rec_stereo_views(
    SmearedHits_stereo, recognized_tracks_y, min_hits
):
    ### PatRec in stereo
    recognized_tracks_stereo = []
    used_hits = []
...
def tracks_combination_using_extrapolation(
    recognized_tracks_12, recognized_tracks_34, z_magnet
):
