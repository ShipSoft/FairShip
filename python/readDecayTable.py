...
def load(
    conffile=os.path.expandvars("$FAIRSHIP/python/DecaySelection.conf"), verbose=True
):
    f = open(conffile, "r")
...
def addHNLdecayChannels(
    ...
    for dec in allowed:
        ...
        if allowed[dec] == "yes" and wanted[dec] == "yes":
            ...
            codes = " ".join(
                [
                    (str(-1 * code) if pdg.GetParticle(-code) != None else str(code))
...
def addDarkPhotondecayChannels(
    ...
    for dec in allowed:
        ...
        if dec not in wanted:
            print(
                "addDarkPhotondecayChannels WARNING: channel not configured! Please add also in conf file.\t",
                dec,
            )
            quit()
        ...
        if allowed[dec] == "yes" and wanted[dec] == "yes":
            ...
            if "hadrons" in dec:
                ...
                if mDP < 3.0:
                    P8gen.SetParameters(
                        "{}:addChannel =  1 {:.12} {} 1 -1".format(
                            dpid, 0.167 * BR, meMode
                        )
                    )
                    P8gen.SetParameters(
                        "{}:addChannel =  1 {:.12} {} 2 -2".format(
                            dpid, 0.666 * BR, meMode
                        )
                    )
                    P8gen.SetParameters(
                        "{}:addChannel =  1 {:.12} {} 3 -3".format(
                            dpid, 0.167 * BR, meMode
                        )
                    )
                if mDP >= 3.0:
                ...
                codes = " ".join(str(code) for code in childrenCodes)
                P8gen.SetParameters(
                    "{}:addChannel =  1 {:.12} {} {}".format(
                        P8gen.GetDPId(), BR, meMode, codes
                    )
                )
                print("debug readdecay table ", particles, children, BR)
