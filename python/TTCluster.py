...
import numpy as np
import ROOT
from ROOT.TMath import Gaus, Landau, Poisson
...
sipm_map = (
    ...
    (-hw + edge + array + biggap + array, biggap / 2.0, False),
    (biggap / 2.0, biggap / 2.0 + charr, (255.5, 319.5)),
    (biggap / 2.0 + charr, biggap / 2.0 + charr + gap, False),
    (biggap / 2.0 + charr + gap, biggap / 2.0 + array, (319.5, 383.5)),
    (biggap / 2.0 + array, biggap / 2.0 + array + biggap, False),
    (
        biggap / 2.0 + array + biggap,
        biggap / 2.0 + array + biggap + charr,
        (383.5, 447.5),
    ),
    (
        biggap / 2.0 + array + biggap + charr,
        biggap / 2.0 + array + biggap + charr + gap,
        False,
    ),
    (
        biggap / 2.0 + array + biggap + charr + gap,
        biggap / 2.0 + array + biggap + charr + array,
        (447.5, 511.5),
    ),
    (
        biggap / 2.0 + array + biggap + charr + array,
        biggap / 2.0 + array + biggap + charr + array + edge,
        False,
    ),
)
...
def GetStationNum(DetID):
    ...
    return int(hit.GetDetectorID() // 1000.0)
...
def global_to_local(DetID, globalpos):
    ...
    return globalpos + ((nmats - 1) / 2.0 - matnum + 1) * scifimat_width
...
def local_to_global(DetID, localpos):
    ...
    matnum = GetMatNum(DetID)
    nmats = GetMatQty(DetID)
    return localpos + (-nmats / 2.0 + matnum - 1 / 2.0) * scifimat_width
...
def create_cluster(amplitude, width, wmp):
    ...
    """
    if not chpos_min < wmp < chpos_max:
        return False
    if wmp is False:
        return False
...
class TTCluster:
    ...
    def ClusterGen(self):
        ...
        if sipm_pos == +1:
            distance = GetMatLength(self.DetID) / 2.0 - second_coord
        elif sipm_pos == -1:
            distance = GetMatLength(self.DetID) / 2.0 + second_coord
...
if __name__ == "__main__":
    ...
    for index, event in enumerate(tree):
        for hit in event.TTPoint:
            pnt = TTCluster(
                hit.GetDetectorID(), hit.GetX(), hit.GetY(), hit.GetEnergyLoss()
            )
