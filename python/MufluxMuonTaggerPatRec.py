...
def execute(TaggerHits, debug=0):
    ...
    # PatRec in zx plane
    tracks_zx = pat_rec_plane(
        TaggerHits_x, coord="x", min_hits=min_hits, max_shared_hits=max_shared_hits
    )

    # PatRec in zy plane
    tracks_zy = pat_rec_plane(
        TaggerHits_y, coord="y", min_hits=min_hits, max_shared_hits=max_shared_hits
    )
...
def pat_rec_plane(TaggerHits, coord="x", min_hits=3, max_shared_hits=2):
    ...
    for atrack in short_tracks:
        [atrack["k_" + coord], atrack["b_" + coord]] = np.polyfit(
            atrack["z_" + coord], atrack[coord + "_" + coord], deg=1
        )
...
def reduce_clones(long_tracks, coord="x", min_hits=3, max_shared_hits=2):
    ...
    for i_track in np.argsort(n_hits)[::-1]:
        ...
        if (
            len(atrack["hits_" + coord]) >= min_hits
