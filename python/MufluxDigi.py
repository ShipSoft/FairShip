...
def StripX(x):
    if x < -total_width / 2.0 or x > total_width / 2.0:
        print("WARNING: x coordinate outside sensitive volume!", x)
    if x < -total_width / 2.0 + EXT_STRIP_XWIDTH_L + V_STRIP_OFF / 2.0:
        strip_x = 184
    elif x > total_width / 2.0 - EXT_STRIP_XWIDTH_R - V_STRIP_OFF / 2.0:
        strip_x = 1
    else:
        x_start = x - total_width / 2.0 + EXT_STRIP_XWIDTH_R
        strip_x = -int(x_start / reduced_width * 182.0) + 1
...
class MufluxDigi:
    ...
    def __init__(self, fout):
        ...
        self.digiLateMufluxSpectrometerBranch = self.sTree.Branch(
            "Digi_LateMufluxSpectrometerHits", self.digiMufluxSpectrometer, 32000, 1
        )
        # muon taggger
        if self.sTree.GetBranch("MuonTaggerPoint"):
            ...
            self.digiMuonTaggerBranch = self.sTree.Branch(
                "Digi_MuonTaggerHits", self.digiMuonTagger, 32000, 1
            )
        ...
        self.sigma_spatial = global_variables.ShipGeo["MufluxSpectrometer"][
            "sigma_spatial"
        ]
    ...
    def digitizeMufluxSpectrometer(self):
        ...
        for aMCPoint in self.sTree.MufluxSpectrometerPoint:
            ...
            aHit.setValid()
            if index > 0 and self.digiMufluxSpectrometer.GetSize() == index:
            ...
            if detID in hitsPerDetId:
                ...
                hitsPerDetId[detID] = index
            if aMCPoint.GetDetectorID() in deadChannelsForMC:
                aHit.setInvalid()
            station = int(aMCPoint.GetDetectorID() // 10000000)
            if ROOT.gRandom.Rndm() < ineffiency[station]:
                aHit.setInvalid()
            index += 1
