...
class SimulationConfig:
    ...
    # Input/output
    input_file: str = "/eos/experiment/ship/data/Charm/Cascade-parp16-MSTP82-1-MSEL4-978Bpot.root"
    default_input_file: bool = True
    output_dir: str = "."
    
    # Geometry designs
    global_designs: Dict[str, Dict[str, Union[int, float]]] = field(default_factory=lambda: {
        '2023': {'dy': 6., 'caloDesign': 3, 'strawDesign': 10},
        '2025': {'dy': 6., 'ds': 8, 'caloDesign': 2, 'strawDesign': 10}
    })
...
class ConfigurationManager:
    ...
    def _initialize_unit_dependent_defaults(self):
        ...
        if self._units_module is None:
            logger.warning("Units module not set, cannot initialize unit-dependent defaults")
            return
    ...
    def load_from_options(self, options):
        ...
        if hasattr(options, 'theprodcouplings') and options.theprodcouplings:
            pc.production_couplings = [float(c) for c in options.theprodcouplings.split(",")]
        ...
        if hasattr(options, 'caloDesign'):
            current_design = sim_config.global_designs.get('custom', 
                            sim_config.global_designs[sim_config.default_design].copy())
        ...
        if hasattr(options, 'testFlag') and options.testFlag:
            sim_config.input_file = "$FAIRSHIP/files/Cascade-parp16-MSTP82-1-MSEL4-76Mpot_1_5000.root"
    ...
    def generate_runtime_config(self, options):
        ...
        if hasattr(options, 'command') and options.command == "PG":
            ...
            engines = ["pythia8", "evtcalc", "pythia6", "genie", "nuradio", 
                      "ntuple", "muonback", "mudis", "fixedTarget", "cosmics"]
        ...
        if sim_config.particle_config.charmonly:
            runtime_config.tag = f"{sim_config.sim_engine}CharmOnly-{sim_config.mc_engine}"
        ...
        runtime_config.parameter_file = str(output_dir / f"ship.params.{runtime_config.tag}.root")
        runtime_config.geometry_file = str(output_dir / f"geofile_full.{runtime_config.tag}.root")
