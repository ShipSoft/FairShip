...
def create_config(
    ...
    if not 0 < nS <= len(slices_gap):
        raise ValueError(f"target nS={nS} out of range for {len(slices_gap)} slices")
    slices_gap[nS - 1] = 0
    print(slices_material, slices_length, slices_gap)

    target_length: float = sum(w + g for w, g in zip(slices_length, slices_gap, strict=True))
