...
class LazyPythiaConfigurator:
    ...
    def configure_hnl(
        ...
        return pythia8_conf.configure(
            P8gen, mass, production_couplings, decay_couplings, inclusive, deep_copy
        )

    @lazy_import_decorator(["pythia8_conf"])
    def configure_rpvsusy(
        self,
        P8gen,
        mass,
        couplings,
        ...
        pythia8_conf = self._import_manager.get_module("pythia8_conf")
        return pythia8_conf.configurerpvsusy(
            P8gen, mass, couplings, sfermionmass, benchmark, inclusive, deep_copy, debug
        )

    @lazy_import_decorator(["pythia8darkphoton_conf"])
    def configure_dark_photon(
        self, P8gen, mass, epsilon, inclusive, mother_mode, deep_copy=False, debug=True
    ):
        ...
        pythia8darkphoton_conf = self._import_manager.get_module(
            "pythia8darkphoton_conf"
        )
        return pythia8darkphoton_conf.configure(
            P8gen, mass, epsilon, inclusive, mother_mode, deep_copy, debug
        )
...
class LazyGeometryConfigurator:
    ...
    def print_weights_and_fields(self, only_with_field=True, exclude=None):
        ...
        return geomGeant4.printWeightsandFields(
            onlyWithField=only_with_field, exclude=exclude
        )
