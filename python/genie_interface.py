...
# import os, shlex, subprocess, ROOT

# def _run(args, env_vars=None):
#     """Run a command with optional env overrides; print a nice shell-like line."""
#     env = os.environ.copy()
#     if env_vars:
#         for k, v in env_vars.items():
#             if v is None:
#                 env.pop(k, None)     # explicitly UNSET var
#             else:
#                 env[k] = str(v)
...
# def make_splines(nupdglist, targetcode, emax, nknots, outputfile, env_vars=None):
#     inputnupdg = ",".join(str(p) for p in nupdglist)
#     args = [
#         "gmkspl", "-p", inputnupdg, "-t", targetcode,
#         "-n", str(nknots), "-e", str(emax), "-o", outputfile
#     ]
#     _run(args, env_vars=env_vars)

# def generate_genie_events(
#     nevents, nupdg, emin, emax, targetcode, inputflux, spline, outputfile,
#     process=None, seed=None, irun=None, env_vars=None
# ):
#     # Build effective env first so we can derive paths like $GENIE/config/...
#     env = os.environ.copy()
#     if env_vars:
#         for k, v in env_vars.items():
#             if v is None:
...
#     msg_xml = None
#     if env.get("GENIE"):
#         msg_xml = os.path.join(env["GENIE"], "config", "Messenger_laconic.xml")
...
#     args += [
#         "-f", f"{inputflux},{get_1D_flux_name(nupdg)}",
...
# def make_ntuples(inputfile, outputfile, env_vars=None):
#     args = ["gntpc", "-i", inputfile, "-f", "gst", "-o", outputfile]
#     _run(args, env_vars=env_vars)
