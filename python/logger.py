...
ROOT.gROOT.ProcessLine(
    '#include "'
    + os.path.join(os.path.expandvars("$FAIRSHIP"), "utils", "logger.hxx")
    + '"'
)
...
def _make_wrapper(funcname):
    def function(*args, **kwargs):
        ...
        args_with_delim = [
            val for pair in zip(args, len(args) * [delim]) for val in pair
        ][:-1]
        return ROOT.ship.__dict__[funcname](*map(type, args_with_delim))(
            *args_with_delim
        )
