...
def execute(SmearedHits, TaggerHits, withNTaggerHits, withDist2Wire, debug=0):
    ...
    #### PatRec in stereo12
    short_tracks_12 = pet_rec_stereo_views(
        SmearedHits_stereo12, short_tracks_y12, min_hits
    )
    ...
    #### Combine tracks
    z_center = 350.75
    track_combinations = combine_tracks_before_and_after_the_magnet(
        short_tracks_12, short_tracks_34, z_center
    )
    ...
    if debug:
        ...
        for i_track in track_hits.keys():
            ...
            print(
                "Z_stereo12",
                [str(np.around(hit["z"], 2)) for hit in atrack["stereo12"]],
            )
            print(
                "X_stereo12",
                [str(np.around(hit["xtop"], 2)) for hit in atrack["stereo12"]],
            )
            ...
            for ahit2 in SmearedHits_stereo:
                ...
                for ahit3 in SmearedHits_stereo:
                    ...
                    in_bin = hit_in_window(
                        z3, y3_center, k_bin, b_bin, window_width=10.0
                    )  # 10.0
                    if in_bin:
...
def combine_tracks_before_and_after_the_magnet(
    short_tracks_12, short_tracks_34, z_center
):
