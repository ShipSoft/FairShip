...
def configurerpvsusy(
    ...
    if inclusive == "c":
        ...
        # add RPVSUSY
        rpvsusy_instance = rpvsusy.RPVSUSY(
            mass, couplings, sfermionmass, benchmark, debug=True
        )
        ctau = rpvsusy_instance.computeNLifetime(system="FairShip") * u.c_light * u.cm
        print("RPVSUSY ctau ", ctau)
        P8gen.SetParameters(
            "9900015:new = N2 N2 2 0 0 {:.12} 0.0 0.0 0.0 {:.12}  0   1   0   1   0".format(
                mass, ctau / u.mm
            )
        )
        ...
        if sumBR < 1.0 and sumBR > 0.0:
            P8gen.SetParameters(
                "431:addChannel      1   {:.12}    0       22      -11".format(
                    1.0 - sumBR
                )
            )
        ...
        if sumBR < 1.0 and sumBR > 0.0:
            P8gen.SetParameters(
                "411:addChannel      1   {:.12}    0       22      -11".format(
                    1.0 - sumBR
                )
            )
    ...
    if inclusive == "b":
        ...
        if sumBR < 1.0 and sumBR > 0.0:
            P8gen.SetParameters(
                "521:addChannel      1   {:.12}    0       22      22".format(
                    1.0 - sumBR
                )
            )
        ...
        if sumBR < 1.0 and sumBR > 0.0:
            P8gen.SetParameters(
                "511:addChannel      1   {:.12}    0       22      22".format(
                    1.0 - sumBR
                )
            )
...
def configure(
    ...
    if process_selection == "c":
        ...
        primary_decays = [
            (ch["id"], [get_br(histograms, ch, mass, production_couplings)])
            for ch in c_channels
        ]
        # The D_s+ can indirectly produce a HNL by first producing a tau+
        secondary_decays = [
            (ds_id, [ds_tau_br, get_br(histograms, ch, mass, production_couplings)])
        ...
        for ch in c_channels:
            add_channel(
                P8gen, ch, histograms, mass, production_couplings, 1 / max_total_br
            )
        ...
        total_tau_br = sum(
            branching_ratios[1] for (_, branching_ratios) in secondary_decays
        )
        assert ds_tau_br * total_tau_br <= max_total_br + 1e-12
        P8gen.SetParameters(
            "431:addChannel      1  {:.12}    0      -15       16".format(
                ds_tau_br * total_tau_br / max_total_br
            )
        )
