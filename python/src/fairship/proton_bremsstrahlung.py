...
def hProdPDF(mDarkPhoton, epsilon, norm, binsp, binstheta, tmin = -0.5 * math.pi, tmax = 0.5 * math.pi, suffix=""):
    ...
    hPDFtheta = ROOT.TH1F("hPDFtheta_eps%s_m%s"%(epsilon,mDarkPhoton),
    ...
    for theta in angles:
        for p in momenta:
            ...
            hPDFp.Fill(p,w)
    hPdfFilename = sys.modules['__main__'].options.outputDir+"/ParaPhoton_eps%s_m%s%s.root"%(epsilon,mDarkPhoton,suffix)
