...
Bool_t SciFiUnpack::DoUnpack(Int_t *data, Int_t size)
{
  ...
  for (auto &&hit : hits) {
    ...
    //unsigned int flag     = (hitinfo      ) & 0X0000000000000001; 
    ...
    //if (channelId < 15999){
    //  trigflag = 0;
    //  uint16_t module = channelId/1536;
    //  if (module > 7) continue;
    //  uint16_t ch = channelId - 1536*module;
    //  uint16_t boardId = channelId/512;
    //  new ((*fRawData)[fNHits]) SciFiHit(0x0900,0, hitTime*25, fineTime*32./680, ch, boardId, module, flag, trigflag);
    //  fNHits++;
    //}
    //else{
    //  trigflag = 1;
    //  uint16_t ch = 0; //there is no channel for trigger signal
    //  uint16_t amp = 0; //there is no amp for trigger signal
    //  uint16_t boardId = channelId - 16000;
