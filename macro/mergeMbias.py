...
ship_geo = ConfigRegistry.loadpy("$FAIRSHIP/geometry/geometry_config.py", Yheight=10.0)
endOfHadronAbsorber = (
    ship_geo["hadronAbsorber"].z + ship_geo["hadronAbsorber"].length / 2.0
) / 100.0
...
cuts = {
    "": "abs(id)>0",
    "_onlyMuons": "abs(id)==13",
    "_onlyNeutrinos": "abs(id)==12||abs(id)==14||abs(id)==16",
}
    ...
    for p in productions:
        ...
        if first:
            ...
            h["ntuple"] = ROOT.TNtuple(
                "pythia8-Geant4",
                "min bias flux after 3m hadron absorber " + opt,
                tuples,
            )
        ROOT.gROOT.cd()
        t.SetEventList(0)
        ...
        for iev in range(nev):
            ...
            if Ekin > 100.0:
                vlist[9] = norm / (
                    pot["CERN-Cracow"][100.0]
                    + pot["Yandex"][5.0]
                    + pot["Yandex2"][10.0]
                )
            elif Ekin > 10.0:
                vlist[9] = norm / (
                    pot["CERN-Cracow"][10.0] + pot["Yandex"][5.0] + pot["Yandex2"][10.0]
                )
            elif Ekin > 5.0:
                ...
                print(
                    "this should not happen, except some rounding errors",
                    p,
                    Ekin,
                    vlist[9],
                )
...
def mergeWithCharm(splitOnly=False, ramOnly=False):
    ...
    if not splitOnly:
        fcascade = ROOT.TFile.Open(
            eospath + "/Charm/Decay-Cascade-parp16-MSTP82-1-MSEL4-76Mpot_1.root"
        )
        t = fcascade.Decay
    ...
    if 1 > 0:
        ...
        for idnu in [16, -16, 14, -14, 12, -12]:
            ...
            ut.bookHist(
                h,
                str(idhnu + 100),
                name + " log10(ptot) vs log10(pt+0.01)",
                100,
                -0.3,
                1.7,
                100,
        ...
        for opt in cuts:
            ...
            ntuple = ROOT.TNtuple(
                "pythia8-Geant4",
                opt.replace("_only", "") + " flux mbias and charm" + opt,
                tuples,
            )
            ROOT.gROOT.cd()
            t.SetEventList(0)
...
def compare():
    ...
    for x in ["", "_>E"]:
        for z in ["p", "pt"]:
            ...
            h["L" + p + x].AddEntry(
                h["T" + p + x], "muon new with charm, cascade, k-fac", "PL"
            )
            h["L" + p + x].AddEntry(
                h["T" + p + "charm" + x],
            ...
            h["L" + p + x].AddEntry(
                h["TP" + p + "charm" + x], "muon from charm old CERN-Cracow prod", "PL"
            )
            h["L" + p + x].Draw()
            h[t].cd(2)
            p = z + "numusum"
            ...
            h["L" + p + x].AddEntry(
                h["TP" + p + "charm" + x], "nu_mu from charm old CERN-Cracow prod", "PL"
            )
            h["L" + p + x].Draw()
            t3 = h[t].cd(3)
            t3.SetLogy(1)
            ...
            h["L" + p + x] = ROOT.TLegend(0.33, 0.69, 0.99, 0.94)
            h["L" + p + x].AddEntry(
                h["T" + p + x], "nu_e new with charm, cascade, k-fac", "PL"
            )
            h["L" + p + x].AddEntry(
                h["T" + p + "charm" + x],
            ...
            h["L" + p + x].AddEntry(
                h["TP" + p + "charm" + x], "nu_e from charm old CERN-Cracow prod", "PL"
            )
            h["L" + p + x].Draw()
            #
            t4 = h[t].cd(4)
            ...
            h["T" + p + x].SetLineColor(4)
            h["Lmuc" + z + x].AddEntry(
                h["T" + p + x], "mu- new with charm, cascade, k-fac", "PL"
            )
            ...
            h["T" + p + x].SetLineColor(2)
            h["Lnumu" + z + x].AddEntry(
                h["T" + p + x], "anti nu_mu new with charm, cascade, k-fac", "PL"
            )
            h["Lnumu" + z + x].AddEntry(
                h["TP" + p + x], "anti nu_mu old Yandex prod", "PL"
            )
            h["Lnumu" + z + x].Draw()
            t6 = h[t].cd(6)
            t6.SetLogy(1)
            ...
            h["T" + p + x].SetLineColor(2)
            h["Lnue" + z + x].AddEntry(
                h["T" + p + x], "anti nu_e new with charm, cascade, k-fac", "PL"
            )
            h["Lnue" + z + x].AddEntry(
                h["TP" + p + x], "anti nu_e old Yandex prod", "PL"
            )
            h["Lnue" + z + x].Draw()
            #
    ...
    for z in ["p", "pt"]:
        ...
        h[z + "muRatio" + x].SetMaximum(
            max(h[z + "muRatio" + x].GetMaximum(), h[z + "numuRatio" + x].GetMaximum())
        )
        ...
        h["Lratio" + z + x].AddEntry(
            h[z + "numuRatio" + x], "nu_mu flux new / old ", "PL"
        )
