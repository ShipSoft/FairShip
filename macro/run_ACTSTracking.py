...
parser.add_argument("-o", "--outputDir", dest="outputDir", help="Output directory", required=False, default="")
parser.add_argument("-n", "--nEvents", dest="nEvents", help="Num of events to process", required=False)
...
parser.add_argument("--DQM", dest="DQM", help="Option to enable ACTS track and vertex DQM", required=False, deafault=True)
...
fTree = inFile["cbmsim"]
options.nEvents = min(fTree.GetEntries(),options.nEvents)
inFile.Close()

#If input file has not been converted to ACTS EDM then convert
if not options.inputFile.find('_ACTS.root') < 0:
