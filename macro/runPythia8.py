...
parser.add_argument(
    "-n", "--pot", dest="NPoT", type=int, help="protons on target", default=1000000
)
parser.add_argument(
...
parser.add_argument(
    "-E", "--Beam", dest="fMom", type=float, help="beam momentum", default=400.0
)
parser.add_argument(
    "-J", "--Jpsi-mainly", action="store_true", dest="JpsiMainly", default=False
)
parser.add_argument(
    "-Y", "--DrellYan", action="store_true", dest="DrellYan", default=False
)
parser.add_argument(
    "-P",
    "--PhotonCollision",
    action="store_true",
    dest="PhotonCollision",
    default=False,
)
    ...
    for g in generators:
        ...
        if len(nmu) == 2:
            ...
            if options.DrellYan:
                ...
                rc = h["M_" + g].Fill(
                    Zstar.m(),
                    py.event[nmu[ks[0]]].y() - ybeam,
                    py.event[nmu[ks[0]]].pT(),
                )
                ...
                cosCS = (
                    ...
                    * 1.0
                    / Zstar.m()
                    / ROOT.TMath.Sqrt(Zstar.m2() + Zstar.pT() ** 2)
                    * A
                )
                rc = h["cosCS_" + g].Fill(
                    cosCS, Zstar.y() - ybeam, py.event[nmu[ks[0]]].pT()
                )
                if Zstar.m() > 2.9 and Zstar.m() < 4.5:
                    rc = h["cosCSJpsi_" + g].Fill(
                        cosCS,
                        py.event[nmu[ks[0]]].y() - ybeam,
                        py.event[nmu[ks[0]]].pT(),
                    )
...
def na50(online=True):
    for g in generators:
        ...
        print(
            "cross section a la NA50 for : %s %5.2F pb"
            % (g, 0.5 * fraction * sigma * 1e9)
        )
    # via cosCS
    for g in generators:
        if online:
        ...
        print(
            "cross section a la NA50, -0.5<cosCS<0.5: %5.2F pb"
            % (fraction * sigma * 1e9)
        )
