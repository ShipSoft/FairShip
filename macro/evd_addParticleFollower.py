...
def execute():
    if not gEve.GetViewers().FindChild("Bar Embedded Viewer side"):
        ...
        for c in cams:
            ...
            )  # gEve.GetViewers().FindChild('Bar Embedded Viewer')
            ...
            if c == "top":
                # doesn't really work
                center = array("d", [0, 500.0, 0])
                cam.Configure(1.0, 0.0, center, 0.0, 0.0)
                co.SetShowOrthographic(True)
                co.SetOrthographicMode(
                    ROOT.TGLCameraOverlay.kAll
                )  # ROOT.TGLCameraOverlay.kAxis
                ...
                cam.Configure(1.0, 0.0, center, 0, 0)
                co.SetShowOrthographic(True)
                co.SetOrthographicMode(
                    ROOT.TGLCameraOverlay.kAll
                )  # ROOT.TGLCameraOverlay.kAxis
        ...
        )  # ROOT.gROOT.FindObject('Root Canvas')
