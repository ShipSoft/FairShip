...
parser.add_argument(
    ...
    "--nEvents",
    dest="nEvents",
    help="Number of events to reconstruct",
    required=False,
    default=999999,
    type=int,
)
...
parser.add_argument(
    "--withT0",
    dest="withT0",
    help="simulate arbitrary T0 and correct for it",
    required=False,
    action="store_true",
)
parser.add_argument(
...
parser.add_argument(
    "--saveDisk",
    dest="saveDisk",
    help="if set, will remove input file, only rec file kept",
    required=False,
    action="store_true",
)
parser.add_argument(
...
parser.add_argument(
    "-dy", dest="dy", help="Max height of tank", required=False, default=None, type=int
)
parser.add_argument(
    "--Debug",
    dest="Debug",
...
if not options.inputFile.find("_rec.root") < 0:
    ...
    if options.inputFile[:7] == "root://":
        os.system("xrdcp " + options.inputFile + " " + outFile)
    elif options.saveDisk:
        os.system("mv " + options.inputFile + " " + outFile)
    else:
...
geoMat = (
    ROOT.genfit.TGeoMaterialInterface()
)  # if only called in ShipDigiReco -> crash, reason unknown
...
if withHists:
    ...
    ut.bookHist(h, "nmeas", "nr measuerements", 100, 0.0, 50.0)
