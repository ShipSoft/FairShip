...
print(
    "Input file: ", FIN, " with ", nEvents, " entries, corresponding to nr-pot=", nrcpot
)
# nEvents=100
...
while n != 0:
    ...
    if p.tau0() > 1:
        ...
        print(
            "Pythia8 configuration: Made %s stable for Pythia, should decay in Geant4",
            p.name(),
        )
P8.init()
...
for n in range(nEvents):
    ...
    P8.event.reset()
    P8.event.append(
        int(sTree.id), 1, 0, 0, sTree.px, sTree.py, sTree.pz, sTree.E, sTree.M, 0.0, 9.0
    )
    next(P8)
    # P8.event.list()
        ...
        if P8.event[n].isFinal():
            ...
            if idabs > 11 and idabs < 17:
                ...
                if idabs == 16 or idabs == 14 or idabs == 12:
                    ...
                    l10pt = min(
                        max(ROOT.TMath.Log10(ROOT.TMath.Sqrt(pt2)), -2.0), 0.4999
                    )
...
if (1.0 - pot / nrcpot) < 1.0e-2:
    print(
        "write ntuple, weight/event=", nrpotspill, "x", chicc, "/", nrcpot, "=", wspill
    )
    Ntup.Write()
    for akey in h:
        h[akey].Write()
    ...
    for idnu in range(12, 18, 2):
        ...
        for idadd in range(-1, 3, 2):
            ...
            if idadd == -1:
                ...
                idw = -idnu
            print(
                idhnu, h[str(idhnu)].GetTitle(), ("%8.3E " % (h[str(idhnu)].Integral()))
            )
    ...
    print("*********** WARNING ***********")
    print(
        "number of POT does not agree between ntuple and hists, i.e.:",
        pot,
        "<>",
        nrcpot,
    )
