...
def print_info(path, node, level, currentlevel, print_sub_det_info=False):
    ...
    for name, _ in sorted(list(sub_nodes.items()), key=operator.itemgetter(1)):
        ...
        if (
            ...
            and fullInfo[name]["node"].GetNodes()
        ):
            print_info(
                fullInfo[name]["path"],
                fullInfo[name]["node"],
...
parser.add_argument(
    "-g", "--geometry", dest="geometry", help="input geometry file", required=True
)
parser.add_argument("-l", "--level", dest="level", help="max subnode level", default=0)
parser.add_argument(
    "-v", "--volume", dest="volume", help="name of volume to expand", default=""
)
parser.add_argument(
    "-X", "--moreInfo", dest="moreInfo", help="print weight and capacity", default=False
)
