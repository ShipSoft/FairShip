...
defaultsplinedir = (
    "/eos/experiment/ship/user/aiuliano/GENIE_FNAL_nu_splines"  # path of splines
)
...
def get_arguments():  # available options
    ...
    ap.add_argument(
        ...
        type=str,
        help="event generator list",
        dest="evtype",
        default=None,
    )  # Possbile evtypes: CC, CCDIS, CCQE, CharmCCDIS, RES, CCRES, see other evtypes in $GENIE/config/EventGeneratorListAssembler.xml
    ap.add_argument(
        "--nudet",
        dest="nudet",
        help="option for neutrino detector",
        required=False,
        action="store_true",
    )
...
def makeSplines():
    ...
    genie_interface.make_splines(
        nupdglist, targetcode, 400, nknots=500, outputfile="xsec_splines.xml"
    )
    ...
    for p in pDict:
        if p < 0:
            print(
                "scale number of "
                + sDict[p]
                + " events with %5.2F" % (1.0 / nuOverNubar[abs(p)])
            )
        if not sDict[p] in os.listdir("."):
            call("mkdir " + sDict[p], shell=True)
        os.chdir("./" + sDict[p])
        ...
        N = nevents
        if p < 0:
            N = int(nevents / nuOverNubar[abs(p)])
...
if "splinedir" not in args:
    ...
    for x in [16, 14, 12]:
        ...
        pDict[-x] = "20" + str(x)
        nuOverNubar[x] = (
            f.Get(pDict[x]).GetSumOfWeights() / f.Get(pDict[-x]).GetSumOfWeights()
        )
    f.Close()
