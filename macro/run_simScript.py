#!/usr/bin/env python
"""SHiP simulation script.

Main script for running SHiP simulations with configurable generators and detectors.
"""
...
parser.add_argument(
    "--Pythia6", dest="pythia6", help="Use Pythia6", action="store_true"
)
parser.add_argument(
    "--Pythia8", dest="pythia8", help="Use Pythia8", action="store_true"
)
# === PG subcommand ===
...
parser.add_argument(
    "--Ntuple", dest="ntuple", help="Use ntuple as input", action="store_true"
)
parser.add_argument(
    "--MuonBack",
    dest="muonback",
...
parser.add_argument(
    "--SmearBeam",
    dest="SmearBeam",
    help="Standard deviation of beam smearing (muon background only) [cm]",
    default=0.8,
    type=float,
)
parser.add_argument(
    "--PaintBeam",
    dest="PaintBeam",
    help="Radius of beam painting (muon background only) [cm]",
    default=5,
    type=float,
)
parser.add_argument(
...
parser.add_argument(
    "--FixedTarget",
    dest="fixedTarget",
    help="Enable fixed target simulation",
    action="store_true",
)
parser.add_argument("--DarkPhoton", help="Generate dark photons", action="store_true")
parser.add_argument(
    "--SusyBench", dest="RPVSUSYbench", help="Generate HP Susy", default=2
)
parser.add_argument(
    "-m",
    "--mass",
    dest="theMass",
    help="Mass of hidden particle, default 1.0 GeV for HNL, 0.2 GeV for DP",
    default=None,
    type=float,
)
parser.add_argument(
...
parser.add_argument(
    "-i", "--firstEvent", help="First event of input file to use", default=0, type=int
)
parser.add_argument(
    "-s",
    "--seed",
...
group.add_argument(
    "-f", dest="inputFile", help="Input file if not default file", default=False
)
parser.add_argument(
    "-g",
    dest="geofile",
...
parser.add_argument(
    "--strawDesign",
    help="Tracker design: 4=sophisticated straw tube design, horizontal wires; 10=straw of 2 cm diameter (default)",
    default=4,
    type=int,
    choices=[4, 10],
)
...
parser.add_argument(
    "-t", "--test", dest="testFlag", help="quick test", action="store_true"
)
parser.add_argument(
    "--dry-run", dest="dryrun", help="stop after initialize", action="store_true"
)
parser.add_argument(
    "-D",
    "--display",
...
parser.add_argument(
    "--z-offset",
    dest="z_offset",
    help="z-offset for the FixedTargetGenerator [mm]",
    default=-84.0,
    type=float,
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=os.path.expandvars("$FAIRSHIP/geometry/target_config_Jun25.yaml"),
)
...
init_result = initialize_simulation_environment(
    seed=options.theSeed, load_ship_libs=True, config_level=0
)
...
for x in os.listdir(config.output_dir):
    if not x.find(tag) < 0:
        os.system(f"rm {config.output_dir}/{x}")
...
primGen, generators = generator_configurator.configure_all_generators(
    run, options, generator_config_values, modules
)

# Update configuration values that may have been modified by generator setup
MCTracksWithHitsOnly = generator_config_values.get(
    "MCTracksWithHitsOnly", MCTracksWithHitsOnly
)
...
def visualizeMagFields():
    """Visualize magnetic fields using the global utility configurator."""
    utility_configurator_global.visualize_mag_fields()


def checkOverlapsWithGeant4():
    """Check geometry overlaps using Geant4.

    Note: Should be called after /run/initialize. May print warnings about TGeo volumes.
    """
