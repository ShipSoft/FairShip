...
strawtubes::strawtubes()
   : FairDetector("strawtubes", kTRUE, kVETO), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
...
void strawtubes::ConstructGeometry()
{
   ...
   for (Int_t vnb = 0; vnb < 2; vnb++) {
      ...
      case 1:
         ...
         nmview = nmveto + "_u";
         break;
      default: angle = 0.; nmview = nmveto + "_x";
      }
      ...
      for (Int_t pnb = 0; pnb < 2; pnb++) {
         ...
         viewframe->AddNode(planebox, 4 * 10000000 + vnb * 1000000 + pnb * 100000,
         ...
         "<<TStationz-(1./2.-vnb)*(fDeltaz_view)+(pnb-1./2.)*fDeltaz_plane12+(lnb-1./2.)*fDeltaz_layer12<<std::endl;
           ...
           TGeoTranslation t5s;
             ...
             //straws are tubes defined along the z-axis, so need to rotate them first by 90 degrees around the x-axis
   ...
   for (Int_t statnb = 0; statnb < 4; statnb++) {
      ...
      switch (statnb) {
      case 0: TStationz = fT1z; break;
      case 1: TStationz = fT2z; break;
      case 2: TStationz = fT3z; break;
      case 3: TStationz = fT4z; break;
      default: break;
      }
      ...
      for (Int_t vnb = 0; vnb < 4; vnb++) {
         ...
         for (Int_t pnb = 0; pnb < 2; pnb++) {
            ...
            // the planebox sits in the viewframe
            // hence z translate the plane wrt to the view
            viewframe->AddNode(planebox, statnb * 10000000 + vnb * 1000000 + pnb * 100000,
                               new TGeoTranslation(0, 0, (pnb - 1. / 2.) * fDeltaz_plane12));
            // planebox->SetVisibility(kFALSE);
            // planebox->VisibleDaughters(kTRUE);
            TGeoRotation r5l;
            ...
            for (Int_t lnb = 0; lnb < 2; lnb++) {
               ...
               for (Int_t snb = 0; snb < fStraws_per_layer; snb++) {
                  ...
                  // x-axis to get them vertical then x translate the straws according to their plane, layer and number
                  r6s.SetAngles(0, 90, 0);
                  TGeoCombiTrans c6s(t6s, r6s);
                  TGeoHMatrix *h6s = new TGeoHMatrix(c6s);

                  layerbox->AddNode(straw, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000 + snb,
                                    h6s);
                  layerbox->AddNode(gas, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000 + snb,
                                    h6s);
                  layerbox->AddNode(wire, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 3000 + snb,
                                    h6s);
                  // only the straws are sensitive
