...
strawtubes::strawtubes()
   : FairDetector("strawtubes", kTRUE, kStraw), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fstrawtubesPointCollection(new TClonesArray("strawtubesPoint"))
{
}

strawtubes::strawtubes(const char *name, Bool_t active)
   : FairDetector(name, active, kStraw), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fstrawtubesPointCollection(new TClonesArray("strawtubesPoint"))
{
}
...
Bool_t strawtubes::ProcessHits(FairVolume *vol)
{
   ...
   if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
      ...
      AddHit(fTrackID, straw_uniqueId, TVector3(xmean, ymean, zmean), TVector3(fMom.Px(), fMom.Py(), fMom.Pz()), fTime,
             deltaTrackLength, fELoss, pdgCode, dist2Wire);
      if (dist2Wire > fInner_Straw_diameter / 2) {
         std::cout << "addhit " << dist2Wire << " straw id " << straw_uniqueId << " pdgcode " << pdgCode << " dot prod "
...
void strawtubes::ConstructGeometry()
{
   ...
   TGeoBBox *detbox1 = new TGeoBBox("detbox1", fStraw_length + fFrame_lateral_width, ftr34ydim + fFrame_lateral_width,
                                    fDeltaz_frame / 2.);
   TGeoBBox *detbox2 = new TGeoBBox("detbox2", fStraw_length + eps, ftr34ydim + eps, fDeltaz_frame / 2. + eps);

   TGeoBBox *detbox1_12 = new TGeoBBox("detbox1_12", fStraw_length_12 + fFrame_lateral_width,
                                       ftr12ydim + fFrame_lateral_width, fDeltaz_frame / 2.);
   ...
   if (fStraw_length_veto > 1) {
      ...
      for (Int_t vnb = 0; vnb < 2; vnb++) {
         ...
         case 1:
            ...
            nmveto = nmveto + "_u";
            break;
         default: angle = 0.; nmveto = nmveto + "_x";
         }
         ...
         for (Int_t pnb = 0; pnb < 2; pnb++) {
            ...
            TGeoBBox *plane_veto = new TGeoBBox("plane box", fStraw_length_veto + eps / 2., fvetoydim + eps / 2.,
            ...
            for (Int_t lnb = 0; lnb < 2; lnb++) {
               ...
               layer_veto = new TGeoBBox("layer box_veto", fStraw_length_veto + eps / 4., fvetoydim + eps / 4.,
                                         layerwidth / 2. + eps / 4.);
               ...
               planebox_veto->AddNode(layerbox_veto, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000,
               ...
               for (Int_t snb = 1; snb < fStraws_per_layer_veto; snb++) {
                  // straw loop
                  t6v.SetTranslation(0, fvetoydim - fStraw_pitch * snb - fOffset_plane12 * pnb + lnb * fOffset_layer12,
                                     0);
      ...
      if ((statnb == 1) || (statnb == 2)) {
         for (Int_t vnb = 0; vnb < 4; vnb++) {
            ...
            case 3:
               ...
               break;
            default: angle = 0.; nmview_12 = nmstation + "_x1";
            }
            ...
            for (Int_t pnb = 0; pnb < 2; pnb++) {
               ...
               nmplane_12 += pnb;
               TGeoBBox *plane_12 = new TGeoBBox("plane box_12", fStraw_length_12 + eps / 2, ftr12ydim + eps / 2,
                                                 planewidth / 2. + 3. * eps / 2);
               ...
               for (Int_t lnb = 0; lnb < 2; lnb++) {
                  ...
                  nmlayer_12 += lnb;
                  TGeoBBox *layer_12 = new TGeoBBox("layer box_12", fStraw_length_12 + eps / 4, ftr12ydim + eps / 4,
                                                    layerwidth / 2. + eps / 4);
                  ...
                  for (Int_t snb = 1; snb < fStraws_per_layer_tr12; snb++) {
                     ...
                     layerbox_12->AddNode(
                        straw_12, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000 + snb, h6s);
                     layerbox_12->AddNode(
      ...
      if ((statnb == 3) || (statnb == 4)) {
         for (Int_t vnb = 0; vnb < 4; vnb++) {
            ...
            case 3:
               ...
               break;
            default: angle = 0.; nmview = nmstation + "_x1";
            }
            ...
            for (Int_t pnb = 0; pnb < 2; pnb++) {
               ...
               nmplane += pnb;
               TGeoBBox *plane = new TGeoBBox("plane box", fStraw_length + eps / 2, ftr34ydim + eps / 2,
                                              planewidth / 2. + 3. * eps / 2);
               ...
               for (Int_t lnb = 0; lnb < 2; lnb++) {
                  ...
                  nmlayer += lnb;
                  TGeoBBox *layer =
                     new TGeoBBox("layer box", fStraw_length + eps / 4, ftr34ydim + eps / 4, layerwidth / 2. + eps / 4);
                  TGeoVolume *layerbox = new TGeoVolume(nmlayer, layer, med);
                  planebox->AddNode(layerbox, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000,
                                    new TGeoTranslation(0, 0, (lnb - 1. / 2.) * fDeltaz_layer12));
                  ...
                  for (Int_t snb = 1; snb < fStraws_per_layer_tr34; snb++) {
                     ...
                     layerbox->AddNode(gas, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000 + snb,
                                       h6s);
                     layerbox->AddNode(
...
// method to get end points by emulating the geometry
{
   ...
   case 3:
      ...
      cosangle = 1.;
      break;
   default: sinangle = 0.; cosangle = 1.;
   }

   // cout << "DetID" << detID << " statnb "<<statnb<<" vnb " << vnb << " pnb " << pnb <<" lnb "<< lnb << " snb " << snb
   // << endl;
   ...
   case 5: TStationz = fT0z; break;
   default: TStationz = fT0z;
   }
   Double_t zpos;
   if (statnb < 5) {
...
strawtubesPoint *strawtubes::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time,
                                    Double_t length, Double_t eLoss, Int_t pdgCode, Double_t dist2Wire)
{
