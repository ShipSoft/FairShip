...
Target::Target()
   : FairDetector("Target", "", kTRUE), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fTargetPointCollection(new TClonesArray("TargetPoint"))
{
}

Target::Target(const char *name, const Double_t Ydist, Bool_t Active, const char *Title)
   : FairDetector(name, true, ktauTarget), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fTargetPointCollection(new TClonesArray("TargetPoint"))
{
   Ydistance = Ydist;
}
...
void Target::SetEmulsionParam(Double_t EmTh, Double_t EmX, Double_t EmY, Double_t PBTh, Double_t EPlW, Double_t LeadTh,
                              Double_t AllPW)
{
...
void Target::SetBrickParam(Double_t BrX, Double_t BrY, Double_t BrZ, Double_t BrPackX, Double_t BrPackY,
                           Double_t BrPackZ, Int_t number_of_plates_)
{
...
void Target::ConstructGeometry()
{
   ...
   if (fDesign != 2 && fDesign != 4) {
      ...
      if (fDesign == 0) // TP
         MagnetVol->AddNode(volTarget, 1,
                            new TGeoTranslation(0, -fMagnetY / 2 + fColumnY + fCoilH2 + YDimension / 2, 0));
      if (fDesign == 1) // NEW
      ...
      if (fDesign == 3) {
         ...
         for (int i = 0; i < fNTarget; i++) {
            volMagRegion->AddNode(volTarget, i + 1,
   ...
   for (Int_t n = 0; n < NPlates; n++) {
      ...
      volBrick->AddNode(
         fDesign < 4 ? volLead : volTungsten, n,
         new TGeoTranslation(0, 0,
                             -BrickZ / 2 + BrickPackageZ / 2 + EmPlateWidth + LeadThickness / 2 + n * AllPlateWidth));
   }
   if (fsingleemulsionfilm) { // simplified configuration, unique sensitive layer for the whole emulsion plate
      ...
      for (Int_t n = 0; n < NPlates + 1; n++) {
         volBrick->AddNode(
            volEmulsionFilm2, n,
            new TGeoTranslation(0, 0,
                                -BrickZ / 2 + BrickPackageZ / 2 + EmulsionThickness / 2 + n * AllPlateWidth)); // BOTTOM
         volBrick->AddNode(volEmulsionFilm, n,
                           new TGeoTranslation(0, 0,
                                               -BrickZ / 2 + BrickPackageZ / 2 + 3 * EmulsionThickness / 2 +
                                                  PlasticBaseThickness + n * AllPlateWidth)); // TOP
         volBrick->AddNode(volPlBase, n,
                           new TGeoTranslation(0, 0,
   ...
   if (fDesign != 2 && fDesign != 4) {
      ...
      if (fsingleemulsionfilm) { // simplified configuration, unique sensitive layer for the whole emulsion plate
         ...
         for (Int_t n = 0; n < NRohacellGap + 1; n++) {
            volCES->AddNode(volEmulsionFilm2CES, n,
                            new TGeoTranslation(0, 0,
                                                -CESWidth / 2 + CESPackageZ / 2 + EmulsionThickness / 2 +
                                                   n * LayerCESWidth)); // BOTTOM
            volCES->AddNode(volEmulsionFilmCES, n,
                            new TGeoTranslation(0, 0,
                                                -CESWidth / 2 + CESPackageZ / 2 + 3 * EmulsionThickness / 2 +
                                                   PlasticBaseThickness + n * LayerCESWidth)); // TOP
            volCES->AddNode(volPlBaseCES, n,
                            new TGeoTranslation(0, 0,
                                                -CESWidth / 2 + CESPackageZ / 2 + EmulsionThickness +
                                                   PlasticBaseThickness / 2 + n * LayerCESWidth)); // PLASTIC BASE
            //	if(n == 2)
   ...
   if (fDesign == 2 || fDesign == 4) {
      ...
      if (fDesign == 2) {
         ...
         tTauNuDet->AddNode(PillarVol, 1,
                            new TGeoTranslation(-XDimension / 2 + fPillarX / 2, -YDimension / 2 - fBaseY - fPillarY / 2,
...
Bool_t Target::ProcessHits(FairVolume *vol)
{
   ...
   if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
      ...
      detID =
         (NWall + 1) * 1E7 + (NRow + 1) * 1E6 + (NColumn + 1) * 1E4 + BrickorCES * 1E3 + (NPlate + 1) * 1E1 + EmTop * 1;
      ...
      //	cout << "ID = "<< fTrackID << "   pdg = " << pdgCode << ";   M = " << fMotherID << "    Npl = " << NPlate<<";
      //NCol = " << NColumn << ";  NRow = " << NRow << "; NWall = " << NWall<< "  P = " << fMom.Px() << ", "<< fMom.Py()
      //<< ", " << fMom.Pz() << endl;
...
void Target::DecodeBrickID(Int_t detID, Int_t &NWall, Int_t &NRow, Int_t &NColumn, Int_t &NPlate, Bool_t &EmCES,
                           Bool_t &EmBrick, Bool_t &EmTop)
{
...
TargetPoint *Target::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
