...
Hpt::Hpt()
   : FairDetector("HighPrecisionTrackers", kTRUE, ktauHpt), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(),
     fTime(-1.), fLength(-1.), fELoss(-1), fHptPointCollection(new TClonesArray("HptPoint"))
{
}

Hpt::Hpt(const char *name, const Double_t DX, const Double_t DY, const Double_t DZ, Bool_t Active, const char *Title)
   : FairDetector(name, Active, ktauHpt), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(), fTime(-1.),
     fLength(-1.), fELoss(-1), fHptPointCollection(new TClonesArray("HptPoint"))
{
   DimX = DX;
   DimY = DY;
   DimZ = DZ;
...
void Hpt::SetSciFiParam(Double_t scifimat_width_, Double_t scifimat_hor_, Double_t scifimat_vert_, Double_t scifimat_z_,
...
void Hpt::ConstructGeometry()
{
   ...
   if (fDesign < 3) {
      ...
      volMudetBox->AddNode(volCbase, 5,
                           new TGeoTranslation(0, -DimY / 2 - fConcreteY / 2, zSizeMudet / 2 - 3 * DimZ / 2 - 55 * cm));
   ...
   if (fDesign == 3) {
      ...
      for (int i = 0; i < n_vert_planes; i++) {
         HPT_scifi_plane_vert_volume->AddNode(
            HPT_scifimat_vert_volume, 100 + i + 1,
      ...
      for (int i = 0; i < fnHPT; i++) {
         for (int j = 0; j < fntarget; j++) {
            volMagRegion->AddNode(volDT, (i + 1) * 1000 + j * fnHPT,
...
HptPoint *Hpt::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
