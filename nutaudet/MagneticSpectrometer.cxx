...
MagneticSpectrometer::MagneticSpectrometer()
   : FairDetector("MagneticSpectrometer", kTRUE, ktauRpc), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(),
     fTime(-1.), fLength(-1.), fELoss(-1), fShipRpcPointCollection(new TClonesArray("ShipRpcPoint"))
{
}
...
void MagneticSpectrometer::ConstructGeometry()
{
   ...
   if (fDesign == 3) {
      ...
      for (Int_t i = 0; i < fNFe; i++) {
         volMSBox->AddNode(volIron, nr + 100 + i,
                           new TGeoTranslation(0, 0, -fZtot / 2 + i * fZFe + fZFe / 2 + i * fZRpc));
      }
      ...
      for (Int_t i = 0; i < fNRpc; i++) {
         volMSBox->AddNode(volRpcContainer, nr + i,
      ...
      tTauNuDet->AddNode(
         Pillar1Vol, 2,
         new TGeoTranslation(fXtot / 2 - fPillarX / 2, -fYtot / 2 - fPillarY / 2, fZcenter - fZtot / 2 + fPillarZ / 2));
...
ShipRpcPoint *MagneticSpectrometer::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time,
