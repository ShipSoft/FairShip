...
TGeoVolume* MTCDetector::CreateSciFiModule(const char* name, Double_t width, Double_t height, Double_t thickness, Int_t LayerId) {
  ...
  auto sciFiLayerMotherUBox = new TGeoBBox(Form("%s_SciFiLayerMother_U", name), width/2, height/2, fiberMatThick/2 / fiber_layer_number);
  auto sciFiLayerMotherUVol = new TGeoVolume(Form("%s_SciFiLayerMother_U", name), sciFiLayerMotherUBox, gGeoManager->GetMedium("SciFiMat"));
...
Bool_t  MTCDetector::ProcessHits(FairVolume* vol)
{
  ...
  if ( gMC->IsTrackExiting()    ||
      ...
      Double_t x,y,z;
      if(detID / 10000000 == 3){
        // mean coordinates within a volume since we don't have spatial info inside scint.
        x = (fPos.X()+Pos.X())/2. ;
        y = (fPos.Y()+Pos.Y())/2. ;
        z = (fPos.Z()+Pos.Z())/2. ;
      }
...
void MTCDetector::Register(){
    FairRootManager::Instance()->RegisterAny("MTCdetPoint", fMTCPointPtr, kTRUE);
    cout << this->GetName() << ",  Register() says: registered " << "MTCdetPoint" <<" collection"<<endl;
  }
