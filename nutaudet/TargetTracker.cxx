...
TargetTracker::TargetTracker()
   : FairDetector("TargetTracker", "", kTRUE), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fTTPointCollection(new TClonesArray("TTPoint"))
{
}

TargetTracker::TargetTracker(const char *name, Double_t TTX, Double_t TTY, Double_t TTZ, Bool_t Active,
                             const char *Title)
   : FairDetector(name, true, ktauTT), fTrackID(-1), fVolumeID(-1), fPos(), fMom(), fTime(-1.), fLength(-1.),
     fELoss(-1), fTTPointCollection(new TClonesArray("TTPoint"))
{
   TTrackerX = TTX;
   TTrackerY = TTY;
...
void TargetTracker::ConstructGeometry()
{
   ...
   for (int i = 0; i < n_hor_planes; i++) {
      TT_scifi_plane_hor_volume->AddNode(TT_scifimat_hor_volume, i + 1,
                                         new TGeoTranslation(0, (-(n_hor_planes - 1) / 2.0 + i) * scifimat_width, 0));
   }
   for (int i = 0; i < n_vert_planes; i++) {
      TT_scifi_plane_vert_volume->AddNode(TT_scifimat_vert_volume, 100 + i + 1,
   ...
   TT_volume->AddNode(TT_scifi_plane_hor_volume, 0,
                      new TGeoTranslation(0, 0, -TTrackerZ / 2 + support_z + scifimat_z / 2));
   TT_volume->AddNode(TT_scifi_plane_vert_volume, 0,
                      new TGeoTranslation(0, 0, -TTrackerZ / 2 + support_z + scifimat_z + scifimat_z / 2));
   TT_volume->AddNode(TT_honeycomb_volume, 0,
   ...
   for (int l = 0; l < fNTT; ++l) {
      volTarget->AddNode(TT_volume, 1000 * (l + 1),
...
TTPoint *TargetTracker::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
