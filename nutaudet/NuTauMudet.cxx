...
NuTauMudet::NuTauMudet()
   : FairDetector("NuTauMudet", kTRUE, ktauRpc), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(), fTime(-1.),
     fLength(-1.), fELoss(-1), fShipRpcPointCollection(new TClonesArray("ShipRpcPoint"))
{
}

NuTauMudet::NuTauMudet(const char *name, const Double_t Zcenter, Bool_t Active, const char *Title)
   : FairDetector(name, Active, ktauRpc), fTrackID(-1), fPdgCode(), fVolumeID(-1), fPos(), fMom(), fTime(-1.),
     fLength(-1.), fELoss(-1), fShipRpcPointCollection(new TClonesArray("ShipRpcPoint"))
{
   fZcenter = Zcenter;
}
...
void NuTauMudet::SetSupportTransverseDimensions(Double_t UpperSupportX, Double_t UpperSupportY, Double_t LowerSupportX,
                                                Double_t LowerSupportY, Double_t LateralSupportX,
                                                Double_t LateralSupportY, Double_t YSpacing)
{
...
void NuTauMudet::ConstructGeometry()
{
   ...
   if (fDesign < 3) {
      ...
      for (int i = 0; i < fNCoil; i++) {
         volCoilContainer->AddNode(volCoil, i,
                                   new TGeoTranslation(0, 0, -40 * cm + fCoilW / 2 + i * (fCoilGap + fCoilW)));
      }
      ...
      for (Int_t i = 0; i < fNFe; i++) {
         volArm2->AddNode(volIron2, nr + 100 + i,
                          new TGeoTranslation(0, 0, -fZArm / 2 + i * (fZFe + fZRpc) + fZFe / 2));
      }

      for (Int_t i = 0; i < fNRpc; i++) {
         volArm2->AddNode(
            volRpcContainer2, nr + i,
            new TGeoTranslation(0, -fYFe / 2 + fYRpc / 2, -fZArm / 2 + (i + 1) * fZFe + i * fZRpc + fZRpc / 2));
      }
      ...
      tTauNuDet->AddNode(Pillar1Vol, 1,
                         new TGeoTranslation(-fXtot / 2 + fPillarX / 2, -fYtot / 2 - fPillarY / 2,
                                             fZcenter - fZArm / 2 - fGapMiddle / 2 + fPillarZ / 2));
      tTauNuDet->AddNode(Pillar1Vol, 2,
                         new TGeoTranslation(fXtot / 2 - fPillarX / 2, -fYtot / 2 - fPillarY / 2,
                                             fZcenter - fZArm / 2 - fGapMiddle / 2 + fPillarZ / 2));
      tTauNuDet->AddNode(Pillar1Vol, 3,
                         new TGeoTranslation(-fXtot / 2 + fPillarX / 2, -fYtot / 2 - fPillarY / 2,
                                             fZcenter + fZArm / 2 + fGapMiddle / 2 - fPillarZ / 2));
      tTauNuDet->AddNode(Pillar1Vol, 4,
                         new TGeoTranslation(fXtot / 2 - fPillarX / 2, -fYtot / 2 - fPillarY / 2,
                                             fZcenter + fZArm / 2 + fGapMiddle / 2 - fPillarZ / 2));
   }
   if (fDesign == 3) {
      ...
      volMudetBox->AddNode(volUpperCover, 1,
                           new TGeoTranslation(0, fYFe / 2 + fYCov / 2 + fUpSuppY, -fZtot / 2 + fZCov / 2));
      ...
      TGeoPara *ArmCross1 =
         new TGeoPara("ArmCross1", fWidthArm / 2., fYCross / 2, fXCross / 2., TMath::RadToDeg() * Inclination, 0.,
                      0.); // length and height are not x and y here, because it will be rotated!
      ArmCross1->SetName("ARMCROSS1");
      TGeoPara *ArmCross2 =
         new TGeoPara("ArmCross2", fWidthArm / 2., fYCross / 2, fXCross / 2., TMath::RadToDeg() * Inclination, 0.,
                      0.); // length and height are not x and y here, because it will be rotated!
      ...
      volMudetBox->AddNode(
         volMuDetCross, 1,
         new TGeoTranslation(-fXFe / 2. - fLatSuppX - fXLateral + fXCross / 2., 0., -fZtot / 2 + fZCross / 2)); // right
      volMudetBox->AddNode(
         volMuDetCross, 2,
         new TGeoTranslation(+fXFe / 2. + fLatSuppX + fXLateral - fXCross / 2., 0., -fZtot / 2 + fZCross / 2)); // left

      //***********************ADDING CUTS AT MID-LATERAL IN WALLS
      IronLayer->SetName("MUDETIRON");
      IronLayer1->SetName("MUDETIRON1");

      Double_t delta = 0.1; // to avoid border effects in the cuts (cut is not visualized in viewer, I do not know if it
                            // can affect simulation)
      TGeoTrd2 *Model =
         new TGeoTrd2("Model", fCutHeight / 2, 0, (fZFe + delta) / 2, (fZFe + delta) / 2,
      ...
      TGeoCompositeShape *mudetcut1 =
         new TGeoCompositeShape("MUDETCUT1", "(MUDETIRON1-MUDETTRIANGLE:MuDetcombright)-MUDETTRIANGLE:MuDetcombleft");
      ...
      TGeoTranslation *lateralupleft = new TGeoTranslation("MuDetlateralupleft", +fXFe / 2. + fLowSuppX / 2.,
                                                           fYFe / 2 - fLowSuppY / 2. - fYSpacing, 0);
      TGeoTranslation *lateralupright = new TGeoTranslation("MuDetlateralupright", -fXFe / 2. - fLowSuppX / 2.,
                                                            fYFe / 2 - fLowSuppY / 2. - fYSpacing, 0);
      TGeoTranslation *laterallowleft = new TGeoTranslation("MuDetlaterallowleft", +fXFe / 2. + fLowSuppX / 2.,
                                                            -fYFe / 2 + fLowSuppY / 2. + fYSpacing, 0);
      TGeoTranslation *laterallowright = new TGeoTranslation("MuDetlaterallowright", -fXFe / 2. - fLowSuppX / 2.,
                                                             -fYFe / 2 + fLowSuppY / 2. + fYSpacing, 0);
      ...
      TString *supportaddition = new TString(
         ...
         "MUDETLOWSUPPORT:MuDetlowleft+MUDETLATERALSUPPORT:MuDetlateralupleft+MUDETLATERALSUPPORT:MuDetlateralupright+"
      ...
      TString *supportaddition1 =
         new TString("MUDETTRIANGCUT1+MUDETUPSUPPORT1:MuDetupright+MUDETUPSUPPORT1:MuDetupleft+MUDETLOWSUPPORT1:"
                     "MuDetlowright+MUDETLOWSUPPORT1:MuDetlowleft+MUDETLOWSUPPORT1:MuDetlowleft+MUDETLATERALSUPPORT1:"
                     "MuDetlateralupleft+MUDETLATERALSUPPORT1:MuDetlateralupright+MUDETLATERALSUPPORT1:"
                     "MuDetlaterallowleft+MUDETLATERALSUPPORT1:MuDetlaterallowright");
      ...
      */
      ...
      TGeoTrd2 *Indentation =
         new TGeoTrd2("Indentation", (fYRpc_outer - 2 * (31.2 * cm + 15. * cm)) / 2, (fYRpc_outer - 2 * 31.2 * cm) / 2,
                      (fZRpc_outer + 0.1 * cm) / 2, (fZRpc_outer + 0.1 * cm) / 2,
      ...
      TGeoTrd2 *Indentation_0 =
         new TGeoTrd2("Indentation_0", (fYRpc_outer - 2 * (31.2 * cm + 15. * cm)) / 2,
                      (fYRpc_outer - 2 * 31.2 * cm) / 2, (fZRpc) / 2, (fZRpc) / 2, (15. * cm + 0.1 * cm) / 2);
      Indentation_0->SetName("INDENTATION_0");
      TGeoCompositeShape *RpcContainer =
         new TGeoCompositeShape("RpcContainer", "RPCCOINTAINER_0-INDENTATION_0:RIGHTINDENT-INDENTATION_0:LEFTINDENT");
      TGeoVolume *volRpcContainer = new TGeoVolume("volRpcContainer", RpcContainer, air);
      //***********************************
      ...
      volRpcContainer->AddNode(volRpcShell, 1,
                               new TGeoTranslation(0., 0., -fZGas / 2 - fZRpcGap - fZStrip - fZRpc_outer / 2));
   ...
   if (fDesign == 4) {
      ...
      volMagRegion->AddNode(
         ...
         new TGeoTranslation(0, 0,
                             -fZtot / 4. - fGapMiddle / 2. - fZRpc / 2.)); // these are INSIDE the magnetized region
      volMagRegion->AddNode(volRPC, 3, new TGeoTranslation(0, 0, -fZtot / 4. + fGapMiddle / 2. + fZRpc / 2.));
      // magnetized region in the middle
      volMagRegion->AddNode(
         volRPC, 4, new TGeoTranslation(0, 0, -fGapMiddle / 2. - fZRpc / 2.)); // these are INSIDE the magnetized region
...
ShipRpcPoint *NuTauMudet::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
