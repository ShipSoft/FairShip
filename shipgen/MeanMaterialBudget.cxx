...
double MeanMaterialBudget(const double* start, const double* end, double* mparam)
{
    ...
    //
    //  SHiP enhancements:
    //        Thomas Ruf,  Thomas.Ruf@cern.ch (interaction length support, Dec 2016)
    //        Anupama Reghunath, anupama.reghunath@cern.ch (error logging, Nov 2024)
    //
    ...
    if (!startnode) {
        LOG(error) << "MeanMaterialBudget: start point out of geometry: x " << start[0] << ", y " << start[1]
                   << ", z " << start[2];
        return 0.0;
    }
