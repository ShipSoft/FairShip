...
Bool_t DPPythia8Generator::ReadEvent(FairPrimaryGenerator *cpg)
{
   ...
   do {
      ...
      if (fpbrem) {
         ...
         if (debug > 1) {
            std::cout << " Adding DP gun with p " << dpmom << " m " << dpm << " e " << dpe << " theta,phi " << thetain
                      << "," << phiin << std::endl
                      << std::flush;
         }
         fPythia->event.append(fDP, 1, 0, 0, dpmom * sin(thetain) * cos(phiin), dpmom * sin(thetain) * sin(phiin),
      ...
      if (iDP == 0) {
         ...
         if (fextFile && *fextFile) {
            ...
            cpg->AddTrack((Int_t)fPythia->event[im].id(), pmx, pmy, pmz, xm / cm + dx, ym / cm + dy, zm / cm, -1, false,
                          em, tm / cm / c_light, w); // convert pythia's (x,y,z[mm], t[mm/c]) to ([cm], [s])
   ...
   fPythiaHadDecay->event[k].e(),  fPythiaHadDecay->event[k].m(), 0., 9. ); dec_chain.push_back( fPythia->event.size()-1
   ...
   for (std::vector<int>::iterator it = dec_chain.begin() + 2; it != dec_chain.end(); ++it) {
      ...
      cpg->AddTrack((Int_t)fPythia->event[k].id(), px, py, pz, xS / cm, yS / cm, zS / cm, im, wanttracking, e,
                    tS / cm / c_light, w);
      // std::cout <<k<< " insert pdg =" <<fPythia->event[k].id() << " pz = " << pz << " [GeV] zS = " << zS << " [mm] tS
      // = " << tS << "[mm/c]" <<  std::endl;
   }
   return kTRUE;
}
