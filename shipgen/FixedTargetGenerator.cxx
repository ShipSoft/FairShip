...
Bool_t FixedTargetGenerator::InitForCharmOrBeauty(TString fInName, Int_t nev, Double_t npot, Int_t nStart)
{
   ...
   fLogger->Info(MESSAGE_ORIGIN, "Input file: %s   with %i entries, corresponding to nr-pot=%f", fInName.Data(),
                 nEvents, nrcpot / chicc);
   fLogger->Info(MESSAGE_ORIGIN, "weight %f corresponding to %f p.o.t. per spill for %i events to process", wspill,
                 nrpotspill, nev);
...
Bool_t FixedTargetGenerator::Init()
{
   ...
   for (const auto &fPythia : plist) {
      ...
      if (tauOnly) {
         fPythia->readString("431:new  D_s+  D_s-  1   3   0    1.96849    0.00000    0.00000    0.00000  1.49900e-01  "
                             " 0   1   0   1   0");
...
Bool_t FixedTargetGenerator::ReadEvent(FairPrimaryGenerator *cpg)
{
   ...
   if (targetName.Data() != "") {
      ...
      // loop over trajectory between start and end to pick an interaction point, copied from GenieGenerator and adapted
      // to hadrons
      Double_t prob2int = -1.;
      Double_t rndm = 0.;
      Double_t sigma;
   ...
   if (G4only) {
      ...
      cpg->AddTrack(int(n_mid), n_mpx, n_mpy, n_mpz, xOff / cm, yOff / cm, zinter / cm, -1, kFALSE, n_mE, 0., wspill,
                    procID);
      // second charm hadron in the event
      nTree->GetEvent(nEntry);
      if (nID1 * n_id > 0) {
         fLogger->Info(MESSAGE_ORIGIN, "same sign charm: %i, %i, %i", nEntry, nID1, n_id);
      }
      nEntry += 1;
      fPythiaP->event.append(int(n_id), 1, 0, 0, n_px, n_py, n_pz, n_E, n_M, 0., 9.);
      fPythiaP->next();
