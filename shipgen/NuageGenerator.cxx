...
Bool_t NuageGenerator::ReadEvent(FairPrimaryGenerator *cpg)
{
   ...
   while (prob2int < gRandom->Uniform(0., 1.)) {
      ...
      if (prob2int > 1.)
         cout << "***WARNING*** NuageGenerator: prob2int > Maximum density????" << prob2int << " maxrho:" << mparam[7]
   ...
   cpg->AddTrack(neu, pout[0], pout[1], pout[2], x, y, z, -1, false,
                 TMath::Sqrt(pout[0] * pout[0] + pout[1] * pout[1] + pout[2] * pout[2]), tof, mparam[0] * mparam[4]);
   if (not fNuOnly) {
      ...
      if (TMath::Abs(oLPdgCode) != 15) {
         cpg->AddTrack(oLPdgCode, pp[0], pp[1], pp[2], x, y, z, 0, true,
                       TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof, mparam[0] * mparam[4]);
         nAddTrk++;
      }
      // cout << "oLpdgCode " << oLPdgCode << " pz "<< pzl << endl;
      if (TMath::Abs(oLPdgCode) == 15) {
         if (fExtDecayer == 0) {
            cpg->AddTrack(oLPdgCode, pp[0], pp[1], pp[2], x, y, z, 0, false,
                          TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof, mparam[0] * mparam[4]);
            nAddTrk++;
            if (TMath::Abs(parent2) == 15) {
               ...
               for (int j = 0; j < nf2; j++) {
                  ...
                  cpg->AddTrack(pdgf2[j], pp[0], pp[1], pp[2], x2, y2, z2, nAddTrk, true,
                                TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof2,
                                mparam[0] * mparam[4]);
            ...
            if (TMath::Abs(parent3) == 15) {
               ...
               for (int j = 0; j < nf3; j++) {
                  ...
                  cpg->AddTrack(pdgf3[j], pp[0], pp[1], pp[2], x3, y3, z3, nAddTrk, true,
                                TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof3,
                                mparam[0] * mparam[4]);
               }
               nAddTrk += nf3;
            }
            if (TMath::Abs(parent4) == 15) {
               ...
               for (int j = 0; j < nf4; j++) {
                  ...
                  cpg->AddTrack(pdgf4[j], pp[0], pp[1], pp[2], x4, y4, z4, nAddTrk, true,
                                TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof4,
                                mparam[0] * mparam[4]);
               }
               nAddTrk += nf4;
            }
      ...
      for (int i = 0; i < nf; i++) {
         ...
         if (Abs(pdgf[i]) != 411 && Abs(pdgf[i]) != 421 && Abs(pdgf[i]) != 431 && Abs(pdgf[i]) != 4122) {
            cpg->AddTrack(pdgf[i], pp[0], pp[1], pp[2], x, y, z, 0, true,
                          TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof, mparam[0] * mparam[4]);
            nAddTrk++;
         }
            ...
            cout << "charm particle " << endl;
         {
            if (fExtDecayer == 0) {
               ...
               cpg->AddTrack(pdgf[i], pp[0], pp[1], pp[2], x, y, z, 0, false,
                             TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof, mparam[0] * mparam[4]);
               nAddTrk++;
               if (TMath::Abs(parent2) == pdgf[i]) {
                  ...
                  for (int j = 0; j < nf2; j++) {
                     ...
                     cpg->AddTrack(pdgf2[j], pp[0], pp[1], pp[2], x2, y2, z2, nAddTrk, true,
                                   TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]), tof2,
                                   mparam[0] * mparam[4]);
                     //	cout << "pdgf2 " << pdgf2[j] << " p2 "<< TMath::Sqrt(pp[0]*pp[0]+pp[1]*pp[1]+pp[2]*pp[2]) <<
                     //endl;
                  }
                  nAddTrk += nf2;
               }
