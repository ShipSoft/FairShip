...
Bool_t GenieGenerator::ReadEvent(FairPrimaryGenerator *cpg)
{
   ...
   while (prob2int < gRandom->Uniform(0., 1.)) {
      ...
      y = tynu * (z - ztarget);
      if (mparam[6] < 0.5) {
         // mparam is number of boundaries along path. mparam[6]=0.: uniform material budget along path, use present
         // x,y,z
         prob2int = 2.;
      } else {
         ...
         if (node && !gGeoManager->IsOutside()) {
            ...
            // cout << "Info GenieGenerator: mat " <<  count << ", " << mat->GetName() << ", " << mat->GetDensity() <<
            // endl; density relative to Prob largest density along this trajectory, i.e. use rho(Pt)
