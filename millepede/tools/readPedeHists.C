...
//__________________________________________________________________________
ReadPedeHists::ReadPedeHists(const char *txtFile) : theStream(txtFile, ios::in)
{
...
TH1 *ReadPedeHists::readNextHist(std::ifstream &file)
{
   ...
   while (readBasic(file, key)) {
      if (key == "bins,") {
         ...
         if (!readBasic(file, key) || key != "limits") {
            ...
            ::Error(method, "failed reading nBins, xLow, xUp (%f %f %f)", nBinsUpLow[0], nBinsUpLow[1], nBinsUpLow[2]);
         } else {
         ...
         if (!readBasic(file, key) || key != "inside" || !readBasic(file, key) || key != "out-high") {
            ::Error(method, "expected keys 'inside' and 'out-high', got (%s)", key.c_str());
                    ...
                    underInOver[2] == -1) {
            ::Error(method, "failed reading under-, 'in-' and overflow (%d %d %d)", underInOver[0], underInOver[1],
                    underInOver[2]);
         }
   ...
   if (nBinsUpLow[1] == nBinsUpLow[2]) { // causes ROOT drawing errors
      ...
      ::Error(method, "Hist %d (version %d): same upper and lower edge (%f), set upper %f.", num, version,
              nBinsUpLow[1], nBinsUpLow[2]);
   }
   ...
   while (true) {
      ...
      if (file.fail() || (type != "Histogram" && type != "XY-Data")) {
         ...
         if (line != "" && line.Length() != line.CountChar(' ')) { // not empty
            ::Error("ReadPedeHists::readNext", "Expect 'Histogram' or 'XY-Data', but failed, line is '%s'",
                    line.Data());
